...
from azureml.train.automl._environment_utilities import is_prod, modify_run_configuration
...
def _automl_static_submit(automl_config_object: 'AutoMLConfig',
    ...
    if run_config is None and compute_target is None:
        ...
        logger.info("LocalTarget provided as compute, submitting a local managed run with docker enabled.")
        is_managed = True
    elif compute_target == "local":
        logger.info("String local provided as compute, submitting a local managed run with docker disabled.")
...
def _register_datasets(workspace: Workspace, dataset: AbstractDataset) -> Optional[str]:
    if dataset is not None:
        return cast(str, dataset._ensure_saved(workspace=workspace))
    else:
        return None


def _compose_args(dataset_args: List[str], name: str, value: Optional[str]) -> List[str]:
    arg = "--{}"
...
def _extract_data(workspace: Workspace, fit_params: Dict[str, Any]) -> Tuple[Dict[str, Any], List[Any]]:
    """Register datasets and create script arguments to pass to the child job."""
    X = fit_params.pop(_DataArgNames.X, None)
    y = fit_params.pop(_DataArgNames.y, None)
    sample_weight = fit_params.pop(_DataArgNames.sample_weight, None)
    X_valid = fit_params.pop(_DataArgNames.X_valid, None)
    y_valid = fit_params.pop(_DataArgNames.y_valid, None)
    sample_weight_valid = fit_params.pop(_DataArgNames.sample_weight_valid, None)
    training_data = fit_params.pop(_DataArgNames.training_data, None)
    validation_data = fit_params.pop(_DataArgNames.validation_data, None)

    dataset_args = []  # type: List[str]

    dataset_args = _compose_args(dataset_args,
                                 _DataArgNames.X, _register_datasets(workspace, X))
    dataset_args = _compose_args(dataset_args,
                                 _DataArgNames.y, _register_datasets(workspace, y))
    dataset_args = _compose_args(dataset_args,
                                 _DataArgNames.sample_weight, _register_datasets(workspace, sample_weight))
    dataset_args = _compose_args(dataset_args,
                                 _DataArgNames.X_valid, _register_datasets(workspace, X_valid))
    dataset_args = _compose_args(dataset_args,
                                 _DataArgNames.y_valid, _register_datasets(workspace, y_valid))
    dataset_args = _compose_args(dataset_args,
                                 _DataArgNames.sample_weight_valid, _register_datasets(workspace, sample_weight_valid))
    dataset_args = _compose_args(dataset_args,
                                 _DataArgNames.training_data, _register_datasets(workspace, training_data))
    dataset_args = _compose_args(dataset_args,
                                 _DataArgNames.validation_data, _register_datasets(workspace, validation_data))
...
def _local_managed(experiment: Experiment,
    ...
    try:
        with tempfile.TemporaryDirectory() as local_path:
            ...
            fit_params, dataset_args = _extract_data(experiment.workspace, fit_params)
...
class AutoMLConfig(object):
        ...
        for k, v in fit_signature.parameters.items():
            ...
            if default_val is inspect.Parameter.empty:
                try:
                    ...
                    raise ConfigException.create_without_pii(
                        "To submit an experiment you will need to provide a "
    ...
    def _validate_config_settings(self, workspace: Optional[Workspace] = None) -> None:
                    ...
                    for split in cv_splits_indices:
                        ...
                        for item in split:
                            if dataprep_utilities.is_dataflow(item):
                                raise ConfigException.create_without_pii(
                                    "Input of type `azureml.dataprep.Dataflow` is deprecated."
                                    " Please see the valid types in our documentation: "
                                    "https://aka.ms/AutoMLConfig")
                            else:
        ...
        if self.user_settings.get('cv_split_column_names') and has_many_feature_dataset_input:
            raise ConfigException.create_without_pii(
                'For X and y input, provide separate `cv_split_indices` data instead of `cv_split_column_names`.',
        ...
        if has_cv_split_indices and has_single_feature_dataset_input:
            raise ConfigException.create_without_pii(
                'For training_data input, provide `cv_split_column_names` instead of `cv_splits_indices`.',
        ...
        if self.user_settings.get('cv_split_column_names'):
            ...
            if not (isinstance(cv_split_column_names_val, list)):
                raise ConfigException.create_without_pii(
                    'cv_split_column_names parameter needs to be of List[str] containing name of the columns',
        ...
        if has_single_feature_dataset_input:
            ...
            if temp_label_column_name == temp_weight_column_name or is_cv_name_duplicate:
                raise ConfigException.create_without_pii(
                    'Same column name is used for `label_column_name`, `weight_column_name`, and/or '
                    '`cv_split_column_names`. Please check the fields and try again.',
