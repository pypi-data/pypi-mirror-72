...
class Formatter:
	...
	def get_delta(self,delta:dt.timedelta, as_dict=False):
		"""
	...
	def past_to_now(self,past:typing.Union[dt.datetime, dt.date], now:typing.Union[dt.datetime, dt.date], as_dict=False):
	...
	def get_difference(self, datetime1:typing.Union[dt.datetime, dt.date], datetime2:typing.Union[dt.datetime, dt.date], as_dict=False):
		"""Past date difference compared to current time
	...
	def natural_date(self, date:dt.date, as_dict=False):
		"""
	...
	def natural_time(self, time:dt.time, as_dict=False, format=12):
		"""
		datetime object to a human readable time
	...
	def natural_datetime(self, date_time:dt.datetime, as_dict=False):
		"""
		datetime object to a human readable time
	...
	def seconds_to_string(self, seconds:int, as_dict=False):
		...
		dur = f'{year} Years ' if year > 0 else '' + f'{month} Months ' if month > 0 else '' + f'{day} Days ' if day > 0 else '' + f'{hour} Hours ' if hour > 0 else '' + f'{minute} Minutes ' if minute > 0 else '' + f'{second} Seconds' if second > 0 else ''
		return dur
...
if __name__ == "__main__":
	app = Formatter()
	seconds = 8283812
	data = app.seconds_to_string(seconds, as_dict=True)
	print(data)
