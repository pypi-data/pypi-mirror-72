...
def get_encoding(value):
    return ENCODING_MAP.get(value.lower())


def bytes_to_readable_string(value, decoder="utf-8"):
    """
    Decode based on content type and then force back to utf-8
    """
    encoding = get_encoding(decoder)

    if six.PY2:
        try:
            return value.decode(encoding, "ignore").encode("utf-8")
        except:
