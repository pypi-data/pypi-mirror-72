...
def read_instance_with_gaz(input_file, gaz, word_alphabet, biword_alphabet, char_alphabet, gaz_alphabet, label_alphabet, segment_label_alphabet, number_normalized, max_sent_length, mode='NER', char_padding_size=-1, char_padding_symbol = '</pad>'):
    in_lines = open(input_file,'r',encoding='utf-8').readlines()
    instence_texts = []
    instence_Ids = []
    words = []
        ...
        if len(line) > 2:
         ...
         ###   print("CHECK:",word, label)
            ...
            if mode == 'NER':
                ...
                label_Ids.append(segment_label_alphabet.get_index(label))
