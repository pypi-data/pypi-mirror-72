...
__all__ = ('chunk_of_generators', 'get_smart_token', 'delete_file', 'write_file', 'upload_to_s3', 'check_file_exist',
           'get_file_link', 'chunk_by_items', 'delete_s3_file', 'chunk_by_parts', 'decode', 'encode',
           'get_full_integrations', 'write_ram_file', 'upload_to_s3_from_ram', 'delete_invalid_integrations')
...
def write_file(filename: str, data: Union[list, dict, Generator], fieldnames: Optional[List[str]] = None,
               delimiter: str = '\t', extension: str = '.tsv') -> str:
    """
...
def upload_to_s3(filename: str, folder: str = "stats/") -> dict:
    ...
    s3.meta.client.upload_file(filename, BUCKET_NAME, f"{folder}{s3_filename}", ExtraArgs={'ACL': 'public-read'},
                               Config=config)
...
def get_file_link(filename: str, folder: str = 'stats/', bucket: str = BUCKET_NAME, suffix: str = '.tsv') -> str:
    """
    :param filename: str
    :param folder: str
    :param bucket:  str
    ...
    if suffix in filename:
        ...
        filename = f"{folder}{filename}{suffix}" if folder not in filename else f"{filename}{suffix}"
...
def check_entities_equal(db_entity: any, new_entity: any, exclude_fields: Union[list, tuple, None] = None) -> bool:
    """
    :param db_entity: any
    ...
    if not exclude_fields:
        exclude_fields = ('_state', 'date_time', 'sc_date_time', 'funnel_stage_duration', 'id')
    return all((field, new_entity[field]) in db_entity.items() for field in db_entity if field not in exclude_fields)
...
class __Crypt(object):
    ...
    def encrypt(self, str_to_enc, str_key=settings.SECRET_KEY):
        try:
            aes_obj = AES.new(hashlib.sha256(str_key.encode()).digest(), AES.MODE_CFB, self.salt)
...
def write_ram_file(f: StringIO, data: Generator, fieldnames: list, delimiter='\t') -> StringIO:
    writer = csv.DictWriter(f, delimiter=delimiter, fieldnames=fieldnames)
    writer.writeheader()
    for row in data:
        writer.writerow(row)
...
def upload_to_s3_from_ram(content: Union[StringIO, BytesIO], filename: str, folder: str = 'yandex_files/',
                          suffix: str = '.tsv'):
    if filename.startswith('/tmp/'):
        s3_filename = filename[5:]
    else:
    ...
    s3 = boto3.resource('s3')
    body = content.getvalue().encode('utf-8') if isinstance(content, StringIO) else content.getvalue()
    s3.meta.client.put_object(Bucket=BUCKET_NAME, Key=f'{folder}{s3_filename}',
                              Body=body, ACL='public-read')
    return {
        "status": "success",
        "s3_filename": s3_filename,
        "s3_folder": folder
...
def delete_invalid_integrations(integrations: list, models: Union[tuple, list],
                                filter_query: Optional[dict] = None) -> list:
    integration_ids = [i['integration_id'] for i in integrations]
    return [
        model.objects.exclude(integration_id__in=integration_ids).delete() if not filter_query else
        model.objects.filter(**filter_query
                             ).exclude(integration_id__in=integration_ids).delete()
        for model in models
    ]
