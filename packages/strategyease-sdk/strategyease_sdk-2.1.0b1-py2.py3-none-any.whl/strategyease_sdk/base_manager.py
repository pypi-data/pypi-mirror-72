...
from strategyease_sdk.client import *
from strategyease_sdk.models import *
from strategyease_sdk.support import *
...
class StrategyTrader(object):
    def __init__(self, logger, config, strategy_context):
        ...
        self._config = config
        self._strategy_context = strategy_context
        self._strategyease_client = Client(self._logger, **config['client'])
    ...
    def purchase_new_stocks(self):
        ...
        self._strategyease_client.purchase_new_stocks()

    def purchase_convertible_bonds(self):
        if not self._pre_check():
            return

        self._strategyease_client.purchase_convertible_bonds()
    ...
    def execute(self, order=None, **kwargs):
        ...
        try:
            ...
            self._logger.exception("[策略易] 下单异常")
    ...
    def cancel(self, order):
        ...
        try:
            ...
            self._logger.exception("[策略易] 撤单异常")
        ...
        try:
            ...
            if self._should_sync(target_portfolio):
                if self._sync_config['pre-clear-for-live'] and not self._config['dry-run']:
                    self._strategyease_client.cancel_all()
    ...
    def _do_cancel(self, order):
        ...
        try:
            ...
            if not order_info['canceled']:
                ...
                self._strategyease_client.cancel(order_id=order_info['id'])
        except KeyError:
            self._logger.warning('[策略易] 未找到对应的委托编号')
    ...
    def _should_run(self):
        if self._config['dry-run']:
            self._logger.debug("[策略易] 当前为排练模式，不执行下单、撤单请求")
            return False
        return True
    ...
    def _create_adjustment(self, target_portfolio):
        ...
        response_json = self._strategyease_client.create_adjustment(request_json=request_json)
...
class StrategyConfig(object):
    ...
    def reload(self):
        content = self._strategy_context.read_file('strategyease_sdk_config.yaml')
