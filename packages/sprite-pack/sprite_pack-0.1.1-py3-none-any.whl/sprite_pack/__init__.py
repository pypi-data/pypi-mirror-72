...
def pack_sprites(sheet_fh, scene_fh, animated, static, static_unpack):
    static.extend(chain.from_iterable(map(PackedSprites.unpack, static_unpack)))
    sizes = list(map(Sprite.get_size, chain(animated, static)))
    ...
    anim_pos, static_pos = split_at(positions, len(animated))
...
class PackedSprites(Sprite):
    def __init__(self, file_width):
        ...
        self.boxes = [
            (x, y, x + self.pack_size.w, y + self.pack_size.h) for x, y in zip(xs, ys)
        ]
...
def main():
    ...
    pack_sprites(args.sheet, args.scene, args.animated, args.static, args.static_unpack)
