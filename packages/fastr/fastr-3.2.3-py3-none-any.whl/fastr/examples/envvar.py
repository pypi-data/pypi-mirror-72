...
def create_network():
    ...
    network = fastr.create_network(id='file_copy')

    # Create a source node in the network
    source1 = network.create_source('TxtFile', id='source')

    # Create a new node in the network using tools
    node = network.create_node('fastr/util/EnvVarTest:1.0', tool_version='1.0', id="copy")

    # Create a link between the source output and an input of the addint node
    node.inputs['input_file'] = source1.output
    node.inputs['env_test'] = 'test 123',

    # Create a sink to save the data
    sink = network.create_sink('TxtFile', id='sink')
