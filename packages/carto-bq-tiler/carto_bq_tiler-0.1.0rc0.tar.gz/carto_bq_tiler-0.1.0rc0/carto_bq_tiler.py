...
from settings import AUTHORS, HELP_CONTEXT_SETTINGS, NAME, VERSION, VIEWER_PORT
...
def cli(ctx, project, credentials):
    ...
    To get help for each command run: carto_bq_tiler COMMAND --help
    """
    # The substring 'carto_bq_tiler' in the multiline string above should be dynamically added :(
...
def load(ctx, mbtiles_file_path, name):
    ...
    tileset_name = bq_client.validate_tileset_name(name)
    tileset_name = bq_client.get_tileset_name(tileset_name)
...
def export_mbtiles(ctx, tileset, file):
    ...
    if not bq_client.exists_tileset(tileset):
        info_message = "{tileset} tileset doesn't exsits in the {project} project".format(
            tileset=click.style(tileset, bold=True), project=click.style(bq_client.project, bold=True)
        )
        click.echo(info_message)
        return
...
def view_local(ctx, tileset_directory, port):
    ...
    if not os.path.isdir(tileset_directory):
        command = '{name} export-viewer {tileset}'.format(name=NAME, tileset=tileset)
        info_message = 'The {tileset} tileset can not be found, run "{command}".'.format(
...
def list(ctx):
    ...
    bq_client = ctx.obj['bq_client']
    with click_spinner.spinner():
        tilesets = bq_client.list_tilesets()
    ...
    for tileset in tilesets:
        tileset_table.append([click.style(tileset['tileset_name'], bold=True), tileset['created_at']])

    column_width = max([len(tileset[0]) for tileset in tileset_table]) + 1

    for tileset in tileset_table:
        info_message = '{tileset} Created at {datetime}'.format(tileset=tileset[0].ljust(column_width),
                                                                datetime=tileset[1])
        click.echo(info_message)
...
def remove(ctx, names):
    ...
    with click_spinner.spinner():
        actual_tilesets, not_tilesets = bq_client.exist_tilesets(names)
...
def info(ctx):
    """
    Get the current Google Cloud project and a valid token.
    """
