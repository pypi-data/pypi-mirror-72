...
from .settings import (CARTO_PARTITION_DEFAULT_STEP, CARTO_PARTITION_VALID_VERSIONS, DEFAULT_LAYER, LINE_SEPARATOR,
                       MAX_BIGQUERY_PARTITIONS, MBTILES_CREATE_STATEMENTS, MBTILES_METADATA_SCHEMA,
                       TILESET_VALID_VERSIONS)
    ...
    def tileset_to_csv(self):
        ...
        with open(csv_file_path, 'w') as file:
            ...
            for row in self.query('SELECT zoom_level, tile_column, tile_row, tile_data FROM tiles'):
                ...
                carto_partition = -CARTO_PARTITION_VALID_VERSIONS[-1]  # Setting a default `carto_partition`
    ...
    def parse_and_clean_metadata(self):
        ...
        self._metadata = {
            ...
            **{
                ...
                'version': TILESET_VALID_VERSIONS[-1]
            }
        }
        ...
        carto_partition_query = """
            SELECT q1.min_zoom_level, q1.max_zoom_level, MIN(q0.tile_column), MAX(q0.tile_column), MIN(q0.tile_row),
                    MAX(q0.tile_row)
                FROM tiles q0,
        ...
        self._metadata['carto_partition'] = {
            'version': CARTO_PARTITION_VALID_VERSIONS[-1],
        ...
        # Getting `vector_layers`, main layer and its id
