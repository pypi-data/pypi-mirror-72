...
class DNN:
    ...
    def init_session (self):
        if self.sess is not None:
            return
        if self.gpu:
            self.sess = tf.compat.v1.Session (graph = self.graph, config = tf.compat.v1.ConfigProto(gpu_options=tf.compat.v1.GPUOptions (per_process_gpu_memory_fraction = self.gpu), log_device_placement = False))
        else:
    ...
    def to_save_model (self, path, predict_def, inputs, outputs):
        ...
        tf.io.write_graph (self.graph.as_graph_def(), dirname, 'graph-def.txt', as_text = True)
        tf.io.write_graph (self.graph.as_graph_def(), dirname, 'graph-def.pb', as_text = False)
        if os.path.isfile (self.norm_file):
