...
from djangohotsauce.adapters import base
...
class LimitedStream(object):
    ...
    def read(self, size=None):
        if size is None:
            stream = self._read_limited()
            result = self.buffer.decode('utf8') + stream.decode('utf8')
            self.buffer = b''
    ...
    def readline(self, size=None):
        while b'\n' not in self.buffer and \
            ...
            if not chunk:
                break
            self.buffer += chunk
        sio = StringIO(self.buffer.decode('utf8'))
        if size:
...
class WSGIHandler(base.BaseHandler):
    ...
    def __call__(self, environ, start_response):
        ...
        if response.status_code == 304:
            # Remove Content-Type from the headers
