...
from frontiersman.client.TraderGui import TradeGui
#from frontiersman.client.GuiConstants import CARD_SIZE, SPACING

#assuming 1920 x 1080 to be res_full = 100.00
...
'''
BANK_SCALE = 1.0
INFO_WIDTH = int(320 * BANK_SCALE)
BANK_HEIGHT = int(100 * BANK_SCALE)
BANK_CARD_WIDTH = int((INFO_WIDTH - 8 * SPACING - BANK_HEIGHT) / 6 * BANK_SCALE)
BANK_HALF_HEIGHT = int((BANK_HEIGHT - 3 * SPACING) / 2 * BANK_SCALE)
PLAYER_HEIGHT = int(150 * BANK_SCALE)
PLAYER_SECTION_WIDTH = int((INFO_WIDTH - 4 * SPACING) / 5 * BANK_SCALE)
PLAYER_QUARTER_HEIGHT = int((PLAYER_HEIGHT - 2 * SPACING) / 4 * BANK_SCALE)
'''


class help_image(UIVerticalScrollBar):
...
class help_window(UIWindow):
    def __init__(self, position, ui_manager):
        ...
        self.help_image = help_image(ui_manager, container=self, parent_element=self)
...
class Client:
        ...
        if curr_player.hiddenVictoryPoints != 0:
            vp_value = pg_g.elements.UITextBox(
                relative_rect=pygame.Rect(vp_logo.relative_rect.left, vp_logo.relative_rect.bottom,
                                          PLAYER_SECTION_WIDTH, PLAYER_QUARTER_HEIGHT),
                container=curr_player_info,
                html_text=str(curr_player.victoryPoints) + '(' + str(
                    curr_player.victoryPoints + curr_player.hiddenVictoryPoints) + ')',
                manager=self.manager,
            )
        else:
            vp_value = pg_g.elements.UITextBox(
    ...
    def initialize_gui(self):
        ...
        screenstring = "localhost:0."+self.player.screen_num
        os.environ["DISPLAY"] = screenstring
        ...
        self.object_textures = {
            'Board': pygame.image.load('../assets/gameboard_grid_v4.png')
        }
        ...
        #prototype
        self.board_center = (
            RESOLUTION[0] / 2 - int(self.hex_size / 2) - 150, RESOLUTION[1] / 2 - int(self.hex_size / 2) - CARD_SIZE[1] / 2)
        ...
        if event.user_type == pg_g.UI_BUTTON_PRESSED:
            ...
            if event.ui_element == self.help_button:
                if self.help_window is None:
                    self.help_window = help_window((10, 10), self.manager)
                else:
    ...
    def draw_board(self):
        ...
        text_surface = self.font.render(INSTRUCTION_TEXT[self.board_action], False, (255, 255, 255))
        coord = self.translate_hex_to_xy(tile.location, self.board_center, int(self.hex_size / 2))
        ...
        for properties in self.road_list:
            ...
            x_off = self.road_length * math.sin(math.pi / 6)
            y_off = self.road_length * math.cos(math.pi / 6)
            ratio = math.sin(math.pi / 3)

            offset = [
                [-self.road_width * 4, -self.road_width * 4],
                [-12 * PROTOTYPE_SCALE, -14 * PROTOTYPE_SCALE],
                [-12 * PROTOTYPE_SCALE, -14 * PROTOTYPE_SCALE]
            ]
