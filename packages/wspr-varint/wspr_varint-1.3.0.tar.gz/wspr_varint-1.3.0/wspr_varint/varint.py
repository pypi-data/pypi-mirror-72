...
class VarInt:
    ...
    @classmethod
    def get_size_from_bytes(cls, encoding: bytes) -> int:
        """
        :param encoding:
        :return:
        """
        if encoding[0] & 0b11111100 == 0b11111000:
            return 1 + cls.get_size_from_bytes(encoding[1:])
        return cls.get_size_from_encoding(encoding[0])

    @staticmethod
    def get_size_from_encoding(encoding: bytes) -> int:
        """"""
        if encoding & 0b11111100 == 0b11111100:
            return 1
        elif encoding & 0b11111100 == 0b11111000:
            raise ValueError(f"When checking the length of a recursive VarInt please use 'get_size_from_bytes'")
        elif encoding & 0b11111100 == 0b11110100:
            return 9
        elif encoding & 0b11111100 == 0b11110000:
            return 5
        elif encoding & 0b11110000 == 0b11100000:
            return 4
        elif encoding & 0b11100000 == 0b11000000:
            return 3
        elif encoding & 0b11000000 == 0b10000000:
            return 2
        elif encoding & 0b10000000 == 0b00000000:
            return 1
        raise ValueError(f"No matching encoding for: '{encoding}'")

    @classmethod
    def from_unknown_length_bytes(cls, bytes_in: bytes) -> Tuple[VarInt, int]:
        """
        Use when the length of the varint is not known.
        This figures out how long the varint is supposed to be and then parses that part.
        The first byte needs to be the start of the variable integer.
        It returns the parsed VarInt and the length.
        """
        length = cls.get_size_from_bytes(bytes_in)
        if len(bytes_in) < length:
            raise ValueError(f"Not enough variable integer bytes provided for VarInt with encoding: '{bytes_in[0]}'")
        return cls.from_bytes(bytes_in[0:length]), length
