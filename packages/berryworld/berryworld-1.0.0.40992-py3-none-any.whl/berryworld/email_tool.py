...
class EmailTool:
	...
	@staticmethod
	def download_file(email_message, extension_files, detach_dir):
	...
	def retrieve_emails(self, unseen=True, folder='Inbox', emails_to_query=10, remove_sender=None, download_type=None,
	                    download_type_arg=None, extension_files=None, detach_dir='./'):
			...
			try:
				...
				for uid in emails_uid:
					...
					if all([sender.upper() not in send.upper() for send in remove_sender]):
						...
						received = pd.to_datetime(received_str[:-6], format='%a, %d %b %Y %H:%M:%S')
						...
						if download_files:
							...
							if (not (download_type == 'time')) | (
								...
								attachment_name = self.download_file(email_message, extension_files, detach_dir)
