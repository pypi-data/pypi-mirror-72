...
class AgentLifeCycle:
    ...
    def update(object, args):
        ...
        try:
            resp = requests.get("https://dev.cognicept.systems/api/v1/aws/assume_role/ecr", headers= headers, timeout = 5)
        ...
        if resp.status_code != 200:
            print('Cognicept REST API error: ' + resp.json()["message"])
            return        
            ...
            try:
                ...
                for status in docker_client.pull(image_name, stream = True, decode = True):
                    if("progress" not in status):
                        status["progress"] = ""
                    print('\r' + image_name + ' - ' + status["status"] + " " + status["progress"], end="")                    
                print(f"\r{image_name} - {bcolors.OKBLUE}OK{bcolors.ENDC}\033[K")                
            except docker.errors.ImageNotFound:
                print(f"\r{image_name} - {bcolors.FAIL}FAILED{bcolors.ENDC}\033[K")
