...
def get_proxies_by_data5u(order, is_random=1):
    ...
    if response.status_code != 200:
        raise Exception("status code: {}: {}".format(response.status_code, response.text))
...
def get_alexa_rank(domain):
    ...
    # 先判断域名是否在alexa排名中，如果不在，则尝试根据上级域名获取alexa排名，直至获取到排名为止，否则返回-1
    results = dict()
    for domain in domains:
        judge_domain = domain
        while True:
            if judge_domain in rank:
                results[domain] = rank[judge_domain]
                break
...
def download_file(url, path, timeout=None, allow_redirects=True, timelimit=None):
    """
    下载文件
    :param url: 文件下载链接
    :param path: 文件保存路径
    :param timeout: 连接超时与读取超时
    :param allow_redirects: 是否允许重定向
    :param timelimit: 总时间限制，用来一些超慢请求
    :return:
    """
    start_time = time.time()
    response = requests.get(url, timeout=timeout, allow_redirects=allow_redirects, stream=True)
