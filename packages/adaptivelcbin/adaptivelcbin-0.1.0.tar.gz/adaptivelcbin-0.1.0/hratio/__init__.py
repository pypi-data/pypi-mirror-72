...
class light_curve(observation):
    ...
    def read_from_fits(self, filename):
        ...
        self._gti_start_ = ff[gti_ind].data['STOP']
    ...
    def dump_info(self):
        logger.info("The light curve has %d bin"%(len(self._times_)))
    ...
    def make_rebin_table(self, min_sn, max_time, gti_threshold, min_bin_size, max_point_sn, single_point_check):
        ...
        GTI_START = self._gti_start_
        GTI_STOP = self._gti_stop_
                ...
                if not _x[i] >= GTI_START[curr_gti_ind] and _x[i] <= GTI_STOP[curr_gti_ind]:
                    ...
                    if curr_gti_ind > 0:
                        ...
                        if _x[i] - GTI_STOP[curr_gti_ind] <= gti_threshold:
                            logger.info("Event does not exceed the GTI gap\n")
                            gti_flag = 0
                        else:
                            logger.info("GTI Threshold exceeded Stop accumulation\n")
                            gti_flag = 1
