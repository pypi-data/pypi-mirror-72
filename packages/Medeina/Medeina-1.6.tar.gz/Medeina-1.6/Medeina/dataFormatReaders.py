...
def retrieveUserProvidedPairMetaData(df, graphType):
    expected = set(['interactionFormat','headingCorner','dataCorner','head','tail','nameDepth','path','metaData'])
    linkMetas = list(set(graphType.keys()) - expected)
    df['metas'] = df.apply(lambda x: {item: x[graphType[item]] for item in linkMetas if item in graphType},axis=1)
