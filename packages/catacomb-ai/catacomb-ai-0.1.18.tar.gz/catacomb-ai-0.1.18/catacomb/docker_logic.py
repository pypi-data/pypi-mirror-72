...
def build():
    ...
    with open('./catacomb-out/Dockerfile', 'w') as f:
        f.write(DOCKERFILE)
    with open('./catacomb-out/server.py', 'w') as f:
        f.write(SERVER)
    ...
    try:
        ...
        if ret_code != 0:
            raise Exception(f"Failed to build image. Return code: {ret_code}")
...
def login_if_required(client):
    if "DOCKER_USER" in os.environ and "DOCKER_PASSWORD" in os.environ:
        client.login(
            username=os.environ["DOCKER_USER"],
            password=os.environ["DOCKER_PASSWORD"],
            registry=os.getenv("DOCKER_REPO", default="registry.hub.docker.com")
        )
