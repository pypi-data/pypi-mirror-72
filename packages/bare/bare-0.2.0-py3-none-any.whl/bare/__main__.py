import argparse
import bare.parser
from bare.ast import BareType, BarePrimitive, StructType, TypeKind, BareEnum, UnionType
...
def _gen_type(type):
    ...
    if isinstance(type, UnionType):
        ...
        result += '\t@classmethod\n'
        result += '\tdef unpack(cls, member, data):\n'
        result += '\t\treturn bare.unpack(cls, data, member)\n\n'
        result += '\n'
        return result
...
def codegen(schema, output, indent, skip=None):
    ...
    result += 'import bare\n'
    result += 'from bare.ast import TypeKind, BarePrimitive, StructType, OptionalType, NamedType, ArrayType, MapType, UnionType\n\n\n'
