...
class LiteNetServer:
    ...
    def active_client_count(self):
        return threading.activeCount() - 2 if threading.activeCount() - 2 == len([addr if self.clients[addr]["active"] for addr in self.clients.keys()]) else (threading.activeCount() - 2, len([addr if self.clients[addr]["active"] for addr in self.clients.keys()]))
