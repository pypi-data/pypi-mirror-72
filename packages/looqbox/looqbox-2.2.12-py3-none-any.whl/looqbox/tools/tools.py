    ...
    for cols in cols_to_compare:
        try:
            if cols in by:
                ...
                d3["D_" + cols] = {rows: (d2[cols][rows] / d1.get(cols, 0)[rows]) - 1 for rows in d1[cols].keys() if rows not in different_keys }
        except TypeError:
    ...
    for cols in cols_to_compare:
        try:
            if cols in by:
                ...
                d3_total["D_" + cols] = (total2[cols] / total1.get(cols, 0)) - 1
        except TypeError:
