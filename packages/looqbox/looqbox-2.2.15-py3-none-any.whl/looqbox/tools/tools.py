...
def data_comp(table1, table2, by=None, cols_to_compare=None, null_as="-"):
    """
    ...
    for cols in cols_to_compare:
        try:
            if cols in by:
                ...
                d3["∆" + cols] = {rows: d2[cols][rows] - d1.get(cols, 0)[rows] for rows in d1[cols].keys() if
                                  rows not in different_keys}
                d3["∆%" + cols] = {rows: (d2[cols][rows] / d1.get(cols, 0)[rows]) - 1 for rows in d1[cols].keys() if
                                   rows not in different_keys}
        except TypeError:
