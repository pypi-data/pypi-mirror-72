...
from azureml.train.hyperdrive.exceptions import HyperDriveConfigException, HyperDriveRehydrateException
...
def _sampling_from_dict(dict_sampling):
    ...
    if not isinstance(dict_sampling, dict):
        raise HyperDriveRehydrateException("Expecting 'dict' type for Sampling dictionary.")

    if "name" not in dict_sampling:
        raise HyperDriveRehydrateException("Couldn't find 'name' entry.")
    ...
    raise HyperDriveRehydrateException("Unknown sampling")
...
class HyperParameterSampling(ABC):
    ...
    def _validate_dict_sampling(dict_sampling, name):
        if not isinstance(dict_sampling, dict) or \
            ...
            raise HyperDriveRehydrateException("Invalid arguments for _validate_dict_sampling method.")

        if "parameter_space" not in dict_sampling:
            raise HyperDriveRehydrateException("Couldn't find 'parameter_space' entry.")
...
class BayesianParameterSampling(HyperParameterSampling):
    ...
    def __init__(self, parameter_space):
        ...
        if parameter_space is None:
            raise HyperDriveConfigException("parameter_space cannot be None")
