...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':ujQhp(perm='m',auth=ujQhN,arg=ujQhN,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=ujQhN)
 time.sleep(3)
def do_start_ftp(port,asynchronous=ujQhN):
...
def main():
 do_start_ftp(ujQhk(sys.argv[1]),asynchronous=ujQhP)
