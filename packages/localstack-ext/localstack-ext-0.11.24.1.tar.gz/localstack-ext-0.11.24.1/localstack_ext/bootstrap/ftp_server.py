...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':xFkNi(perm='m',auth=xFkNU,arg=xFkNU,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=xFkNU)
 time.sleep(3)
def do_start_ftp(port,asynchronous=xFkNU):
...
def main():
 do_start_ftp(xFkNp(sys.argv[1]),asynchronous=xFkNr)
