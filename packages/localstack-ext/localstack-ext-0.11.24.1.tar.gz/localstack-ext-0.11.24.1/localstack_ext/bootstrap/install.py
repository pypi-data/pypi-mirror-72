...
def install_postgres():
 if not in_docker():
  return
 try:
  run('which postgres',print_error=UhHcd)
 except UhHca:
  install_alpine_package('postgresql','RDS')
 try:
  run('which pg_config',print_error=UhHcd)
 except UhHca:
  install_alpine_package('postgresql-dev','RDS')
def install_redis():
 try:
  run('which redis-server',print_error=UhHcd)
 except UhHca:
  install_alpine_package('redis','ElastiCache')
 return 'redis-server'
def install_mqtt():
 try:
  run('which mosquitto',print_error=UhHcd)
 except UhHca:
