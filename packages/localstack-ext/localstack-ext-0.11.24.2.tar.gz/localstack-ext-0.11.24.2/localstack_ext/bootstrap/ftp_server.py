...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':yPXTC(perm='m',auth=yPXTE,arg=yPXTE,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=yPXTE)
 time.sleep(3)
def do_start_ftp(port,asynchronous=yPXTE):
...
def main():
 do_start_ftp(yPXTW(sys.argv[1]),asynchronous=yPXTx)
