...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':AnyfQ(perm='m',auth=Anyfr,arg=Anyfr,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=Anyfr)
 time.sleep(3)
def do_start_ftp(port,asynchronous=Anyfr):
...
def main():
 do_start_ftp(Anyfm(sys.argv[1]),asynchronous=Anyfb)
