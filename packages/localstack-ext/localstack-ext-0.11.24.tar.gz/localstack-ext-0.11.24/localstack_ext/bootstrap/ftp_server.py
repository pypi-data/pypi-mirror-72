...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':uwlDR(perm='m',auth=uwlDA,arg=uwlDA,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  do_start_ftp(asynchronous=uwlDA)
 time.sleep(3)
def do_start_ftp(port,asynchronous=uwlDA):
...
def main():
 do_start_ftp(uwlDm(sys.argv[1]),asynchronous=uwlDn)
