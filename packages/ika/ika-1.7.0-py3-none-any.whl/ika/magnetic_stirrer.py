...
from ika.utilities.data_analysis_visualization import stable_against_known, \
    find_stable_against_known_custom_one_window, plot_axes, plot_plateau
...
class MagneticStirrer(Runnable):
    ...
    READ_RATED_SET_SAFETY_TEMPERATURE_VALUE = "IN_SP_3"
    ...
    def __init__(self,
        ...
        :param save_folder: path to folder to save data from running the hotplate
        ...
        self.datetime_manager = datetimeManager(string_format=self.datetime_format)  # datetime format for data
        # recording
        self.time_column_heading = f'Time ({self.datetime_format})'
        self.columns = [self.time_column_heading,
                        self.time_s_column_heading,
                        self.time_min_column_heading,
                        self.time_hour_column_heading,
                        self.probe_temperature_column_heading,
                        self.plate_surface_temperature_column_heading,
                        self.target_temperature_column_heading,
                        self.stir_rate_column_heading,
                        self.target_stir_rate_column_heading,
                        ]
        ...
        self.save_folder = save_folder if save_folder is not None else Path.cwd()
        self.state_csv_path: Path = None
        self.command_csv_path: Path = None
        self.state_data: pd.DataFrame = None
        self.command_data: pd.DataFrame = None
        self.set_up_save_folder(save_folder=save_folder)

        self.monitor_temperature_thread = None
    ...
    def set_up_save_folder(self, save_folder: Path):
        self.save_folder = save_folder if save_folder is not None else Path.cwd()
        self.state_csv_path = self.save_folder.joinpath('ika_state_data.csv')
        self.command_csv_path = self.save_folder.joinpath('ika_command_data.csv')
        self.state_data: pd.DataFrame = None
        self.command_data: pd.DataFrame = None
        self.set_up_data()

    def set_up_data(self):
        start_time: str = self.datetime_manager.now_string()
    ...
    def set_up_state_data(self, start_time) -> None:
        ...
        +-----------------------------+----------+------------+-------------+-----------------+
        | Time (self.datetime_format) | Time (s) | Time (min) | Time (hour) | Temperature (C) |
        +-----------------------------+----------+------------+-------------+-----------------+
        |          datetime_1         |     0    |      0     |      0      |      TEMP_1     |
        +-----------------------------+----------+------------+-------------+-----------------+
        |          datetime_2         |    30    |     0.5    |    0.0083   |      TEMP_2     |
        +-----------------------------+----------+------------+-------------+-----------------+
        |          datetime_3         |    60    |      1     |    0.0167   |      TEMP_3     |
        +-----------------------------+----------+------------+-------------+-----------------+
        ...
        plate_surface_temperature_column_heading = self.hotplate_sensor_temperature
        target_temperature_column_heading = self.target_temperature
        stir_rate_column_heading = self.stir_rate
        target_stir_rate_column_heading = self.target_stir_rate

        add_data = {self.time_column_heading: start_time,
                    self.time_s_column_heading: 0,
                    self.time_min_column_heading: 0,
                    self.time_hour_column_heading: 0,
                    self.probe_temperature_column_heading: probe_temperature,
                    self.plate_surface_temperature_column_heading: plate_surface_temperature_column_heading,
                    self.target_temperature_column_heading: target_temperature_column_heading,
                    self.stir_rate_column_heading: stir_rate_column_heading,
                    self.target_stir_rate_column_heading: target_stir_rate_column_heading,
                    }
        df = df.append(add_data, ignore_index=True)
        self.state_data = df
        try:
            df.to_csv(self.state_csv_path, sep=',', index=False, mode='w')
    ...
    def set_up_command_data(self, start_time) -> None:
        ...
        columns = [self.time_column_heading, 'Time (s)', 'Time (min)', 'Time (hour)', 'Command']
        df = pd.DataFrame(columns=columns)
        add_data = {self.time_column_heading: start_time,
                    'Time (s)': 0,
                    'Time (min)': 0,
        ...
        df = df.append(add_data, ignore_index=True)
        self.command_data = df
        try:
            df.to_csv(self.command_csv_path, sep=',', index=False, mode='w')
        except PermissionError as e:
            pass

    def save_csv_files(self):
        self.save_state_csv()
        self.save_command_csv()

    def save_state_csv(self):
        state_data = self.state_data
        state_data = state_data.drop_duplicates(subset=self.time_s_column_heading)
        try:
            state_data.to_csv(self.state_csv_path, sep=',', index=False, mode='w')
        except PermissionError as e:
            pass

    def save_command_csv(self):
        command_data = self.command_data
        command_data = command_data.drop_duplicates(subset=self.time_s_column_heading)
        try:
            command_data.to_csv(self.command_csv_path, sep=',', index=False, mode='w')
        except PermissionError as e:
            pass
    ...
    def register_state(self) -> None:
        ...
        current_time_str: str = self.datetime_manager.now_string()
        current_time: datetime = datetimeManager.str_to_datetime(current_time_str, datetime_format=self.datetime_format)[0]
        first_row = self.state_data.head(1)
        start_time: str = first_row[self.time_column_heading][0]
        row_index = len(self.state_data) - 1
        last_row = self.state_data.tail(1)
        last_time: str = last_row[self.time_column_heading][row_index]
        last_time: datetime = datetimeManager.str_to_datetime(last_time,
                                                              datetime_format=self.datetime_format)[0]
        if (current_time - last_time).seconds < self.record_data_interval:
            return

        start_time: datetime = datetimeManager.str_to_datetime(start_time,
                                                               datetime_format=self.datetime_format)[0]
        times = [start_time, current_time]

        relative_s = datetimeManager.relative_datetime(times,
                                                       units=self._unit_seconds,
                                                       rounding=self.datetime_manager_rounding)[1]
        relative_min = datetimeManager.relative_datetime(times,
                                                         units=self._unit_minutes,
                                                         rounding=self.datetime_manager_rounding)[1]
        relative_hour = datetimeManager.relative_datetime(times,
                                                          units=self._unit_hours,
                                                          rounding=self.datetime_manager_rounding)[1]

        add_data = {self.time_column_heading: current_time_str,
                    self.time_s_column_heading: relative_s,
                    self.time_min_column_heading: relative_min,
                    self.time_hour_column_heading: relative_hour,
                    self.probe_temperature_column_heading: probe_temperature,
                    self.plate_surface_temperature_column_heading: plate_surface_temperature_column_heading,
                    self.target_temperature_column_heading: target_temperature_column_heading,
                    self.stir_rate_column_heading: stir_rate_column_heading,
        ...
        self.state_data = self.state_data.append(add_data, ignore_index=True)

        nrows = len(self.state_data)
    ...
    def register_command(self, command: str) -> None:
        current_time_str: str = self.datetime_manager.now_string()
        current_time: datetime = datetimeManager.str_to_datetime(current_time_str, datetime_format=self.datetime_format)[0]
        first_row = self.command_data.head(1)
        start_time: str = first_row[self.time_column_heading][0]
        start_time: datetime = datetimeManager.str_to_datetime(start_time,
                                                               datetime_format=self.datetime_format)[0]
        times = [start_time, current_time]
        ...
        add_data = {self.time_column_heading: current_time_str,
                    'Time (s)': relative_s,
                    'Time (min)': relative_min,
                    'Time (hour)': relative_hour,
                    'Command': command,
                    }
        self.command_data = self.command_data.append(add_data, ignore_index=True)
        self.save_command_csv()
    ...
    def wait_until_temperature_stable(self,
        ...
        """
        Check for when the temperature has stabilized, and pause code execution until this happes or when
        timeout number of seconds has passed. Every measurement takes about 1 second.

        :param float, time_out: every measurement takes roughly 1 second. The time_out value is the maximum number
            seconds after this function has been started that it can run for until it must return. This is to prevent
            the function from holding something up for too long
        ...
        start_time: datetime = datetimeManager.str_to_datetime(start_time,
                                                               datetime_format=self.datetime_format)[0]
        while True:
            current_time: str = self.datetime_manager.now_string()
            current_time: datetime = datetimeManager.str_to_datetime(current_time,
                                                                     datetime_format=self.datetime_format)[0]
            seconds_passed: timedelta = datetimeManager.relative_datetime([start_time, current_time],
                                                                          units=datetimeManager._unit_seconds,
                                                                          rounding=1)[1]
            if seconds_passed > time_out:
                ...
                time.sleep(1)
                self.register_state()
                times = self.state_data['Time (min)'].tolist()
                temperatures = self.state_data[self.probe_temperature_column_heading].tolist()
                if len(times) < n:
                    continue
                else:
                    last_n_x_values = times[-n:]
                    last_n_y_values = temperatures[-n:]
                    stable = self.stable(
                        x=last_n_x_values,
                        y=last_n_y_values,
                    )
...
class MagneticStirrerVisual(MagneticStirrer):
    ...
    def save_graph(self,
        ...
        if file_name is None:
            file_name = 'ika_temperature_time_graph.png'
        if file_name[-4:] != '.png':
            file_name += '.png'
        graph_save_path: Path = self.save_folder.joinpath(file_name)
