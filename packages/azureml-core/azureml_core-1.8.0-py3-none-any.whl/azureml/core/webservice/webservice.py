...
class Webservice(ABC):
    ...
    @staticmethod
    def deploy_from_image(workspace, name, image, deployment_config=None, deployment_target=None, overwrite=False):
        """
    ...
    @staticmethod
    def _check_for_existing_webservice(workspace, name, overwrite=False):
        if not overwrite:
            try:
...
class WebServiceAccessToken(object):
    ...
    def deserialize(payload_obj):
        ...
        :rtype: :class:`azureml.core.webservice.WebServiceAccessToken`
        """
