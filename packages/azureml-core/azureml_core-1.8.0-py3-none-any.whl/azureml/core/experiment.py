...
class Experiment(ChainedIdentity, HasExperimentPortal):
    ...
    def reactivate(self, new_name=None):
        ...
        .. remarks::
            ...
            the same name. In the case of a name conflict, the archived experiment can be renamed when
            calling **reactivate** by passing new_name.

        :param new_name: Optional new name for renaming an archived experiment during reactivation.
        :type new_name: str
        """
        updated_experiment = self._workspace_client.reactivate_experiment(experiment_id=self._id, new_name=new_name)
        self._archived_time = updated_experiment.archived_time
