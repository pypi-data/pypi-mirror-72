...
class ImageBuildDetails(object):
    ...
    def status(self):
        ...
        return self.environment_client._get_cloud_image_build_status(self.build_id)["status"]
...
please set exist_ok=True""".format(wheel_name, wheel_name)
    ...
    def build(self, workspace, image_build_compute=None):
        ...
        environment_client = EnvironmentClient(workspace.service_context)
        try:
            image_build_details = environment_client._start_cloud_image_build(
        ...
        return ImageBuildDetails(environment_client, log_url, build_id)
    ...
    def build_local(self, workspace, platform=None, **kwargs):
        ...
        .. remarks::
            ...
            To build a docker image set `useDocker=True`
            To puch a result image into AzureML workspace container registry after image build completion set
