...
class Settings(dict):
    ...
    def set_dependent_defaults(self, only_missing=True):
        ...
        if should_update("pp_model_slice"):
            self["pp_model_slice"] = list(range(self.dim))[:2]
            if len(self["pp_model_slice"]) == 1:
                self["pp_model_slice"].append(self["pp_model_slice"][0])
                self["pp_model_slice"].append(50)
            else:
                self["pp_model_slice"].append(25)
        else:
            self["pp_model_slice"][0] -= 1  # user expected not to use python indexing
            self["pp_model_slice"][1] -= 1
        self["pp_model_slice"] = np.asarray(self["pp_model_slice"])
