    ...
    def load_data(self):
        ...
        if bo_results is not None:
            ...
            if 0 < minfreq < totalpts:
                imins = np.arange(initpts - 1, totalpts - 1, minfreq)
                imins = np.append(imins, totalpts - 1)
            else:
                imins = np.array([totalpts - 1])

            min_preds = np.c_[
                imins,
                np.concatenate(bo_results.convergence[:, dim + 1]),
                bo_results.convergence[:, dim + 2 :],
            ]

        else:
            rst_data = parse.parse_input_file(
                self.rstfile, skip="keywords", split_rst_data=True
            )
            acqs = rst_data["acqs"]
            mod_par = rst_data["mod_par"]
