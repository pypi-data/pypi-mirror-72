...
class Lock:
    ...
    :param lifetime: The expected maximum lifetime of the lock, as a
        timedelta or integer number of seconds.  Defaults to 15 seconds.
    ...
    def refresh(self,
        ...
        :param lifetime: If given, this sets the lock's new lifetime.
    ...
    def lock(self, timeout: Optional[Interval] = None) -> None:
        ...
        while True:
            ...
            if (release_time != -1 and
                    datetime.now() > release_time + CLOCK_SLOP):
    ...
    def transfer_to(self, pid: int) -> None:
        ...
        self._touch()
        # Get the current claim's file name.
        winner = self._read()
    ...
    def _set_claimfile(self, pid: Optional[int] = None) -> None:
    ...
    @retry_errnos.setter
    def retry_errnos(self, errnos: List[int]):
        self._retry_errnos = []
