...
def get_installed_packages() -> Set[str]:
    installed_special_reqs = []
    for special_req in SPECIAL_REQUIREMENTS:
        try:
            import_module(special_req)
            installed_special_reqs.append(special_req)
        except:
            print(f"Missing {special_req}")
            pass

    return set([p.project_name for p in pkg_resources.working_set] + installed_special_reqs)
