...
def _load_rules(runner, params=None, check_signature=True, storage=runtime):
    ...
    if runner.interface_manager is not None:
        ...
        callbacks.extend(chain.from_iterable(
            runner.interface_manager.call_all("instrumentation_callbacks", runner, storage)))
...
def features_change(runner, params):
    ...
    if use_signals is not None:
        runner.set_signal_client(
            params.get("batch_size", 0), params.get("max_staleness", 0),
    ...
    if params.get("batch_size") is not None:
        runner.set_deliverer(
            ...
            use_signals=use_signals or False,
        )
...
def tracing_enable(runner, params):
    ...
    if not isinstance(tracing_identifier, str) or not isinstance(tracing_scope_definition, Mapping):
        return {
            "status": False,
