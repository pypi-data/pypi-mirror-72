...
def igwn_frames(scope, name, detector, dataset, gps_prefix):
    """
    Map the GWF files according to the modern IGWN schema:

    O3GK:K-K1_C10-1268039680-4096.gwf ->
...
def igwn_sfts(scope, name):
    """
    Map SFT files using their metadata
    """
    # metadata: datatype, observatory, calibration, window

    # Get metadata
    client = Client()
    did_meta = client.get_did_meta(scope, name)
    try:
        datatype = did_meta['datatype']
    except KeyError:
        raise KeyError("%s:%s has no datatype metadata" % (scope, name))
    try:
        obs = did_meta['obs']
    except KeyError:
        raise KeyError("%s:%s has no obs metadata" % (scope, name))
    try:
        calibration = did_meta['calibration']
    except KeyError:
        raise KeyError("%s:%s has no calibration metadata" % (scope, name))
    try:
        window = did_meta['window']
    except KeyError:
        raise KeyError("%s:%s has no window metadata" % (scope, name))
    try:
        gps_start = did_meta['gps-start-time']
    except KeyError:
        raise KeyError("%s:%s has no gps-start-time metadata" % (scope, name))
    gps_prefix = gps_start[:4]

    dir_hash = "%s-%s-%s" % (obs, datatype, gps_prefix)

    pfn = "%s/sfts/%s/%s/%s/%s/%s" % (scope, obs, calibration, window,
                                      dir_hash, name)

    return pfn


def igwn(scope, name, rse, rse_attrs, proto_attrs):
    """
    ...
    if extension == 'gwf':
        pfn = igwn_frames(scope, name, detector, dataset, gps_prefix)
    elif extension == 'sft':
        pfn = igwn_sfts(scope, name)
    else:
...
if __name__ == '__main__':
    ...
    def test_mapping(algorithm, scope, name, pfn):
        ...
        if algorithm.__name__ not in [
                'ligo_legacy', 'kagra_offline', 'igwn_frames'
        ]:
    ...
    test_mapping(igwn_frames, "O3GK", "K-K1_C10-1268039680-4096.gwf",
                 "O3GK/K1_C10/K-K1_C10-126/K-K1_C10-1268039680-4096.gwf")
