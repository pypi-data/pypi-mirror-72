...
def login(url,config):
    ...
    inforlogin.update(ti,access_token, expires_in, refresh_token, token_type,start_session,session_expire,saak,sask,client_id,client_secret)
...
def reconnect(url,headers):
    ...
    #print (" Old session expired")
    #print (headers)

    headers = header2(r['access_token'])
...
def header2(token):
    ...
    headers = {
        'Content-Type':         'application/json',
        'X-TenantId':           inforlogin._GLOBAL_ti,
        'Connection':           'keep-alive',
        'Host':                 'mingle-ionapi.eu1.inforcloudsuite.com',
        'Accept-Encoding':      'gzip, deflate, br',
        'Cache-Control':        'no-cache',
        'User-Agent':           'FellowConsultingAGRuntime/7.22.0',
        'X-ClientId':           inforlogin._GLOBAL_client_id,
        'Accept':               '*/*',
