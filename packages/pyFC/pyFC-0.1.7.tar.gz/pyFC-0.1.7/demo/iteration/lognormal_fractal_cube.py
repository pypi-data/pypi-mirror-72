...
if plot:
    ...
    if history:
        for fci in fc.yield_cubes():
            pyFC.plot_field_stats(fci, scaling='log', cmap=cmap, plottype=plottype, vmin=-1.9, vmax=1.9)
            if saveplot:
                if plottype == 'imshow':
    ...
    # Or just the final one
        ...
        fc.gen_cube()
        pyFC.plot_field_stats(fc, scaling='log', cmap=cmap, plottype=plottype, vmin=-1.9, vmax=1.9)
        if saveplot:
            if plottype == 'imshow':
                pl.savefig(pt.unique_fname(pname1))
                pl.savefig(pt.unique_fname(pname2))
