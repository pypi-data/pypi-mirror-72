...
class ParameterisedCircuit(QuantumCircuit, ABC):
    ...
    def get_parameter_resolver(self, vals:np.ndarray) -> Union['cirq.ParamResolver',List['cirq.ParamResolver']]:
        ...
        resolver = [ParamResolver({param:value for param, value in 
                                   np.column_stack((self.parameters,val))}) for val in vals]
        if self.expr_map:
            resolver = [self.expr_map.transform_params(params) for params in resolver]
    ...
    def assemble(self):
        ...
        cq_flat, expr_map = cirq.flatten(cq)
        self += cq_flat
