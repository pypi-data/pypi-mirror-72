...
from .enums import SignalByteOrder, SignalMultiplexMode, AttributeOwner, SignalType
...
class Signal(object):
    ...
    def __init__(self, db, message, sh,
                 name=None, type=None,
                 byte_order=None,
                 mode=None,
                 representation=None,
                 size=None,
                 scaling=None,
                 limits=None, unit=None, comment=None):
    ...
    def __repr__(self):
        return ("Signal(name={!r}, type={!r}, byte_order={!r}, mode={!r}, size={!r}, "
                "scaling={!r}, limits={!r}, unit={!r}, comment={!r})".
                format(self.name, self.type, self.byte_order, self.mode,
                       self.size, self.scaling, self.limits, self.unit,
                       self.comment))
    ...
    def data_from(self, can_data, phys=None, raw=None):
        ...
        if phys is not None:
            ...
            dll.kvaDbStoreSignalValuePhys(
                self._handle, c_buf, ct.sizeof(c_buf), c_phys)
        if raw is not None:
            c_raw = ct.c_uint64(raw)
            dll.kvaDbStoreSignalValueRaw64(
                self._handle, c_buf, ct.sizeof(c_buf), c_raw)
    ...
    def qualified_name(self):
        ...
        """
        buf = ct.create_string_buffer(255)
        dll.kvaDbGetSignalQualifiedName(
    ...
    def phys_from(self, can_data):
        ...
        c_phys = ct.c_double()
        c_buf = (ct.c_char * len(can_data)).from_buffer_copy(can_data)
        dll.kvaDbRetrieveSignalValuePhys(
            self._handle, ct.byref(c_phys), c_buf, ct.sizeof(c_buf))
        return c_phys.value
    ...
    def raw_from(self, can_data):
        ...
        raw = ct.c_uint64()
        c_buf = (ct.c_char * len(can_data)).from_buffer_copy(can_data)
        dll.kvaDbRetrieveSignalValueRaw64(
            self._handle, ct.byref(raw), c_buf, ct.sizeof(c_buf))
        return raw.value
    ...
    def scaling(self):
        ...
        dll.kvaDbGetSignalValueScaling(
            self._handle, ct.byref(c_factor), ct.byref(c_offset))
        return ValueScaling(factor=c_factor.value, offset=c_offset.value)

    @scaling.setter
    def scaling(self, value):
        """`ValueScaling`: Set the signals factor and offset"""
        dll.kvaDbSetSignalValueScaling(
            self._handle, value.factor, value.offset)
    ...
    def size(self):
        ...
        c_length = ct.c_int(0)
        dll.kvaDbGetSignalValueSize(
            self._handle, ct.byref(c_startbit), ct.byref(c_length))
    ...
    def size(self, value):
        ...
        """
        dll.kvaDbSetSignalValueSize(
            self._handle, value.startbit, value.length)
    ...
    def limits(self):
        ...
        c_min = ct.c_double(0)
        c_max = ct.c_double(0)
        dll.kvaDbGetSignalValueLimits(
            self._handle, ct.byref(c_min), ct.byref(c_max))
    ...
    def limits(self, value):
        ...
        """
        dll.kvaDbSetSignalValueLimits(
            self._handle, value.min, value.max)
...
class EnumSignal(Signal):
    ...
    def __init__(self, db, message, sh,
                 ...
                 limits=None, unit=None, comment=None, enums={}):
        super(EnumSignal, self).__init__(db, message=message,
                                         sh=sh, name=name, type=type,
                                         byte_order=byte_order, mode=mode,
                                         size=size, scaling=scaling,
    ...
    def add_enum_definition(self, enums):
        ...
        for key, value in enums.items():
            ...
            dll.kvaDbAddEnumValue(
                self._handle, c_value, c_key)
