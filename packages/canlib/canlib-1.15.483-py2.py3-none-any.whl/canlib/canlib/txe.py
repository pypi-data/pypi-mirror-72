...
class Txe(object):
    ...
    def _size_of_item_data(self, item):
        ...
        dll.kvScriptTxeGetData(self._c_path, item, None, buffer_size)  # pylint: disable=maybe-no-member
    ...
    def _read_item(self, item, expected_buffer_type):
        ...
        dll.kvScriptTxeGetData(self._c_path, item, data, buffer_size)  # pylint: disable=maybe-no-member
    ...
    def description(self):
        ...
        return self._read_variable_size_item(TxeDataItem.DESCRIPTION).rstrip(b'\x00').decode('utf-8')
