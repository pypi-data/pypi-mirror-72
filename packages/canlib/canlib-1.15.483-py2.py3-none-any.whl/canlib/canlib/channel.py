...
from .enums import MessageFlag, DeviceMode, Driver, Open, ScriptRequest, Stat, ScriptStatus
from .enums import ScriptStop
from .envvar import EnvVar
from .exceptions import CanError
from .structures import CanBusStatistics
...
def openChannel(channel, flags=0, bitrate=None, data_bitrate=None):
    ...
    Args:
        ...
        bitrate (canBITRATE_XXX or canFD_BITRATE_XXX): The desired
            bitrate. Must be one of the predefined canBITRATE_XXX (for classic
            CAN) or canFD_BITRATE_XXX (for CAN FD) constants,
            e.g. `canBITRATE_1M` and `canFD_BITRATE_1M_80P`. For CAN FD, this
            is the arbitration bitrate.

        data_bitrate (canFD_BITRATE_XXX): The desired data bitrate for CAN
            FD. Must be one of the predefined canFD_BITRATE_XXX constants,
            e.g. `canFD_BITRATE_500K_80P`. This is only valid when opening a
            CAN FD channel.
...
class Channel(object):
    ...
    def scriptEnvvarOpen(self, name):
        ...
        This should normally not by used directly, instead opening and closing
        ...
        envHandle = dll.kvScriptEnvvarOpen(self.handle, envvarName,
                                           ct.byref(envvarType),
                                           ct.byref(envvarSize))
    ...
    def scriptEnvvarSetInt(self, envHandle, value):
        ...
        This should normally not by used directly, instead set and get the
    ...
    def fileCopyToDevice(self, hostFileName, deviceFileName=None):
        ...
        dll.kvFileCopyToDevice(self.handle, hostFileName.encode('utf-8'),
                               deviceFileName.encode('utf-8'))
    ...
    def fileCopyFromDevice(self, deviceFileName, hostFileName=None):
        ...
        dll.kvFileCopyFromDevice(self.handle, deviceFileName.encode('utf-8'),
                                 hostFileName.encode('utf-8'))
