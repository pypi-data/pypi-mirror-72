...
_ct_dll = dllLoader.load_dll(win_name='linlib.dll',
                             linux_name='liblinlib.so')
...
def getChannelData(channel_number, item=ChannelData.CARD_FIRMWARE_REV):
    ...
    if item != ChannelData.CARD_FIRMWARE_REV:
        raise NotImplementedError(
            "linlib.getChannelData only supports getting CARD_FIRMWARE_REV")
...
def getTransceiverData(channel_number):
    ...
    ttype = ct.c_int()
    dll.linGetTransceiverData(
        channel_number, ean, serial, ct.byref(ttype))
