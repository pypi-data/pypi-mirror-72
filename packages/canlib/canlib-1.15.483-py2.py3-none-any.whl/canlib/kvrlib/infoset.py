...
from .discovery import stored_devices, store_devices, DeviceInfo, start_discovery
...
class DeviceInfoSet(MutableSet):
    ...
    def add(self, info):
        ...
        if not isinstance(info, DeviceInfo):
            raise TypeError("{cls} objects can only contain DeviceInfo objects".format(
    ...
    def find(self, ean, serial):
        ...
        if elemid in self._elements:
            ...
            raise DeviceNotInSetError("No device info matching ean={ean} and serial={serial} found".format(
