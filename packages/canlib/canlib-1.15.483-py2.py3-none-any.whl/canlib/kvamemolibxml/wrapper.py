...
_ct_dll = dllLoader.load_dll(win_name='kvaMemoLibXML.dll',
                             linux_name='libkvamemolibxml.so')
...
class KvaMemoLibXml(object):
    ...
    kvaMemoLibXmlDll = dllLoader.load_dll(win_name='kvaMemoLibXML.dll',
                                          linux_name='libkvamemolibxml.so')
...
def xmlGetValidationStatusCount():
    ...
    dll.kvaXmlGetValidationStatusCount(
        ct.byref(countErr), ct.byref(countWarn))
...
def xmlGetValidationError():
    ...
    dll.kvaXmlGetValidationError(
        ct.byref(validationStatus), text, len(text))
...
def xmlGetValidationWarning():
    ...
    validationStatus = ct.c_int(-666)
    text = ct.create_string_buffer(1048)
    dll.kvaXmlGetValidationWarning(
        ct.byref(validationStatus), text, len(text))
    return (validationStatus.value, text.value.decode())
