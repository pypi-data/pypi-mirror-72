...
class Property(CEnum):
    ...
    SIGNAL_BASED = 1001  # Writes signals and not data frames. Used only with kvlcIsPropertySupported().
    SHOW_SIGNAL_SELECT = 1002  # Requires a database. Used only with kvlcIsPropertySupported().
    ATTACHMENTS = 1003  # Can attach files to converted data. Use kvlcAttachFile() to add a file. Used only with kvlcIsPropertySupported().
