...
from airflow.contrib.auth.backends.password_auth import generate_token,certify_token
...
# 控制访问者权限
def visitor_control(fun):
    def swapper(*args,**kwargs):
        if current_user.is_visitor():
            flash("This requires Admin privileges", "error")
...
def dag_link(v, c, m, p):
    ...
    url = url_for(
        ...
        # execution_date=m.last_scheduler_run)
    return Markup(
...
class Airflow(BaseView):
    ...
    def index(self):
        if current_user.is_visitor():
            return self.render('airflow/visitor_dags.html')
    ...
    def login(self):
        if settings.P_URL:
            return redirect(settings.P_URL + '/etl/airflow')
        else:
    ...
    def dag_edit(self):
        if settings.P_URL:
            return redirect(settings.P_URL + '/admin/etl/dagsetup/')
        else:
            flash('You should setup your p_url in aiflow.cfg')
            return redirect('/admin')

    @expose('/org_login', methods=['GET', 'POST'])
    def org_login(self):
        return airflow.login.login(self, request)

    # 新增获取token登录
    @expose('/gettoken', methods=['GET'])
    def get_EmbedToken(self):
        APPKEY = settings.APPKEY
        if request.method == 'GET' and request.args.get('token') and request.args.get('user'):
            token = request.args.get('token')
            user = request.args.get('user')
            if APPKEY == token:
                EmbedToken = generate_token(APPKEY+user, 60)
                return EmbedToken
            else:
                return 'Wrong Key!'
        else:
    ...
    def logout(self):
        ...
        # return redirect(url_for('admin.index'))
        return redirect('/admin/airflow/org_login')

    # 外部登录
    @expose('/quick_login',methods=['GET'])
    def quick_login(self):
        return airflow.login.quick_login(self,request)
    ...
    def tree(self, session=None):
        ...
        form = DateTimeWithNumRunsForm(data={'base_date': max_date,
                                             'num_runs': num_runs})
        # 由于我们只用python operator, 变更
        operators = {}
        for op in dag.tasks:
            if hasattr(op,'driver'):
                operators[op.driver] = op.ui_color
            else:
                operators[op.__class__.__name__] = op.ui_color
        if current_user.is_visitor():
            template = 'airflow/visitor_tree.html'
        else:
            template = 'airflow/tree.html'
    ...
    def graph(self, session=None):
        ...
        for op in dag.tasks:
            if hasattr(op,'driver'):
                ...
                operators[op.__class__.__name__] = op.ui_color
        if current_user.is_visitor():
            template = 'airflow/visitor_graph.html'
        else:
            template = 'airflow/graph.html'
        return self.render(
            template,
            dag=dag,
            form=form,
...
class HomeView(AdminIndexView):
    ...
    def index(self, session=None):
        ...
        if do_filter and owner_mode == 'ldapgroup':
            ...
            if current_user.get_authority():
                temp = current_user.get_authority().split(',')
            else:
                temp = ''
            sql_query = sql_query.filter(
                ~DM.is_subdag, DM.is_active,
                # DM.owners == current_user.user.username
                DM.owners.in_(temp if temp else [current_user.user.username])
            )
        else:
            sql_query = sql_query.filter(
                ~DM.is_subdag, DM.is_active
        ...
        if do_filter and owner_mode == 'ldapgroup':
            ...
            if current_user.get_authority():
                # print(current_user.user.username,current_user.get_authority())
                temp = current_user.get_authority().split(',')
            else:
                temp = ''
            ...
            webserver_dags = {
                ...
                if dag.owner in (temp if temp else current_user.user.username)
            }
        else:
            webserver_dags = {
                dag.dag_id: dag
...
class VariableView(wwwutils.DataProfilingMixin, AirflowModelView):
    ...
    column_list = ('key', 'val', 'is_encrypted','remark')
