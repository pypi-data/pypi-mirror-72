...
class Library(CnvrgBase):
    ...
    @staticmethod
    def load_model(module_name,path=None,install_req=False):
        try:
            if path is None:
            ...
            raise CnvrgError("Can't find library to load, check path of the library")
            print(e)
            eixt(1)
    ...
    def connect(self, **kwargs):
        if self.model is None:
            ...
            sys.exit(1)
        self.conn = self.model.connect(**kwargs)
    def query(self, query):
        if self.model is None:
            raise CnvrgError("Can't find model")
            sys.exit(1)
        self.model.run(cur=self.conn.cursor(),query=query)
    def to_csv(self, query,filename):
        if self.model is None:
            raise CnvrgError("Can't find model")
            sys.exit(1)
        self.model.to_csv(cur=self.conn.cursor(),query=query,filename=filename)
    def to_df(self, query,**kwargs):
        if self.model is None:
            raise CnvrgError("Can't find model")
            sys.exit(1)
        self.model.to_df(conn=self.conn,query=query,**kwargs)
    def close_connection(self):
        if self.model is None:
            raise CnvrgError("Can't find model")
            sys.exit(1)
        self.model.close_connection(cur=self.conn.cursor())
