...
class Variables(object):
    ...
    def _getVars(self, prefix=None):
        return [var if not prefix or var.key.startswith(prefix) for var in self._getAllVars()]
    ...
    def getall(self, prefix=None):
        return {self._getRelativeKey(self._getOriginKey(arg.key), prefix=prefix): arg.get() for arg in self._getVars(prefix=prefix)}
