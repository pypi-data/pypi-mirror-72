...
class API(object):
    ...
    def get_data(self, identifiers, productIds=None, packageIds=None, fieldClusterIds=None, dtype='json', fieldIds=None,
        ...
        #Check what is assigned
        assert identifiers is not None, "Identifiers must be passed as a list of Sustainalytics EntityId, ISINs, CUSIPs etc"
        ...
        with tqdm(total=len(identiers_group_list)) as pbar:
            ...
            for i, ids100 in enumerate(identiers_group_list):
                ...
                # For packages ids
                if packageIds is not None and isinstance(packageIds, list) and len(packageIds) > 0:
                    params = params + (('packageIds', packageIds),)

                if fieldClusterIds is not None and isinstance(fieldClusterIds, list) and len(fieldClusterIds) > 0:
                    fieldClusterIds_str = ','.join([str(elem) for elem in fieldClusterIds])
                    params = params + (('fieldClusterIds', fieldClusterIds_str),)
                if fieldIds is not None and isinstance(fieldIds, list) and len(fieldIds) > 0:
                    fieldIds_str = ','.join([str(elem) for elem in fieldIds])
                    params = params + (('fieldIds', fieldIds_str),)
                #else:
                 #   raise AttributeError('Identifier argument must be passed with only ONE of these other parameters productIds, packageIds, fieldClusterIds or fieldIds')
