...
def second_user(transactional_db, django_user_model, backend, ngrok_url):
    ...
    Membership.objects.create(user=user, account=account, is_admin=True, limit=4)
    with dask.config.set(
        {
            "coiled": {
                "cloud": {
...
async def cloud(sample_user):
    async with Cloud(account=account, asynchronous=True,) as cloud:
        ...
        await asyncio.gather(
            *[
                cloud.delete_software_environment(name=name, account=info["account"])
        ...
        await asyncio.gather(
            *[
                cloud.delete_cluster_configuration(name=name, account=info["account"])
...
async def test_default_org_username(second_user):
    async with Cloud(asynchronous=True) as cloud:
        assert cloud.default_account == second_user.username
...
@pytest.mark.asyncio
async def tests_list_clusters_account(
    ...
    await cloud.create_cluster(
        name="cluster-1",
        configuration=cluster_configuration,
        account=sample_user.account.slug,
    )

    # Creat cluster in second account
    await cloud.create_software_environment(
        name="env-2", conda=["toolz"], account=second_account.account.slug
    )
    await cloud.create_cluster_configuration(
        name="config-2", software="env-2", account=second_account.account.slug
    )
    await cloud.create_cluster(
        name="cluster-2", configuration="config-2", account=second_account.account.slug
    )
