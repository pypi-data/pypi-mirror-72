...
- :ref:`Connecting to Coiled Cloud via the coiled Python package <cloud>`
- :ref:`Managing software environments <software-envs>`
- :ref:`Managing cluster configurations <cluster-config>`
- :ref:`Creating Dask clusters <cluster-creation>`
...
.. code-block:: python
    ...
    cloud = Cloud(user="<your-username>")

You'll then be prompted to enter your account password and, optionally, save an
authentication token to a :doc:`configuration file <configuration>` to avoid
the need to enter your credentials each time you create a ``Cloud`` object.
...
packages to install into the environment. For example:
...
:meth:`Cloud.create_cluster_configuration` method. For example:
...
.. code-block:: python
    ...
    cloud.create_cluster_configuration(
        ...
        scheduler_cpu=1,
        software_environment="daskdev/dask:latest",
    )
...
.. code-block:: python
    ...
    cluster = CoiledCluster(configuration="my-cluster-config", n_workers=5)
