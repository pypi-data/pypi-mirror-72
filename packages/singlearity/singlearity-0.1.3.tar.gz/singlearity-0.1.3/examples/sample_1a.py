...
def get_best_veteran():
    ...
    candidate_batters = sing.get_players(name="Nelson Cruz")
    candidate_batters.append(sing.get_players(name="Yuli Gurrie")[0])
    #Note that get_players returns a list (even if it returns only one player).
    #So we need to grab the first player.
    candidate_pitchers = [sing.get_players(name = p)[0] for p in ['James Paxton', 'Jordan Montgomery']]
    #candidate_pitchers = sing.get_players(name="Tyler Anderson")
    #candidate_pitchers.append(sing.get_players(name="Clayton Kershaw")[0]) 
