...
class NvmAccessProviderCmsisDapUpdi(NvmAccessProviderCmsisDapAvr):
    ...
    def __init__(self, transport, device_info, frequency=None, options=""):
        ...
        if "hvpulse" in self.options:
            self.logger.info("Activating UPDI using simple high-voltage pulse")
            use_hv = Avr8Protocol.UPDI_HV_SIMPLE_PULSE
        elif "hvautopounce" in self.options:
            self.logger.info("Activating UPDI using high-voltage pulse with auto power-toggle")
            use_hv = Avr8Protocol.UPDI_HV_AUTO_POWER_TOGGLE
        elif "hvuserpounce" in self.options:
    ...
    def start(self, user_interaction_callback=None):
        ...
        self.avr.sib_read()
        if "chip_erase_by_key" in self.options:
        ...
        try:
            ...
            if self.avr.protocol.error_as_string(error.code) == "AVR8_FAILURE_OCD_LOCKED":
                if "locked_write_user_row" in self.options:
                    self.logger.info("Preparing to write user row on a locked device...")
