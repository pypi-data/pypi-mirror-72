...
# Some of the tests might trigger logging up to level ERROR even when the test pass so to avoid confusing log
# outputs the level is set higher than any logging within the pymcuprog module.
...
class TestBackend(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.backend = Backend(LOG_LEVEL)
    ...
    def test_connect_to_tool_usb_hid_connection_calls_hid_transport_with_correct_serial_and_product(self):
        ...
        serial = "DUMMYSERIAL"
        tool = "DUMMYTOOL"
        connection = ToolUsbHidConnection(serial_number=serial, tool_name=tool)
    ...
    def test_is_isolated_erase_possible_flash_mega4809_returns_false(self):
        self.mock_hid_transport()
        self.mock_is_connected_to_hid_tool()
        self.mock_housekeepingprotocol()
        mock_programmer = self.mock_programmer()
        ...
        self.assertFalse(self.backend.is_isolated_erase_possible(MemoryNames.FLASH))
