...
from h2o4gpu.base import BaseEstimator, clone, is_classifier
...
from h2o4gpu.base import TransformerMixin
...
class VargEstimator(BaseEstimator):
    """h2o4gpu estimators shouldn't have vargs."""
...
def test_clone():
    ...
    from h2o4gpu.feature_selection import SelectFpr, f_classif
...
class TreeBadVersion(DecisionTreeClassifier):
    def __getstate__(self):
        return dict(self.__dict__.items(), _h2o4gpu_version="something")
...
def test_pickle_version_warning_is_issued_upon_different_version():
    ...
    message = pickle_error_message.format(estimator="TreeBadVersion",
                                          ...
                                          current_version=h2o4gpu.__version__)
...
def test_pickle_version_no_warning_is_issued_with_non_h2o4gpu_estimator():
    iris = datasets.load_iris()
    tree = TreeNoVersion().fit(iris.data, iris.target)
    tree_pickle_noversion = pickle.dumps(tree)
    try:
        module_backup = TreeNoVersion.__module__
        TreeNoVersion.__module__ = "noth2o4gpu"
...
def test_pickling_when_getstate_is_overwritten_by_mixin_outside_of_h2o4gpu():
    try:
        estimator = MultiInheritanceEstimator()
        text = "this attribute should not be pickled"
        estimator._attribute_not_pickled = text
        old_mod = type(estimator).__module__
        type(estimator).__module__ = "noth2o4gpu"
...
def test_regressormixin_score_multioutput():
    ...
    X = [[1], [2], [3]]
    y = [1, 2, 3]
    reg = LinearRegressionSklearn().fit(X, y)
