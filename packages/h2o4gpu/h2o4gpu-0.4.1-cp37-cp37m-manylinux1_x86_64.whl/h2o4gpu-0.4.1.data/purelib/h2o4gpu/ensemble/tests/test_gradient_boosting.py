"""
Testing for the gradient boosting module (h2o4gpu.ensemble.gradient_boosting).
"""
import warnings
import numpy as np
...
from h2o4gpu.datasets import (make_classification, fetch_california_housing,
                              make_regression)
...
from h2o4gpu.ensemble.gradient_boosting import ZeroEstimator
from h2o4gpu.ensemble._gradient_boosting import predict_stages
from h2o4gpu.preprocessing import OneHotEncoder
from h2o4gpu.svm import LinearSVC
from h2o4gpu.metrics import mean_squared_error
from h2o4gpu.model_selection import train_test_split
from h2o4gpu.utils import check_random_state, tosequence
...
from h2o4gpu.utils.testing import assert_less
...
from h2o4gpu.utils.testing import skip_if_32bit
from h2o4gpu.exceptions import DataConversionWarning
from h2o4gpu.exceptions import NotFittedError
from h2o4gpu.dummy import DummyClassifier, DummyRegressor
from h2o4gpu.pipeline import make_pipeline
from h2o4gpu.linear_model import LinearRegressionSklearn
from h2o4gpu.svm import NuSVR


GRADIENT_BOOSTING_ESTIMATORS = [GradientBoostingClassifierSklearn,
...
def check_classification_toy(presort, loss):
    ...
    clf = GradientBoostingClassifierSklearn(loss=loss, n_estimators=10,
...
def test_classifier_parameter_checks():
    # Check input parameter validation for GradientBoostingClassifierSklearn.
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(n_estimators=0).fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(n_estimators=-1).fit, X, y)

    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(learning_rate=0.0).fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(learning_rate=-1.0).fit, X, y)

    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(loss='foobar').fit, X, y)

    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(min_samples_split=0.0).fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(min_samples_split=-1.0).fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(min_samples_split=1.1).fit, X, y)

    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(min_samples_leaf=0).fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(min_samples_leaf=-1.0).fit, X, y)

    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(min_weight_fraction_leaf=-1.).fit,
                  X, y)
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(min_weight_fraction_leaf=0.6).fit,
                  X, y)

    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(subsample=0.0).fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(subsample=1.1).fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(subsample=-0.1).fit, X, y)

    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(max_depth=-0.1).fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(max_depth=0).fit, X, y)

    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(init={}).fit, X, y)

    # test fit before feature importance
    assert_raises(ValueError,
                  lambda: GradientBoostingClassifierSklearn().feature_importances_)

    # deviance requires ``n_classes >= 2``.
    assert_raises(ValueError,
                  lambda X, y: GradientBoostingClassifierSklearn(
    ...
    assert_raise_message(ValueError,
                         ...
                         GradientBoostingClassifierSklearn(presort='invalid')
                         .fit, X, y)


def test_regressor_parameter_checks():
    # Check input parameter validation for GradientBoostingRegressorSklearn
    assert_raise_message(ValueError, "alpha must be in (0.0, 1.0) but was 1.2",
                         GradientBoostingRegressorSklearn(loss='huber', alpha=1.2)
                         .fit, X, y)
    assert_raise_message(ValueError, "alpha must be in (0.0, 1.0) but was 1.2",
                         GradientBoostingRegressorSklearn(loss='quantile', alpha=1.2)
                         .fit, X, y)
    assert_raise_message(ValueError, "Invalid value for max_features: "
                         "'invalid'. Allowed string values are 'auto', 'sqrt'"
                         " or 'log2'.",
                         GradientBoostingRegressorSklearn(max_features='invalid').fit,
                         X, y)
    assert_raise_message(ValueError, "n_iter_no_change should either be None"
                         " or an integer. 'invalid' was passed",
                         GradientBoostingRegressorSklearn(n_iter_no_change='invalid')
                         .fit, X, y)
    allowed_presort = ('auto', True, False)
    assert_raise_message(ValueError,
                         "'presort' should be in {}. "
                         "Got 'invalid' instead.".format(allowed_presort),
                         GradientBoostingRegressorSklearn(presort='invalid')
                         .fit, X, y)


def test_loss_function():
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(loss='ls').fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(loss='lad').fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(loss='quantile').fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingClassifierSklearn(loss='huber').fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingRegressorSklearn(loss='deviance').fit, X, y)
    assert_raises(ValueError,
                  GradientBoostingRegressorSklearn(loss='exponential').fit, X, y)


def check_classification_synthetic(presort, loss):
    # Test GradientBoostingClassifierSklearn on synthetic dataset used by
    ...
    gbrt = GradientBoostingClassifierSklearn(n_estimators=100, min_samples_split=2,
    ...
    gbrt = GradientBoostingClassifierSklearn(n_estimators=200, min_samples_split=2,
                                      max_depth=1, loss=loss,
...
def check_boston(presort, loss, subsample):
    ...
    for sample_weight in None, ones, 2 * ones:
        clf = GradientBoostingRegressorSklearn(n_estimators=100,
                                        loss=loss,
                                        max_depth=4,
                                        subsample=subsample,
                                        min_samples_split=2,
...
def check_iris(presort, subsample, sample_weight):
    ...
    clf = GradientBoostingClassifierSklearn(n_estimators=100,
                                     loss='deviance',
                                     random_state=1,
                                     subsample=subsample,
                                     presort=presort)
...
def test_regression_synthetic():
    ...
    for presort in True, False:
        clf = GradientBoostingRegressorSklearn(presort=presort)
    ...
    for presort in True, False:
        ...
        clf = GradientBoostingRegressorSklearn(**regression_params)
        clf.fit(X_train, y_train)
        mse = mean_squared_error(y_test, clf.predict(X_test))
...
def test_feature_importances():
    ...
    for presort in True, False:
        clf = GradientBoostingRegressorSklearn(n_estimators=100, max_depth=5,
...
def test_probability_log():
    ...
    clf = GradientBoostingClassifierSklearn(n_estimators=100, random_state=1)
...
def test_check_inputs_predict():
    ...
    clf = GradientBoostingRegressorSklearn(n_estimators=100, random_state=1)
...
def test_max_feature_regression():
    ...
    gbrt = GradientBoostingClassifierSklearn(n_estimators=100, min_samples_split=5,
...
def test_feature_importance_regression():
    ...
    reg = GradientBoostingRegressorSklearn(loss='huber', learning_rate=0.1,
...
def test_max_feature_auto():
    ...
    gbrt = GradientBoostingClassifierSklearn(n_estimators=1, max_features='auto')
    gbrt.fit(X_train, y_train)
    assert_equal(gbrt.max_features_, int(np.sqrt(n_features)))

    gbrt = GradientBoostingRegressorSklearn(n_estimators=1, max_features='auto')
    gbrt.fit(X_train, y_train)
    assert_equal(gbrt.max_features_, n_features)

    gbrt = GradientBoostingRegressorSklearn(n_estimators=1, max_features=0.3)
    gbrt.fit(X_train, y_train)
    assert_equal(gbrt.max_features_, int(n_features * 0.3))

    gbrt = GradientBoostingRegressorSklearn(n_estimators=1, max_features='sqrt')
    gbrt.fit(X_train, y_train)
    assert_equal(gbrt.max_features_, int(np.sqrt(n_features)))

    gbrt = GradientBoostingRegressorSklearn(n_estimators=1, max_features='log2')
    gbrt.fit(X_train, y_train)
    assert_equal(gbrt.max_features_, int(np.log2(n_features)))

    gbrt = GradientBoostingRegressorSklearn(n_estimators=1,
...
def test_staged_predict():
    ...
    X_train, y_train = X[:200], y[:200]
    X_test = X[200:]
    clf = GradientBoostingRegressorSklearn()
...
def test_staged_predict_proba():
    ...
    X_train, y_train = X[:200], y[:200]
    X_test, y_test = X[200:], y[200:]
    clf = GradientBoostingClassifierSklearn(n_estimators=20)
...
def test_quantile_loss():
    ...
    clf_quantile = GradientBoostingRegressorSklearn(n_estimators=100, loss='quantile',
    ...
    clf_lad = GradientBoostingRegressorSklearn(n_estimators=100, loss='lad',
...
def test_oob_multilcass_iris():
    ...
    clf = GradientBoostingClassifierSklearn(n_estimators=100, loss='deviance',
...
def test_warm_start(Cls):
    ...
    if Cls is GradientBoostingRegressorSklearn:
...
def test_complete_classification():
    ...
    from h2o4gpu.tree._tree import TREE_LEAF
    X, y = datasets.make_hastie_10_2(n_samples=100, random_state=1)
    k = 4

    est = GradientBoostingClassifierSklearn(n_estimators=20, max_depth=None,
...
def test_complete_regression():
    ...
    est = GradientBoostingRegressorSklearn(n_estimators=20, max_depth=None,
                                    random_state=1, max_leaf_nodes=k + 1)
...
def test_zero_estimator_reg():
    ...
    est = GradientBoostingRegressorSklearn(n_estimators=20, max_depth=1,
...
def test_zero_estimator_clf():
    ...
    est = GradientBoostingClassifierSklearn(n_estimators=20, max_depth=1,
                                     random_state=1, init='zero')
    est.fit(X, y)
...
def test_warm_start_wo_nestimators_change():
    ...
    clf = GradientBoostingClassifierSklearn(n_estimators=10, warm_start=True)
    clf.fit([[0, 1], [2, 3]], [0, 1])
...
def test_probability_exponential():
    # Predict probabilities.
    clf = GradientBoostingClassifierSklearn(loss='exponential',
...
def test_non_uniform_weights_toy_edge_case_reg():
    ...
    for loss in ('huber', 'ls', 'lad', 'quantile'):
        gb = GradientBoostingRegressorSklearn(learning_rate=1.0, n_estimators=2,
                                       loss=loss)
...
def test_non_uniform_weights_toy_edge_case_clf():
    ...
    for loss in ('deviance', 'exponential'):
        gb = GradientBoostingClassifierSklearn(n_estimators=5, loss=loss)
        gb.fit(X, y, sample_weight=sample_weight)
...
def check_sparse_input(EstimatorClass, X, X_sparse, y):
    ...
    if isinstance(EstimatorClass, GradientBoostingClassifierSklearn):
...
@pytest.mark.parametrize(
        'EstimatorClass',
        (GradientBoostingClassifierSklearn, GradientBoostingRegressorSklearn))
...
def test_gradient_boosting_early_stopping():
    ...
    gbc = GradientBoostingClassifierSklearn(n_estimators=1000,
                                     n_iter_no_change=10,
                                     learning_rate=0.1, max_depth=3,
                                     random_state=42)

    gbr = GradientBoostingRegressorSklearn(n_estimators=1000, n_iter_no_change=10,
                                    learning_rate=0.1, max_depth=3,
                                    random_state=42)
    ...
    gbc = GradientBoostingClassifierSklearn(n_estimators=100, learning_rate=0.1,
                                     max_depth=3, random_state=42)
    gbc.fit(X, y)
    gbr = GradientBoostingRegressorSklearn(n_estimators=200, learning_rate=0.1,
                                    max_depth=3, random_state=42)
    gbr.fit(X, y)
...
def test_gradient_boosting_validation_fraction():
    ...
    gbc = GradientBoostingClassifierSklearn(n_estimators=100,
                                     n_iter_no_change=10,
    ...
    gbr = GradientBoostingRegressorSklearn(n_estimators=100, n_iter_no_change=10,
                                    learning_rate=0.1, max_depth=3,
                                    validation_fraction=0.1,
                                    random_state=42)
...
def test_early_stopping_stratified():
    ...
    gbc = GradientBoostingClassifierSklearn(n_iter_no_change=5)
    with pytest.raises(
            ValueError,
...
@pytest.mark.parametrize(
    ...
    [(GradientBoostingClassifierSklearn, make_classification, DummyClassifier),
     (GradientBoostingClassifierSklearn, _make_multiclass, DummyClassifier),
     (GradientBoostingRegressorSklearn, make_regression, DummyRegressor)],
...
def test_gradient_boosting_with_init(gb, dataset_maker, init_estimator):
    # Check that GradientBoostingRegressorSklearn works when init is a h2o4gpu
    # estimator.
...
def test_gradient_boosting_with_init_pipeline():
    ...
    init = make_pipeline(LinearRegressionSklearn())
    gb = GradientBoostingRegressorSklearn(init=init)
...
@pytest.mark.parametrize('estimator, missing_method', [
    (GradientBoostingClassifierSklearn(init=LinearSVC()), 'predict_proba'),
    (GradientBoostingRegressorSklearn(init=OneHotEncoder()), 'predict')
])
...
def test_early_stopping_n_classes():
    ...
    gb = GradientBoostingClassifierSklearn(n_iter_no_change=5, random_state=0,
...
def test_gbr_degenerate_feature_importances():
    ...
    X = np.zeros((10, 10))
    y = np.ones((10,))
    gbr = GradientBoostingRegressorSklearn().fit(X, y)
