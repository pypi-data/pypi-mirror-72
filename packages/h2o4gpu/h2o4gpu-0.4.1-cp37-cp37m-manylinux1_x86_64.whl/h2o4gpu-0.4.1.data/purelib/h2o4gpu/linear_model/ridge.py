...
def ridge_regression(X, y, alpha, sample_weight=None, solver='auto',
    ...
    alpha : {float, array-like},
        ...
        LogisticRegressionSklearn or LinearSVC. If an array is passed, penalties are
    ...
    solver : {'auto', 'svd', 'cholesky', 'lsqr', 'sparse_cg', 'sag', 'saga'}
        ...
        - 'svd' uses a Singular Value Decomposition of X to compute the RidgeSklearn
        ...
        - 'sag' uses a Stochastic Average Gradient descent, and 'saga' uses
          ...
          scaler from h2o4gpu.preprocessing.
    ...
    return_intercept : boolean, default False
        ...
        data, use h2o4gpu.linear_model._preprocess_data before your regression.
...
def _ridge_regression(X, y, alpha, sample_weight=None, solver='auto',
    ...
    if return_intercept and solver != 'sag':
        raise ValueError("In RidgeSklearn, only 'sag' solver can directly fit the "
...
class _BaseRidgeSklearn(LinearModel, MultiOutputMixin, metaclass=ABCMeta):
    @abstractmethod
...
class RidgeSklearn(_BaseRidgeSklearn, RegressorMixin):
    ...
    the l2-norm. Also known as RidgeSklearn Regression or Tikhonov regularization.
    ...
    See also
    --------
    RidgeSklearnClassifier : RidgeSklearn classifier
    RidgeSklearnCV : RidgeSklearn regression with built-in cross validation
    :class:`h2o4gpu.kernel_ridge.KernelRidgeSklearn` : Kernel ridge regression
        combines ridge regression with the kernel trick

    Examples
    --------
    >>> from h2o4gpu.linear_model import RidgeSklearn
    >>> import numpy as np
    >>> n_samples, n_features = 10, 5
    >>> rng = np.random.RandomState(0)
    >>> y = rng.randn(n_samples)
    >>> X = rng.randn(n_samples, n_features)
    >>> clf = RidgeSklearn(alpha=1.0)
    >>> clf.fit(X, y) # doctest: +NORMALIZE_WHITESPACE
    RidgeSklearn(alpha=1.0, copy_X=True, fit_intercept=True, max_iter=None,
    ...
    def fit(self, X, y, sample_weight=None):
        """Fit RidgeSklearn regression model
...
class RidgeSklearnClassifier(LinearClassifierMixin, _BaseRidgeSklearn):
    """Classifier using RidgeSklearn regression.
    ...
    alpha : float
        ...
        the estimates. Larger values specify stronger regularization.
        Alpha corresponds to ``C^-1`` in other linear models such as
        LogisticRegressionSklearn or LinearSVC.
    ...
    Examples
    --------
    >>> from h2o4gpu.datasets import load_breast_cancer
    >>> from h2o4gpu.linear_model import RidgeSklearnClassifier
    >>> X, y = load_breast_cancer(return_X_y=True)
    >>> clf = RidgeSklearnClassifier().fit(X, y)
    >>> clf.score(X, y) # doctest: +ELLIPSIS
    0.9595...

    See also
    --------
    RidgeSklearn : RidgeSklearn regression
    RidgeSklearnClassifierCV :  RidgeSklearn classifier with built-in cross validation

    Notes
    -----
    For multi-class classification, n_class classifiers are trained in
    a one-versus-all approach. Concretely, this is implemented by taking
    advantage of the multi-variate response support in RidgeSklearn.
    """
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if not self._label_binarizer.y_type_.startswith('multilabel'):
            ...
            # we don't (yet) support multi-label classification in RidgeSklearn
            raise ValueError(
...
class _RidgeSklearnGCV(LinearModel):
    """RidgeSklearn regression with built-in Generalized Cross-Validation

    It allows efficient Leave-One-Out cross-validation.

    This class is not intended to be used directly. Use RidgeSklearnCV instead.
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if error:
            ...
            # they are already computed efficiently by _RidgeSklearnGCV. This
...
class _BaseRidgeSklearnCV(LinearModel, MultiOutputMixin):
    def __init__(self, alphas=(0.1, 1.0, 10.0),
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        cv = self.cv
        if cv is None:
            estimator = _RidgeSklearnGCV(self.alphas,
            ...
            gs = GridSearchCV(RidgeSklearn(fit_intercept=self.fit_intercept,
                                    normalize=self.normalize,
                                    solver=solver),
...
class RidgeSklearnCV(_BaseRidgeSklearnCV, RegressorMixin):
    """RidgeSklearn regression with built-in cross-validation.
    ...
    cv : int, cross-validation generator or an iterable, optional
        ...
        :class:`h2o4gpu.model_selection.StratifiedKFold` is used, else,
        :class:`h2o4gpu.model_selection.KFold` is used.
    ...
    Examples
    --------
    >>> from h2o4gpu.datasets import load_diabetes
    >>> from h2o4gpu.linear_model import RidgeSklearnCV
    >>> X, y = load_diabetes(return_X_y=True)
    >>> clf = RidgeSklearnCV(alphas=[1e-3, 1e-2, 1e-1, 1]).fit(X, y)
    >>> clf.score(X, y) # doctest: +ELLIPSIS
    0.5166...
...
class RidgeSklearnClassifierCV(LinearClassifierMixin, _BaseRidgeSklearnCV):
    """RidgeSklearn classifier with built-in cross-validation.
    ...
    --------
    >>> from h2o4gpu.datasets import load_breast_cancer
    >>> from h2o4gpu.linear_model import RidgeSklearnClassifierCV
    >>> X, y = load_breast_cancer(return_X_y=True)
    >>> clf = RidgeSklearnClassifierCV(alphas=[1e-3, 1e-2, 1e-1, 1]).fit(X, y)
    >>> clf.score(X, y) # doctest: +ELLIPSIS
    0.9630...
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        _BaseRidgeSklearnCV.fit(self, X, Y, sample_weight=sample_weight)
        return self
