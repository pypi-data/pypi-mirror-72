...
from h2o4gpu.metrics import make_scorer
from h2o4gpu.metrics import get_scorer

from h2o4gpu.linear_model.base import LinearRegressionSklearn
from h2o4gpu.linear_model.ridge import ridge_regression
from h2o4gpu.linear_model.ridge import RidgeSklearn
from h2o4gpu.linear_model.ridge import _RidgeSklearnGCV
from h2o4gpu.linear_model.ridge import RidgeSklearnCV
from h2o4gpu.linear_model.ridge import RidgeSklearnClassifier
from h2o4gpu.linear_model.ridge import RidgeSklearnClassifierCV
from h2o4gpu.linear_model.ridge import _solve_cholesky
from h2o4gpu.linear_model.ridge import _solve_cholesky_kernel
from h2o4gpu.linear_model.ridge import _check_gcv_mode
from h2o4gpu.linear_model.ridge import _X_operator
from h2o4gpu.datasets import make_regression

from h2o4gpu.model_selection import GridSearchCV
from h2o4gpu.model_selection import KFold, GroupKFold, cross_val_predict

from h2o4gpu.utils import check_random_state
from h2o4gpu.datasets import make_multilabel_classification
...
def test_ridge(solver):
    # RidgeSklearn regression convergence test using score
    ...
    ridge = RidgeSklearn(alpha=alpha, solver=solver)
    ridge.fit(X, y)
...
def test_ridge_sample_weights():
    ...
    for n_samples, n_features in ((6, 5), (5, 10)):
        ...
        for (alpha, intercept, solver) in param_grid:
            ...
            # RidgeSklearn with explicit sample_weight
            est = RidgeSklearn(alpha=alpha, fit_intercept=intercept,
...
def test_ridge_vs_lstsq():
    # On alpha=0., RidgeSklearn and OLS yield the same solution.
    ...
    ridge = RidgeSklearn(alpha=0., fit_intercept=False)
    ols = LinearRegressionSklearn(fit_intercept=False)
...
def test_ridge_individual_penalties():
    ...
    coef_cholesky = np.array([
        RidgeSklearn(alpha=alpha, solver="cholesky").fit(X, target).coef_
        for alpha, target in zip(penalties, y.T)])

    coefs_indiv_pen = [
        RidgeSklearn(alpha=penalties, solver=solver, tol=1e-8).fit(X, y).coef_
        for solver in ['svd', 'sparse_cg', 'lsqr', 'cholesky', 'sag', 'saga']]
    for coef_indiv_pen in coefs_indiv_pen:
        assert_array_almost_equal(coef_cholesky, coef_indiv_pen)

    # Test error is raised when number of targets and penalties do not match.
    ridge = RidgeSklearn(alpha=penalties[:-1])
...
def test_compute_gram(shape, uniform_weights):
    ...
    gcv = _RidgeSklearnGCV(fit_intercept=True)
...
def test_ridge_gcv_vs_ridge_loo_cv(
    ...
    loo_ridge = RidgeSklearnCV(cv=n_samples, fit_intercept=fit_intercept,
                        alphas=alphas, scoring='neg_mean_squared_error',
                        normalize=normalize)
    gcv_ridge = RidgeSklearnCV(gcv_mode=gcv_mode, fit_intercept=fit_intercept,
...
def test_ridge_gcv_sample_weights(
    ...
    ridge_reg = RidgeSklearn(alpha=kfold.alpha_, fit_intercept=fit_intercept)
    splits = cv.split(X_tiled, y_tiled, groups=indices)
...
def test_check_gcv_mode_error(mode):
    ...
    gcv = RidgeSklearnCV(gcv_mode=mode)
...
def _test_ridge_loo(filter_):
    ...
    ridge_gcv = _RidgeSklearnGCV(fit_intercept=fit_intercept)
    ...
    ridge_gcv2 = RidgeSklearnCV(fit_intercept=False, scoring=scoring)
    ...
    ridge_gcv3 = RidgeSklearnCV(fit_intercept=False, scoring=scoring)
    f(ridge_gcv3.fit)(filter_(X_diabetes), y_diabetes)
    assert ridge_gcv3.alpha_ == pytest.approx(alpha_)

    # check that we get same best alpha with a scorer
    scorer = get_scorer('neg_mean_squared_error')
    ridge_gcv4 = RidgeSklearnCV(fit_intercept=False, scoring=scorer)
...
def _test_ridge_cv_normalize(filter_):
    ridge_cv = RidgeSklearnCV(normalize=True, cv=3)
    ridge_cv.fit(filter_(10. * X_diabetes), y_diabetes)

    gs = GridSearchCV(RidgeSklearn(normalize=True, solver='sparse_cg'), cv=3,
...
def _test_ridge_diabetes(filter_):
    ridge = RidgeSklearn(fit_intercept=False)
...
def _test_ridge_classifiers(filter_):
    ...
    for reg in (RidgeSklearnClassifier(), RidgeSklearnClassifierCV()):
    ...
    cv = KFold(5)
    reg = RidgeSklearnClassifierCV(cv=cv)
    reg.fit(filter_(X_iris), y_iris)
    y_pred = reg.predict(filter_(X_iris))
...
def _test_tolerance(filter_):
    ridge = RidgeSklearn(tol=1e-5, fit_intercept=False)
    ridge.fit(filter_(X_diabetes), y_diabetes)
    score = ridge.score(filter_(X_diabetes), y_diabetes)

    ridge2 = RidgeSklearn(tol=1e-3, fit_intercept=False)
...
def test_ridge_sparse_svd():
    ...
    y = rng.rand(100)
    ridge = RidgeSklearn(solver='svd', fit_intercept=False)
...
def test_class_weights():
    ...
    reg = RidgeSklearnClassifier(class_weight=None)
    reg.fit(X, y)
    assert_array_equal(reg.predict([[0.2, -1.0]]), np.array([1]))

    # we give a small weights to class 1
    reg = RidgeSklearnClassifier(class_weight={1: 0.001})
    reg.fit(X, y)
    ...
    reg = RidgeSklearnClassifier(class_weight='balanced')
    reg.fit(X, y)
    assert_array_equal(reg.predict([[0.2, -1.0]]), np.array([1]))
    ...
    reg = RidgeSklearnClassifier(class_weight=None)
    reg.fit(X, y)
    rega = RidgeSklearnClassifier(class_weight='balanced')
    rega.fit(X, y)
...
@pytest.mark.filterwarnings('ignore: The default value of cv')  # 0.22
@pytest.mark.parametrize('reg', (RidgeSklearnClassifier, RidgeSklearnClassifierCV))
...
def test_class_weights_cv():
    ...
    reg = RidgeSklearnClassifierCV(class_weight=None, alphas=[.01, .1, 1])
    reg.fit(X, y)

    # we give a small weights to class 1
    reg = RidgeSklearnClassifierCV(class_weight={1: 0.001}, alphas=[.01, .1, 1, 10])
    reg.fit(X, y)
...
def test_ridgecv_store_cv_values():
    ...
    r = RidgeSklearnCV(alphas=alphas, cv=None, store_cv_values=True)
    ...
    r = RidgeSklearnCV(cv=3, store_cv_values=True)
...
def test_ridge_classifier_cv_store_cv_values():
    ...
    r = RidgeSklearnClassifierCV(alphas=alphas, cv=None, store_cv_values=True)
...
def test_ridgecv_sample_weight():
    ...
    for n_samples, n_features in ((6, 5), (5, 10)):
        ...
        cv = KFold(5)
        ridgecv = RidgeSklearnCV(alphas=alphas, cv=cv)
        ridgecv.fit(X, y, sample_weight=sample_weight)

        # Check using GridSearchCV directly
        parameters = {'alpha': alphas}
        gs = GridSearchCV(RidgeSklearn(), parameters, cv=cv)
...
def test_sparse_design_with_sample_weights():
    ...
    sparse_ridge = RidgeSklearn(alpha=1., fit_intercept=False)
    dense_ridge = RidgeSklearn(alpha=1., fit_intercept=False)
...
def test_ridgecv_int_alphas():
    ...
    # Integers
    ridge = RidgeSklearnCV(alphas=(1, 10, 100))
    ridge.fit(X, y)
...
def test_ridgecv_negative_alphas():
    ...
    # Negative floats
    ridge = RidgeSklearnCV(alphas=(-0.1, -1.0, -10.0))
    assert_raises_regex(ValueError,
                        "alphas must be positive",
                        ridge.fit, X, y)
...
def test_sparse_cg_max_iter():
    reg = RidgeSklearn(solver="sparse_cg", max_iter=1)
...
def test_n_iter():
    ...
    for max_iter in range(1, 4):
        for solver in ('sag', 'saga', 'lsqr'):
            reg = RidgeSklearn(solver=solver, max_iter=max_iter, tol=1e-12)
            reg.fit(X, y_n)
            assert_array_equal(reg.n_iter_, np.tile(max_iter, n_targets))

    for solver in ('sparse_cg', 'svd', 'cholesky'):
        reg = RidgeSklearn(solver=solver, max_iter=1, tol=1e-1)
        reg.fit(X, y_n)
...
def test_ridge_fit_intercept_sparse():
    ...
    for solver in ['sag', 'sparse_cg']:
        dense = RidgeSklearn(alpha=1., tol=1.e-15, solver=solver, fit_intercept=True)
        sparse = RidgeSklearn(alpha=1., tol=1.e-15, solver=solver, fit_intercept=True)
        dense.fit(X, y)
    ...
    for solver in ['saga', 'lsqr']:
        sparse = RidgeSklearn(alpha=1., tol=1.e-15, solver=solver, fit_intercept=True)
        assert_raises_regex(ValueError, "In RidgeSklearn,", sparse.fit, X_csr, y)
...
def test_ridge_regression_check_arguments_validity(return_intercept,
    ...
    if solver not in ['sag', 'auto'] and return_intercept:
        assert_raises_regex(ValueError,
                            "In RidgeSklearn, only 'sag' solver",
...
def test_ridge_classifier_no_support_multilabel():
    ...
    assert_raises(ValueError, RidgeSklearnClassifier().fit, X, y)
...
def test_dtype_match(solver):
    ...
    ridge_32 = RidgeSklearn(alpha=alpha, solver=solver, max_iter=500, tol=1e-10,)
    ridge_32.fit(X_32, y_32)
    coef_32 = ridge_32.coef_

    # Check type consistency 64 bits
    ridge_64 = RidgeSklearn(alpha=alpha, solver=solver, max_iter=500, tol=1e-10,)
    ...
    if solver == "sparse_cg" and sys.platform == "darwin":
        pytest.xfail(
            ...
            "https://github.com/h2o4gpu/h2o4gpu/issues/13868")
...
def test_dtype_match_cholesky():
    ...
    # Check type consistency 32bits
    ridge_32 = RidgeSklearn(alpha=alpha, solver='cholesky')
    ridge_32.fit(X_32, y_32)
    coef_32 = ridge_32.coef_

    # Check type consistency 64 bits
    ridge_64 = RidgeSklearn(alpha=alpha, solver='cholesky')
    ridge_64.fit(X_64, y_64)
    coef_64 = ridge_64.coef_
