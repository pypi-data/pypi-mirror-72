...
def fetch_mldata(dataname, target_name='label', data_name='data',
    ...
    works well with the most common cases:
      ...
      3) the data array is stored as `n_features x n_samples` , and thus needs
         to be transposed to match the `h2o4gpu` standard
    ...
    # set axes to h2o4gpu conventions
    if transpose_data:
...
def setup_module(module):
    ...
    from h2o4gpu.utils.testing import install_mldata_mock
    install_mldata_mock({
        'iris': {
            'data': np.empty((150, 4)),
            'label': np.empty(150),
...
def teardown_module(module):
    from h2o4gpu.utils.testing import uninstall_mldata_mock
