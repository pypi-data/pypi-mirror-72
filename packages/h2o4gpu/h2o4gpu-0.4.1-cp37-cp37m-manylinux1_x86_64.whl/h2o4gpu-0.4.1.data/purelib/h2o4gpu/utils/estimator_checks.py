...
from ..linear_model import RidgeSklearn
...
def _yield_transformer_checks(name, transformer):
    ...
    external_solver = ['Isomap', 'KernelPCASklearn', 'LocallyLinearEmbedding',
                       'RandomizedLassoSklearn', 'LogisticRegressionSklearnCV']
...
def check_estimator(Estimator):
    """Check if estimator adheres to h2o4gpu conventions.
...
def set_checking_parameters(estimator):
    ...
    if name == 'TruncatedSVDSklearn':
        # TruncatedSVDSklearn doesn't run with n_components = n_features
        # This is ugly :-/
    ...
    if name in ('HistGradientBoostingClassifierSklearn',
...
def check_methods_subset_invariance(name, estimator_orig):
    ...
    for method in ["predict", "transform", "decision_function",
        ...
        if (name, method) in [('NuSVC', 'decision_function'),
                              ('SparsePCASklearn', 'transform'),
                              ('MiniBatchSparsePCASklearn', 'transform'),
...
def check_fit2d_1feature(name, estimator_orig):
    ...
    if hasattr(estimator, "n_clusters"):
        estimator.n_clusters = 1
    # ensure two labels in subsample for RandomizedLogisticRegressionSklearn
    if name == 'RandomizedLogisticRegressionSklearn':
...
def check_estimators_pickle(name, estimator_orig):
    ...
    if estimator.__module__.startswith('h2o4gpu.'):
...
def check_non_transformer_estimators_n_iter(name, estimator_orig):
    ...
    not_run_check_n_iter = ['RidgeSklearn', 'SVR', 'NuSVR', 'NuSVC',
                            'RidgeSklearnClassifier', 'SVC', 'RandomizedLassoSklearn',
                            'LogisticRegressionSklearnCV', 'LinearSVC',
    ...
    # LassoSklearnLars stops early for the default alpha=1.0 the iris dataset.
    if name == 'LassoSklearnLars':
