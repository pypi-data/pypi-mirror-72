...
import h2o4gpu
from h2o4gpu.inspection import partial_dependence
from h2o4gpu.inspection import plot_partial_dependence
from h2o4gpu.inspection.partial_dependence import (
    _grid_from_X,
...
from h2o4gpu.linear_model import LogisticRegressionSklearn
from h2o4gpu.linear_model import MultiTaskLassoSklearn
from h2o4gpu.datasets import load_boston, load_iris
...
from h2o4gpu.preprocessing import PolynomialFeatures
from h2o4gpu.dummy import DummyClassifier
from h2o4gpu.base import BaseEstimator, ClassifierMixin
...
@pytest.mark.parametrize('Estimator, method, data', [
    (GradientBoostingClassifierSklearn, 'recursion', binary_classification_data),
    (GradientBoostingClassifierSklearn, 'recursion', multiclass_classification_data),
    (GradientBoostingClassifierSklearn, 'brute', binary_classification_data),
    (GradientBoostingClassifierSklearn, 'brute', multiclass_classification_data),
    (GradientBoostingRegressorSklearn, 'recursion', regression_data),
    (GradientBoostingRegressorSklearn, 'brute', regression_data),
    (LinearRegressionSklearn, 'brute', regression_data),
    (LinearRegressionSklearn, 'brute', multioutput_regression_data),
    (LogisticRegressionSklearn, 'brute', binary_classification_data),
    (LogisticRegressionSklearn, 'brute', multiclass_classification_data),
    (MultiTaskLassoSklearn, 'brute', multioutput_regression_data),
    ])
...
@pytest.mark.parametrize(
    'est, method',
    [(LinearRegressionSklearn(), 'brute'),
     (GradientBoostingRegressorSklearn(random_state=0), 'recursion'),
     (GradientBoostingRegressorSklearn(random_state=0), 'brute')]
)
...
def test_recursion_decision_function(target_feature):
    ...
    est = GradientBoostingClassifierSklearn(random_state=0, loss='deviance')
    est.fit(X, y)
...
@pytest.mark.parametrize('est', (LinearRegressionSklearn(),
                                 GradientBoostingRegressorSklearn(random_state=0)))
...
def test_partial_dependence_easy_target(est, power):
    ...
    lr = LinearRegressionSklearn().fit(new_X, new_y)
...
@pytest.mark.parametrize('Estimator',
                         (h2o4gpu.tree.DecisionTreeClassifier,
                          h2o4gpu.tree.ExtraTreeClassifier,
                          h2o4gpu.ensemble.ExtraTreesClassifier,
                          h2o4gpu.neighbors.KNeighborsClassifier,
                          h2o4gpu.neighbors.RadiusNeighborsClassifier,
                          h2o4gpu.ensemble.RandomForestClassifierSklearn))
...
@pytest.mark.parametrize(
    ...
    [(KMeansSklearn(),
     ...
     (GradientBoostingClassifierSklearn(random_state=0),
      {'features': [0], 'response_method': 'predict_proba',
       'method': 'recursion'},
...
@pytest.mark.parametrize(
    'estimator',
    [LinearRegressionSklearn(), GradientBoostingClassifierSklearn(random_state=0)]
)
...
def test_warning_recursion_non_constant_init():
    ...
    gbc = GradientBoostingClassifierSklearn(init=DummyClassifier(), random_state=0)
    gbc.fit(X, y)
...
def test_partial_dependence_sample_weight():
    ...
    clf = GradientBoostingRegressorSklearn(n_estimators=10, random_state=1)
...
def test_plot_partial_dependence_multiclass(pyplot):
    ...
    iris = load_iris()
    clf = GradientBoostingClassifierSklearn(n_estimators=10, random_state=1)
...
def test_plot_partial_dependence_error(pyplot, data, params, err_msg):
    X, y = data
    estimator = LinearRegressionSklearn().fit(X, y)
