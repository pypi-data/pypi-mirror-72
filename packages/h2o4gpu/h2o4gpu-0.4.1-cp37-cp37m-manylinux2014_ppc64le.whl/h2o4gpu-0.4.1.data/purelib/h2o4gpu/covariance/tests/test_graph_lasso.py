...
from h2o4gpu.covariance import (graph_lasso, GraphLassoSklearn, GraphLassoSklearnCV,
                                empirical_covariance)
from h2o4gpu.datasets.samples_generator import make_sparse_spd_matrix
...
def test_graph_lasso(random_state=0):
    ...
    model = GraphLassoSklearn(alpha=.25).fit(X)
    model.score(X)
    ...
    for assume_centered in (False, True):
        prec_ = GraphLassoSklearn(assume_centered=assume_centered).fit(Z).precision_
        precs.append(prec_)
...
def test_graph_lasso_cv(random_state=1):
    ...
    try:
        ...
        GraphLassoSklearnCV(verbose=100, alphas=5, tol=1e-1).fit(X)
    finally:
        sys.stdout = orig_stdout

    # Smoke test with specified alphas
    GraphLassoSklearnCV(alphas=[0.8, 0.5], tol=1e-1, n_jobs=1).fit(X)
