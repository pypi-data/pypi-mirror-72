...
from h2o4gpu.linear_model import LogisticRegressionSklearn, SGDClassifier, LassoSklearn
from h2o4gpu.svm import LinearSVC
from h2o4gpu.feature_selection import SelectFromModel
from h2o4gpu.ensemble import RandomForestClassifierSklearn
from h2o4gpu.linear_model import PassiveAggressiveClassifier
...
def test_input_estimator_unchanged():
    ...
    est = RandomForestClassifierSklearn()
...
def test_max_features_error(max_features, err_type, err_msg):
    clf = RandomForestClassifierSklearn(n_estimators=50, random_state=0)
...
def test_max_features():
    ...
    est = RandomForestClassifierSklearn(n_estimators=50, random_state=0)
    ...
    transformer1 = SelectFromModel(estimator=LassoSklearn(alpha=0.025,
                                                   random_state=42))
    X_new1 = transformer1.fit_transform(X, y)
    ...
    for n_features in range(1, X_new1.shape[1] + 1):
        transformer2 = SelectFromModel(estimator=LassoSklearn(alpha=0.025,
                                       random_state=42),
...
def test_sample_weight():
    ...
    est = LogisticRegressionSklearn(random_state=0, fit_intercept=False)
    transformer = SelectFromModel(estimator=est)
...
def test_coef_default_threshold():
    ...
    # For the LassoSklearn and related models, the threshold defaults to 1e-5
    transformer = SelectFromModel(estimator=LassoSklearn(alpha=0.1,
                                  random_state=42))
    transformer.fit(X, y)
    X_new = transformer.transform(X)
...
def test_2d_coef():
    ...
    for threshold, func in zip(["mean", "median"], [np.mean, np.median]):
        for order in [1, 2, np.inf]:
            ...
            transformer = SelectFromModel(estimator=LogisticRegressionSklearn(),
                                          threshold=threshold,
                                          norm_order=order)
            transformer.fit(X, y)
...
def test_partial_fit():
    ...
    transformer = SelectFromModel(estimator=RandomForestClassifierSklearn())
