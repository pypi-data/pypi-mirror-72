...
from h2o4gpu.utils.testing import assert_almost_equal, assert_array_equal
...
from h2o4gpu.ensemble import VotingClassifier, VotingRegressor
from h2o4gpu.model_selection import GridSearchCV
from h2o4gpu import datasets
from h2o4gpu.model_selection import cross_val_score, train_test_split
...
def test_predictproba_hardvoting():
    eclf = VotingClassifier(estimators=[('lr1', LogisticRegressionSklearn()),
...
def test_majority_label_iris():
    ...
    clf1 = LogisticRegressionSklearn(random_state=123)
    clf2 = RandomForestClassifierSklearn(random_state=123)
    clf3 = GaussianNB()
    eclf = VotingClassifier(estimators=[
...
def test_tie_situation():
    ...
    clf1 = LogisticRegressionSklearn(random_state=123, multi_class='ovr',
                              solver='liblinear')
    clf2 = RandomForestClassifierSklearn(random_state=123)
...
def test_gridsearch():
    ...
    clf1 = LogisticRegressionSklearn(random_state=1)
    clf2 = RandomForestClassifierSklearn(random_state=1)
    clf3 = GaussianNB()
    eclf = VotingClassifier(estimators=[
                ('lr', clf1), ('rf', clf2), ('gnb', clf3)],
                voting='soft')
...
def test_set_params():
    ...
    clf1 = LogisticRegressionSklearn(random_state=123, C=1.0)
    clf2 = RandomForestClassifierSklearn(random_state=123, max_depth=None)
    clf3 = GaussianNB()
...
def test_set_estimator_none(drop):
    ...
    assert all(isinstance(est, (LogisticRegressionSklearn, GaussianNB))
...
@pytest.mark.parametrize(
    ...
    [(X, y, VotingClassifier(
        [('lr', LogisticRegressionSklearn()),
         ('rf', RandomForestClassifierSklearn(n_estimators=5))])),
     (X_r, y_r, VotingRegressor(
         [('lr', LinearRegressionSklearn()),
          ('rf', RandomForestRegressorSklearn(n_estimators=5))]))]
)
@pytest.mark.parametrize("drop", [None, 'drop'])
def test_none_estimator_with_weights(X, y, voter, drop):
    # check that an estimator can be set to None and passing some weight
    # regression test for
    # https://github.com/h2o4gpu/h2o4gpu/issues/13777
