...
from h2o4gpu.utils.testing import assert_dict_equal
from h2o4gpu.utils.testing import assert_no_warnings

from h2o4gpu.base import clone, BaseEstimator
from h2o4gpu.pipeline import Pipeline, FeatureUnion, make_pipeline, make_union
from h2o4gpu.svm import SVC
from h2o4gpu.linear_model import LogisticRegressionSklearn, LassoSklearn
from h2o4gpu.linear_model import LinearRegressionSklearn
from h2o4gpu.cluster import KMeansSklearn
from h2o4gpu.feature_selection import SelectKBest, f_classif
from h2o4gpu.dummy import DummyRegressor
from h2o4gpu.decomposition import PCASklearn, TruncatedSVDSklearn
from h2o4gpu.datasets import load_iris
from h2o4gpu.preprocessing import StandardScaler
from h2o4gpu.feature_extraction.text import CountVectorizer
from h2o4gpu.utils._joblib import Memory
from h2o4gpu.utils._joblib import __version__ as joblib_version
...
def test_pipeline_methods_anova():
    ...
    X = iris.data
    y = iris.target
    # Test with Anova + LogisticRegressionSklearn
    clf = LogisticRegressionSklearn()
...
def test_pipeline_raise_set_params_error():
    ...
    pipe = Pipeline([('cls', LinearRegressionSklearn())])
...
def test_pipeline_methods_pca_svm():
    ...
    X = iris.data
    y = iris.target
    # Test with PCASklearn + SVC
    clf = SVC(gamma='scale', probability=True, random_state=0)
    pca = PCASklearn(svd_solver='full', n_components='mle', whiten=True)
...
def test_pipeline_methods_preprocessing_svm():
    ...
    pca = PCASklearn(n_components=2, svd_solver='randomized', whiten=True)
    clf = SVC(gamma='scale', probability=True, random_state=0,
...
def test_fit_predict_on_pipeline():
    ...
    km_for_pipeline = KMeansSklearn(random_state=0)
...
def test_fit_predict_on_pipeline_without_fit_predict():
    ...
    assert_raises_regex(AttributeError,
                        "'PCASklearn' object has no attribute 'fit_predict'",
...
def test_feature_union():
    ...
    X -= X.mean(axis=0)
    y = iris.target
    svd = TruncatedSVDSklearn(n_components=2, random_state=0)
...
def test_feature_union_weights():
    ...
    X = iris.data
    y = iris.target
    pca = PCASklearn(n_components=2, svd_solver='randomized', random_state=0)
    select = SelectKBest(k=1)
...
def test_classes_property():
    ...
    reg = make_pipeline(SelectKBest(k=1), LinearRegressionSklearn())
    reg.fit(X, y)
    assert_raises(AttributeError, getattr, reg, "classes_")

    clf = make_pipeline(SelectKBest(k=1), LogisticRegressionSklearn(random_state=0))
...
def test_step_name_validation():
    ...
    for cls, param in [(Pipeline, 'steps'),
        ...
        # we validate in construction (despite h2o4gpu convention)
...
def test_set_params_nested_pipeline():
    ...
    estimator.set_params(a__b__alpha=0.001, a__b=LassoSklearn())
    estimator.set_params(a__steps=[('b', LogisticRegressionSklearn())], a__b__C=5)
...
def test_pipeline_param_error():
    clf = make_pipeline(LogisticRegressionSklearn())
