...
from h2o4gpu.utils.testing import assert_not_equal
...
from h2o4gpu.ensemble import GradientBoostingRegressorSklearn, RandomForestClassifierSklearn
from h2o4gpu.exceptions import NotFittedError
from h2o4gpu.utils._joblib import cpu_count
from h2o4gpu.linear_model import LassoSklearn
...
from h2o4gpu.linear_model import SGDRegressor
from h2o4gpu.metrics import jaccard_score, mean_squared_error
from h2o4gpu.multiclass import OneVsRestClassifier
from h2o4gpu.multioutput import ClassifierChain, RegressorChain
from h2o4gpu.multioutput import MultiOutputClassifier
from h2o4gpu.multioutput import MultiOutputRegressor
from h2o4gpu.svm import LinearSVC
from h2o4gpu.base import ClassifierMixin
...
def test_multi_target_regression():
    ...
    for n in range(3):
        rgr = GradientBoostingRegressorSklearn(random_state=0)
        rgr.fit(X_train, y_train[:, n])
        references[:, n] = rgr.predict(X_test)

    rgr = MultiOutputRegressor(GradientBoostingRegressorSklearn(random_state=0))
...
def test_multi_target_regression_partial_fit():
    ...
    assert_almost_equal(references, y_pred)
    assert not hasattr(MultiOutputRegressor(LassoSklearn), 'partial_fit')
...
def test_multi_target_sparse_regression():
    ...
    for sparse in [sp.csr_matrix, sp.csc_matrix, sp.coo_matrix, sp.dok_matrix,
                   sp.lil_matrix]:
        rgr = MultiOutputRegressor(LassoSklearn(random_state=0))
        rgr_sparse = MultiOutputRegressor(LassoSklearn(random_state=0))
...
def test_multi_target_sample_weights_api():
    ...
    rgr = MultiOutputRegressor(LassoSklearn())
...
def test_multi_target_sample_weights():
    ...
    w = [2., 1.]
    rgr_w = MultiOutputRegressor(GradientBoostingRegressorSklearn(random_state=0))
    rgr_w.fit(Xw, yw, w)
...
def test_multi_output_classification():
    ...
    forest = RandomForestClassifierSklearn(n_estimators=10, random_state=1)
...
def test_multiclass_multioutput_estimator_predict_proba():
    ...
    clf = MultiOutputClassifier(LogisticRegressionSklearn(
...
def test_classifier_chain_fit_and_predict_with_sparse_data():
    ...
    classifier_chain = ClassifierChain(LogisticRegressionSklearn())
...
def test_classifier_chain_vs_independent_models():
    ...
    ovr = OneVsRestClassifier(LogisticRegressionSklearn())
    ovr.fit(X_train, Y_train)
    Y_pred_ovr = ovr.predict(X_test)

    chain = ClassifierChain(LogisticRegressionSklearn())
...
def test_base_chain_fit_and_predict():
    ...
    X, Y = generate_multilabel_dataset_with_correlations()
    chains = [RegressorChain(RidgeSklearn()),
              ClassifierChain(LogisticRegressionSklearn())]
    for chain in chains:
        chain.fit(X, Y)
...
def test_base_chain_fit_and_predict_with_sparse_data_and_cv():
    ...
    X, Y = generate_multilabel_dataset_with_correlations()
    X_sparse = sp.csr_matrix(X)
    base_chains = [ClassifierChain(LogisticRegressionSklearn(), cv=3),
                   RegressorChain(RidgeSklearn(), cv=3)]
...
def test_base_chain_random_order():
    ...
    X, Y = generate_multilabel_dataset_with_correlations()
    for chain in [ClassifierChain(LogisticRegressionSklearn()),
                  RegressorChain(RidgeSklearn())]:
