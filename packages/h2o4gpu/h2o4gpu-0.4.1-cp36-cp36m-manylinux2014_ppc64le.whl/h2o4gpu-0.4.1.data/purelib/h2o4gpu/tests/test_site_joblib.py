import os
import pytest
from h2o4gpu.utils._joblib import Parallel, delayed, Memory, parallel_backend


def test_old_pickle(tmpdir):
    import joblib

    # Check that a pickle that references h2o4gpu.external.joblib can load
    f = tmpdir.join('foo.pkl')
    f.write(b'\x80\x02ch2o4gpu.externals.joblib.numpy_pickle\nNumpyArrayWrappe'
