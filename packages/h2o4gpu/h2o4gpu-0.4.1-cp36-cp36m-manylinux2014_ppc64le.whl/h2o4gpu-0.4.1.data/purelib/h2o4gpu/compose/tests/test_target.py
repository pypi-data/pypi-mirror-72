...
from h2o4gpu.linear_model import LinearRegressionSklearn, LassoSklearn

from h2o4gpu import datasets

from h2o4gpu.compose import TransformedTargetRegressor
...
def test_transform_target_regressor_error():
    ...
    regr = TransformedTargetRegressor(regressor=LinearRegressionSklearn(),
    ...
    regr = TransformedTargetRegressor(regressor=LassoSklearn(),
                                      transformer=StandardScaler())
...
def test_transform_target_regressor_functions():
    ...
    lr = LinearRegressionSklearn().fit(X, regr.func(y))
...
def test_transform_target_regressor_1d_transformer(X, y):
    # All transformer in h2o4gpu expect 2D data. FunctionTransformer with
