...
from h2o4gpu.decomposition.pca import _assess_dimension_
from h2o4gpu.decomposition.pca import _infer_dimension_
...
def test_pca():
    # PCASklearn on dense arrays
    X = iris.data

    for n_comp in np.arange(X.shape[1]):
        pca = PCASklearn(n_components=n_comp, svd_solver='full')
    ...
    pca = PCASklearn(svd_solver='full')
    pca.fit(X)
...
def test_pca_arpack_solver():
    ...
    for n_comp in np.arange(1, d):
        pca = PCASklearn(n_components=n_comp, svd_solver='arpack', random_state=0)
    ...
    pca = PCASklearn(n_components=0, svd_solver='arpack', random_state=0)
    ...
    pca = PCASklearn(n_components=d, svd_solver='arpack', random_state=0)
    assert_raises(ValueError, pca.fit, X)
    assert_equal(pca.n_components,
                 PCASklearn(n_components=d,
                     svd_solver='arpack', random_state=0).n_components)
    assert_equal(pca.svd_solver,
                 PCASklearn(n_components=0,
                     svd_solver='arpack', random_state=0).svd_solver)
...
def test_pca_randomized_solver():
    ...
    for n_comp in np.arange(1, X.shape[1]):
        pca = PCASklearn(n_components=n_comp, svd_solver='randomized', random_state=0)
    ...
    pca = PCASklearn(n_components=0, svd_solver='randomized', random_state=0)
    assert_raises(ValueError, pca.fit, X)
...
def test_whitening():
    # Check that PCASklearn output has unit-variance
    ...
    for solver, copy in product(solver_list, (True, False)):
        ...
        pca = PCASklearn(n_components=n_components, whiten=True, copy=copy,
        ...
        X_ = X.copy()
        pca = PCASklearn(n_components=n_components, whiten=False, copy=copy,
...
def test_explained_variance():
    ...
    pca = PCASklearn(n_components=2, svd_solver='full').fit(X)
    apca = PCASklearn(n_components=2, svd_solver='arpack', random_state=0).fit(X)
    assert_array_almost_equal(pca.explained_variance_,
                              apca.explained_variance_, 1)
    assert_array_almost_equal(pca.explained_variance_ratio_,
                              apca.explained_variance_ratio_, 3)

    rpca = PCASklearn(n_components=2, svd_solver='randomized', random_state=42).fit(X)
    assert_array_almost_equal(pca.explained_variance_,
    ...
    pca = PCASklearn(n_components=2).fit(X)
    rpca = PCASklearn(n_components=2, svd_solver='randomized',
...
def test_singular_values():
    # Check that the PCASklearn output has the correct singular values
    ...
    pca = PCASklearn(n_components=2, svd_solver='full',
              random_state=rng).fit(X)
    apca = PCASklearn(n_components=2, svd_solver='arpack',
               random_state=rng).fit(X)
    rpca = PCASklearn(n_components=2, svd_solver='randomized',
               random_state=rng).fit(X)
    ...
    pca = PCASklearn(n_components=3, svd_solver='full', random_state=rng)
    apca = PCASklearn(n_components=3, svd_solver='arpack', random_state=rng)
    rpca = PCASklearn(n_components=3, svd_solver='randomized', random_state=rng)
...
def test_pca_check_projection():
    ...
    for solver in solver_list:
        Yt = PCASklearn(n_components=2, svd_solver=solver).fit(X).transform(Xt)
        Yt /= np.sqrt((Yt ** 2).sum())
...
def test_pca_inverse():
    ...
    for solver in solver_list:
        pca = PCASklearn(n_components=2, whiten=True, svd_solver=solver)
        pca.fit(X)
        Y = pca.transform(X)
        Y_inverse = pca.inverse_transform(Y)
        assert_almost_equal(X, Y_inverse, decimal=3)
...
def test_pca_validation(solver):
    ...
    for data in [X, X.T]:
        ...
        if solver == 'arpack':
            ...
            assert_raises_regex(ValueError,
                                ...
                                PCASklearn(n_components, svd_solver=solver)
                                .fit, data)
    ...
    assert_raise_message(ValueError,
                         ...
                         PCASklearn(n_components, svd_solver=solver).fit, data)
...
def test_n_components_none(solver):
    ...
    # We conduct the same test on X.T so that it is invariant to axis.
    for data in [X, X.T]:
        pca = PCASklearn(svd_solver=solver)
        pca.fit(data)
        if solver == 'arpack':
...
def test_randomized_pca_check_projection():
    # Test that the projection by randomized PCASklearn on dense data is correct
    rng = np.random.RandomState(0)
    n, p = 100, 3
    X = rng.randn(n, p) * .1
    X[:10] += np.array([3, 4, 5])
    ...
    Yt = PCASklearn(n_components=2, svd_solver='randomized',
...
def test_randomized_pca_check_list():
    # Test that the projection by randomized PCASklearn on list data is correct
    X = [[1.0, 0.0], [0.0, 1.0]]
    X_transformed = PCASklearn(n_components=1, svd_solver='randomized',
...
def test_randomized_pca_inverse():
    # Test that randomized PCASklearn is inversible on dense data
    rng = np.random.RandomState(0)
    n, p = 50, 3
    X = rng.randn(n, p)  # spherical data
    X[:, 1] *= .00001  # make middle component relatively small
    ...
    pca = PCASklearn(n_components=2, svd_solver='randomized', random_state=0).fit(X)
    Y = pca.transform(X)
    Y_inverse = pca.inverse_transform(Y)
    assert_almost_equal(X, Y_inverse, decimal=2)

    # same as above with whitening (approximate reconstruction)
    pca = PCASklearn(n_components=2, whiten=True, svd_solver='randomized',
...
def test_n_components_mle():
    ...
    n_components_dict = {}
    for solver in solver_list:
        pca = PCASklearn(n_components='mle', svd_solver=solver)
...
def test_pca_dim():
    ...
    pca = PCASklearn(n_components='mle', svd_solver='full').fit(X)
...
def test_infer_dim_1():
    ...
    X = (rng.randn(n, p) * .1 + rng.randn(n, 1) * np.array([3, 4, 5, 1, 2]) +
         np.array([1, 0, 7, 4, 6]))
    pca = PCASklearn(n_components=p, svd_solver='full')
    pca.fit(X)
...
def test_infer_dim_by_explained_variance():
    X = iris.data
    pca = PCASklearn(n_components=0.95, svd_solver='full')
    pca.fit(X)
    assert_equal(pca.n_components, 0.95)
    assert_equal(pca.n_components_, 2)

    pca = PCASklearn(n_components=0.01, svd_solver='full')
    pca.fit(X)
    ...
    X = rng.rand(5, 20)
    pca = PCASklearn(n_components=.5, svd_solver='full').fit(X)
    assert_equal(pca.n_components, 0.5)
    assert_equal(pca.n_components_, 2)


def test_pca_score():
    # Test that probabilistic PCASklearn scoring yields a reasonable score
    n, p = 1000, 3
    rng = np.random.RandomState(0)
    X = rng.randn(n, p) * .1 + np.array([3, 4, 5])
    for solver in solver_list:
        pca = PCASklearn(n_components=2, svd_solver=solver)
        pca.fit(X)
        ll1 = pca.score(X)
...
def test_pca_score2():
    # Test that probabilistic PCASklearn correctly separated different datasets
    n, p = 100, 3
    rng = np.random.RandomState(0)
    X = rng.randn(n, p) * .1 + np.array([3, 4, 5])
    for solver in solver_list:
...
def test_pca_score3():
    # Check that probabilistic PCASklearn selects the right model
    n, p = 200, 3
    rng = np.random.RandomState(0)
    ...
    ll = np.zeros(p)
    for k in range(p):
        pca = PCASklearn(n_components=k, svd_solver='full')
        pca.fit(Xl)
        ll[k] = pca.score(Xt)
...
def test_pca_score_with_different_solvers():
    ...
    pca_dict = {svd_solver: PCASklearn(n_components=30, svd_solver=svd_solver,
                                random_state=0)
    ...
    for pca in pca_dict.values():
        ...
        # https://github.com/h2o4gpu/h2o4gpu/issues/7568
        # https://github.com/h2o4gpu/h2o4gpu/issues/8541
        # https://github.com/h2o4gpu/h2o4gpu/issues/8544
...
def test_pca_zero_noise_variance_edge_cases():
    ...
    for svd_solver in svd_solvers:
        pca = PCASklearn(svd_solver=svd_solver, n_components=p)
        pca.fit(X)
...
def test_svd_solver_auto():
    ...
    pca = PCASklearn(n_components=.5)
    pca.fit(X)
    pca_test = PCASklearn(n_components=.5, svd_solver='full')
    pca_test.fit(X)
    assert_array_almost_equal(pca.components_, pca_test.components_)

    # case: max(X.shape) <= 500 => 'full'
    pca = PCASklearn(n_components=5, random_state=0)
    Y = X[:10, :]
    pca.fit(Y)
    pca_test = PCASklearn(n_components=5, svd_solver='full', random_state=0)
    pca_test.fit(Y)
    assert_array_almost_equal(pca.components_, pca_test.components_)

    # case: n_components >= .8 * min(X.shape) => 'full'
    pca = PCASklearn(n_components=50)
    pca.fit(X)
    pca_test = PCASklearn(n_components=50, svd_solver='full')
    pca_test.fit(X)
    assert_array_almost_equal(pca.components_, pca_test.components_)

    # n_components >= 1 and n_components < .8 * min(X.shape) => 'randomized'
    pca = PCASklearn(n_components=10, random_state=0)
    pca.fit(X)
    pca_test = PCASklearn(n_components=10, svd_solver='randomized', random_state=0)
    pca_test.fit(X)
    assert_array_almost_equal(pca.components_, pca_test.components_)
...
def test_pca_sparse_input(svd_solver):
    ...
    pca = PCASklearn(n_components=3, svd_solver=svd_solver)
...
def test_pca_bad_solver():
    X = np.random.RandomState(0).rand(5, 4)
    pca = PCASklearn(n_components=3, svd_solver='bad_argument')
    assert_raises(ValueError, pca.fit, X)
...
def test_pca_deterministic_output():
    ...
    for solver in solver_list:
        ...
        for i in range(20):
            pca = PCASklearn(n_components=2, svd_solver=solver, random_state=rng)
...
def check_pca_float_dtype_preservation(svd_solver):
    # Ensure that PCASklearn does not upscale the dtype when input is float32
    X_64 = np.random.RandomState(0).rand(1000, 4).astype(np.float64,
                                                         copy=False)
    X_32 = X_64.astype(np.float32)

    pca_64 = PCASklearn(n_components=3, svd_solver=svd_solver,
                 random_state=0).fit(X_64)
    pca_32 = PCASklearn(n_components=3, svd_solver=svd_solver,
