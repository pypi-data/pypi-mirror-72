...
from h2o4gpu.linear_model.base import _rescale_data
...
from h2o4gpu.datasets.samples_generator import make_sparse_uncorrelated
from h2o4gpu.datasets.samples_generator import make_regression
...
def test_linear_regression():
    # Test LinearRegressionSklearn on a simple dataset.
    # a simple dataset
    X = [[1], [2]]
    Y = [1, 2]
...
def test_linear_regression_sample_weights():
    ...
    for n_samples, n_features in ((6, 5), ):
        ...
        for intercept in (True, False):
            ...
            # LinearRegressionSklearn with explicit sample_weight
            reg = LinearRegressionSklearn(fit_intercept=intercept)
...
def test_fit_intercept():
    ...
    lr2_without_intercept = LinearRegressionSklearn(fit_intercept=False).fit(X2, y)
    lr2_with_intercept = LinearRegressionSklearn(fit_intercept=True).fit(X2, y)

    lr3_without_intercept = LinearRegressionSklearn(fit_intercept=False).fit(X3, y)
    lr3_with_intercept = LinearRegressionSklearn(fit_intercept=True).fit(X3, y)
...
def test_linear_regression_sparse_equal_dense(normalize, fit_intercept):
    ...
    clf_dense = LinearRegressionSklearn(**params)
    clf_sparse = LinearRegressionSklearn(**params)
    clf_dense.fit(X, y)
...
def test_linear_regression_multiple_outcome(random_state=0):
    ...
    reg = LinearRegressionSklearn(fit_intercept=True)
    reg.fit((X), Y)
