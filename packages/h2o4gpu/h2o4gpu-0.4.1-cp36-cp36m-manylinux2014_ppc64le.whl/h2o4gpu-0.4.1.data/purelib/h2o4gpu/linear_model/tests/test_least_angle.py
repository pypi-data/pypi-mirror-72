...
from h2o4gpu import linear_model, datasets
from h2o4gpu.linear_model.least_angle import _lars_path_residues, LassoSklearnLarsIC
...
def test_lars_lstsq():
    ...
    clf = linear_model.LassoSklearnLars(alpha=0.)
    clf.fit(X1, y)
...
def test_lasso_gives_lstsq_solution():
    # Test that Lars LassoSklearn gives least square solution at the end
    # of the path
...
@pytest.mark.parametrize(
        'classifier',
        [linear_model.Lars, linear_model.LarsCV, linear_model.LassoSklearnLarsIC])
...
def test_rank_deficient_design():
    # consistency test that checks that LARS LassoSklearn is handling rank
    # deficient input data (with n_features < rank) in the same way
    # as coordinate descent LassoSklearn
    y = [5, 0, 5]
    for X in (
              [[5, 0],
               [0, 5],
        ...
        lars = linear_model.LassoSklearnLars(.1, normalize=False)
        coef_lars_ = lars.fit(X, y).coef_
        obj_lars = (1. / (2. * 3.)
                    * linalg.norm(y - np.dot(X, coef_lars_)) ** 2
                    + .1 * linalg.norm(coef_lars_, 1))
        coord_descent = linear_model.LassoSklearn(.1, tol=1e-6, normalize=False)
...
def test_lasso_lars_vs_lasso_cd():
    # Test that LassoSklearnLars and LassoSklearn using coordinate descent give the
    # same results.
    X = 3 * diabetes.data

    alphas, _, lasso_path = linear_model.lars_path(X, y, method='lasso')
    lasso_cd = linear_model.LassoSklearn(fit_intercept=False, tol=1e-8)
    ...
    for alpha in np.linspace(1e-2, 1 - 1e-2, 20):
        clf1 = linear_model.LassoSklearnLars(alpha=alpha, normalize=False).fit(X, y)
        clf2 = linear_model.LassoSklearn(alpha=alpha, tol=1e-8,
    ...
    X = diabetes.data
    alphas, _, lasso_path = linear_model.lars_path(X, y, method='lasso')
    lasso_cd = linear_model.LassoSklearn(fit_intercept=False, normalize=True,
                                  tol=1e-8)
    for c, a in zip(lasso_path.T, alphas):
        if a == 0:
            continue
...
def test_lasso_lars_vs_lasso_cd_early_stopping():
    ...
    for alpha_min in alphas_min:
        alphas, _, lasso_path = linear_model.lars_path(X, y, method='lasso',
                                                       alpha_min=alpha_min)
        lasso_cd = linear_model.LassoSklearn(fit_intercept=True, normalize=True,
                                      tol=1e-8)
        lasso_cd.alpha = alphas[-1]
        lasso_cd.fit(X, y)
        error = linalg.norm(lasso_path[:, -1] - lasso_cd.coef_)
...
def test_lasso_lars_path_length():
    # Test that the path length of the LassoSklearnLars is right
    lasso = linear_model.LassoSklearnLars()
    lasso.fit(X, y)
    lasso2 = linear_model.LassoSklearnLars(alpha=lasso.alphas_[2])
    lasso2.fit(X, y)
...
def test_lasso_lars_vs_lasso_cd_ill_conditioned2():
    ...
    lars = linear_model.LassoSklearnLars(alpha=alpha, normalize=False)
    assert_warns(ConvergenceWarning, lars.fit, X, y)
    lars_coef_ = lars.coef_
    lars_obj = objective_function(lars_coef_)

    coord_descent = linear_model.LassoSklearn(alpha=alpha, tol=1e-4, normalize=False)
...
def test_multitarget():
    ...
    n_targets = Y.shape[1]
    estimators = [
        linear_model.LassoSklearnLars(),
        linear_model.Lars(),
        # regression test for gh-1615
        linear_model.LassoSklearnLars(fit_intercept=False),
...
@pytest.mark.filterwarnings('ignore: The default value of cv')  # 0.22
def test_lars_cv():
    # Test the LassoSklearnLarsCV object by checking that the optimal alpha
    # increases as the number of samples increases.
    # This property is not actually guaranteed in general and is just a
    # property of the given dataset, with the given steps chosen.
    old_alpha = 0
    lars_cv = linear_model.LassoSklearnLarsCV()
...
def test_lars_cv_max_iter(recwarn):
    ...
    with np.errstate(divide='raise', invalid='raise'):
        ...
        lars_cv = linear_model.LassoSklearnLarsCV(max_iter=5, cv=5)
        lars_cv.fit(X, y)
...
def test_lasso_lars_ic():
    # Test the LassoSklearnLarsIC object by checking that
    # - some good features are selected.
    # - alpha_bic > alpha_aic
    # - n_nonzero_bic < n_nonzero_aic
    lars_bic = linear_model.LassoSklearnLarsIC('bic')
    lars_aic = linear_model.LassoSklearnLarsIC('aic')
    rng = np.random.RandomState(42)
    X = diabetes.data
    ...
    lars_broken = linear_model.LassoSklearnLarsIC('<unknown>')
...
def test_lars_path_readonly_data():
    ...
    # https://github.com/h2o4gpu/h2o4gpu/issues/4597
...
estimator_parameter_map = {'LassoSklearnLars': {'alpha': 0.1},
                           'LassoSklearnLarsCV': {},
                           'LassoSklearnLarsIC': {}}
...
def test_lasso_lars_vs_lasso_cd_positive():
    ...
    alphas, _, lasso_path = linear_model.lars_path(X, y, method='lasso',
                                                   positive=True)
    lasso_cd = linear_model.LassoSklearn(fit_intercept=False, tol=1e-8, positive=True)
    for c, a in zip(lasso_path.T, alphas):
        if a == 0:
            continue
        lasso_cd.alpha = a
    ...
    # to the circumstance that the Lars-LassoSklearn algorithm does not converge to
    # the least-squares-solution for small alphas, see 'Least Angle Regression'
    # by Efron et al 2004. The coefficients are typically in congruence up to
    # the smallest alpha reached by the Lars-LassoSklearn algorithm and start to
    # diverge thereafter.  See
    # https://gist.github.com/michigraber/7e7d7c75eca694c7a6ff

    for alpha in np.linspace(6e-1, 1 - 1e-2, 20):
        clf1 = linear_model.LassoSklearnLars(fit_intercept=False, alpha=alpha,
                                      normalize=False, positive=True).fit(X, y)
        clf2 = linear_model.LassoSklearn(fit_intercept=False, alpha=alpha, tol=1e-8,
                                  normalize=False, positive=True).fit(X, y)
        err = linalg.norm(clf1.coef_ - clf2.coef_)
        assert_less(err, 1e-3)
...
def test_lasso_lars_vs_R_implementation():
    # Test that h2o4gpu LassoSklearnLars implementation agrees with the LassoSklearnLars
    ...
    ###########################################################################
    # Scenario 1: Let's compare R vs h2o4gpu when fit_intercept=False and
    # normalize=False
    ###########################################################################
    #
    ...
    model_lasso_lars = linear_model.LassoSklearnLars(alpha=0, fit_intercept=False,
                                              normalize=False)
    ...
    ###########################################################################
    # Scenario 2: Let's compare R vs h2o4gpu when fit_intercept=True and
    # normalize=True
    #
    # Note: When normalize is equal to True, R returns the coefficients in
    # their original units, that is, they are rescaled back, whereas h2o4gpu
    ...
    model_lasso_lars2 = linear_model.LassoSklearnLars(alpha=0, fit_intercept=True,
                                               normalize=True)
    model_lasso_lars2.fit(X, y)
    skl_betas2 = model_lasso_lars2.coef_path_

    # Let's rescale back the coefficients returned by h2o4gpu before comparing
...
def test_lasso_lars_copyX_behaviour(copy_X):
    ...
    """
    lasso_lars = LassoSklearnLarsIC(copy_X=copy_X, precompute=False)
    rng = np.random.RandomState(0)
...
def test_lasso_lars_fit_copyX_behaviour(copy_X):
    ...
    """
    lasso_lars = LassoSklearnLarsIC(precompute=False)
    rng = np.random.RandomState(0)
    X = rng.normal(0, 1, (100, 5))
    X_copy = X.copy()
    y = X[:, 2]
