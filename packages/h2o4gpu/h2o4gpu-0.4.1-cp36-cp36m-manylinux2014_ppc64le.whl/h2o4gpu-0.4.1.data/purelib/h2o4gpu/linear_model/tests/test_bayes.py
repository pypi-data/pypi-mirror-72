...
from h2o4gpu.linear_model.bayes import BayesianRidgeSklearn, ARDRegression
from h2o4gpu.linear_model import RidgeSklearn
from h2o4gpu import datasets
from h2o4gpu.utils.extmath import fast_logdet
...
def test_n_iter():
    ...
    clf = BayesianRidgeSklearn(n_iter=0)
...
def test_bayesian_ridge_scores():
    ...
    clf = BayesianRidgeSklearn(compute_score=True)
    clf.fit(X, y)
...
def test_bayesian_ridge_score_values():
    ...
    # compute score with BayesianRidgeSklearn
    clf = BayesianRidgeSklearn(alpha_1=alpha_1, alpha_2=alpha_2,
...
def test_bayesian_ridge_parameter():
    ...
    # A RidgeSklearn regression model using an alpha value equal to the ratio of
    # lambda_ and alpha_ from the Bayesian RidgeSklearn model must be identical
    br_model = BayesianRidgeSklearn(compute_score=True).fit(X, y)
    rr_model = RidgeSklearn(alpha=br_model.lambda_ / br_model.alpha_).fit(X, y)
...
def test_bayesian_sample_weights():
    ...
    # A RidgeSklearn regression model using an alpha value equal to the ratio of
    # lambda_ and alpha_ from the Bayesian RidgeSklearn model must be identical
    br_model = BayesianRidgeSklearn(compute_score=True).fit(X, y, sample_weight=w)
    rr_model = RidgeSklearn(alpha=br_model.lambda_ / br_model.alpha_).fit(
        X, y, sample_weight=w)
    assert_array_almost_equal(rr_model.coef_, br_model.coef_)
    assert_almost_equal(rr_model.intercept_, br_model.intercept_)
...
def test_toy_bayesian_ridge_object():
    # Test BayesianRidgeSklearn on toy
    X = np.array([[1], [2], [6], [8], [10]])
    Y = np.array([1, 2, 6, 8, 10])
    clf = BayesianRidgeSklearn(compute_score=True)
    clf.fit(X, Y)
...
def test_prediction_bayesian_ridge_ard_with_constant_input():
    # Test BayesianRidgeSklearn and ARDRegression predictions for edge case of
    ...
    for clf in [BayesianRidgeSklearn(), ARDRegression()]:
        y_pred = clf.fit(X, y).predict(X)
        assert_array_almost_equal(y_pred, expected)


def test_std_bayesian_ridge_ard_with_constant_input():
    # Test BayesianRidgeSklearn and ARDRegression standard dev. for edge case of
