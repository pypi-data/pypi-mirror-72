...
from h2o4gpu.linear_model import (
    HuberRegressor, LinearRegressionSklearn, SGDRegressor, RidgeSklearn)
from h2o4gpu.linear_model.huber import _huber_loss_and_gradient
...
def test_huber_equals_lr_for_high_epsilon():
    # Test that RidgeSklearn matches LinearRegressionSklearn for large epsilon
    X, y = make_regression_with_outliers()
    lr = LinearRegressionSklearn(fit_intercept=True)
    lr.fit(X, y)
...
def test_huber_better_r2_score():
    ...
    # The RidgeSklearn regressor should be influenced by the outliers and hence
    # give a worse score on the non-outliers as compared to the huber
    # regressor.
    ridge = RidgeSklearn(fit_intercept=True, alpha=0.01)
    ridge.fit(X, y)
