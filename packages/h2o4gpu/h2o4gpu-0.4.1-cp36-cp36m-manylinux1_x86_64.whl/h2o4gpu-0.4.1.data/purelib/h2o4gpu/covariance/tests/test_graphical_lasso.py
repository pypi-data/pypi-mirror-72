...
from h2o4gpu.covariance import (graphical_lasso, GraphicalLassoSklearn,
                                GraphicalLassoSklearnCV, empirical_covariance)
...
def test_graphical_lasso(random_state=0):
    ...
    model = GraphicalLassoSklearn(alpha=.25).fit(X)
    model.score(X)
...
def test_graphical_lasso_cv(random_state=1):
    ...
    try:
        ...
        GraphicalLassoSklearnCV(verbose=100, alphas=5, tol=1e-1).fit(X)
    finally:
        sys.stdout = orig_stdout

    # Smoke test with specified alphas
    GraphicalLassoSklearnCV(alphas=[0.8, 0.5], tol=1e-1, n_jobs=1).fit(X)
