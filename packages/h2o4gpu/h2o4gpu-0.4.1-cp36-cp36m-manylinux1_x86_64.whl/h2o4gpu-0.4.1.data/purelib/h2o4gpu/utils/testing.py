...
import h2o4gpu
from h2o4gpu.base import (BaseEstimator, ClassifierMixin, ClusterMixin,
                          RegressorMixin, TransformerMixin)
from h2o4gpu.utils import deprecated, IS_PYPY, _IS_32BIT
from h2o4gpu.utils._joblib import joblib
from h2o4gpu.utils._unittest_backport import TestCase

additional_names_in_all = []
try:
    from nose.tools import raises as _nose_raises
    deprecation_message = (
        'h2o4gpu.utils.testing.raises has been deprecated in version 0.20 '
        'and will be removed in 0.22. Please use '
        'h2o4gpu.utils.testing.assert_raises instead.')
...
try:
    ...
    deprecation_message = (
        'h2o4gpu.utils.testing.with_setup has been deprecated in version 0.20 '
...
# assert_raises_regex but lets keep the backward compat in h2o4gpu with
...
def all_estimators(include_meta_estimators=None,
    ...
    """Get a list of all estimators from h2o4gpu.
    ...
    all_classes = []
    # get parent folder
    path = h2o4gpu.__path__
    for importer, modname, ispkg in pkgutil.walk_packages(
            path=path, prefix='h2o4gpu.', onerror=lambda x: None):
...
def create_memmap_backed_data(data, mmap_mode='r', return_folder=False):
    ...
    return_folder
    """
    temp_folder = tempfile.mkdtemp(prefix='h2o4gpu_testing_')
...
def check_docstring_parameters(func, doc=None, ignore=None, class_name=None):
    ...
    if (not func_name.startswith('h2o4gpu.') or
            func_name.startswith('h2o4gpu.externals')):
        return incorrect
...
def assert_run_python_script(source_code, timeout=60):
    ...
    """
    fd, source_file = tempfile.mkstemp(suffix='_src_test_h2o4gpu.py')
    os.close(fd)
    try:
        with open(source_file, 'wb') as f:
            f.write(source_code.encode('utf-8'))
        cmd = [sys.executable, source_file]
        cwd = op.normpath(op.join(op.dirname(h2o4gpu.__file__), '..'))
        env = os.environ.copy()
        kwargs = {
            'cwd': cwd,
            'stderr': STDOUT,
