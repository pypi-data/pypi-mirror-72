...
from h2o4gpu.base import clone
from h2o4gpu.datasets import load_iris, make_classification
from h2o4gpu.metrics import log_loss
from h2o4gpu.metrics.scorer import get_scorer
from h2o4gpu.model_selection import StratifiedKFold
from h2o4gpu.model_selection import GridSearchCV
from h2o4gpu.model_selection import train_test_split
from h2o4gpu.preprocessing import LabelEncoder
from h2o4gpu.utils import compute_class_weight, _IS_32BIT
from h2o4gpu.utils.testing import assert_almost_equal
from h2o4gpu.utils.testing import assert_allclose
from h2o4gpu.utils.testing import assert_array_almost_equal
from h2o4gpu.utils.testing import assert_array_equal
from h2o4gpu.utils.testing import assert_equal
from h2o4gpu.utils.testing import assert_greater
from h2o4gpu.utils.testing import assert_raise_message
from h2o4gpu.utils.testing import assert_raises
from h2o4gpu.utils.testing import assert_warns
from h2o4gpu.utils.testing import ignore_warnings
from h2o4gpu.utils.testing import assert_warns_message
from h2o4gpu.utils.testing import assert_no_warnings
from h2o4gpu.linear_model import SGDClassifier
from h2o4gpu.preprocessing import scale
from h2o4gpu.utils.testing import skip_if_no_parallel

from h2o4gpu.exceptions import ConvergenceWarning
from h2o4gpu.exceptions import ChangedBehaviorWarning
from h2o4gpu.linear_model.logistic import (
    LogisticRegressionSklearn,
    logistic_regression_path,
    _logistic_regression_path, LogisticRegressionSklearnCV,
...
def test_predict_2_classes():
    ...
    check_predictions(LogisticRegressionSklearn(random_state=0), X, Y1)
    check_predictions(LogisticRegressionSklearn(random_state=0), X_sp, Y1)

    check_predictions(LogisticRegressionSklearn(C=100, random_state=0), X, Y1)
    check_predictions(LogisticRegressionSklearn(C=100, random_state=0), X_sp, Y1)

    check_predictions(LogisticRegressionSklearn(fit_intercept=False,
                                         random_state=0), X, Y1)
    check_predictions(LogisticRegressionSklearn(fit_intercept=False,
...
def test_error():
    ...
    assert_raise_message(ValueError, msg,
                         LogisticRegressionSklearn(C=-1).fit, X, Y1)
    assert_raise_message(ValueError, msg,
                         LogisticRegressionSklearn(C="test").fit, X, Y1)

    msg = "is not a valid scoring value"
    assert_raise_message(ValueError, msg,
                         LogisticRegressionSklearnCV(scoring='bad-scorer', cv=2).fit,
                         X, Y1)

    for LR in [LogisticRegressionSklearn, LogisticRegressionSklearnCV]:
...
def test_logistic_cv_mock_scorer():
    ...
    lr = LogisticRegressionSklearnCV(Cs=Cs, scoring=mock_scorer, cv=cv)
    lr.fit(X, Y1)
...
def test_logistic_cv_score_does_not_warn_by_default():
    lr = LogisticRegressionSklearnCV(cv=2, multi_class='ovr')
    lr.fit(X, Y1)
...
def test_lr_liblinear_warning():
    ...
    lr = LogisticRegressionSklearn(solver='liblinear', multi_class='ovr', n_jobs=2)
...
def test_predict_3_classes():
    check_predictions(LogisticRegressionSklearn(C=10), X, Y2)
    check_predictions(LogisticRegressionSklearn(C=10), X_sp, Y2)
...
def test_predict_iris():
    ...
    for clf in [LogisticRegressionSklearn(C=len(iris.data), solver='liblinear',
                                   multi_class='ovr'),
                LogisticRegressionSklearn(C=len(iris.data), solver='lbfgs',
                                   multi_class='multinomial'),
                LogisticRegressionSklearn(C=len(iris.data), solver='newton-cg',
                                   multi_class='multinomial'),
                LogisticRegressionSklearn(C=len(iris.data), solver='sag', tol=1e-2,
                                   multi_class='ovr', random_state=42),
                LogisticRegressionSklearn(C=len(iris.data), solver='saga', tol=1e-2,
                                   multi_class='ovr', random_state=42)
                ]:
...
def test_multinomial_validation(solver):
    lr = LogisticRegressionSklearn(C=-1, solver=solver, multi_class='multinomial')
    assert_raises(ValueError, lr.fit, [[0, 1], [1, 0]], [0, 1])


@pytest.mark.filterwarnings('ignore: Default multi_class will')  # 0.22
@pytest.mark.parametrize('LR', [LogisticRegressionSklearn, LogisticRegressionSklearnCV])
...
@pytest.mark.parametrize('model, params, warn_solver',
                         [(LogisticRegressionSklearn, {}, True),
                          (LogisticRegressionSklearnCV, {'cv': 5}, False)])
...
def test_multinomial_binary(solver):
    ...
    clf = LogisticRegressionSklearn(solver=solver, multi_class='multinomial',
    ...
    mlr = LogisticRegressionSklearn(solver=solver, multi_class='multinomial',
...
def test_multinomial_binary_probabilities():
    ...
    clf = LogisticRegressionSklearn(multi_class='multinomial', solver='saga')
    clf.fit(X, y)
...
def test_sparsify():
    ...
    n_samples, n_features = iris.data.shape
    target = iris.target_names[iris.target]
    clf = LogisticRegressionSklearn(random_state=0).fit(iris.data, target)
...
def test_inconsistent_input():
    ...
    clf = LogisticRegressionSklearn(random_state=0)
...
def test_nan():
    ...
    Xnan[0, 1] = np.nan
    logistic = LogisticRegressionSklearn(random_state=0)
...
def test_consistency_path():
    ...
    for solver in ['sag', 'saga']:
        ...
        for i, C in enumerate(Cs):
            lr = LogisticRegressionSklearn(C=C, fit_intercept=False, tol=1e-5,
    ...
    for solver in ('lbfgs', 'newton-cg', 'liblinear', 'sag', 'saga'):
        ...
        lr = LogisticRegressionSklearn(C=Cs[0], fit_intercept=True, tol=1e-4,
...
def test_liblinear_dual_random_state():
    ...
    lr1 = LogisticRegressionSklearn(random_state=0, dual=True, max_iter=1, tol=1e-15,
                             solver='liblinear', multi_class='ovr')
    lr1.fit(X, y)
    lr2 = LogisticRegressionSklearn(random_state=0, dual=True, max_iter=1, tol=1e-15,
                             solver='liblinear', multi_class='ovr')
    lr2.fit(X, y)
    lr3 = LogisticRegressionSklearn(random_state=8, dual=True, max_iter=1, tol=1e-15,
                             solver='liblinear', multi_class='ovr')
    lr3.fit(X, y)
...
def test_logistic_cv():
    # test for LogisticRegressionSklearnCV object
    n_samples, n_features = 50, 5
    rng = np.random.RandomState(0)
    X_ref = rng.randn(n_samples, n_features)
    y = np.sign(X_ref.dot(5 * rng.randn(n_features)))
    X_ref -= X_ref.mean()
    X_ref /= X_ref.std()
    lr_cv = LogisticRegressionSklearnCV(Cs=[1.], fit_intercept=False,
                                 solver='liblinear', multi_class='ovr')
    lr_cv.fit(X_ref, y)
    lr = LogisticRegressionSklearn(C=1., fit_intercept=False,
                            solver='liblinear', multi_class='ovr')
    lr.fit(X_ref, y)
...
@pytest.mark.parametrize('scoring, multiclass_agg_list',
                         [('accuracy', ['']),
                          ...
                          # h2o4gpu/h2o4gpu/pull/
                          # 11578#discussion_r203250062)
                          ('f1', ['_macro', '_weighted']),
                          ('neg_log_loss', ['']),
                          ('recall', ['_macro', '_weighted'])])
def test_logistic_cv_multinomial_score(scoring, multiclass_agg_list):
    # test that LogisticRegressionSklearnCV uses the right score to compute its
    # cross-validation scores when using a multinomial scoring
    # see https://github.com/h2o4gpu/h2o4gpu/issues/8720
    X, y = make_classification(n_samples=100, random_state=0, n_classes=3,
                               n_informative=6)
    train, test = np.arange(80), np.arange(80, 100)
    lr = LogisticRegressionSklearn(C=1., solver='lbfgs', multi_class='multinomial')
...
def test_multinomial_logistic_regression_string_inputs():
    # Test with string labels for LogisticRegressionSklearn(CV)
    ...
    lr = LogisticRegressionSklearn(solver='lbfgs', multi_class='multinomial')
    lr_cv = LogisticRegressionSklearnCV(solver='lbfgs', multi_class='multinomial')
    lr_str = LogisticRegressionSklearn(solver='lbfgs', multi_class='multinomial')
    lr_cv_str = LogisticRegressionSklearnCV(solver='lbfgs', multi_class='multinomial')
    ...
    lr_cv_str = LogisticRegressionSklearn(
...
def test_logistic_cv_sparse():
    ...
    clf = LogisticRegressionSklearnCV(fit_intercept=True)
    clf.fit(X, y)
    clfs = LogisticRegressionSklearnCV(fit_intercept=True)
    clfs.fit(csr, y)
...
def test_ovr_multinomial_iris():
    ...
    clf = LogisticRegressionSklearnCV(cv=precomputed_folds, multi_class='ovr')
    clf.fit(train, target)

    # Conflate classes 0 and 1 and train clf1 on this modified dataset
    clf1 = LogisticRegressionSklearnCV(cv=precomputed_folds, multi_class='ovr')
    ...
    for solver in ['lbfgs', 'newton-cg', 'sag', 'saga']:
        ...
        clf_multi = LogisticRegressionSklearnCV(
        ...
        # Test attributes of LogisticRegressionSklearnCV
...
def test_logistic_regression_solvers():
    ...
    ncg = LogisticRegressionSklearn(solver='newton-cg', **params)
    lbf = LogisticRegressionSklearn(solver='lbfgs', **params)
    lib = LogisticRegressionSklearn(solver='liblinear', **params)
    sag = LogisticRegressionSklearn(solver='sag', **params)
    saga = LogisticRegressionSklearn(solver='saga', **params)
    ncg.fit(X, y)
    lbf.fit(X, y)
    sag.fit(X, y)
    saga.fit(X, y)
...
def test_logistic_regression_solvers_multiclass():
    ...
    lbf = LogisticRegressionSklearn(solver='lbfgs', **params)
    lib = LogisticRegressionSklearn(solver='liblinear', **params)
    sag = LogisticRegressionSklearn(solver='sag', max_iter=1000, **params)
    saga = LogisticRegressionSklearn(solver='saga', max_iter=10000, **params)
    ncg.fit(X, y)
    lbf.fit(X, y)
    sag.fit(X, y)
    saga.fit(X, y)
...
def test_logistic_regressioncv_class_weights():
    for weight in [{0: 0.1, 1: 0.2}, {0: 0.1, 1: 0.2, 2: 0.5}]:
        ...
        for class_weight in (weight, 'balanced'):
            ...
            clf_lbf = LogisticRegressionSklearnCV(solver='lbfgs', Cs=1,
                                           fit_intercept=False,
                                           multi_class='ovr',
                                           class_weight=class_weight)
            clf_ncg = LogisticRegressionSklearnCV(solver='newton-cg', Cs=1,
                                           fit_intercept=False,
                                           multi_class='ovr',
                                           class_weight=class_weight)
            clf_lib = LogisticRegressionSklearnCV(solver='liblinear', Cs=1,
                                           fit_intercept=False,
                                           multi_class='ovr',
                                           class_weight=class_weight)
            clf_sag = LogisticRegressionSklearnCV(solver='sag', Cs=1,
                                           fit_intercept=False,
                                           multi_class='ovr',
                                           class_weight=class_weight,
                                           tol=1e-5, max_iter=10000,
                                           random_state=0)
            clf_saga = LogisticRegressionSklearnCV(solver='saga', Cs=1,
                                            fit_intercept=False,
                                            multi_class='ovr',
                                            class_weight=class_weight,
                                            tol=1e-5, max_iter=10000,
...
def test_logistic_regression_sample_weights():
    ...
    clf_cw = LogisticRegressionSklearn(
        solver="liblinear", fit_intercept=False, class_weight={0: 1, 1: 2},
        penalty="l1", tol=1e-5, random_state=42, multi_class='ovr')
    clf_cw.fit(X, y)
    clf_sw = LogisticRegressionSklearn(
...
def test_logistic_regression_class_weights():
    ...
    for solver in solvers:
        clf1 = LogisticRegressionSklearn(solver=solver, multi_class="multinomial",
                                  class_weight="balanced")
        clf2 = LogisticRegressionSklearn(solver=solver, multi_class="multinomial",
    ...
    for solver in solvers:
        clf1 = LogisticRegressionSklearn(solver=solver, multi_class="ovr",
                                  class_weight="balanced")
        clf2 = LogisticRegressionSklearn(solver=solver, multi_class="ovr",
                                  class_weight=class_weight_dict)
        clf1.fit(X, y)
        clf2.fit(X, y)
...
def test_logistic_regression_multinomial():
    ...
    solver = 'lbfgs'
    ref_i = LogisticRegressionSklearn(solver=solver, multi_class='multinomial')
    ref_w = LogisticRegressionSklearn(solver=solver, multi_class='multinomial',
                               fit_intercept=False)
    ref_i.fit(X, y)
    ref_w.fit(X, y)
    ...
    for solver in ['sag', 'saga', 'newton-cg']:
        clf_i = LogisticRegressionSklearn(solver=solver, multi_class='multinomial',
                                   random_state=42, max_iter=2000, tol=1e-7,
                                   )
        clf_w = LogisticRegressionSklearn(solver=solver, multi_class='multinomial',
                                   random_state=42, max_iter=2000, tol=1e-7,
                                   fit_intercept=False)
        clf_i.fit(X, y)
        clf_w.fit(X, y)
    ...
    for solver in ['lbfgs', 'newton-cg', 'sag', 'saga']:
        clf_path = LogisticRegressionSklearnCV(solver=solver, max_iter=2000, tol=1e-6,
...
def test_liblinear_decision_function_zero():
    ...
    # See Issue: https://github.com/h2o4gpu/h2o4gpu/issues/3600
    # and the PR https://github.com/h2o4gpu/h2o4gpu/pull/3623
    X, y = make_classification(n_samples=5, n_features=5, random_state=0)
    clf = LogisticRegressionSklearn(fit_intercept=False, solver='liblinear',
                             multi_class='ovr')
    clf.fit(X, y)
...
def test_liblinear_logregcv_sparse():
    ...
    clf = LogisticRegressionSklearnCV(solver='liblinear', multi_class='ovr')
...
def test_saga_sparse():
    ...
    X, y = make_classification(n_samples=10, n_features=5, random_state=0)
    clf = LogisticRegressionSklearnCV(solver='saga')
    clf.fit(sparse.csr_matrix(X), y)
...
def test_logreg_intercept_scaling():
    ...
    for i in [-1, 0]:
        clf = LogisticRegressionSklearn(intercept_scaling=i, solver='liblinear',
                                 multi_class='ovr')
...
def test_logreg_intercept_scaling_zero():
    ...
    clf = LogisticRegressionSklearn(fit_intercept=False)
    clf.fit(X, Y1)
...
def test_logreg_l1():
    ...
    lr_liblinear = LogisticRegressionSklearn(penalty="l1", C=1.0, solver='liblinear',
                                      fit_intercept=False, multi_class='ovr',
                                      tol=1e-10)
    lr_liblinear.fit(X, y)

    lr_saga = LogisticRegressionSklearn(penalty="l1", C=1.0, solver='saga',
                                 fit_intercept=False, multi_class='ovr',
...
def test_logreg_l1_sparse_data():
    ...
    lr_saga_dense = LogisticRegressionSklearn(penalty="l1", C=1.0, solver='saga',
                                       fit_intercept=False, multi_class='ovr',
                                       max_iter=1000, tol=1e-10)
...
def test_logistic_regression_cv_refit(random_seed, penalty):
    ...
    # Internally the LogisticRegressionSklearnCV model uses a warm start to refit on
    ...
    lr_cv = LogisticRegressionSklearnCV(Cs=[1.0], refit=True, **common_params)
    lr_cv.fit(X, y)
    lr = LogisticRegressionSklearn(C=1.0, **common_params)
    lr.fit(X, y)
...
def test_logreg_predict_proba_multinomial():
    ...
    clf_multi = LogisticRegressionSklearn(multi_class="multinomial", solver="lbfgs")
    clf_multi.fit(X, y)
    clf_multi_loss = log_loss(y, clf_multi.predict_proba(X))
    clf_ovr = LogisticRegressionSklearn(multi_class="ovr", solver="lbfgs")
    clf_ovr.fit(X, y)
        ...
        for solver in solvers:
            for multi_class in ['ovr', 'multinomial']:
                if solver == 'liblinear' and multi_class == 'multinomial':
                    continue
                lr = LogisticRegressionSklearn(max_iter=max_iter, tol=1e-15,
...
def test_n_iter(solver):
    ...
    clf = LogisticRegressionSklearn(tol=1e-2, multi_class='ovr',
                             solver=solver, C=1.,
    ...
    clf = LogisticRegressionSklearnCV(tol=1e-2, multi_class='ovr',
    ...
    clf = LogisticRegressionSklearn(tol=1e-2, multi_class='multinomial',
                             solver=solver, C=1.,
                             random_state=42, max_iter=100)
    clf.fit(X, y)
    assert_equal(clf.n_iter_.shape, (n_classes,))

    clf = LogisticRegressionSklearnCV(tol=1e-2, multi_class='multinomial',
                               solver=solver, Cs=n_Cs, cv=n_cv_fold,
                               random_state=42, max_iter=100)
    clf.fit(X, y)
    assert_equal(clf.n_iter_.shape, (n_classes, n_cv_fold, n_Cs))
...
def test_warm_start(solver, warm_start, fit_intercept, multi_class):
    ...
    clf = LogisticRegressionSklearn(tol=1e-4, multi_class=multi_class,
    ...
    for (X, y) in ((X_bin, y_bin), (X_sparse, y_sparse)):
        for penalty in ['l1', 'l2']:
            ...
            for alpha in np.logspace(-1, 1, 3):
                saga = LogisticRegressionSklearn(
                    C=1. / (n_samples * alpha),
                    solver='saga',
                    multi_class='ovr',
                    max_iter=200,
                ...
                liblinear = LogisticRegressionSklearn(
                    C=1. / (n_samples * alpha),
                    solver='liblinear',
                    multi_class='ovr',
                    max_iter=200,
...
def test_dtype_match(solver, multi_class):
    ...
    lr_32 = LogisticRegressionSklearn(solver=solver, multi_class=multi_class,
                               random_state=42)
    lr_32.fit(X_32, y_32)
    assert_equal(lr_32.coef_.dtype, X_32.dtype)

    # check consistency with sparsity
    lr_32_sparse = LogisticRegressionSklearn(solver=solver,
                                      multi_class=multi_class,
                                      random_state=42)
    ...
    lr_64 = LogisticRegressionSklearn(solver=solver, multi_class=multi_class,
                               random_state=42)
    lr_64.fit(X_64, y_64)
...
def test_warm_start_converge_LR():
    ...
    lr_no_ws = LogisticRegressionSklearn(multi_class='multinomial',
                                  solver='sag', warm_start=False,
                                  random_state=0)
    lr_ws = LogisticRegressionSklearn(multi_class='multinomial',
...
def test_elastic_net_coeffs():
    ...
    for penalty in ('elasticnet', 'l1', 'l2'):
        lr = LogisticRegressionSklearn(penalty=penalty, C=C, solver='saga',
...
def test_elastic_net_l1_l2_equivalence(C, penalty, l1_ratio):
    ...
    lr_enet = LogisticRegressionSklearn(penalty='elasticnet', C=C, l1_ratio=l1_ratio,
                                 solver='saga', random_state=0)
    lr_expected = LogisticRegressionSklearn(penalty=penalty, C=C, solver='saga',
                                     random_state=0)
    lr_enet.fit(X, y)
    lr_expected.fit(X, y)
...
def test_elastic_net_vs_l1_l2(C):
    ...
    enet_clf = LogisticRegressionSklearn(penalty='elasticnet', C=C, solver='saga',
                                  random_state=0)
    gs = GridSearchCV(enet_clf, param_grid, cv=5, iid=False, refit=True)

    l1_clf = LogisticRegressionSklearn(penalty='l1', C=C, solver='saga',
                                random_state=0)
    l2_clf = LogisticRegressionSklearn(penalty='l2', C=C, solver='saga',
                                random_state=0)
...
def test_LogisticRegressionSklearn_elastic_net_objective(C, l1_ratio):
    ...
    lr_enet = LogisticRegressionSklearn(penalty='elasticnet', solver='saga',
                                 random_state=0, C=C, l1_ratio=l1_ratio,
                                 fit_intercept=False)
    lr_l2 = LogisticRegressionSklearn(penalty='l2', solver='saga', random_state=0,
...
@pytest.mark.parametrize('multi_class', ('ovr', 'multinomial'))
def test_LogisticRegressionSklearnCV_GridSearchCV_elastic_net(multi_class):
    # make sure LogisticRegressionSklearnCV gives same best params (l1 and C) as
    # GridSearchCV when penalty is elasticnet

    if multi_class == 'ovr':
        # This is actually binary classification, ovr multiclass is treated in
        # test_LogisticRegressionSklearnCV_GridSearchCV_elastic_net_ovr
        X, y = make_classification(random_state=0)
    else:
    ...
    lrcv = LogisticRegressionSklearnCV(penalty='elasticnet', Cs=Cs, solver='saga',
                                cv=cv, l1_ratios=l1_ratios, random_state=0,
                                multi_class=multi_class)
    lrcv.fit(X, y)

    param_grid = {'C': Cs, 'l1_ratio': l1_ratios}
    lr = LogisticRegressionSklearn(penalty='elasticnet', solver='saga',
...
def test_LogisticRegressionSklearnCV_GridSearchCV_elastic_net_ovr():
    # make sure LogisticRegressionSklearnCV gives same best params (l1 and C) as
    ...
    # LogisticRegressionSklearnCV with multi_class='ovr' will have one C and one
    # l1_param for each class, while LogisticRegressionSklearn will share the
...
@pytest.mark.parametrize('multi_class', ('ovr', 'multinomial'))
def test_LogisticRegressionSklearnCV_no_refit(multi_class):
    # Test LogisticRegressionSklearnCV attribute shapes when refit is False
...
@pytest.mark.filterwarnings('ignore: Default multi_class will')  # 0.22
def test_LogisticRegressionSklearnCV_elasticnet_attribute_shapes():
...
def test_l1_ratio_param(l1_ratio):
    ...
    assert_raise_message(ValueError, msg,
                         LogisticRegressionSklearn(penalty='elasticnet',
                                            solver='saga',
    ...
    if l1_ratio is not None:
        ...
        assert_warns_message(UserWarning, msg,
                             LogisticRegressionSklearn(penalty='l1', solver='saga',
                                                l1_ratio=l1_ratio).fit, X, Y1)
...
def test_l1_ratios_param(l1_ratios):
    ...
    assert_raise_message(ValueError, msg,
                         LogisticRegressionSklearnCV(penalty='elasticnet',
                                              solver='saga',
    ...
    if l1_ratios is not None:
        ...
        function = LogisticRegressionSklearnCV(penalty='l1', solver='saga',
                                        l1_ratios=l1_ratios, cv=2).fit
...
def test_elastic_net_versus_sgd(C, l1_ratio):
    # Compare elasticnet penalty in LogisticRegressionSklearn() and SGD(loss='log')
    n_samples = 500
...
@pytest.mark.parametrize('est', [LogisticRegressionSklearn(random_state=0),
                                 LogisticRegressionSklearnCV(random_state=0, cv=3),
                                 ])
...
def test_logistic_regression_multi_class_auto(est, solver):
    ...
    if solver == 'liblinear':
        ...
        if sys.platform == 'darwin' and solver == 'lbfgs':
            pytest.xfail('Issue #11924: LogisticRegressionSklearnCV(solver="lbfgs", '
...
def test_penalty_none(solver):
    ...
    lr = LogisticRegressionSklearn(penalty='none', solver=solver, C=4)
    assert_warns_message(UserWarning, msg, lr.fit, X, y)

    lr_none = LogisticRegressionSklearn(penalty='none', solver=solver,
                                 random_state=0)
    lr_l2_C_inf = LogisticRegressionSklearn(penalty='l2', C=np.inf, solver=solver,
                                     random_state=0)
    pred_none = lr_none.fit(X, y).predict(X)
    pred_l2_C_inf = lr_l2_C_inf.fit(X, y).predict(X)
    assert_array_equal(pred_none, pred_l2_C_inf)

    lr = LogisticRegressionSklearnCV(penalty='none')
