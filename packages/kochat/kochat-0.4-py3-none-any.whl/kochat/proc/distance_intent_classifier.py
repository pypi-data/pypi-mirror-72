...
from kochat.loss import BaseLoss
from kochat.proc.base import TorchProcessor
from kochat.proc.intent.distance_estimator import DistanceEstimator
from kochat.proc import FallbackDetector


@intent
class DistanceIntentClassifier(TorchProcessor):
    ...
    def _forward(self, feats: Tensor, labels: Tensor = None, lengths: Tensor = None) -> tuple:
        ...
        feats = self.model.distance_features(feats)
        logits = self.model.distance_classifier(feats)
