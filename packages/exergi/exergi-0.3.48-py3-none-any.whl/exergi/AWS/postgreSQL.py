...
def config(filePath: str ='/home/ec2-user/SageMaker/.config/database.ini', 
    ...
    Returns:
        - db_connector  - All connection parameters in the specified filePath   
                          and section
    """
    ...
    db_connector = {}                     # Get section, default to postgresql
    
    # Checks to see if section (postgresql) parser exists
    if parser.has_section(section):
        params = parser.items(section)
        for param in params:
            db_connector[param[0]] = param[1]
...
def checkPostgreSQLConnection(db_connector: dict):
    """A function that checks connection settings and version of postgreSQL
    
    Arguments:
        - db_connector        - Connection parameters from config file
    """

    import psycopg2

    try:
        connection = psycopg2.connect(**db_connector)
...
def importDataFromPostgreSQL(sql_query: str,db_connector: dict):
