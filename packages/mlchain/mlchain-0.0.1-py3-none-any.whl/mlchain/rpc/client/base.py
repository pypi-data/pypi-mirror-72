...
class MLClient:
    def __init__(self, api_key=None, api_address=None, serializer='msgpack', image_encoder=None, timeout=None,
                 name=None,
    ...
    def store(self):
        if self.store_ is None:
            self.store_ = AsyncStorage(self.__get_function('store_get'))
        return self.store_
    ...
    def post(self, function_name, headers=None, args=None, kwargs=None):
        return self._post(function_name, self.headers(), args, kwargs)

    def _get(self, api_name, headers=None):
        raise NotImplementedError

    def get(self, api_name, headers=None):
        return self._get(api_name, self.headers())
...
class BaseFunction:
    ...
    def __call__(self, *args, **kwargs):
        ...
        if 'error' in output:
            ...
            logger.info("MLCHAIN VERSION: {} API VERSION: {}".format(output.get('mlchain_version', None),
