...
class ServeModel(object):
    ...
    def call_function(self, function_name_, id_=None, __headers__=None, *args, **kwargs):
        ...
        if isinstance(function_name, str):
            if len(function_name) == 0:
                ...
                start = time.time()
                if '__headers__' in signature(func_).parameters:
                    kwargs['__headers__'] = __headers__
    ...
    async def call_async_function(self, function_name_, id_=None, __headers__=None, *args, **kwargs):
        """
        Flow request values into function_name and return output
        """
        function_name, id = function_name_, id_
