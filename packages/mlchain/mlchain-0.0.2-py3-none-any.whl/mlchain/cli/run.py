...
def run_command(entry_file, host, port, bind, wrapper, server, queue, workers, trace, log, config, name,mode,dump_request, kws):
    ...
    if server == 'grpc':
        ...
        app = GrpcServer(app, name=name, trace=trace, monitor_sampling_rate=monitor_sampling_rate)
        app.run(host, port)
            ...
            def load(self):
                ...
                if isinstance(app, ServeModel):
                    if self.server == 'flask':
                        ...
                        app = FlaskServer(app, name=name, trace=trace,
                                          monitor_sampling_rate=monitor_sampling_rate,dump_request=dump_request,version=version,api_keys=api_keys)
                        ...
                        app = QuartServer(app, name=name, trace=trace,
                                          monitor_sampling_rate=monitor_sampling_rate,dump_request=dump_request,version=version,api_keys=api_keys)
                        app.register_swagger(host, port)
                        if cors:
        ...
        from mlchain.rpc.server.quart_server import QuartServer
        app = get_model(entry_file, serve_model=True, queue=queue, trace=trace, name=name)
        app = QuartServer(app, name=name, trace=trace, monitor_sampling_rate=monitor_sampling_rate,version=version,dump_request=dump_request,api_keys=api_keys)
    ...
    if isinstance(app, MLServer):
        ...
        if server == 'flask':
            from mlchain.rpc.server.flask_server import FlaskServer
            app = FlaskServer(app, name=name, trace=trace, log=log, monitor_sampling_rate=monitor_sampling_rate,dump_request=dump_request,version=version,api_keys=api_keys)
            app.run(host, port, cors=cors, gunicorn=False)
        elif server == 'quart':
            from mlchain.rpc.server.quart_server import QuartServer
            app = QuartServer(app, name=name, trace=trace, monitor_sampling_rate=monitor_sampling_rate,dump_request=dump_request,version=version,api_keys=api_keys)
            app.run(host, port, cors=cors, gunicorn=False, hypercorn=False)
