...
from .constants import holidays, workdays
...
def _validate_date(*dates):
    ...
    if not (min_year <= date.year <= max_year):
        raise NotImplementedError(
            "no available data for year {}, only year between [{}, {}] supported, please update chinesecalendar".format(date.year, min_year, max_year)
        )
    return date
