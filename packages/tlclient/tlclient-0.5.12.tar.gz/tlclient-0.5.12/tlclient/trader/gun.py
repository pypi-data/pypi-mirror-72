...
class ConsoleEntrance(object):
    ...
    FIST_TYPES = ['master', 'tg', 'mg', 'tr', 'mr', 'oms', 'rms', 'bs', 'algo', 'rr', 'nn']  # ALL fist type supported
...
'''.format(FIST_TYPES)
        ...
        if fist_type in ['tg', 'mg']:
            ...
            try:
                if args.package_name:
                    PythonGateway(args.package_name, fist_type, args.gateway_name, args.router_name, sub_config=args.sub,
    ...
    def ip(self):
        import requests
        ip = requests.get('http://ifconfig.me').content.decode(errors='ignore').strip()
    ...
    def remote(self):
        ...
        if args.option == 'run':
            ...
            requester = RemoteExecRequester(args.port)
            requester.run()
