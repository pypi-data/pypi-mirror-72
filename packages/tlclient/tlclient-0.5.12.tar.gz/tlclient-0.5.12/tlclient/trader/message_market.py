...
class MktBasic(ctypes.Structure):
    ...
    def __str__(self):
        return '{}-{}-{}-{}-{}'.format(
            ExchangeID.read(self.exchange), self.ticker.decode(errors='ignore'), AssetType.read(self.asset_type),
            self.trading_day.decode(errors='ignore'), Timer.nano_to_datetime(self.mkt_time).strftime('%H:%M:%S')
        )
