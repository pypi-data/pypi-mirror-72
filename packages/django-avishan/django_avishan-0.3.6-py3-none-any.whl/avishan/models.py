...
class IdentifierVerification(AvishanModel):
    ...
    def create_verification_code(cls) -> str:
        import random
        if cls.__class__ is PhoneVerification:
...
class AuthenticationType(AvishanModel):
    ...
    @classmethod
    def _register(cls, user_user_group: UserUserGroup, key: str, **kwargs):
    ...
    @classmethod
    def login(cls, key: str, user_group: UserGroup = None, **kwargs):
        from avishan.exceptions import AuthException

        try:
            if not user_group:
                found_object: KeyValueAuthentication = cls.objects.get(
                    key=cls.key_field().related_model.get(key))
            else:
                found_object: KeyValueAuthentication = cls.objects.get(**{
                    'key': cls.key_field().related_model.get(key),
...
class OtpAuthentication(AuthenticationType):
    ...
    @classmethod
    def register(cls, user_user_group: UserUserGroup, key: str):
    ...
    def _login_post_check(cls, **kwargs):
        ...
        if cls.__class__ is PhoneOtpAuthenticate:
        ...
        if not kwargs.get('verify', False):
            ...
            if found_object.date_sent and (datetime.datetime.now() - found_object.date_sent) < gap:
                raise ErrorMessageException(AvishanTranslatable(
                    EN='Verification code sent recently, Please try again later',
                    FA='برای ارسال مجدد کد، کمی صبر کنید'
                ))
    ...
    @classmethod
    def verify(cls, key: str, entered_code: str, user_group: UserGroup = None):

        try:
            cls.key_field().related_model.get(key)
        except cls.key_field().related_model.DoesNotExist:
