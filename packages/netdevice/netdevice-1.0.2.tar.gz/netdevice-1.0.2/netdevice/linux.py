...
class LinuxDevice(object):
    ...
    def login(self, terminal_type = 'ansi', login_timeout = 10):
        ...
        if (self.attributes.get("type", 'Linux') == 'Junos'):
            # Second phase
    ...
    def log (self, message, level = LOG_NOTICE, leading = None, color =
        ...
        '''
        level = log_level and log_level or level
        if level > self["log_level"] or (not kwargs.get("with_log", True)):
            return
    ...
    def sleep (self, timeout, total = 50, char = '>', description = "sleep"):
        ...
        for i in range(1, count + 1):
            ...
            sys.stdout.write('\r%s %s/%s[%s][%s%%]'%(description, i/10, timeout,
                str, i * 100 / count))
            sys.stdout.flush()
            time.sleep(0.1)
    ...
    def get_file(self, filename, localname = '.', scheme = "scp", timeout = -1):
        ...
        neveer timeout
        '''
        cmd = 'scp %s@%s:%s %s' %(self["username"], self["hostname"], filename, localname)
    ...
    def put_file(self, filename, remotedir, timeout = -1):
        ...
        clild = pexpect.spawn('scp %s %s@%s:%s' %(filename, self["username"],
    ...
    def x_configure_interface (self, *args):
        ...
        client.x_configure_interface("eth1", "41.0.0.2", "2002::2")
        client.x_configure_interface(name = "eth1", inet = "41.0.0.2", inet6 = "2002::2")
