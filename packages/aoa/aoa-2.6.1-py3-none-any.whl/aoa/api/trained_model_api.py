...
class TrainedModelApi(BaseApi):
    ...
    def save(self, trained_model: Dict[str, str], artefacts: List[str]):
        ...
        self.upload_artefacts(artefacts, response['id'])

        return response

    def upload_artefacts(self, artefacts, trained_model_id):
        for artefact in artefacts:
            query_params = {
                'objectKey': "{}/{}".format(trained_model_id, os.path.basename(artefact))
            }
            signed_url = self.aoa_client.get_request('/api/s3/signedPutUrl', {}, query_params)
    ...
    def approve(self, trained_model_id: str, note: str):
        ...
        self.transition(trained_model_id, transition_request)
    ...
    def deploy(self, trained_model_id: str):
        transition_request = dict()
        transition_request['status'] = 'DEPLOYED'

        self.transition(trained_model_id, transition_request)

    def retire(self, trained_model_id: str):
        transition_request = dict()
