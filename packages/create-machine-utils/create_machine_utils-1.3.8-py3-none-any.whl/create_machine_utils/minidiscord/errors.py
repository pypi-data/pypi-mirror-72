...
class ErrorHandler(commands.Cog):
    ...
    def handles(self, exception, ignore_non_exceptions=False):
        def decorator(func):
            if isinstance(exception, tuple) or issubclass(exception, Exception) or ignore_non_exceptions:
        ...
        if (isinstance(exception, typing.Callable) or inspect.iscoroutinefunction(exception)) \
                and not issubclass(exception, Exception) and not ignore_non_exceptions:
    ...
    async def on_command_error(self, context, error):
        try:
            for handles, handler in self._handlers:
                if not isinstance(error, handles) or issubclass(error, handles) or error == handles:
                    continue
