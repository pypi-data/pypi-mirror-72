...
class EDAVisualizer(Visualizer): #pylint: disable=invalid-name
    ...
    def distribution_plot(self, data, column, bins, title=None, x_axis_fontsize=10, #pylint: disable= too-many-locals, too-many-arguments
        ...
        data_ = data.dropna(subset=[column])
    ...
    def scatter_plot(self, data, x, y, x_bins, y_bins, title=None, label=True, #pylint: disable=too-many-locals, too-many-arguments, too-many-statements, invalid-name
        ...
        x_bin_size = round(float(x_diff)/float(x_bins), 3)
        x_axis_ticks = ['[{}, {})'.format(round(math.floor(x_mini / x_bin_size) * x_bin_size + item * x_bin_size, 3),\
                        round(math.floor(x_mini / x_bin_size) * x_bin_size + (item + 1) * x_bin_size, 3)) for item in range(0, x_bins + 1)]
        ...
        y_bin_size = round(float(y_diff)/float(y_bins), 3)
        y_axis_ticks = ['[{}, {})'.format(round(math.floor(y_mini / y_bin_size) * y_bin_size + item * y_bin_size, 3),\
                        round(math.floor(y_mini / y_bin_size) * y_bin_size + (item + 1) * y_bin_size, 3)) for item in range(y_bins, -1, -1)]
        query = "SELECT *, FLOOR({0}/{1}) AS BAND_X,".format(quotename(x), x_bin_size)
        query += " '[' || ROUND(FLOOR({0}/{1})*{1}, {2}) || ', ' || ".format(quotename(x), x_bin_size, rounding_precision)
        query += "ROUND(((FLOOR({0}/{1})*{1})".format(quotename(x), x_bin_size)
        query += "+{0}), {1}) || ')' AS BANDING_X,".format(x_bin_size, rounding_precision)
        query += " FLOOR({0}/{1}) AS BAND_Y, '[' || ".format(quotename(y), y_bin_size)
        query += "ROUND(FLOOR({0}/{1})*{1}, {2}) || ', ' || ".format(quotename(y), y_bin_size, rounding_precision)
        query += "ROUND(((FLOOR({0}/{1})*{1})+{1}), {2}) || ')'".format(quotename(y), y_bin_size, rounding_precision)
        ...
        ax.set_xticks(np.arange(len(x_axis_ticks)))
        ax.set_xticklabels(x_axis_ticks)
        ax.set_xlabel(x, fontdict={'fontsize':label_fontsize})
        ax.xaxis.set_tick_params(rotation=90)
        ax.set_yticks(np.arange(len(y_axis_ticks)))
        ax.set_yticklabels(y_axis_ticks)
        ax.set_ylabel(y, fontdict={'fontsize':label_fontsize})
        # Turn spines off and create white grid.
        for edge, spine in ax.spines.items(): #pylint: disable=unused-variable
            spine.set_visible(False)
        ticks = np.arange(0, len(x_axis_ticks), 1)-0.5
        ax.set_xticks(ticks, minor=True)
        ticks = np.arange(0, len(y_axis_ticks), 1)-0.5
        ax.set_yticks(ticks, minor=True)
    ...
    def bar_plot(self, data, column, aggregation, title=None, label_fontsize=12): #pylint: disable=too-many-branches, too-many-statements
        """
    ...
    def box_plot(self,
                 ...
                 upper_outlier_fence_factor=0): #pylint: disable=too-many-locals, too-many-arguments, too-many-branches, too-many-statements
        """
