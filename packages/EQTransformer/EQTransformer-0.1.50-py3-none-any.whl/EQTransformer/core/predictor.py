...
def predictor(input_dir=None,
    ...
    Args:        
        input_dir (str, optional): Directory name containing hdf5 and csv files-preprocessed data. Defaults to None. 
        ...
        input_model (str, optional): Path to a trained model. Defaults to None.           
        output_dir (str, optional): Output directory. Defaults to None.   
        output_probabilities (bool, optional): If True, it will output probabilities and estimated uncertainties for each trace into an HDF file. Defaults to False.              
        detection_threshold (float, optional): A value in which the detection probabilities above it will be considered as an event. Defaults to 0.3.                
        P_threshold (float, optional): A value which the P probabilities above it will be considered as P arrival. Defaults to 0.1.            
        S_threshold (float, optional): A value which the S probabilities above it will be considered as S arrival. Defaults to 0.1.               
        number_of_plots (int, optional): The number of plots for detected events outputed for each station data. Defaults to 10.        
        plot_mode (str, optional): The type of plots: 'time': only time series or 'time_frequency': time and spectrograms. Defaults to 'time'.             
        estimate_uncertainty (bool, optional): If True uncertainties in the output probabilities will be estimated. Defaults to False.             
        number_of_sampling (int, optional): The number of Monte Carlo sampling to estimate the uncertainties. Defaults to 5.             
        ...
        normalization_mode (str, optional): Mode of normalization for data preprocessing, 'max': maximum amplitude among three components, 'std': standard deviation. Defaults to 'std'           
        ...
        gpu_limit (tuple, optional): Set the maximum percentage of memory usage for the GPU. Defaults to None.
        number_of_cpus (int, optional): Number of CPUs used for the parallel preprocessing and feeding of data for prediction. Defaults to 5.
        ...
        keepPS (bool, optional): If True, only detected events that have both P and S picks will be written otherwise those events with either P or S pick. Defaults to False.
        spLimit (int, optional): S - P time in seconds. It will limit the results to those detections with events that have a specific S-P time limit. Defaults to 60.
    ...
    Returns:
        ...
        ./output_dir/STATION_OUTPUT/output_probabilities.hdf5: An hdf5 containing the predicted probabilities for all samples. 
