...
def run_associator(input_dir,
    ...
    Args:        
        input_dir (str): Path to a dicrectory containing detection results. Defaults to None.
        start_time (str): Start of a time period of interest in 'YYYY-MM-DD hh:mm:ss.f' format. Defaults to None.
        end_time (str): End of a timeperiod of interest in 'YYYY-MM-DD hh:mm:ss.f' format. Defaults to None.
        moving_window (int): The length of time window used for association in second. Defaults to 15.
        pair_n (int): The minimum number of stations used for the association. Defaults to 2.
        output_dir (str): Path to the directory to write the output file. Defaults to '.'.
        consider_combination (bool): If True, it will write down all possible combinations of picked arrival times for each event. 
           This will generate multiple events with the same ID, and you will need to remove those with poor solutions after location. This helps to remove the false positives from the associated event. Defaults to False.

         
        input_model (str): Path to a trained model. Defaults to None.           
        stations_json (str): Path to a JSON file containing station information.Defaults to None.           
        output_dir (str): Output directory. Defaults to None.              
        detection_threshold (float): A value in which the detection probabilities above it will be considered as an event. Defaults to 0.3.                
        P_threshold (float): A value which the P probabilities above it will be considered as P arrival. Defaults to 0.1.            
        S_threshold (float): A value which the S probabilities above it will be considered as S arrival. Defaults to 0.1.               
        number_of_plots (int): The number of plots for detected events outputed for each station data. Defaults to 10.        
        plot_mode (str): The type of plots: 'time': only time series or 'time_frequency': time and spectrograms. Defaults to 'time'.             
        loss_weights (list): Loss weights for detection, P picking, and S picking respectively. Defaults to [0.03, 0.40, 0.58].               
        loss_types (list): Loss types for detection, P picking, and S picking respectively. Defaults to ['binary_crossentropy', 'binary_crossentropy', 'binary_crossentropy']        
        normalization_mode (str): Mode of normalization for data preprocessing, 'max': maximum amplitude among three components, 'std': standard deviation. Defaults to 'std'           
        batch_size: (int): Batch size. Defaults to 500.           
        overlap (float): If set, detection, and picking are performed in overlapping windows. Defaults to 0.3.           
        gpuid (int): Id of GPU used for the prediction. If using CPU set to None. Defaults to None.           
        gpu_limit (int): Set the maximum percentage of memory usage for the GPU. Defaults to None.
