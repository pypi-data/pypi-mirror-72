...
std::vector <std::function <double(double, std::vector <double>)> > partialsvector_lin = { partial1_lin, partial2_lin };
...
int main()
{
    ...
    */
    ...
    //example of usage with >1 independent ("x") variable (weighted data, with priors, but no error pivots)
...
//    result = requestHandlerWeighted(xt, yt, gt, wt, funcInt, xt.size(), rejTech, 0, pP, hP, bt);
    ...
    // //EXAMPLE with priors, but weighted and no error pivots (1D case)
...
//
//
//    Priors testPriors = Priors(MIXED_PRIORS, gaussianParams, boundedParams); //construction of priors object
...
//     FunctionalForm model = FunctionalForm(func_linear, x, y, partialsvector_lin, tolerance, guess, w, testPriors, getAvg, xb_guess);// , getAvg, xb_guess); //setting up for functional form
    FunctionalForm model = FunctionalForm(func_linear, x, y, partialsvector_lin, tolerance, guess);//, w);//, testPriors, getAvg, xb_guess);// , getAvg, xb_guess);
     RCR rcr = RCR(SS_MEDIAN_DL); //setting up for RCR with this rejection technique
     rcr.setParametricModel(model);
     rcr.performBulkRejection(y); //running Bulk rejection RCR
