...
from tensorflow_probability.python.internal.backend.numpy import private as array_ops
...
def default_swap_proposal_fn(prob_swap, name=None):
  ...
  def adjacent_swaps(num_replica, batch_shape=(), seed=None):
    ...
    with tf.name_scope(name or 'adjacent_swaps'):
      ...
      u = tf.random.stateless_uniform(u_shape, seed=seed()) < 0.5
      ...
      return tf.where(
          tf.random.stateless_uniform(batch_shape, seed=seed()) < prob_swap, y, x)
  ...
  def one_step(self, current_state, previous_kernel_results):
    ...
    with tf.name_scope(mcmc_util.make_name(self.name, 'remc', 'one_step')):
      ...
      log_uniform = tf.math.log(
          tf.random.stateless_uniform(shape=replica_and_batch_shape,
                            dtype=dtype,
