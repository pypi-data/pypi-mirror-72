...
GKE_DEFAULT_SERVINGGROUP_TYPE = "n1-standard-1"  # Instance type of default GKE serving groups
...
def gke_init(
    ...
    if not use_existing:
        ...
        if not click.confirm("Are you sure you want to use {}?".format(gke_cluster_name)):
            raise ExitException("Please run without --use-existing or specify a different --kube-cluster-name")
        cmd = [
            "gcloud",
            "container",
            "clusters",
...
def gke_add_nodepool(serving_group_name, config_contents, gcp_creds):
    from googleapiclient import discovery
    from googleapiclient.errors import HttpError
