...
class SoftwarePatchStatus:
    ...
    def collect_patch_data_status(self):
        ...
        for item in res:
            ...
            if "metadata" in item and "platform" in item["metadata"]:
        ...
        for item in per_update_totals.to_numpy():
            software_updates_by_popularity.labels(item[0], item[1]).set(item[2])

        totals = df.sum(0, numeric_only=True)

        # goal: number of software updates, grouped by status:
        #
        # NEW: count_requested > 0 && count_assigned == 0
        # IN PROGRESS: count_remaining 
        # FAILED: assigned_clients.error + assigned_clients.warning
        # DONE: is_completed
        df_completed = df.loc[df['is_completed']]
        software_updates_by_state.labels('Completed').set(df_completed['is_completed'].sum())

        df_inprogress = df_not_completed.loc[df_not_completed['remaining'] > 0]
        software_updates_by_state.labels('Remaining').set(len(df_inprogress))

        # software_updates_by_state.labels('Requested').set(totals["requested"])
