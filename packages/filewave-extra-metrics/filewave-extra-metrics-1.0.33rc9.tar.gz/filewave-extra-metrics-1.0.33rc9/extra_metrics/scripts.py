...
@click.option('-v', '--skip-verify-tls', 'verify_tls', is_flag=True, default=False, help='whether or not to verify the server certificate (gets stored in config)')
...
def log_config_summary(cfg, major, minor, patch):
    ...
    logger.info(f"Skip TLS Verify  : {cfg.get_verify_tls()}")
