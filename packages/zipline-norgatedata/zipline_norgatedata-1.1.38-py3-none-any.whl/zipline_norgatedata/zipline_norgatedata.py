...
def create_norgatedata_equities_bundle(bundlename,stock_price_adjustment_setting,start_session,end_session, symbol_list = None, watchlists = None, excluded_symbol_list = None):
    def ingest(environ,
               asset_db_writer,
               minute_bar_writer,
               daily_bar_writer,
        ...
        symbols = determine_symbols(start_session,symbol_list,watchlists,excluded_symbol_list)
...
def create_norgatedata_futures_bundle(bundlename,start_session,end_session,symbol_list = None,watchlists = None, session_symbols = None, excluded_symbol_list = None):
    def ingest(environ,
               asset_db_writer,
               minute_bar_writer,
               daily_bar_writer,
        ...
        symbols,root_symbols = determine_futures_symbols(start_session, symbol_list = symbol_list,session_symbols=session_symbols, watchlists = watchlists, excluded_symbol_list = excluded_symbol_list)
        dtype = [('start_date', 'datetime64[ns]'),
                    ('end_date', 'datetime64[ns]'),
                    ('auto_close_date', 'datetime64[ns]'),
                    ('symbol', 'object'),
        ...
        if len(metadata) != len(set(metadata['symbol'])):
            separator=','
            logger.error('There are duplicate symbols in the metadata - this is probably due to a duplicated futures root symbol and will cause Zipline to abort.  Duplicate symbols are  : ' + separator.join(Repeat(metadata['symbol'])))
...
def determine_symbols(startdate,symbol_list = None, watchlists = None, excluded_symbol_list = None):
...
def determine_futures_symbols(startdate,symbol_list = None, watchlists = None, session_symbols = None, excluded_symbol_list = None):
...
def _pricing_iter_equities(symbols, metadata, sessions, show_progress, stock_price_adjustment_setting, start_session, end_session):
    ...
    with maybe_show_progress(symbols, show_progress,
        ...
        for sid, symbol in enumerate(it):
            ...
            slqd = norgatedata.second_last_quoted_date(symbol, format='pandas-timestamp')
            if slqd is None:
            ...
            df = df.fillna(method='bfill',axis=1,limit=3) # back fill close into OHL.  (For some reason, inplace=True doesn't work here with Pandas 0.18)
...
def _pricing_iter_futures(symbols, metadata, sessions, show_progress, start_session, end_session):
    with maybe_show_progress(symbols, show_progress,
        ...
        for sid, symbol in enumerate(it):
            ...
            norgate_data_symbol = symbol
            norgate_data_assetid = norgatedata.assetid(symbol)
            notice_date = norgatedata.first_notice_date(symbol, format='pandas-timestamp')
            expiration_date = norgatedata.last_quoted_date(symbol, format='pandas-timestamp')
            if norgatedata.base_type(symbol) == 'Futures Market':
            ...
            if notice_date is None:
                ac_date = pd.Timestamp(expiration_date, tz="utc")
                notice_date = ac_date
            else:
                ac_date = df.index[df.index.searchsorted(pd.Timestamp(notice_date)) - 1] # Day prior to FND
            all_dates = sessions.snap('D')
            valid_dates = all_dates.slice_indexer(start=start_date,end=end_date)
            ...
            if len(all_dates[valid_dates]) > 0 and all_dates[valid_dates][0].tz_localize("UTC") not in df.index:
                # Create a reindexed DF with one row that is forward filled
                single_date_slice = all_dates.slice_indexer(start=all_dates[valid_dates][0],end=all_dates[valid_dates][0])
                single_date_df = df.reindex(all_dates[single_date_slice].tz_localize("UTC"),method='ffill')
...
def register_norgatedata_equities_bundle(bundlename,stock_price_adjustment_setting=norgatedata.StockPriceAdjustmentType.TOTALRETURN,start_session='1990-01-01',end_session='now',calendar_name='NYSE',symbol_list = None,watchlists = None):
    if not isinstance(start_session,pd.Timestamp):
        start_session = norgatedata.norgatehelper.decode_date(start_session,format='pandas-timestamp')
    if not isinstance(end_session,pd.Timestamp):
        end_session = norgatedata.norgatehelper.decode_date(end_session,format='pandas-timestamp')
    start_session,end_session = normalize_daily_start_end_session(calendar_name,start_session,end_session)
    register(bundlename, 
...
def register_norgatedata_futures_bundle(bundlename,start_session='1990-01-01',end_session='now',calendar_name='us_futures', symbol_list = None,session_symbols = None,watchlists= None):
    if not isinstance(start_session,pd.Timestamp):
        start_session = norgatedata.norgatehelper.decode_date(start_session,format='pandas-timestamp')
    if not isinstance(end_session,pd.Timestamp):
        end_session = norgatedata.norgatehelper.decode_date(end_session,format='pandas-timestamp')
...
def zipline_futures_root_symbols_dict():
    ...
    for session_symbol in session_symbols:
        ...
        if zipline_symbol in zipline_markets:
            logger.error('Zipline already has a session symbol of ' + zipline_symbol + ':' + zipline_markets[zipline_symbol] + '.  ' + name + ', derived from norgate session symbol ' + session_symbol + ' is a duplicate ' )
