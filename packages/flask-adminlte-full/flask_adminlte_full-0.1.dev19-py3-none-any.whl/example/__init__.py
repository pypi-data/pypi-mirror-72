...
from flask import Flask, render_template, url_for, flash, request, abort, current_app, redirect, session
...
# p = os.path.join(os.path.dirname(adminlte_base.__file__), 'translations')
# domain = Domain(dirname=p, domain='adminlte_full')
...
@app.route('/widgets')
def widgets():
    print(adminlte.manager.get_locale())
    print(adminlte.manager.get_translations())
