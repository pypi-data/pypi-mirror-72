...
class SolutionStateManager:
    ...
    def sagemaker_create_solution(self, TrainingJobName: str, newSolutionName: str):
        """
        ...
        for key_, values_ in prev_training_job.items():
            if key_ not in self.required_new_job_fields:
                new_training_job.update({
                    key_ : values_
                })

        # update fields
        # in python3, a time suffix is automatically added
        new_training_job.update({
            'TrainingJobName': newSolutionName
        })
    ...
    def update_state(self, solution: dict, solution_base_name: str):
        ...
        new_solution_arn = self.sagemaker_create_solution(
            TrainingJobName=solution['arn'],
            newSolutionName=solution_base_name
        )
        ...
        if next_version_creation_time:
            new_solution.update({
                'next_version_creation_time': next_version_creation_time
            })
