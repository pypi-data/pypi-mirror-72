def thread_function(speed_queue):
    import gi
    ...
    class SensorFactory(GstRtspServer.RTSPMediaFactory):
        ...
        def on_need_data(self, src, lenght):
            frame = speed_queue.get(timeout=1)
            data = frame.tostring()
            ...
            print("Running... Number of frames: " + str(self.number_frames))
...
def start_rtsp_streaming():
    ...
    x = threading.Thread(target=thread_function, args=(frame_queue,)).start()
    return frame_queue


def update_frame(speed_queue,frame):
    speed_queue.put(frame)
