...
def calculate_maxEnt(jnc):
    ex_df = jnc
    if ex_df.empty:
        max_ent_series_5 = pd.DataFrame(columns = ["5'bases_maxEnt", 'maxEnt_5'])
        max_ent_series_3 = pd.DataFrame(columns = ["3'bases_maxEnt", 'maxEnt_3'])
    else:
        lt_5 = ex_df["5'bases_maxEnt"].to_list()
        with open('data/fivemaxent.txt', mode='wt', encoding='utf-8') as fiveprfile:
            fiveprfile.write('\n'.join(lt_5))
        lt_3 = ex_df["3'bases_maxEnt"].tolist()
        ...
        max_ent_series_5 = pd.DataFrame([x.split('\t') for x in results_in_list_5], columns = ["5'bases_maxEnt", 'maxEnt_5'])
        ...
        max_ent_series_3 = pd.DataFrame([x.split('\t') for x in results_in_list_3], columns = ["3'bases_maxEnt", 'maxEnt_3'])
...
if __name__ == '__main__':
    ...
    ex_df["5'_bases"] = 'atcgtcgaa'
    ex_df["3'_bases"] = 'ctctactactatctatctagatc'
