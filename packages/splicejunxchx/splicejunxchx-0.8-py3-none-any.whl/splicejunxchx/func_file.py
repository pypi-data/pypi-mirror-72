...
def extract_spljnc(splice_jnc, gtf, strand, cons_exons, gtf_exons):
    ...
    junctions_five = set(junctions_five).tolist()
    junctions_three = set(junctions_three).tolist()
    if dict_five['gene'] == 1:
    ...
    if dict_five['exon'] == 1:
        dict_five['exon'] = ';'.join([s for s in junctions_five if ':exon:' in s])
    if dict_three['exon'] == 1:
        dict_three['exon'] =';'.join([s for s in junctions_three if ':exon:' in s])
    if dict_five['intron'] == 1:
        dict_five['intron'] = ';'.join([s for s in junctions_five if ':intron:' in s])
    if dict_three['intron'] == 1:
        dict_three['intron'] = ';'.join([s for s in junctions_three if ':intron:' in s])
    #dict_five['intron'] = five_intron
    #dict_three['intron'] = three_intron
    dict_five['strandofgene'] = ';'.join(set(junctions_five).tolist())
    dict_three['strandofgene'] = ';'.join(set(junctions_three).tolist())
