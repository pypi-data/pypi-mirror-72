...
impl CharTrigrams for &str {
    fn char_quadgrams(&self) -> Vec<&str> {
        ...
        let mut t = vec![];
        for (start, end) in indices[..indices.len() - 3].iter().zip(indices[3..].iter()) {
...
impl StringAnalysisDict {
    fn new() -> Self {
        StringAnalysisDict { map: Default::default(), fst: Default::default() }
    }
}
...
impl StringAnalysisDict {
    ...
    fn get_by_any_superstring(&self, ks: Vec<Option<&str>>, min_length: usize) -> Vec<(&str, &PyObject)> {
        ks.iter().flat_map(|k| self.get_by_superstring(k.as_ref().map(|i| *i), min_length)).collect()
    }
