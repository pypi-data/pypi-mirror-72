...
class SparkMonitorOtherHandler(IPythonHandler):
    def get(self):
        return 'yunker'
...
class SparkMonitorHandler(IPythonHandler):
    ...
    def get(self):
        ...
        port = os.environ.get("SPARKMONITOR_UI_PORT", SparkContext._active_spark_context.uiWebUrl.split(':')[-1])
...
def load_jupyter_server_extension(nb_server_app):
    ...
    while not SparkContext._active_spark_context:
        sleep(0.01)
    web_app = nb_server_app.web_app
    host_pattern = ".*$"
    port_route_pattern = url_path_join(web_app.settings["base_url"], proxy_root + "4045")
    # route_pattern = url_path_join(
    #     web_app.settings["base_url"], proxy_root + ".*")
    web_app.add_handlers(host_pattern, [(port_route_pattern, SparkMonitorOtherHandler)])
