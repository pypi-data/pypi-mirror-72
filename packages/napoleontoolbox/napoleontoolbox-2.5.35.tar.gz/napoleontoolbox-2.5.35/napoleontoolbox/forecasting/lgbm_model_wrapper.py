...
class LGBMModel():
    ...
    def __init__(self, preprocessing = False, threshold = 0.6, number_per_year=252):
        self.model = lgb.LGBMRegressor()
        self.preprocessing = preprocessing
        self.threshold = threshold
        self.columns_to_drop = np.array([])
        self.binarization_threshold = dict()
    ...
    def fit(self, X_train, y_train, X_val, y_val):#, method = 'standard'):
        if self.preprocessing:
            ...
            self.binarization_threshold = preprocessing_features.get_binarization_threshold(X_train)
            X_train = preprocessing_features.binarize_features(X_train,self.binarization_threshold)
            X_val = preprocessing_features.binarize_features(X_val, self.binarization_threshold)
    ...
    def predict(self, X_test):#, method = 'standard'):
        if self.preprocessing:
            ...
            X_test = preprocessing_features.binarize_features(X_test, self.binarization_threshold)
