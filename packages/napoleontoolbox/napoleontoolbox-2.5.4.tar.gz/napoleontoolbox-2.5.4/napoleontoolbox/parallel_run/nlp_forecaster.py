...
class AbstractRunner(ABC):
    def __init__(self, starting_date = None, running_date = None, drop_token=None, dropbox_backup = True, underlying=None, frequence = None, target='target', stationarize_ohlc=False, apply_zscore=False, skipping_point=10, text_features_filename=None, underlying_filename = None, local_root_directory='../data/', user = 'napoleon',number_per_year=252, use_light=False, starting_iterations_to_pass=0):
        super().__init__()
        ...
        self.target=target
        self.stationarize_ohlc =stationarize_ohlc
        self.apply_zscore = apply_zscore
        self.skipping_point = skipping_point
        self.text_features_filename=text_features_filename
        self.underlying_filename = underlying_filename
...
class  NLP_ForecasterRunner(AbstractRunner):
    def runTrial(self, saver, seed, n, s, s_eval, calibration_step, signal_type, idios_string):
        continuous_saving_key, two_states_saving_key, two_states_saving_key_lo, three_states_saving_key, three_states_saving_key_lo , idios= saver.create_saving_key(seed, n, s, s_eval, calibration_step, signal_type, idios_string)
        ...
        if s_eval == 0:
            s_eval = None
        print('reading file')
        text_features_df = pd.read_csv(self.local_root_directory+self.text_features_filename)
        text_features_df.index = text_features_df.date
        text_features_df = text_features_df.asfreq(freq='H')
        text_features_df = text_features_df.tz_convert(None)
        underlying_df = pd.read_pickle(self.local_root_directory+self.underlying_filename)

        if self.stationarize_ohlc:
            print('Starting Stationarization')
            for col in underlying_df.columns:
                print('Checking stationarity of item', col)
                isStationary = tester.testStationarityDickeyFuller(underlying_df[col], displayResult=False)
                if not isStationary:
                    underlying_df[col] = np.log(underlying_df[col])
                    underlying_df[col] = underlying_df[col].fillna(0)
            print('Stationarization Over')


        timeseries_df = text_features_df.merge(underlying_df, left_index=True, right_index=True)
        timeseries_df['return'] = timeseries_df['close'].pct_change().shift(-1)

        timeseries_df = timeseries_df[:4000]
        if self.apply_zscore:
            print('Starting zscore')
            print('...')
            for col in timeseries_df.columns:
                if col not in ['date', 'return']+underlying_df.columns.tolist():
                    try:
                        timeseries_df[col] = signal_utility.expanding_zscore(timeseries_df[col], skipping_point=self.skipping_point)
        ...
        timeseries_df = timeseries_df.loc[:,timeseries_df.columns != 'date']
        timeseries_df = timeseries_df.fillna(0)

        X, y = timeseries_df.loc[:, timeseries_df.columns != self.target], timeseries_df[self.target]
        ...
        perf_df = signal_utility.reconstitute_prediction_perf(y_pred=forecasted_series, y_true = y, transaction_cost=transaction_cost, print_turnover=False)
        ...
        perf_df = signal_utility.reconstitute_prediction_perf(y_pred= discrete_two_states_forecasting_series , y_true = y, transaction_cost=transaction_cost, print_turnover=False)
        sharpe_strat = metrics.sharpe(perf_df['perf_return'].dropna(), period= self.number_per_year, from_ret=True)
        sharpe_under = metrics.sharpe(perf_df['close_return'].dropna(), period= self.number_per_year, from_ret=True)
        print('underlying sharpe')
        print(sharpe_under)
        ...
        perf_df = signal_utility.reconstitute_prediction_perf(y_pred=discrete_two_states_forecasting_series_lo, y_true=y,
        ...
        perf_df = signal_utility.reconstitute_prediction_perf(y_pred=discrete_three_states_forecasting_series, y_true = y, transaction_cost=transaction_cost, print_turnover=False)
        sharpe_strat = metrics.sharpe(perf_df['perf_return'].dropna(), period= self.number_per_year, from_ret=True)
        sharpe_under = metrics.sharpe(perf_df['close_return'].dropna(), period= self.number_per_year, from_ret=True)
        print('underlying sharpe')
        print(sharpe_under)
        ...
        perf_df = signal_utility.reconstitute_prediction_perf(y_pred=discrete_three_states_forecasting_series_lo, y_true=y,
                                                              transaction_cost=transaction_cost, print_turnover=False)
        sharpe_strat = metrics.sharpe(perf_df['perf_return'].dropna(), period=self.number_per_year, from_ret=True)
        sharpe_under = metrics.sharpe(perf_df['close_return'].dropna(), period=self.number_per_year, from_ret=True)
        print('underlying sharpe')
