...
class AbstractRunner(ABC):
    def __init__(self, starting_date = None, running_date = None, drop_token=None, dropbox_backup = True, underlying=None, frequence = None, target='target', stationarize_ohlc=False, apply_zscore=False, skipping_point=5, sentiment_grouping=False, add_bbbands=False, add_rsi_indicator=False, apply_binarization=False, local_root_directory='../data/', user = 'napoleon',number_per_year=252, use_light=False, starting_iterations_to_pass=0):
        super().__init__()
        ...
        self.sentiment_grouping = sentiment_grouping
        self.skipping_point = skipping_point
        self.add_bbbands=add_bbbands
        self.add_rsi_indicator = add_rsi_indicator
        ...
        self.predictors_file_name = 'features_'+str(self.apply_zscore)+str(self.sentiment_grouping)+str(self.add_bbbands)+str(self.add_rsi_indicator)+str(self.stationarize_ohlc)+str(sel'_set.pkl'
...
class  NLP_ForecasterRunner(AbstractRunner):
    def runTrial(self, saver, seed, n, s, s_eval, calibration_step, signal_type, idios_string):
        ...
        if s_eval == 0:
            s_eval = None
        print('reading file')
        timeseries_df = pd.read_pickle(self.local_root_directory+self.predictors_file_name)
