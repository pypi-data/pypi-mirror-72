...
class RealTimeSignalEnsemblingParalleLauncher():
    ...
    def daily_apply_standard_mean_discretization(self, signals_df=None, s=21, method ='find_best_three_states_discretization_lo'):
        ...
        for row_index in range(s, signals_np.shape[0]):
            ...
            if method == 'find_best_three_states_discretization':
                splitting_quantile_threshold, splitting_threshold, sharpe_strat, sharpe_under = signal_utility.find_best_three_states_discretization(y_train_pred, y_train_true)
                y_next_pred = np.nanmean(signals_np[row_index, :])
                y_next_pred_discrete = signal_utility.apply_discretization_three_states(y_next_pred,splitting_threshold)
            elif method == 'find_best_three_states_discretization_lo':
                splitting_quantile_threshold, splitting_threshold, sharpe_strat, sharpe_under = signal_utility.find_best_three_states_discretization_lo(y_train_pred, y_train_true)
                y_next_pred = np.nanmean(signals_np[row_index, :])
                y_next_pred_discrete = signal_utility.apply_discretization_three_states_lo(y_next_pred,splitting_threshold)
    ...
    def minutely_apply_standard_mean_two_states_discretization(self, signals_df=None, s=21, method ='find_best_two_states_discretization'):
