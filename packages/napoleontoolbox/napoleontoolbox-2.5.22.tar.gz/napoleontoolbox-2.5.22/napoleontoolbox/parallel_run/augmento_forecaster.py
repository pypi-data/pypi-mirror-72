...
class AbstractRunner(ABC):
    def __init__(self, starting_date = None, running_date = None, drop_token=None, dropbox_backup = True, underlying=None, frequence = None, target='target', local_root_directory='../data/', user = 'napoleon',number_per_year=252):
        super().__init__()
        ...
        self.predictors_output_file_name = f'augmento_{self.underlying}_{self.frequence}_data.pkl'
...
class  Augmento_ForecasterRunner(AbstractRunner):
    def runTrial(self, saver, seed, n, s, s_eval, calibration_step, signal_type, idios_string):
        ...
        if self.dropbox_backup:
            pkl_file_name = f'augmento_{self.underlying}_{self.frequence}_data.pkl'
            timeseries_df = self.dbx.download_pkl(pkl_file_name)
        ...
        generic_columns = [f'{self.frequence}_{gen_col}' for gen_col in ['open', 'high', 'low', 'close', 'volume']]
        features_colmuns = [col for col in timeseries_df.columns if col not in generic_columns]

        timeseries_df[f'{self.frequence}_close_return']= timeseries_df[f'{self.frequence}_close'].pct_change()
