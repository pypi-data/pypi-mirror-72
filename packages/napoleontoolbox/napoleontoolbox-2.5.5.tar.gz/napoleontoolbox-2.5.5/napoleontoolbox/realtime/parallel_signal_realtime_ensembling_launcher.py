    ...
    if np.isnan(actual_open):
        ...
        if len(high_dict) > 0 and len(low_dict) > 0 and len(volu_dict) > 0 and len(close_dict) > 0 and len(open_dict) > 0:
            ...
            if (len(fetcher.minute_np.shape)>1 and len(fetcher.minute_np)>3):
                #print('last three minutes '+minute_name)
...
class RealTimeSignalEnsemblingParalleLauncher():
    ...
    def recreate_minutely_histo(self, minutely_df, up_to_date):
        ...
        discrete_daily_signals = self.minutely_apply_standard_mean_two_states_discretization(signals_df=cumulated_signals, s=self.minutely_ia_lookback_window)
        ...
        discretized_saving_path = saving_path.replace('pkl','')
    ...
    def recreate_daily_histo(self, daily_df, up_to_date):
        ...
        counter = 1
        for me_signal in self.day_calib_signals_list:
            print('backtesting '+str(counter)+' over '+str(len(self.day_calib_signals_list)))
            # we have to reload the file each time
        ...
        print('discretizing the signal')
        discrete_daily_signals = self.daily_apply_standard_mean_two_states_discretization(signals_df=cumulated_signals, s=self.daily_ia_lookback_window)
        print('discretization done')
        cumulated_signals['trend']=discrete_daily_signals
    ...
    def daily_apply_standard_mean_two_states_discretization_last_signal(self):
        signal_columns = [self.day_signal_mapping[me_sig] for me_sig in self.day_calib_signals_list]
        prediction_index = self.day_signal_df.shape[0]
        starting_train = prediction_index - 1 - self.daily_ia_lookback_window
        stopping_train = prediction_index - 1
        ...
        y_train_pred = np.mean(local_rolling_np, axis=1)
        ...
        y_next_pred = local_signal_df.values[prediction_index, :].mean()
        print('day continuous prediction ' + str(y_next_pred))
    ...
    def minutely_apply_standard_mean_two_states_discretization_last_signal(self):
        signal_columns = [self.minute_signal_mapping[me_sig] for me_sig in self.min_calib_signals_list]
        prediction_index = self.minute_signal_df.shape[0]
        starting_train = prediction_index - 1 - self.minutely_ia_lookback_window
        stopping_train = prediction_index - 1
        ...
        y_next_pred = local_signal_df.values[prediction_index, :].mean()
        print('minute continuous prediction '+str(y_next_pred))
        y_next_pred_discrete = signal_utility.apply_discretization_two_states(y_next_pred, splitting_threshold)
        print('minute discrete prediction '+str(y_next_pred_discrete))
        return y_next_pred_discrete

    def daily_apply_standard_mean_two_states_discretization(self, signals_df=None, s=21):
        ...
        for row_index in range(s + 1, signals_np.shape[0]):
            starting_train = row_index - 1 - s
            stopping_train = row_index - 1
            ...
            splitting_quantile_threshold, splitting_threshold, sharpe_strat, sharpe_under = signal_utility.find_best_two_states_discretization(
                y_train_pred, y_train_true)
            y_next_pred = signals_np[row_index, :].mean()
            y_next_pred_discrete = signal_utility.apply_discretization_two_states(y_next_pred, splitting_threshold)
            y_pred_discretized[prediction] = y_next_pred_discrete
        return y_pred_discretized

    def minutely_apply_standard_mean_two_states_discretization(self, signals_df=None, s=21):
    ...
    def reload_daily_histo(self, from_cryptocompare = False,running_date = None):
        ...
        if from_cryptocompare :
            ...
            saving_path = self.local_root_directory+backed_up_signal_histo_name
            print('loading histo recomputation to '+backed_up_signal_histo_name)
    ...
    def launchDaySequential(self):
        ...
        if len(self.day_signal_np.shape)>1 and len(self.day_signal_np)>3:
            ...
            print('forward filling the triggered daily signals ')
            counter = 0
            for meArg in self.day_calib_signals_list:
    ...
    def launchMinuteSequential(self):
        ...
        if len(self.minute_signal_np.shape)>1 and len(self.minute_signal_np)>3:
            ...
            print('forward filling the triggered minute signals ')
            counter = 0
            for meArg in self.min_calib_signals_list:
                triggered = trigger_results[counter]
                signal_counter = len(self.generic_columns) + counter
    ...
    def day_signals_ia_mixing(self):
        ...
        print('still '+str(nb_nan_signals)+' not triggered day signals')
        print('last daily alphas signal')
        print(last_day_signals)
        print('discretizing the signal')
        if  self.day_signal_np.shape[0]> self.daily_ia_lookback_window + 1:
            discrete_signals = self.daily_apply_standard_mean_two_states_discretization_last_signal()
        else :
    ...
    def minute_signals_ia_mixing(self):
        ...
        print('still '+str(nb_nan_signals)+' not triggered minute signals')
        print('last minute alphas signal')
        print(last_minute_signals)
        print('discretizing the signal')
        if  self.minute_signal_np.shape[0]> self.minutely_ia_lookback_window + 1:
            discrete_signals = self.minutely_apply_standard_mean_two_states_discretization_last_signal()
        else :
