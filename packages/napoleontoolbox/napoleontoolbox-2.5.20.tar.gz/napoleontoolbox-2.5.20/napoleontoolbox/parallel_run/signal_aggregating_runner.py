...
def daily_bc_apply_standard_mean_two_states_discretization(signals_df=None, s=21):
...
class AbstractRunner(ABC):
    def __init__(self, calibration_starting_date = None,  calibration_running_date = None, starting_date = None,  running_date = None,  drop_token=None, dropbox_backup = True, underlying = None, frequence='daily', selected_algo='',algo='' ,freqly_return_pkl_filename_suffix='freqly_candels.pkl',aggregated_pkl_file_suffix='freqly_to_mix.pkl', aggregated_pkl_mapping_file_suffix = 'freqly_to_mix_mapping.pkl', list_pkl_file_suffix = 'my_list.pkl', local_root_directory='../data/', user = 'napoleon', add_turn_over = False, skipping_size=200, recompute_all = True, db_path_suffix=None, daily_ia_lookback_window=21):
        ...
        self.dates_stub = self.calibration_starting_date.strftime('%d_%b_%Y') + '_' + self.calibration_running_date.strftime('%d_%b_%Y')
...
class AggregatingSignalRunner(AbstractRunner):
    ...
    def saving_cumulated_signals(self, cumulated_signals_df, mapping_dictionary):
        ...
        discrete_daily_signals = daily_bc_apply_standard_mean_two_states_discretization(signals_df=cumulated_signals_df, s=self.daily_ia_lookback_window)
