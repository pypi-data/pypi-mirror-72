...
class Queues():
    ...
    def start():
        if not Queues.__globalqueue:
            ...
            t.start()
    def __messagequeue():
        kcwsqlite.sqlite().connect(queuesdbpath).table("model_kcweb_queues").where("code",3).delete()
        while True:
    ...
    def getall(pagenow=1,pagesize=20,where=None):
        ...
        if not os.path.isfile(queuesdbpath):
            ...
            model_kcweb_tasks.create_table()
        try:
            lists=kcwsqlite.sqlite().connect(queuesdbpath).where(where).table("model_kcweb_queues").page(pagenow,pagesize).select()
    ...
    def iscomplete():
        ...
        return Boolean
        """
        if kcwsqlite.sqlite().connect(queuesdbpath).table("model_kcweb_queues").where("code",3).count():
            return True
        else:
            return False
