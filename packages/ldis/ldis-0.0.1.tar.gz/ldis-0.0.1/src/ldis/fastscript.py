...
class Param:
    ...
    def __init__(
        ...
        self.help, self.type, self.opt, self.action, self.nargs = (
            help,
            type,
            opt,
            action,
    ...
    @property
    def kwargs(self):
        return {k: v for k, v in self.__dict__.items() if v is not None and k != "opt"}
...
def call_parse(func):
    ...
    def _f(*args, **kwargs):
        ...
        if xtra is not None:
            if xtra == 1:
                xtra = p.prog
            for k, v in args_from_prog(func, xtra).items():
                setattr(args, k, v)
        del args.xtra
        func(**args.__dict__)
