...
class basehistogram(object):
    ...
    def set_heights(self, heights):
        ...
        Set bin heights
        '''
        assert heights.shape == self.binsizes.shape
...
def histdiv(a, b, corr=None, ignore_denominator_uncertainty=False):
    ...
    r = histogram(r.binedges, r.areas, areas=False, name=r.name, uncorr_variations=r.uncorr_variations, corr_variations=r.corr_variations, attributes=r.attributes, plot_attributes=r.plot_attributes)
    if corr:
...
class histogram2d(basehistogram):
    ...
    def project(self, axis, name=''):
        ...
        :param axis: which axis to project onto
    ...
    def iterbins(self):
        ...
        :returns: x-axis bin egdes, y-axis bin edges, and nominal bin height
        :rtype: ``tuple`` of the following: ``tuple`` of two ``float``, ``tuple`` of two ``float``, and one ``float``
        ...
        .. code-block:: python
            ...
            >>> for binedges_x, binedges_y, height in h.iterbins(): print(binedges_x, binedges_y, height)
            (-7.0, 0.0) (-1.0, 0.0) 1.0
            (-7.0, 0.0) (0.0, 1.0) 5.0
            (0.0, 5.0) (-1.0, 0.0) 2.0
            (0.0, 5.0) (0.0, 1.0) 6.0
        ...
        expanded_heights = np.ravel(self.heights)
        for bin_lower_edge_x, bin_upper_edge_x, bin_lower_edge_y, bin_upper_edge_y, height in zip(bin_lower_edges_x, bin_upper_edges_x, bin_lower_edges_y, bin_upper_edges_y, expanded_heights):
