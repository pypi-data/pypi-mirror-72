    ...
    for i, p in enumerate(panels):
        ...
        if not p.nolegend:
            if legend_outside:
                ...
                # NOTE: untested with LaTeX-rendered labels
...
def make_uncertainty_breakdown(histogram, separator='__', ylims=None, xlabel='', **kwargs):
    ...
    for i, name in enumerate(uncertainty_names):
        ...
        for variation in all_variations.keys():
            ...
            bh = (heppy.histdiv(histogram.extract_variation_histogram(variation), histogram, ignore_denominator_uncertainty=True))
            bh.name = name
            bh.plot_attributes = {'linestyle' : linestyle}
    ...
    histogram_for_total_uncertainty.plot_attributes = {'edgecolor' : '0.8'}
