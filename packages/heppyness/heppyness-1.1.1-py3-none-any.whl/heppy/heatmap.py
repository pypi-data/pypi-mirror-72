...
class TextFormatter:
    ...
    def brief(significants=3, fixedprec=None, nominal=None, uncert_up=None, uncert_down=None, **ignore):
        ...
        if all([s == '' for s in [n, u, d]]): return ''
...
def make_heatmap(histogram, areas=False, title='', figsize=(8, 5), text_format=TextFormatter.brief, text_precision=3, text_autocolor=True, monowidth=False, write='', xmax=None, **kwargs):
    ...
    The following keys in :code:`histogram.plot_attributes` can be used to set labels:

    - :code:`"title"` – plot title
    - :code:`"xlabel"` – x-axis label
    - :code:`"ylabel"` – y-axis label
    - :code:`"zlabel"` – z-axis (colour palette) label
    '''
    ...
    contents = np.ravel(histogram.areas) if areas else heights
    h2d = plt.hist2d(vis_midx, vis_midy, weights=contents, bins=vis_binedges, **plot_options)

    max_value_of_heatmap = np.max(contents)
    if text_format is not None:
        # Write bin contents as text onto the plot
        for x, y, nominal in zip(vis_midx, vis_midy, contents):
            values = {
                'nominal' : nominal,
                'uncert_up' : -99.,
                'uncert_down' : -99.,
    ...
    if 'title' in histogram.plot_attributes:
        axes.set_title(histogram.plot_attributes['title'], x=0., ha='left', size='x-large')
