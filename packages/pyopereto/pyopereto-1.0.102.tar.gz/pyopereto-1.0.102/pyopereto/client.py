...
class OperetoClient(object):
    ...
    def __new__(cls, *args, **kwargs):
            ...
            try:
                with open(file, 'r') as f:
                    if file.endswith('.json'):
                        ...
                        kwargs.update(yaml.load(f.read(), Loader=yaml.FullLoader))
        ...
        def _get_agent_credentials():
            ...
            if os.path.exists(token_file):
                with open(token_file, 'r') as tf:
                    kwargs.update({'opereto_auth_token':tf.read()})
        ...
        if not set(['opereto_auth_token', 'opereto_host']) <= set(kwargs):
            _get_agent_credentials()
            if os.path.exists(os.path.join(work_dir,'arguments.json')):
                get_credentials_from_file(os.path.join(work_dir,'arguments.json'))
            elif os.path.exists(os.path.join(work_dir,'arguments.yaml')):
                get_credentials_from_file(os.path.join(work_dir,'arguments.yaml'))
            elif os.path.exists(os.path.join(home_dir,'opereto.yaml')):
                get_credentials_from_file(os.path.join(home_dir,'opereto.yaml'))


        if set(['opereto_auth_token', 'opereto_host']) <= set(kwargs):
            instance = object.__new__(OperetoClient)
        else:
    ...
    def __init__(self, **kwargs):
        ...
        self.input['opereto_user'] = self.get_current_opereto_token['username']
