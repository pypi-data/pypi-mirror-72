...
class CFM(nn.Module):
    ...
    def forward(self, left, down):
        # left: High-level features input
        # down: Low-level features input
        if down.size()[2:] != left.size()[2:]:
            down = F.interpolate(down, size=left.size()[2:], mode='bilinear', align_corners=False)
        ...
        # out4h: High-level features output
        # out4v: Low-level features output
        return out4h, out4v
...
class Decoder(nn.Module):
    ...
    def forward(self, out2h, out3h, out4h, out5v, feedback=None):
        if feedback is not None:
            refine5 = F.interpolate(feedback, size=out5v.size()[2:], mode='bilinear', align_corners=False)
            refine4 = F.interpolate(feedback, size=out4h.size()[2:], mode='bilinear', align_corners=False)
            refine3 = F.interpolate(feedback, size=out3h.size()[2:], mode='bilinear', align_corners=False)
            refine2 = F.interpolate(feedback, size=out2h.size()[2:], mode='bilinear', align_corners=False)
            out5v = out5v + refine5
...
class F3Net(nn.Module):
    ...
    def forward(self, x, shape=None):
        ...
        pred1 = F.interpolate(self.linearp1(pred1), size=shape, mode='bilinear', align_corners=False)
        pred2 = F.interpolate(self.linearp2(pred2), size=shape, mode='bilinear', align_corners=False)

        out2h = F.interpolate(self.linearr2(out2h), size=shape, mode='bilinear', align_corners=False)
        out3h = F.interpolate(self.linearr3(out3h), size=shape, mode='bilinear', align_corners=False)
        out4h = F.interpolate(self.linearr4(out4h), size=shape, mode='bilinear', align_corners=False)
        out5h = F.interpolate(self.linearr5(out5v), size=shape, mode='bilinear', align_corners=False)
