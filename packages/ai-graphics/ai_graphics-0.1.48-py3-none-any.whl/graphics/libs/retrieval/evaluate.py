...
class OverAll:
    ...
    def __init__(self):
        self.default_hyper_params["recall_k"] = np.sort(self.default_hyper_params["recall_k"])
    ...
    def compute_recall_at_k(self, gt: List[bool], result_dict: Dict) -> None:
        ...
        """
        ks = self.default_hyper_params["recall_k"]
        gt = gt[:ks[-1]]
    ...
    def __call__(self, query_result: List, gallery_info: List) -> (float, Dict):
        ...
        recall_at_k = dict()
        for k in self.default_hyper_params["recall_k"]:
            recall_at_k[k] = 0
