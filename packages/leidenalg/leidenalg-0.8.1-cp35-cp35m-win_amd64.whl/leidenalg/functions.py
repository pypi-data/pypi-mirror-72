    ...
    for u_slice in v_slice.neighbors(mode=_ig.OUT):
      if v_slice.index < u_slice.index or G_coupling.is_directed():
        ...
        if len(set(nodes_v)) != len(nodes_v):
          err = '\n'.join(
            ['\t{0} {1} times'.format(item, count) for item, count in list(Counter(nodes_v).items()) if count > 1]
            )
        ...
        if len(set(nodes_u)) != len(nodes_u):
          err = '\n'.join(
            ['\t{0} {1} times'.format(item, count) for item, count in list(Counter(nodes_u).items()) if count > 1]
            )
        ...
        edges = list(zip(nodes_v, nodes_u))
        e_start = G.ecount()
        G.add_edges(edges)
        e_end = G.ecount()
        e_idx = list(range(e_start, e_end))
