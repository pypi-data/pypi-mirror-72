...
def safe_get_json(json_string, key, log=logging.getLogger('spotmax-agent')):
    try:
...
def safe_read_simple_url(url, log=logging.getLogger('spotmax-agent')):
    try:
...
def get_gcp_metadata(url, path_params, log=logging.getLogger('spotmax-agent')):
    try:
        url = url + path_params
        log.debug("reading simple {} ...".format(url))
        req = Request(url)
...
def get_azure_metadata(url, log=logging.getLogger('spotmax-agent')):
    try:
        log.debug("reading simple {} ...".format(url))
        req = Request(url)
...
def safe_get_simple_url(url, log=logging.getLogger('spotmax-agent')):
    try:
        log.debug("reading simple {} ...".format(url))
        req = Request(url)
        req.add_header('Content-Type', 'application/json')
...
def safe_get_url(url, token=None, log=logging.getLogger('spotmax-agent')):
    try:
...
def safe_post_url(url, message, token=None, log=logging.getLogger('spotmax-agent')):
    try:
...
def safe_put_url(url, message, token, log=logging.getLogger('spotmax-agent')):
    try:
...
def parse_yml(yml, log=logging.getLogger('spotmax-agent')):
    # Initialize Config
    with open(yml) as f:
        try:
            return yaml.load(f)
