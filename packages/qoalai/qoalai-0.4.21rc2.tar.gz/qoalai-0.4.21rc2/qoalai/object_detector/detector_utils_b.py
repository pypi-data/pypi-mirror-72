...
class ObjectDetector(object):
    ...
    def grid_mask(self):
        ...
        for i in range(3):
            self.grid_position_mask_onx_np.append(np.zeros((1, 
                                                            self.num_vertical_grid[i], 
                                                            self.num_horizontal_grid[i], 
                                                            1),
                                                        dtype=np.float32))
            self.grid_position_mask_ony_np.append(np.zeros((1, 
                                                            self.num_vertical_grid[i], 
                                                            self.num_horizontal_grid[i], 
                                                            1),
                                                        dtype=np.float32))
    ...
    def yolo_loss(self, outputs: [], labels: []):
        ...
        for i in range(3):
            ...
            for idx, val in enumerate(self.anchor[border_a:border_b]):
                ...
                a = w_pred_real / 416. #self.grid_width[i]
                b = w_label_real / 416. #self.grid_width[i]
                c = h_pred_real / 416. #self.grid_height[i]
                d = h_label_real / 416. #self.grid_height[i]
                sz_loss =  self.size_loss_alpha * (tf.sqrt(mse_loss_sum(a, b)) + tf.sqrt(mse_loss_sum(c, d)) )
    ...
    def build_yolov3_net(self, inputs, network_type, is_training):
        ...
        if self.is_multilabel: 
            total_properties = sum(self.multilabel_dict.values())
