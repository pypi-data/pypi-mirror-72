...
def is_local_ip(ip):
    """
    Test weather the given ip is a local ip, returns true if a local ip provided.
    :param ip:
    :return: Boolean, True if a private ip or local ip.
    """
    first = ip.split('.')[0]
    second = ip.split('.')[1]
    if ip == '127.0.0.1':
    ...
    if first in ['10', '192', '172']:
        if first == '10':
            ...
            return second in [str(each) for each in range(16, 32)]
        elif first == '192':
            return second == '168'
    else:
        return False
