...
class connect_Flow(connect_Base):
    ...
    _hx_statics = ["STEP_PARAM_ID", "getFormattedRequest", "getFormattedData", "getDataTable"]
    _hx_interfaces = []
    _hx_super = connect_Base
    ...
    def prepareRequestAndOpenLogSection(self,model):
        ...
        tierAccount = (assetRequest.asset.tiers.customer.id if (((assetRequest is not None) and ((assetRequest.asset.tiers.customer is not None)))) else (tierConfigRequest.configuration.tiers.customer.id if (((tierConfigRequest is not None) and ((tierConfigRequest.configuration.tiers.customer is not None)))) else "tier_account"))
        if ((assetRequest is not None) or ((tierConfigRequest is not None))):
            connect_Env.getLogger().setFilename((((((((((("" + ("null" if provider is None else provider)) + "/") + ("null" if hub is None else hub)) + "/") + ("null" if marketplace is None else marketplace)) + "/") + ("null" if product is None else product)) + "/") + ("null" if tierAccount is None else tierAccount)) + ".md"))
        elif ((listing is not None) or ((usageFile is not None))):
            connect_Env.getLogger().setFilename((((("usage/" + ("null" if provider is None else provider)) + "/") + ("null" if marketplace is None else marketplace)) + ".md"))
    ...
    def processAbortAndCloseLogSection(self,index,requestStr,dataStr):
        if self.abortRequested:
            if (self.abortMessage is None):
                ...
                saveResult = connect_storage_StepStorage.save(self.model,connect_storage_StepData(index,self.data,connect_storage_StorageType.ConnectStorage,(self.stepAttempt + 1)),param,Reflect.field(self.model,"update"))
...
class connect_models_Item(connect_models_IdModel):
    ...
    __slots__ = ("mpn", "quantity", "oldQuantity", "renewal", "params", "status", "displayName", "globalId", "itemType", "period", "type", "name")
    _hx_fields = ["mpn", "quantity", "oldQuantity", "renewal", "params", "status", "displayName", "globalId", "itemType", "period", "type", "name"]
    _hx_methods = ["getParamById"]
    _hx_statics = []
    _hx_interfaces = []
    _hx_super = connect_models_IdModel
