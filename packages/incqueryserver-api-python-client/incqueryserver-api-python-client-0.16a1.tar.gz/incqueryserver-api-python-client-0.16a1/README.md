...
with iqs_client.ApiClient(configuration) as api_client:
    ...
    close_acquisition_request = iqs_client.CloseAcquisitionRequest() # CloseAcquisitionRequest | Close an open acquisition, either successfully or with failure (discard). 

    try:
        # End the acquistion of a compartment, either by finalizing or discarding
        api_response = api_instance.close_acquisition(close_acquisition_request)
        pprint(api_response)
    except ApiException as e:
        print("Exception when calling AcquisitionApi->close_acquisition: %s\n" % e)
