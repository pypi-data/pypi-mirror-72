...
def plot_fit_parameters(fit_by_bin, pn, t1, dt1, r, dr, hr, dhr, plot_latex=False, latex_label_dict=None,
                        skip_factors=True, skipped=['factor', 'cstat', 'plot_filename'], save_plot=True):
    from matplotlib import rc

    fit_by_par = rearrange_fit_parameters(fit_by_bin)

    keys = list(fit_by_bin.keys())
    n_pars = len(fit_by_bin[keys[0]].keys()) + 1
...
def dump_latex_table(fit_by_bin, latex_label_dict=default_latex_label_dict, to_skip=[], get_unit=None):
...
'''
    def default_get_unit(par, kk, ff, par_name):
        ...
        if 'flux' in par:
            ff *= 1e7
            unit = '$10^{-7}\\mathrm{erg\\,s^{-1}\\,cm^{-2}}$'

        if 'norm' in par and 'bbody' in kk:
            ff = np.sqrt(ff)
            unit = 'km'
            par_name = 'r'
        if 'kT' in par or 'peak' in par:
            unit = 'keV'
