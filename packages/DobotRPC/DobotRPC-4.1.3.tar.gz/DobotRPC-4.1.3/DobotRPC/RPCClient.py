...
class RPCClient(object):
    def __init__(self,
        ...
        self.__ws: websockets.server.WebSocketServer = None
    ...
    async def send(self,
        ...
        if not self.is_connected:
            raise NetworkError.CommNotConnectError(
                "Had not connected Dobotlink!")
        ...
        loggers.get(LOGGER_NAME).debug(f"rpc client >>>> {payload}")
        ...
        if cb_feedback:
            ...
            if exchange["error"] is not None:
                raise NetworkError.NetworkError(exchange["error"])
    ...
    async def notify(self, method: str, data: Any) -> Any:
        ...
        loggers.get(LOGGER_NAME).debug(f"rpc client >>>> {rpc_packet}")
