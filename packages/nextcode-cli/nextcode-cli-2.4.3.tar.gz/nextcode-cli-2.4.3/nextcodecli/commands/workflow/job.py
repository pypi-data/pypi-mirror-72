...
def logs(ctx, log_group, log_filter):
    ...
    if not log_group:
        ...
        echo("The following log groups are available for job %s:\n  %s" % (workflow_job.job_id, "\n  ".join(log_groups.keys())))
        sys.exit(1)
    try:
...
def process_pod(ctx, is_raw):
    ...
    echo("Showing pod information for pod {} in job {} from when the process was run...".format(process["process_id"], ctx.obj.job.job_id))
    keys = [
        'name',
        'namespace',
        'creation_timestamp',
...
def process_node(ctx, is_raw):
    ...
    if is_raw:
        echo(dumps(node))
        return
    echo("Showing node information for pod {} in job {} from when the process was run...".format(process["process_id"], ctx.obj.job.job_id))
