...
def ipce_from_typelike_tr_(T: TypeLike, c: IFTContext, ieso: IESO) -> TRE:
    ...
    if T is float:
        res = cast(
            JSONSchema, {JSC_TYPE: JSC_NUMBER, SCHEMA_ATT: SCHEMA_ID, JSC_TITLE: JSC_TITLE_FLOAT},
        )
        res = sorted_dict_cbor_ord(res)
        return TRE(res)
    ...
    if T is Decimal:
        res = cast(
            JSONSchema, {JSC_TYPE: JSC_STRING, JSC_TITLE: JSC_TITLE_DECIMAL, SCHEMA_ATT: SCHEMA_ID},
        )
        res = sorted_dict_cbor_ord(res)
        return TRE(res)
    ...
    if T is datetime.datetime:
        res = cast(
            JSONSchema,
            {JSC_TYPE: JSC_STRING, JSC_TITLE: JSC_TITLE_DATETIME, SCHEMA_ATT: SCHEMA_ID,},
        )
        res = sorted_dict_cbor_ord(res)
        return TRE(res)
    ...
    if is_Type(T):
        ...
        res = cast(JSONSchema, {SCHEMA_ATT: SCHEMA_ID, JSC_TYPE: "subtype", "subtype": r.schema},)
        res = sorted_dict_cbor_ord(res)
