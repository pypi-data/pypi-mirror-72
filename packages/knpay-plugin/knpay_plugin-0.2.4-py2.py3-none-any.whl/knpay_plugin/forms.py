...
class PaymentForm(forms.Form):
    ...
    def connect(self):
        ...
        try:
            ...
            response = requests.post(config.get_knpay_url(
                is_payment_request=self.is_payment_request),
                json=data, timeout=35, verify=False)
        ...
        if response.status_code in [200, 201]:
            ...
            try:
                ...
                error_log = "Error occurred while parsing knpay response %s and response content is %s " % (
                    e, response.content)
                self._errors = {'__all__': str(error_log)}
                logger.error(error_log)
                return
