...
def download_gs(gsp: str, lp: str, api_key: str):
    bucket, path = split_rpath(gsp)
    path = quote(path, safe='')
    url = f'{GCP_ENDPOINT}/b/{bucket}/o/{path}?alt=media&key={api_key}'
    r = requests.get(url, stream=True)
    with open(lp, 'wb+') as f:
        for chunk in r.iter_content(chunk_size=1024):
            f.write(chunk)

def download_s3(s3p: str, lp: str):
    client = boto3.client('s3')
    bucket, path = split_rpath(s3p)
    client.download_file(bucket, path, lp)
    return lp
