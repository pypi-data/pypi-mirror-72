...
class TASTCode(object):
        ...
        for l in range(L):
            for m in range(M):
                for q in range(Q):
                    ...
                    self.codes[Q * M * l + m * Q + q] = np.matmul(
                        apsksymbols[l] * msymbols[m] * np.linalg.matrix_power(G, m), np.diag(qsymbols))

    def putRate(self):
        print("B / M = %d / %d = %d [bit/symbol]" % (self.B, self.M, self.B / self.M))

    @classmethod
    def evaluate(cls, M, Q, Lp, u):
        LDM = Lp * Q
        pmin = np.inf
    ...
    @classmethod
    def search(cls, M, Q, Lp):
        maxp = -1

        while True:
            u = np.random.randint(Q * Lp, size = M)
            #print(u)
            p = cls.evaluate(M, Q, Lp, u)
            if p > maxp:
                maxp = p
                bestu = u
    ...
    def getDiversityMaximizingFactors(self, M, Q, La, Lp):
        ...
        if M == 2 and Q == 1 and La == 1 and Lp == 2:
            ...
            u = [0,1,3,0,2,2,1,3,1,2,0,0,0,1,0,3,2,2,0,2,1,2,1,1,0,1,2,2,0,2,0,1,0,3,1,2,3,3,3,2,3,1,2,1,1,0,3,3,2,0,0,0,3,3,3,2,2,3,0,1,2,0,1,0]
        elif M == 64 and Q == 4 and La == 1 and Lp == 1:  # rate = 0.125, maxp = 0
            u = [0,3,2,2,1,0,1,2,0,3,1,0,0,3,0,1,2,2,0,0,3,2,1,2,2,3,0,2,3,1,3,2,2,2,3,2,3,0,0,3,0,0,3,1,0,3,3,3,2,2,3,2,2,1,3,2,0,2,3,1,3,1,1,2]
        elif M == 64 and Q == 1 and La == 1 and Lp == 4: # rate = 0.125, maxp = 0.502715
            u = [2,1,1,2,1,2,0,0,1,2,1,1,2,0,1,1,0,0,0,1,0,1,0,0,1,0,1,2,3,2,1,3,3,1,1,1,3,3,2,0,1,2,2,2,1,2,3,2,3,0,2,0,2,1,2,0,2,0,0,1,0,2,2,1]
        else:
