...
class DiagonalUnitaryCode:
    ...
    def __init__(self, M, L, initu = None):
        """
        Args:
        ...
        if 'None' in str(type(initu)):
            self.u = self.getDiversityMaximizingFactors(M, L)
        elif 'str' in str(type(initu)) and initu == "random":
            self.u = self.getRandomFactors(M, L)
        else:
            self.u = initu
        for l in range(L):
            self.codes[l] = np.diag(np.exp(1.0j * 2.0 * np.pi * self.u * l / L))

    def putRate(self):
        print("B / M = %d / %d = %d [bit/symbol]" % (self.B, self.M, self.B / self.M))

    def getRandomFactors(self, M, L):
        ret = np.ones(M)
    ...
    def getDiversityMaximizingFactors(self, M, L):
        if M == 1 and L == 1:
            ...
            # maxp = 1.08268e-06
            u = [1, 138, 167, 182, 193, 238, 243, 250, 254, 264, 264, 273, 274, 275, 278, 291, 303, 308, 315, 350, 351, 358, 366, 373, 374, 377, 402, 411, 425, 439, 469, 494]
        elif M == 64 and L == 64:
            # maxp = 2.13874e-11
