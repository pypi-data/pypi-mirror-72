...
class UpdateConnector:
    ...
    def new_employee_with_first_contract(self, data: dict, overload_fields={}):
        ...
        fields_to_update.update({"PsPv": data['send_payslip']}) if 'send_payslip' in data else fields_to_update
        fields_to_update.update({"YsPv": data['send_annual_statement']}) if 'send_annual_statement' in data else fields_to_update

        for key in overload_fields.keys():
            fields_to_update.update({key: overload_fields[key]})
        body['AfasEmployee']['Element']['Fields'].update(fields_to_update)
