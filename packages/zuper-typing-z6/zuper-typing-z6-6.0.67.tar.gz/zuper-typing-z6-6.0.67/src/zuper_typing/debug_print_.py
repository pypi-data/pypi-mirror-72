...
eop_log = replace(get_default_dpoptions(), max_initial_levels=10)  # <lower
...
def debug_print_dict(
    ...
    if type(x) is frozendict:
        lbrace = color_ops_light("f" + lbrace)
        rbrace = color_ops_light(rbrace)
    else:
        lbrace = color_ops(lbrace)
        rbrace = color_ops(rbrace)
    ...
    if len(x) == 0:
        if opt.omit_type_if_empty:
            return lbrace + rbrace + ps
        else:
            return dps(type(x)) + " " + lbrace + rbrace + ps
    ...
    if nlines == 0 and tlen < get_max_col(stack):
        # x = "," if len(x) == 1 else ""
        res = lbrace + color_ops(", ").join(ss) + rbrace + ps
...
def debug_print_set(
    ...
    if type(x) is frozenset:
        popen = color_ops_light("f" + popen)
        pclose = color_ops_light(pclose)
    else:
        popen = color_ops(popen)
        pclose = color_ops(pclose)
