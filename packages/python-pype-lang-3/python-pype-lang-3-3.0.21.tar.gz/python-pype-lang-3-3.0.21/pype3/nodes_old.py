...
def callable_node(fArg,params):
    ...
    return callable_node_with_args(fArg,[params['accum']])
...
def index_arg_node(fArg,params):

    return Subscript(value=params['accum'],
...
def index_node(fArgs,accum=ACCUM_LOAD,getFunc=get_call_or_false,**params):
...
def lambda_node(fArgs,accum=ACCUM_LOAD,**params):
...
def map_dict_or_list_node(fArg,accum=ACCUM_LOAD,**params):
...
def reduce_node(fArgs,accumNode=ACCUM_LOAD,**params):
...
def filter_dict_node(fArgs,
    ...
    return dict_comp(accum,loadedDictValue,ifAnyNode,**params)
    

def filter_list_or_dict_node(fArg,accum=ACCUM_LOAD,**params):
...
def switch_dict_node(fArg,accum=ACCUM_LOAD,**params):
...
def dict_assoc_node(fArgs,accum=ACCUM_LOAD,**params):
...
def dict_dissoc_node(fArgs,accum=ACCUM_LOAD,**params):
...
def list_build_node(fArgs,accum=ACCUM_LOAD,**params):
...
def dict_build_node(fArg,accum=ACCUM_LOAD,**params):
...
def embedded_pype_chain(fArgs,accum,**params):
...
def embedded_pype_node(fArgs,accum=ACCUM_LOAD,**params):
...
def do_lambda_node(node,**params):
...
def do_node(fArgs,accum=ACCUM_LOAD,**params):
...
def name_bookmark_node(fArg,accumNode,**params):

    print('*'*30)
    print('name_bookmark_node')
    print(f'{fArg} is fArg')

    bookmarkName=fArg.get_name()
    fArgVal=fArg.get_f_arg_val()

    print(f'{fArgVal} is fArgVal')
...
def optimize_rec(fArg,
    ...
    node=optimizer(fArg,accumNode,**params)
...
def optimize_f_args(fArgs,startNode,embeddingNodes):
    ...
    defaultParams={'accum':ACCUM_LOAD,
                   'optimizePairs':OPTIMIZE_PAIRS,
                   'embeddingNodes':embeddingNodes}

    for fArg in fArgs:

        opt=optimize_rec(fArg,**defaultParams)
