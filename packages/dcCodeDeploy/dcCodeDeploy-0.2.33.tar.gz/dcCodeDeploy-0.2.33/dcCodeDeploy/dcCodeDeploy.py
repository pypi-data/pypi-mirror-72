...
def build(branch, ngBuild='', ngApp='', angularBuild='', angularEnv='', installPath='.', node=''):
 ...
 # For more info: https://github.com/yarnpkg/yarn/issues/683#issuecomment-346038525
    yarn_cache=os.getenv('PWD','') + '/.yarn_cache/'
    local('mkdir -p .yarn_cache/')
    ...
    # ensure yarn installs all build tools
        ...
        if ngBuild:
            ...
            local('cp -R migrations/ dist/migrations')
...
#    local('cp config/keyset-prod.json deploy/worker/config/keyset.json')
#    local('cp config/logback.xml deploy/worker/config/')
#    local('cp config/application-prod-worker.yml deploy/worker/config/')
