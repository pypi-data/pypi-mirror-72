...
def get_diamond():
    ...
    if is_in_path('diamond') is False:
        ...
        if platform.system() == 'Darwin':
            url = 'http://github.com/bbuchfink/diamond/releases/download/v0.9.34/diamond-linux64.tar.gz'
        else:
            url = 'http://github.com/bbuchfink/diamond/releases/download/v0.9.34/diamond-linux64.tar.gz'
        extract(download(url))
...
def get_blast():
    ...
    if is_in_path('blastn') is False:
        linux_url = 'https://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/LATEST/ncbi-blast-2.10.1+-x64-linux.tar.gz'
        mac_url = 'https://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/LATEST/ncbi-blast-2.10.1+-x64-macosx.tar.gz'
        if platform.system() == 'Darwin':
