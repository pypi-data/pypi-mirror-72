...
def check_if_change_required(case: Case, execute_in_order, store_response):
    ...
    if ('sm/user/entities/{id}' in path) and (method == 'get' or method == 'delete'  or method == 'patch'):
        case.path_parameters['id'] = store_response.get_store_result('post:/sm/user/entities', 'id')
    if ('sm/user/entities' in path) and (method == 'post' or method == 'patch'):
        case.body["short_name"] = str(time.time()) + 't'
  
    if ('sm/user/securities/{id}' in path) and (method == 'get' or method == 'delete'  or method == 'patch'):
        case.path_parameters['id'] = store_response.get_store_result('post:/sm/user/securities', 'id')
    if ('sm/user/securities' in path) and (method == 'post' or method == 'patch'):
        case.body["short_name"] = str(time.time()) + 't'
        case.body["issuer_id"] = store_response.get_store_result('post:/sm/user/entities', 'id')

    if ('sm/user/quotes/{id}' in path) and (method == 'get' or method == 'delete'  or method == 'patch' ):
        case.path_parameters['id'] = store_response.get_store_result('post:/sm/user/quotes', 'id')
    if ('sm/user/quotes' in path) and (method == 'post' or method == 'patch'):
        case.body["short_name"] = str(time.time()) + 't'
        case.body["security_id"] = store_response.get_store_result('post:/sm/user/securities', 'id')
     
    if '/notes' in path and case.path_parameters and 'note_id' in case.path_parameters:
...
def check_if_storing_required(case, response, execute_in_order, store_response):
    path = case.endpoint.path.lower()
    method = case.endpoint.method.lower()
    if '/sm/user/entities' in path and method == 'post':
        store_response.store_result(method+":"+path, 'id', json.loads(response.text).get('id', None)) 
    elif path == '/notes' and method == 'post':
        store_response.store_result(method + ":" + path, 'id', json.loads(response.text).get('id', None))
    elif path == '/sm/user/securities' and method == 'post':
        store_response.store_result(method + ":" + path, 'id', json.loads(response.text).get('id', None))
    elif path == '/sm/user/quotes' and method == 'post':
        store_response.store_result(method + ":" + path, 'id', json.loads(response.text).get('id', None))         
