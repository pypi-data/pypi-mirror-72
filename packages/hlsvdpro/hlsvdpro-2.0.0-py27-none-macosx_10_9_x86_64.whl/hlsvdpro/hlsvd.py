"""The HLSVDPRO package computes a 'sum of lorentzians' model for the complex
'signals' data passed in via a 'black box' state space approach. We are using
the PROPACK Fortran library to compute SVDs and the algorithm is based on the
description in:
...
def hlsvdpro(data, nsv_sought, m=None):
    """
    Computes a 'sum of lorentzians' model for the complex 'data' passed in
    using the PROPACK version 2.1 SVD library and based on the algorithm in:
    ...
    if mode == "f":
        x = sp.linalg.hankel(xx[:l + 1], xx[l:])
    else:
        ...
        x = sp.linalg.hankel(xx[:m - 1:-1], xx[m::-1])  # SVD of data matrix and truncation of U to form Uk
    ...
    #   'reverse' puts them largest->smallest like singular values

    roots = sp.linalg.eigvals(zp)
...
def convert_hlsvd_result(result, dwell):
    ...
    Args:
        result (tuple): an 8-tuple output from hlsvd() method
        dwell (float): Dwell time in [ms]

    Returns:
        tuple: an 8-tuple containing -
            Same as 8-tuple from hlsvd() method, but frequencies convert to
            [kHz] and damping factors to [ms]. Phases convert to [degrees].
            Singular values and row and column matrices are maintained at
            their same values and output tuple locations.
    ...
    phases = np.array([phase * (180 / 3.1415926) for phase in phases])
...
TESTDATA = {"step_size": 0.256, "npts_acq": 4096, "npts_svd": 1024,
