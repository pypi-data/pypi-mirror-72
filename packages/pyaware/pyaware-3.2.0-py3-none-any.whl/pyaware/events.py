    ...
    def publish(self, topic, **data):
        ...
        if pyaware.from_coroutine():
            ...
            for handle, parse_topic in self.handles_async.values():
                ...
                if parse_topic:
                    futures.append(
                        _loop.call_soon_threadsafe(_loop.create_task, pyaware.async_call(handle, topic=topic, **data)))
                else:
            ...
            for handle, parse_topic in self.handles_executor.values():
                ...
                if parse_topic:
                    futures.append(
                        _loop.call_soon_threadsafe(_loop.run_in_executor(None, partial(handle, topic=topic, **data))))
                else:
