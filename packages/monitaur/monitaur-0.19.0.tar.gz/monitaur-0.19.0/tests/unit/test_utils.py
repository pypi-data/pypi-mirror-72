...
@pytest.mark.parametrize(
    "enabled, drift_dict, drift_type",
    [
        (False, {"age": range(9, 10)}, "feature"),
        (False, {"age": range(9, 10)}, "model"),
        (None, {"age": range(9, 10)}, "feature"),
        (None, {"age": range(9, 10)}, "model"),
    ],
)
def test_validate_validate_drift_metrics_invalid(enabled, drift_dict, drift_type):
    with pytest.raises(MetricsError) as excinfo:
        validate_drift_metrics(enabled, drift_dict, drift_type)
    assert (
