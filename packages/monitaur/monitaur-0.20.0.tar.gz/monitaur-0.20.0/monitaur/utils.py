...
def record_training_file_save(
    model_set_id, aws_credentials, x_train, y_train, y_labels=None
):
    """
    Description:
    ...
    try:
        ...
        upload_file_to_s3(model_set_id, "x", x_path, aws_credentials)

        # save and upload y_train csv to s3
        np.savetxt(y_path, y_train, delimiter=",")
        upload_file_to_s3(model_set_id, "y", x_path, aws_credentials)
        ...
        if y_labels is not None:
            ...
            upload_file_to_s3(model_set_id, "labels", aws_credentials)
        return True
    except Exception:
        return False
