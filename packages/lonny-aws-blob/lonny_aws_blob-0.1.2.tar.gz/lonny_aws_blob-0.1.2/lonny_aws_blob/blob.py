...
def _query(selectors = dict(), *, next_token = None):
    # No downside to removing "none" values. Would form an illegal query anyway...
    predicates = [f"{k} = '{v}'" for k,v in selectors.items() if v is not None]
    ...
    if next_token is not None:
        for blob in _query(selectors, next_token = next_token):
            yield blob

def query(** selectors):
    return _query(selectors)
