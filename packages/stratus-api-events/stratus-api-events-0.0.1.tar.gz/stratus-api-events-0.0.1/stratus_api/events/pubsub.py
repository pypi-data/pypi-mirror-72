...
def format_pubsub_name(name, app_settings, project_id=None, pubsub_type='topic'):
...
def get_topic(name, project_id=None):
    ...
    topic_name = format_pubsub_name(name=name, pubsub_type='topic', project_id=project_id, app_settings=settings)
...
def create_topic(name, project_id=None) -> tuple:
    ...
    logger = get_logger()
    publisher = create_publisher()
    topic_name = format_pubsub_name(name=name, pubsub_type='topic', project_id=project_id, app_settings=app_settings)
    created = False
    try:
...
def create_subscription(topic, subscription, path, project_id=None):
    ...
    push_config = dict()
    created = False
    topic_name = format_pubsub_name(name=topic, pubsub_type='topic', project_id=project_id, app_settings=app_settings)
...
def push_to_topic(topic, attributes, payload: dict):
    import json
    publisher = create_publisher()
    publisher.publish(
