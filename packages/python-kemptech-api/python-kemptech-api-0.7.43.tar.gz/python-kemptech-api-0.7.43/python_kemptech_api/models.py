...
# For compatablilty with both 2.7 and 3.4
try:
...
class BaseKempAppliance(HttpClient, AccessInfoMixin):
    ...
    def _get_curl_command_list(self, command):
        ...
        """
        curl = [
            'curl', '-f', '-s', '-k', '--connect-timeout', str(utils.TIMEOUT)
        ]
    ...
    def download_user_cert(self, user, location=os.curdir):
        ...
        cmd = self._get_curl_command_list('userdownloadcert?user={}'
                                          .format(user))
        ret = subprocess.call(cmd, stdout=file)
        if ret != 0:
            # If the command did not succeed, remove the file
            file.close()
            os.remove(file.name)
    ...
    def restore_cert(self, password, file, restore_type="full"):
        params = {"password": password, "type": restore_type}
        response = self._post('/restorecert', file=file,
                              parameters=params)
        return send_response(response)
    ...
    def get_azure_ha_params(self):
        """Get HA parameters from LM deployed on Azure

        :return: Dict containing HA params
        """
        response = self._get('/getazurehaparams')
        if is_successful(response):
            data = get_data(response)
            return data.get('AzureHA', {})
        else:
            raise KempTechApiException(get_error_msg(response))

    def get_aws_ha_params(self):
        """Get HA parameters from LM deployed on AWS

        :return: Dict containing HA params
        """
        response = self._get('/getawshaparams')
        if is_successful(response):
            data = get_data(response)
            return data.get('AwsHA', {})
        else:
            raise KempTechApiException(get_error_msg(response))
...
class LoadMaster(BaseKempAppliance):
    ...
    def backup_cert(self, password, file_name=None, location='backups'):
        """
        Create certificate backup with the provided password and optional
        filename
        """
        ...
        #with open(file_name, 'wb') as file:
        file = open(file_name, 'wb')
        cmd = self._get_curl_command_list(
            'backupcert?{}'.format(
                urlencoder.urlencode({"password": password})
            )
        )
        ret = subprocess.call(cmd, stdout=file)
        if ret != 0:
    ...
    def create_ldap(self, ldap_obj):
        return LDAP(self.access_info, ldap_obj)

    def get_ldap_endpoint(self, ldap_obj=None):
        if ldap_obj:
            response = self._get("/showldapendpoint", {'name': ldap_obj['name']})
        else:
            response = self._get("/showldaplist")
        if is_successful(response):
