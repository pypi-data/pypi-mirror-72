...
class Protocol(asyncio.Protocol):
    def __init__(self, daemon, *args, **kwargs):
        ...
        self.logger.debug(self._avro_protocol)
        ...
        try:
            for hs, meta, name, params in self.unpacker:
                if hs is not None:
                    ...
                    self.logger.debug(f"hs response {out}")
                    if hs.match == "NONE":
                        name = ""
                ...
                self.logger.info(f"Wrote meta, {meta}, {out_meta.getvalue()}")
                try:
                    ...
                    self.logger.info(f"Wrote non-error flag")
                ...
                self.logger.debug("Wrote end frame")
                if name == "shutdown":
