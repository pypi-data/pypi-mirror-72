...
float quisk_make_sidetone(int sample_rate, int rewind)
{
	float rise_time;
	...
	static float phase = 0.0;
        ...
        if (rewind) {    // rewind the phase
	        phase -= 2.0 * M_PI * quisk_sidetoneFreq / sample_rate * rewind;
		//printf("rewind phase %.3f\n", phase);
                return 0.0;
        }
	rise_time = 0.707 / (sample_rate * 4.0 / 1000.0);	// milliseconds rise time
	...
	if (envelopeVol > 0) {
		phase += 2.0 * M_PI * quisk_sidetoneFreq / sample_rate;
		if (phase > 2.0 * M_PI)
			phase -= 2.0 * M_PI;
		return sinf(phase) * envelopeVol * quisk_sidetoneVolume;
	}
	return 0.0;
}
...
#ifdef MS_WINDOWS
      else if (quisk_sound_state.use_soundio)
         dev->driver = DEV_DRIVER_SOUNDIO;
      else
         dev->driver = DEV_DRIVER_DIRECTX;
#else
...
PyObject * quisk_set_sound_name(PyObject * self, PyObject * args)	// Called from GUI thread
{
	...
	if (!PyArg_ParseTuple (args, "iieses", &index, &play, utf8, &name, utf8, &device_name))
		return NULL;
	if (play) {
		strncpy(PlaybackDevices[index]->name, name, QUISK_SC_SIZE);
		strncpy(PlaybackDevices[index]->device_name, device_name, QUISK_SC_SIZE);
	}
	else {
		strncpy(CaptureDevices[index]->name, name, QUISK_SC_SIZE);
...
void quisk_open_sound(void)	// Called from GUI thread
{
...
#ifdef QUISK_NO_SOUNDIO
	quisk_sound_state.use_soundio = 0;
#else
	if (QuiskGetConfigInt ("use_fast_sound", 0))
		quisk_sound_state.use_soundio = 1;
	else
		quisk_sound_state.use_soundio = 0;
#endif
	...
	Capture.is_Playback = 0;
	Playback.is_Playback = 1;
	MicCapture.is_Playback = 0;
	MicPlayback.is_Playback = 0;
	DigitalInput.is_Playback = 0;
	DigitalOutput.is_Playback = 0;
	RawSamplePlayback.is_Playback = 0;
	quisk_DigitalRx1Output.is_Playback = 0;
...
void quisk_set_sidetone(void)
{
	...
	if (quisk_sound_state.verbose_sound)
		printf("quisk_set_sidetone %d\n", quisk_active_sidetone);
}
