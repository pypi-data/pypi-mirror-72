		...
		if (restart || quisk_play_state == STARTING) {		// play silence
			...
			case Int16:
				for (i = 0; i < frames; i++) {
					...
					*(int16_t *)areas[ch_I].ptr = i16;
					areas[ch_I].ptr += areas[ch_I].step;
					if (two) {
						*(int16_t *)(areas[ch_Q].ptr) = i16;
						areas[ch_Q].ptr += areas[ch_Q].step;
					}
				}
				break;
			case Int32:
				for (i = 0; i < frames; i++) {
					...
					*(int32_t *)areas[ch_I].ptr = i32;
					areas[ch_I].ptr += areas[ch_I].step;
					if (two) {
						*(int32_t *)areas[ch_Q].ptr = i32;
						areas[ch_Q].ptr += areas[ch_Q].step;
					}
				}
				break;
			case Float32:
				for (i = 0; i < frames; i++) {
					...
					*(float *)areas[ch_I].ptr = f32;
					areas[ch_I].ptr += areas[ch_I].step;
					if (two) {
						*(float *)areas[ch_Q].ptr = f32;
						areas[ch_Q].ptr += areas[ch_Q].step;
					}
				}
				break;
...
static void open_soundio_playback(struct sound_dev * dev)
{
	...
	if (dev->dev_index == t_Playback)	// radio sound playback
		latency = quisk_sound_state.data_poll_usec * 1E-6;
	else
		latency = quisk_sound_state.latency_millisecs * 1E-3;
	if (quisk_sound_state.verbose_sound) {
