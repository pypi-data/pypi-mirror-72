...
static int have_soundio = 0;		// do we have the libsoundio module?
#else
static int have_soundio = 1;
#endif
...
static PyObject * watfall_RgbData(PyObject * self, PyObject * args)	// Called by the GUI thread
{
	...
	if (!PyArg_ParseTuple (args, "y*y*y*ii", &red, &green, &blue, &width, &max_height))
		return NULL;
...
static PyObject * watfall_GetPixels(PyObject * self, PyObject * args)	// Called by the GUI thread
{
	...
	if (!PyArg_ParseTuple (args, "y*w*iii", &rgb_data, &pixels, &x_origin, &width, &height))
		return NULL;
