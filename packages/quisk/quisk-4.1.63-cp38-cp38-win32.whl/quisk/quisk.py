...
"""
  ...
  def GetNameValue(self):
    if self.params[0] == '?':
      return '?', ''
    args = self.params.split(maxsplit=1)
    if len(args) == 1:
      return args[0], ''
...
class App(wx.App):
  ...
  def OnInit(self):
    ...
    # These are parallel to lists in sound.c:
    self.sound_play_devices = ("name_of_sound_play", "name_of_mic_play", "digital_output_name", "sample_playback_name", "digital_rx1_name")
    self.sound_capt_devices = ("name_of_sound_capt", "microphone_name", "digital_input_name")
...
The new code supports multiple corrections per band.""")
  ...
  def MakeSoundDeviceList(self):
    # Create the list of capture and play devices (play, prefix, name, device, alsa_name).
    # Play is 1 for play, 0 for capture. Prefix is "alsa:", "pulse:", ""
    self.sound_devices = []
    if sys.platform == 'win32':
      if QS.get_params("have_soundio") and conf.use_fast_sound:
        dev_capt, dev_play = QS.sio_sound_devices("wasapi")
        for name, device, raw in dev_capt:
          if raw:
            self.sound_devices.append((0, '', name, device, ''))
        for name, device, raw in dev_play:
          if raw:
            self.sound_devices.append((1, '', name, device, ''))
      else:	# Use old DirectX
        dev_capt, dev_play = QS.sound_devices()
        for name in dev_capt:
          self.sound_devices.append((0, '', name, name, ''))
        for name in dev_play:
          self.sound_devices.append((1, '', name, name, ''))
    else:
      if QS.get_params("have_soundio") and conf.use_fast_sound:
        dev_capt, dev_play = QS.sio_sound_devices("alsa")
        for name, device, raw in dev_capt:
          if raw:
            self.sound_devices.append((0, 'alsa:', name, device, device))
        for name, device, raw in dev_play:
          if raw:
            self.sound_devices.append((1, 'alsa:', name, device, device))
      else:	# Create the names from our old Alsa module
        dev_capt, dev_play = QS.sound_devices()
        for name in dev_capt:
          i = name.rfind(" (hw:")
          if i > 5:
      ...
      for device, name, alsa_name in dev_capt:
        i = alsa_name.rfind(" (hw:")
        if i > 5:
          alsa_name = alsa_name[i + 2 : -1]
        self.sound_devices.append((0, 'pulse:', name, device, alsa_name))
      for device, name, alsa_name in dev_play:
        i = alsa_name.rfind(" (hw:")
        if i > 5:
          alsa_name = alsa_name[i + 2 : -1]
        self.sound_devices.append((1, 'pulse:', name, device, alsa_name))
    # Create the list of sound card names
    self.dev_capt = ['']
    self.dev_play = ['']
    pulse_capt = ['pulse: default']
    pulse_play = ['pulse: default']
    for play, prefix, name, device, alsa_name in self.sound_devices:
      #print (play, ';', prefix, ';', name, ';', device, ';', alsa_name)
      if alsa_name:
        t = "%s%s (%s)" % (prefix, name, alsa_name)
      else:
        t = "%s%s" % (prefix, name)
      if prefix == "pulse:":
        if play:
  ...
  def GetSoundDevice(self, in_play, in_name):
    # Given a sound card name, return (the corrected name, the device)
    if not in_name:
      return '', ''
    if in_name[0:9] == 'portaudio':
      return in_name, ''
    i = in_name.rfind(" (hw:")		# strip off ending " (hw:1,0)"
    if i > 5:
      in_name = in_name[0:i]
    if in_name[0:5] == 'alsa:':
      in_prefix = 'alsa:'
      in_name = in_name[5:]
    elif in_name[0:6] == 'pulse:':
      in_prefix = 'pulse:'
      in_name = in_name[6:]
    else:
      in_prefix = ''
    ...
    for play, prefix, name, device, alsa_name in self.sound_devices:
      ...
      if in_prefix != prefix:
        continue
      if in_name == name or in_name == device:
        return in_prefix + name, device
      if in_name in name:
        try_name = name
        try_device = device
    if try_name:
      return in_prefix + try_name, try_device
    return in_name, in_name
  def OpenSound(self):
    ...
    index = 0
    for key in self.sound_capt_devices:
      name = getattr(conf, key)		# capture devices
      name, device = self.GetSoundDevice(0, name)
      QS.set_sound_name(index, 0, name, device)
      index += 1
    index = 0
    for key in self.sound_play_devices:
      name = getattr(conf, key)		# playback devices
      name, device = self.GetSoundDevice(1, name)
      QS.set_sound_name(index, 1, name, device)
      index += 1
