...
class HyperparameterExplorer(object):
    ...
    def download_runs(self, store, ws_name, run_group_name, run_group_type, hp_config_cloud_path, hp_config_local_dir):
        ...
        # of run records to read at a time in mongo (keep small to avoid "maximum server message size exceeded" errors)
        batch_size = 20  
        ...
        if run_group_name == "experiment":
            ...
            allrun_records = store.get_all_runs(run_group_name, ws_name, run_group_type, batch_size=batch_size)
        else:
    ...
    def define_hyperparameters(self, hp_config_file_path):
        ...
        hp_dict = hp_config_yaml[constants.HPARAM_DIST]
