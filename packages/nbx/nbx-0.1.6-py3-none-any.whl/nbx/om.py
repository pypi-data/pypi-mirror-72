...
__all__ = ['extract_tag', 'contains_tag', 'is_nbx', 'is_nbx_cell', 'is_magic_or_shell', 'strip', 'parse_xarg',
           ...
           'NbxBundle', 'BUNDLE_SUMMARY', 'create_script', 'create_om_files', 'add_if_necessary', 'create_run_script',
           'create_job_script', 'create_wrapper_script', 'create_folders', 'create_experiment_script', 'INSTRUCTIONS']
...
#Cell
def create_script(tname, fname, vars):
    tpath = Path(pkg_resources.resource_filename(
                     __name__, f"/templates/{tname}"))
    create_file_from_template(tpath, fname, vars)

def create_om_files(nbname, target_dir, lang, num_jobs, simg, job_header, arr_size=1000, step=20):
    ...
    create_wrapper_script(target_dir, lang)
    create_experiment_script(target_dir, lang, nbname)
...
def create_wrapper_script(target_dir, lang):
    tname = f"wrapper_{lang}.tpl"
    fname = Path(target_dir)/f"wrapper.{lang}"
    create_script(tname, fname, {})
...
def create_experiment_script(target_dir, lang, nbname):
    ...
    create_script(tname, fname, vars(nb))

    tname = f"pspace_{lang}.tpl"
    fname = Path(target_dir)/f"psapce.{lang}"
    create_script(tname, fname, {})
