    ...
    def _handle_keypress(self, key) -> Optional[List[Command]]:
        ...
        # execute the command on enter
            ...
            # else parse the various : commands
                ...
                if command_parts[0] == "pdf":
                    commands[0].suppress_clear = True

                    commands += [
                        SetStatusLineTextCommand("Generating PDF...", Position.CENTER),
                        PDFCommand(path=possible_path),
                    ]
...
class Editor(Component):
    ...
    def _handle_command(self, command: Command) -> Optional[List[Command]]:
        ...
        elif isinstance(command, PDFCommand):
            return self.__handle_pdf_command(command)

    def __handle_pdf_command(self, command: PDFCommand) -> List[Command]:
        """Handle PDF command."""
        tmp = secrets.token_hex(16)

        self.__write_score_to_file(tmp)

        child = subprocess.Popen(["lilypond", str(tmp)], stdout=subprocess.DEVNULL)

        if child.returncode == 0:
            return [
                SetStatusLineTextCommand("PDF generated successfully.", Position.CENTER)
            ]
        else:
            return [
                SetStatusLineTextCommand(
                    "PDF generation failed. Is LilyPond installed?", Position.CENTER
                )
            ]
    ...
    def __handle_save_command(self, command: SaveCommand) -> List[Command]:
        ...
        try:
            self.__write_score_to_file(self.current_file_path)
            self.changed_since_saving = False
    ...
    def __write_score_to_file(self, path: str):
