...
class MapControls(wx.Panel):
    ...
    def build_ctrls(self):
        ...
        self.toolbar.AddTool(PEN_COLOR, "Pen color",
                             self.color_icon.bitmap(self.pen_color),
                             shortHelp="Pen color")
        self.toolbar.AddIconSelector(LINE_WIDTH_SELECTOR, "Line width",
                                     LineWidthIcons(),
                                     shortHelp="Line width")
        self.toolbar.AddIconSelector(MINI_SELECTOR, "Select miniature", self.minilib, shortHelp="Select miniature")
        ...
        self.toolbar.AddTool(MAP_OPEN, "Open map",
                             orpg.tools.bitmap.create_from_file("tool_open.png"),
                             shortHelp="Open a map")
        self.toolbar.AddTool(MAP_OPEN, "Save map",
        ...
        self.Bind(wx.EVT_TOOL, self.on_pen_color, id=PEN_COLOR)
        self.Bind(wx.EVT_TOOL, self.on_line_width, id=LINE_WIDTH_SELECTOR)
        self.Bind(wx.EVT_TOOL, self.on_mini_selector, id=MINI_SELECTOR)
        ...
        self.Bind(wx.EVT_TOOL, self.map_frame.on_open, id=MAP_OPEN)
        self.Bind(wx.EVT_TOOL, self.map_frame.on_save, id=MAP_SAVE)
    ...
    def on_pen_color(self,evt):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.toolbar.SetToolNormalBitmap(PEN_COLOR, self.color_icon.bitmap(self.pen_color))
        dlg.Destroy()

    def on_line_width(self, evt):
        self.toolbar.PopupIconSelector(LINE_WIDTH_SELECTOR)

    def on_mini_selector(self, evt):
        self.toolbar.PopupIconSelector(MINI_SELECTOR)
        self.toolbar.ToggleTool(MODE_ADD_MINI, True)
        self._set_mode(MODE_ADD_MINI)
    ...
    def on_left_up(self,evt):
        ...
        if self.mode == MODE_FREEFORM:
            ...
            mini_tmpl = self.toolbar.GetSelected(MINI_SELECTOR)
    ...
    def polyline_add_point(self, pos):
        ...
        if not self.drawing:
            self.working_line = self.new_line()
    ...
    def new_line(self):
        width = self.toolbar.GetSelected(LINE_WIDTH_SELECTOR).width
