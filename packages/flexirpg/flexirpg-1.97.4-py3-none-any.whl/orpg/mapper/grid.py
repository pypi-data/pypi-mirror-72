...
class grid_layer(layer_base):
    ...
    def _snap_to_corner(self, pos, w, h):
        xl = int((pos.x + self.unit_size/2)     / self.unit_size) * self.unit_size
        xr = int((pos.x + self.unit_size/2 + w) / self.unit_size) * self.unit_size - w
        yt = int((pos.y + self.unit_size/2)     / self.unit_size) * self.unit_size
        yb = int((pos.y + self.unit_size/2 + h) / self.unit_size) * self.unit_size - h
