...
class JupyterS3EC2RoleAuthentication(JupyterS3Authentication):
    ...
    def get_credentials(self):
        ...
        if now > self.expiration:
            request = HTTPRequest(
                'http://169.254.169.254/latest/meta-data/iam/security-credentials/' + self.role_name, method='GET')
...
def _get(context, path, content, type, format):
    ...
    format_to_get = format if format is not None else _format_from_type_and_path(
        context, type_to_get, path)
    ...
    def _parse_list_response(response):
        ...
        for el in root:
            if el.tag == f'{namespace}Contents':
                ...
                last_modified_str = _first_child_text(
                    el, f'{namespace}LastModified')
                last_modified = datetime.datetime.strptime(
            ...
            if el.tag == f'{namespace}CommonPrefixes':
                ...
                directories.append(_first_child_text(
                    el, f'{namespace}Prefix')[:-1])
...
def _aws_sig_v4_headers(access_key_id, secret_access_key, pre_auth_headers,
    ...
    def signature():
        ...
        date_key = sign(
            ('AWS4' + secret_access_key).encode('utf-8'), datestamp)
