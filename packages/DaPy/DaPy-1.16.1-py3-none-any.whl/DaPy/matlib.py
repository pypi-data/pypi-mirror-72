...
from math import sqrt, exp as math_exp, factorial
...
def sign(x):
    ...
    if not hasattr(x, '__abs__') or not hasattr(x, '__div__'):
        x = Matrix(x)
    return ((x + 0.1) / abs(x + 0.1) + (x - 0.1) / abs(x - 0.1)) / 2.0
...
def diff(seq, lag=1, keep_head=False):
    return Series(seq).diff(lag, keep_head)
...
def mean(data, axis=None):
    ...
    if axis is None:
        if hasattr(data, 'shape'):
            return float(_sum(data, None)) / reduce(mul, data.shape)
...
def describe(data, detail=0):
    ...
    size = float(len(data))
    if size == 0:
        return statistic(*chain(repeat('', 7), [mode, '', '']))
