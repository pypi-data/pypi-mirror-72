...
from .SafetyContainer import ThreadSafeContainer
from .IndexArray import SortedIndex
from .Row import Row
from .Series import Series
from .utils import (argsort, auto_plus_one, auto_str2value,
                    fast_str2value, hash_sort, is_dict, is_empty, is_iter,
                    is_math, is_seq, is_str, is_value, get_isnan, range, split,
...
AGG_FUNC = {'min': Series.min,
            'max': Series.max,
            'sum': Series.sum,
            'count': Series.__len__,
            'avg': Series.mean,
            'mean': Series.mean,
            'skew': Series.skew,
            'kurt': Series.kurt,
...
class BaseSheet(ThreadSafeContainer):
    ...
    def columns(self, item):
        ...
        for old, new in zip_longest(old_col, self.columns):
            seq = self.data.get(old, Series(NaN=self.nan))
    ...
    def nan(self, item):
        ...
        self._nan = item
        self._isnan = get_isnan(self._nan)
    ...
    def __setstate__(self, read_dict):
        ...
        ThreadSafeContainer.__setstate__(self, read_dict)
    ...
    def __getitem__(self, interval):
        ...
        if isinstance(interval, Series):
            ...
            return self._iloc(type(self)(), [i for i, val in enumerate(interval) if val is True])
        ...
        if isinstance(key, int):
            ...
            if isinstance(value, type(self)):
                if len(value.columns) == 1:
                    ...
                    for subkey, subvalue in value.iter_items():
                        self[key + '_' + subkey] = subvalue
    ...
    def _accumulate(self, func=None, cols=None, skipna=True):
        for col in self._check_columns_index(cols):
            ...
            if self._missing[index] != 0:
                self._missing[index] = seq.count_nan()
        return self
    ...
    def _add_row(self, row):
        ...
        if is_dict(row):
            ...
            for key, value in dic_row.items():
                if key not in self._data:
                    row.append(value)
                    seq = Series(repeat(self.nan, self.shape.Ln), self.nan)
        ...
        if lenth_bias > 0 and self.shape.Ln == 0:
            for i in xrange(lenth_bias):
                self._append_col(Series(NaN=self.nan))
        elif lenth_bias > 0:
            for _ in xrange(lenth_bias):
                series = Series(repeat(self._nan, self.shape.Ln), NaN=self.nan)
    ...
    def _agg(self, func_set, *args, **kwrds):
        '''
        Parameters
        ----------
        func_set : {function: list of columns}

        Returns
        -------
        iterator
        '''
        assert isinstance(func_set, dict)
        for func, keys in func_set.items():
            result = {'Func': str(func)}
            if callable(func) is False:
                assert func in AGG_FUNC, '`funct` must be str or callable object'
                func = AGG_FUNC[func]
            for key in self._check_columns_index(keys):
    ...
    def _check_sequence(self, series, size):
        ...
        '''
        if is_value(series):
            return 0, Series(repeat(series, size), self.nan)

        if is_str(series) or not is_iter(series):
            series = [series]
        series = Series(series, self.nan)
        if len(series) < size:
            series.extend([self._nan] * (size - len(series)))
        return series.count_nan(), series
    ...
    def _check_columns_index(self, col):
        ...
        raise TypeError('unknow type for getting value')
    ...
    def _extend(self, item):
        ...
        for title, sequence in item.iter_items():
            ...
            if title not in self.columns:
                ...
                seq = Series(repeat(self._nan, self._dim.Ln), self.nan)
            else:
    ...
    def _getitem_by_tuple(self, key, subset):
        ...
        err = "DO NOT get subset by columns and rows at the " +\
    ...
    def _group_index_by_column_value(self, columns, engine=list):
        ...
        #for i, row in enumerate(self[columns].iter_rows()):
    ...
    def _get_ranks(self, ranks, cols, duplicate):
        ...
        for col in cols:
            ...
            ranks._quickly_append_col('%s_rank' % col, rank_col, 0)
        return ranks
    ...
    def _init_like_iter(self, series, columns):
        ...
        for i, row in enumerate(series):
            ...
            more_col = [Series(repeat(self.nan, i), self.nan) for _ in xrange(bias)]
    ...
    def _iloc(self, subset, indexs):
        ...
        for miss, (key, sequence) in zip(self._missing, self.iter_items()):
            sequence = Series(sequence) if not isinstance(sequence, Series) else sequence
            seq = sequence[indexs]
            if isinstance(seq, Series) is False:
                seq = Series([seq], self.nan)
            if miss != 0:
                miss = seq.count_nan()
    ...
    def _iter_groupby(self, keys, func=None, apply_col=None):
        def operate_subset(subset, key):
            ...
            ret.insert_col(0, repeat(key, len(ret)), 'GROUPBY')
            return ret[0]
    ...
    def _map(self, func, cols, process=1):
        ...
        assert isinstance(process, (int, bool))
        if process in (1, True) and len(self) >= 10000:
            process = min(len(cols), SYSTEM_CPU)

        def process_func(seq):
            seq = seq.map(func)
            nan = seq.count_nan()
            return seq, nan
        if isinstance(process, int) and process > 1:
            with Pool(processes=process) as pool:
                result = pool.map(process_func, (self.data[name] for name in cols))
        else:
            result = (process_func(self.data[name]) for name in cols)
            
        for name, result in zip(cols, result):
            self.data[name] = result[0]
            ind = self.columns.index(name)
            self._missing[ind] = result[1]
        return self
    ...
    def make_table(cls, shape, fill_with=None, nan=NaN):
        ...
        for col in range(col):
            table.append_col(Series(repeat(fill_with, ln), nan))
        return table      
    ...
    def _normalized(self, process, cols):
        ...
        for title in cols:
            if process in ('NORMAL', 'STANDAR'):
                ...
                if None in statis or \
                   isinstance(statis.Min, str) or \
                   isinstance(statis.Mean, str):
                    continue
    ...
    def _setitem_slice_row(self, start, stop, value):
        ...
        for i, arr in enumerate(self.iter_values()):
            ...
            self._missing[i] -= block.count-nan()
    ...
    def _setitem_cell(self, ln, col, val):
        if col not in self._data:
            self._append_col(Series(NaN=self.nan), col)
