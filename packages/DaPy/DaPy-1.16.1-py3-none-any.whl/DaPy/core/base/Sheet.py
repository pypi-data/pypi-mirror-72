...
from .SafetyContainer import check_thread_locked
from .IndexArray import SortedIndex
from .Row import Row
from .Series import Series
from .utils import (argsort, auto_plus_one, auto_str2value, count_nan, beautiful_str_val,
                    fast_str2value, hash_sort, is_dict, is_empty, is_iter,
                    is_math, is_seq, is_str, is_value, isnan, range, split, string_align,
...
class SeriesSet(BaseSheet):
    ...
    def agg(self, func, cols=None, *args, **kwrds):
        '''Aggregate using one or more operations over specified column(s)
        ...
        cols : str, str in list (default='all')
            str of column names that you wish to process
        ...
        *kwrds
        ...
        >>> table.agg(func=['count', 'mean', 'avg', 'std']).show()
          Func |      A       |      B       |      C       
        -------+--------------+--------------+--------------
         count |           5  |           5  |           5  
          mean | 2.4000000000 | 2.6000000000 | 3.0000000000 
          avg  | 2.4000000000 | 2.6000000000 | 3.0000000000 
          std  | 1.6733200531 | 1.1401754251 | 1.5811388301
          
        >>> table.agg(lambda seq: len(seq) ** 2).show()
                    Func           |     A      |     B      |     C      
        ---------------------------+------------+------------+------------
         <function <lambda> at 0x0 |         25 |         25 |         25 
              0000260E94B1EA0>     |            |            |            

        '''
        if callable(func) or is_str(func):
            agg = self._agg({func: cols}, *args, **kwrds)
        elif is_iter(func):
            agg = self._agg(dict((_, cols) for _ in func), *args, **kwrds)
        else:
            agg = self._agg(func, *args, **kwrds)
        result = SeriesSet(columns=['Func'])
        for row in agg:
    ...
    def append_col(self, series, variable_name=None):
        ...
        >>> sheet
        A: <1, 2, 3>
        >>> sheet.append_col([1, 2, 3, 4, 5], variable_name='B')
        >>> sheet
        A: <1, 2, 3, nan, nan>
        B: <1, 2, 3, 4, 5>
    ...
    def apply(self, func, cols=None, axis=0, n_jobs=1, *args, **kwrds):
        ...
        assert isinstance(n_jobs, int) and n_jobs >= 1, '`n_jobs` must be an integer and greater than 0'
        cols = self._check_columns_index(cols)
        if n_jobs > 1:
            try:
                from joblib import Parallel, delayed
            except ImportError:
                raise ImportError('DaPy requires `joblib` to implement parallel operation')
            worker = Parallel(n_jobs=n_jobs, backend="threading")
            func = delayed(func)
        ...
        if axis == 0:
            try:
                ...
                subset = subset.iter_rows()
            except:
                pass
            if n_jobs > 1:
                return SeriesSet(worker(func(row, *args, **kwrds) for row in subset))
            return SeriesSet([func(row, *args, **kwrds) for row in subset])

        result = SeriesSet(nan=self.nan)
        if n_jobs > 1:
            for val, col in zip(worker(func(val, *args, **kwrds) for val in subset.iter_values()), cols):
                result[col] = val
            return result
        for key in cols:
            value = func(self[key], *args, **kwrds)
            result[key] = value if is_seq(value) else [value,]
        return result
    ...
    def describe(self, level=0):
        ...
        info = SeriesSet(None, ['Name', 'Miss', 'Min', 'Mean', 'Max', 'S', 'Mode', 'Skew', 'Kurt'])
        for sequence, num_nan in zip(self.iter_values(), self._missing):
            if num_nan != 0:
                sequence = sequence.drop(self.nan)
            info.append_row(describe(sequence))
        info['Name'] = self.columns
        info['Miss'] = self.missing
        if level == 0:
            del info['Skew', 'Kurt', 'S', 'Sn', 'CV', 'Range']
        if level == 1:
            del info['Skew', 'Kurt',]

        info.map(beautiful_str_val, info.columns, inplace=True)
        blank_size = [max(4, max(col.apply(count_str_printed_length))) + 2 for col in info.iter_values()]
        # Draw the title line of description
        message = '|'.join(string_align(title, length) for title, length in zip(info.columns, blank_size))
        message += '\n%s\n' % '+'.join(map(lambda x: '-' * x, blank_size))

        # Draw the main table of description
        for row in info:
            message += '|'.join(string_align(v, l) for l, v in zip(blank_size, row)) + '\n'

        lenth = 8 + sum(blank_size)
    ...
    def drop_index(self, columns):
        '''drop_index(cols) -> SeriesSet
        remove index from the current object
        '''
        for col in self._check_columns_index(columns):
            error = '`%s` is not an index' % col
            assert col in self._sorted_index, error
            del self._sorted_index[col]
    ...
    def flatten(self, axis=0):
        ...
        return Series(self._flatten(axis), self.nan)
    ...
    def from_file(cls, addr, **kwrd):
        ...
        dtypes = []
        prefer_dtypes = kwrd.get('dtypes', {})
        if is_seq(prefer_dtypes):
            prefer_dtypes = dict(enumerate(prefer_dtypes))
        assert isinstance(prefer_dtypes, dict), '`dtypes` arguments must be a dict object'
        assert all(callable(_) or _ in fast_str2value for _ in prefer_dtypes.values()), '`dtypes` values should be str in %s or a callable object' % (fast_str2value.keys())
        ...
        def get_val_dtype(_val, _pre_dtypes, len_dtype):
            val = auto_str2value(_val, _pre_dtypes.get(len_dtype))
            type_name = str(val.__class__).split()[1][1:-2].split('.')[0]
            return val, prefer_dtypes.get(len_dtype, fast_str2value[type_name])
            ...
            for i in xrange(first_line):
                ...
                if i == title_line:
                    ...
                    for col_num, key in enumerate(columns):
                        if key in prefer_dtypes:
                            prefer_dtypes[col_num] = prefer_dtypes[key]
            ...
            for i, row in enumerate(file_):
                ...
                for mis, seq, transfer, val in zip_longest(miss, data, dtypes, split(strip(row), sep)):  
                    ...
                    try:
                        ...
                        # different types of data in the same variable
                        seq.append(auto_str2value(val))

                    except TypeError:
                        _ = sorted([(len(col), i) for i, col in enumerate(data)])
                        val, dtype = get_val_dtype(val, prefer_dtypes, _[0][1])
                        seq.append(val)
                        dtypes[_[0][1]] = dtype
                        ...
                        if val in miss_symbol:
                            ...
                            miss_val.append(1)
                            dtypes.append(None)
                        elif len(dtypes) in prefer_dtypes:
                            transfer = prefer_dtypes[len(dtypes)]
                            transfer = transfer if callable(transfer) else fast_str2value[transfer]
                            try:
                                val = transfer(val)
                            except:
                                raise RuntimeError('transfer failed with value `%s` with function `%s` at column `%s`' % (val, transfer, columns[len(dtypes)]))
                            dtypes.append(transfer)
                        else:
                            val, dtype = get_val_dtype(val, prefer_dtypes, len(dtypes))
                            dtypes.append(dtype)

                        if len(data) == 0:
                            data.append(Series([val], nan),)
                            
                        else:
                            missed = len(data[0]) - 1
                            miss_val.append(missed)
                            data.append(Series(chain(repeat(nan, missed), [val]), nan),)
    ...
    def show(self, max_lines=None, max_display=75, max_col_size=25, multi_line=True):
        ...
        for key, arr in str_series.iter_items():
            str_series[key] = Series(arr).apply(beautiful_str_val)
    ...
    @check_thread_locked
    def map(self, func, cols=None, process=1, inplace=False):
        '''apply(func, col=None, *args, **kwrds)
            apply a function to columns or rows
        ...
        assert inplace in (True, False), '`inplace` must be True or False'
        if inplace is False:
            return SeriesSet(self)._map(func, cols, process)
        return self._map(func, cols, process)

    def max(self, cols=None):
        '''return maximum values in each columns'''
        return self.agg('max', cols)[0]

    def mean(self, cols=None):
        '''return average values in each columns'''
        return self.agg('mean', cols)[0]
    ...
    def merge(self, other, how='inner', self_on=0, right_on=0):
        ...
        assert len(self_on) == len(right_on), 'keys of left table must equal to right tbale'
        
        if len(self_on) != 1:
            self['___TEMP1___'] = zip(*[self[_] for _ in self_on])
            self_on = '___TEMP1___'
            other['___TEMP2___'] = zip(*[other[_] for _ in right_on])
            right_on = '___TEMP2___'
            temp_variable = '___TEMP2___'
        else:
            temp_variable = None
        ...
        if how == 'left':
            merge_table = left_join(self, other, self_on, right_on, joined)
        elif how == 'outer':
            merge_table = outer_join(self, other, self_on, right_on, joined)
        elif how == 'inner':
            merge_table = inner_join(self, other, self_on, right_on, joined)
        else:
            merge_table = left_join(other, self, right_on, self_on, SeriesSet(nan=other.nan))
        if temp_variable is not None:
            del self['___TEMP1___'], other['___TEMP2___']
            del merge_table['___TEMP1___', '___TEMP2___']
        return merge_table
        
    def min(self, cols=None):
        '''return minimum values in each columns'''
        return self.agg('in', cols)[0]
    ...
    def std(self, cols=None):
        '''return standard deviation in each columns'''
        return self.agg('std', cols)[0]
    ...
    def var(self, cols=None):
        '''return variances in each columns'''
        return self.agg('var', cols)[0]
