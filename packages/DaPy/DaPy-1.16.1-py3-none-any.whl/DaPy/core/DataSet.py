...
class DataSet(object):
    ...
    def __getstate__(self):
        pkl = {}
        pkl['_data'] = self.__dict__['_data']
        pkl['_sheets'] = self.__dict__['_sheets']
        pkl['_types'] = self.__dict__['_types']
        return pkl
    ...
    def save(self, addr, **kwrds):
        ...
        col_size_str : int (default=30)
            the maximum number of characters in a string column when
            a new table is created

        col_size_int : int (default=10)
            the maximum number of characters in an int column when
            a new table is created

        col_size_float : int (default=10)
            the maximum number of characters in a float column when
            a new table is created
        '''
        fpath, fname, fbase, ftype = parse_addr(addr)
        ...
        if ftype in ('csv', 'txt'):
            ...
            with sql.connect(addr) as conn:
                for data, sheet in zip(self._data, self._sheets):
                    write_db(cur=conn.cursor(), sheet=sheet, data=data,
                             if_exists=kwrds.get('if_exists', 'fail'),
                             mode='sqlite3',
                             int_len=kwrds.get('col_size_int', 10),
                             float_len=kwrds.get('col_size_float', 10),
                             str_len=kwrds.get('col_size_str', 30),)
            ...
            with sql.connect(host=host, port=int(port), user=user, passwd=psd, db=fname[0], charset='utf8') as conn:
                for data, sheet in zip(self._data, self._sheets):
                    write_db(cur=conn, sheet=sheet, data=data,
                             if_exists=kwrds.get('if_exists', 'fail'),
                             mode='mysql',
                             int_len=kwrds.get('col_size_int', 10),
                             float_len=kwrds.get('col_size_float', 10),
