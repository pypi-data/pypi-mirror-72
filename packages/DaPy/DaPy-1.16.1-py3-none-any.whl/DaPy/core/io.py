...
from .base import Matrix, SeriesSet, Series, is_iter, is_math, is_seq, is_value
...
def create_sheet(dtype, data, titles, nan):
    if dtype.upper() in ('COL', 'SERIESSET', 'TABLE'):
...
def write_xls(worksheet, data):
    ...
    try:
        if isinstance(data, (Matrix, SeriesSet)):
...
def write_html(f, data):
    ...
    f.write('<tbody>')
    if isinstance(data, (Matrix, SeriesSet)) or \
...
def write_db(cur, sheet, data, if_exists, mode, int_len, float_len, str_len):
