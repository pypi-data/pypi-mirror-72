...
def rows_to_dataframe(rows, allow_git_different_rows=False,
...
def extract_gridsearch_parameters(
    df, score_name='mean_test_score', drop_outcome=True,
    ...
    args:
        ...
        score_name: name of the field that the score should be extracted from.
            default value is 'mean_test_score', which is the mean score of all
            cross validation runs for a parameter combination.
    ...
    for _, row in df.iterrows():
        ...
        for combination_id, combination in cv['params'].items():
            ...
            result_row['score'] = cv[score_name][combination_id]
            result_row.update(
