...
from pyqalx.factories.utils import _msg_user
...
class BotSource:
    ...
    def __init__(self, bot_name, bot, build_path_dir, *args, **kwargs):
    ...
    def _msg_user(self, msg, **kwargs):
        """
        Helper method that prints and also logs a debug message if a logger
        instance attribute exists
        """
        _msg_user(title=self.bot_name, fore=self.FORE, msg=msg, **kwargs)
    ...
    def _install_bot(self):
        ...
        try:
            subprocess.check_call(
                [shutil.which("pip"), "install", "."], cwd=self.build_path
            )
            return
        ...
        try:
            ...
            msg = (
                f"Dependencies could not be installed. "
                f"Check that a `setup.py` or a `pyproject.toml` file exists "
                f"or provide a valid path to a requirements.txt file in the "
                f"bot definition on the factory plan.  Error was `{exc}`"
            )
...
class PyPIBotSource(BotSource):
    ...
    def install(source_dict):
        ...
        if version is not None:
            pypi += f"=={version}"
        try:
            subprocess.check_call([shutil.which("pip"), "install", pypi])
        except subprocess.CalledProcessError as exc:
            msg = f"Failed to install {pypi}.  Error was `{exc}`"
            raise QalxFactoryBuildError(msg)
