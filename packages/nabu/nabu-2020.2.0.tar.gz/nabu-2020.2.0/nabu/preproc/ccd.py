...
#     and the actual radio index in scan.
# (2) radios: dict of DataUrl, flats/darks: dict of DataUrl
#     In this case we have the information on "scan index", but we have to read
#     the radios.
# (3) radios: array, flats/darks: array
#     same problem as in (1), we have to keep the mapping "array range" <-> "scan_range"
#
# For now only the first case is supported, but it would be nice to have (2) and (3) in the future.
# Besides, the current implementation assumes than the mapping "array range" <-> "scan_range"
# is the identity, i.e array_idx == scan_idx.
#
class FlatField(CCDProcessing):
    ...
    def normalize_radios(self, radios):
        ...
        dark = np.ascontiguousarray(
            self.darks_arr[0], dtype=np.float32
        )
...
class CCDCorrection(CCDProcessing):
    ...
    def median_clip_correction(self, radios, inplace=True):
        """
        ...
        radios: numpy.ndarray, optional
            Radios stack. If provided, it must have the same shape as the one
            provided when instanciating the class.
        inplace: bool, optional
            Whether to compute the correction in-place, i.e replace the radios
            array with the corrected images. Default is True to spare memory.
        """
        assert radios.ndim == 3
        assert radios.shape[1:] == self.shape
        ...
        for i in range(n_radios):
            ...
            output[i][invalid_mask] = medians[invalid_mask]
        return output
