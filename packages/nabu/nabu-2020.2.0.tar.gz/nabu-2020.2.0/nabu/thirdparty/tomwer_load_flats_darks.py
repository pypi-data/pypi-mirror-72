...
def is_process_node(node):
    return (node.name.split('/')[-1].startswith('tomwer_process_') and
            ...
            'program' in node and
            node['program'] [()] == 'tomwer_dark_refs' and
            'version' in node and
...
def main(argv):
    ...
    parser.add_argument('tomwer_process_file_path',
                        help='path to the tomwer file containing the '
                             '`dark ref` process to read')
    parser.add_argument('--entry',
                        help='Specific entry to get',
                        default=None)

    options = parser.parse_args(argv[1:])
    with h5py.File(options.tomwer_process_file_path, 'r', swmr=True) as h5f:
        entries = get_process_entries(root_node=h5f, depth=MAX_DEPTH)
        print('entries found are', entries)

    flats = get_flats_frm_process_file(options.tomwer_process_file_path,
                                       entry=options.entry)
    flats_indexes = [str(index) for index in flats]
    print('   - %s flats found, with indexes %s' % (len(flats), flats_indexes))

    darks = get_darks_frm_process_file(options.tomwer_process_file_path,
                                       entry=options.entry)
    darks_indexes = [str(index) for index in darks]
    print('   - %s darks found, with indexes %s' % (len(darks), darks_indexes))
