...
def estimate_required_memory(process_config, chunk_size=None, radios_and_slices=True):
    ...
    if "ccd_filter" in processing_steps:
...
def estimate_chunk_size(available_memory_GB, process_config, chunk_step=50):
    ...
    while (estimate_required_memory(process_config, chunk_size=chunk_size)/1e9 < available_memory_GB) and chunk_size < process_config.dataset_infos.radio_dims[1]:
        chunk_size += chunk_step
    return chunk_size
