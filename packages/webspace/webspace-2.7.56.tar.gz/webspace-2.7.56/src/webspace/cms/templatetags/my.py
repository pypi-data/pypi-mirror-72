...
def cached_form(form, request):
    if form:
        if 'current_form_id' in request.session and request.session['current_form_id'] and form.id == request.session['current_form_id']:
...
register.filter('cached_form', cached_form)
