...
class NBApi(FastAPI):
    def __init__(self, title: str, cors_origins: list = []):
        ...
        async def http_middleware(request: Request, call_next):
            ts = time.time()
            request.state.set(KEY_ID, str(uuid.uuid4()))
            request.state.set(KEY_CID, 'unknown')
            request.state.set(KEY_AUTH, request.headers['authorization'])
            resp = await call_next(request)
            c.labels(path=request.url.path, status_code=resp.status_code, cid=request.state.get(KEY_CID)).inc()
            g.labels(path=request.url.path, status_code=resp.status_code, cid=request.state.get(KEY_CID)).set(time.time() - ts)
            return resp
