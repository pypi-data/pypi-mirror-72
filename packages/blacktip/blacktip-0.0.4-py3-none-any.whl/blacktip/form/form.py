...
class Form:
    ...
    def liability_sheet(self):
        ...
        """
        return self.get("Liability|Liabilities")
...
class Form10K(Form):
    ...
    def calc_ROE(self):
        ...
        """
        net_income =  self.get("^NetIncomeLoss$")
        stockholders_equity = self.get("^StockholdersEquity$")
        years = net_income.columns.to_list()[2:]

        for year in years:
            net_income[year] = net_income[year][0] / stockholders_equity[year][0]
            net_income["tag"] = "ROE"
            net_income["uom"] = "ratio"
    ...
    def calc_CurrentRatio(self):
        ...
        """
        current_assets = self.get("^AssetsCurrent$")
        current_liabilities = self.get("^LiabilitiesCurrent$")
        years = current_assets.columns.to_list()[2:]

        for year in years:
            current_assets[year] = current_assets[year][0] / current_liabilities[year][0]
            current_assets["tag"] = "CurrentRatio"
            current_assets["uom"] = "ratio"
    ...
    def calc_DebtToEquity(self):
        ...
        """
        total_liabilities = self.get("^Liabilities$")
        stockholders_equity = self.get("^StockholdersEquity$")
        years = total_liabilities.columns.to_list()[2:]

        for year in years:
            total_liabilities[year] = total_liabilities[year][0] / stockholders_equity[year][0]
            total_liabilities["tag"] = "DebtToEquity"
            total_liabilities["uom"] = "ratio"
    ...
    def calc_BookValue(self):
        ...
        years = assets.columns.to_list()[2:]

        for year in years:
            assets[year] = assets[year][0] - liabilities[year][0]
            assets["tag"] = "BookValue"
