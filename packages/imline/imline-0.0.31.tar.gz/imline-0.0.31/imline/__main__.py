...
def start_sub(args):
    ...
    elif args["sub_jobs"] == "db-to-csv":

        from imline.mapping import Mapper
        imline.mapper = Mapper(args)

        imline.mapper.database_to_csv(output=args["output"])
...
def initiate():
    ...
    ap_object.add_argument("--method", help="Multiplying method for the images. Either `rotate`, `resize`, `shift` or `zoom`", type=str, default="shift", choices=["rotate", "resize", "shift", "zoom"])
    ...
    ap_db_csv = sub_p.add_parser("db-to-csv", help="Convert the database info to csv file.")
    ap_db_csv.add_argument("--output", help="Data folder path that will be keep databases converted csv file.", type=str)
