...
os.system("cp ../data/aeqdskd3d aeqdsk")	# [Cannot tab or indent these 3 lines]
os.system("cp ../data/neqdskd3d neqdsk")
...
com.nxleg[0,0] = 2		#pol. mesh pts from inner plate to flx.x-point
com.nxcore[0,0] = 2		#pol. mesh pts from flx.x-point to top on inside
com.nxcore[0,1] = 2		#pol. mesh pts from top to flx.x-point on outside
com.nxleg[0,1] = 2		#pol. mesh pts from flx.x-point to outer plate
com.nysol[0] = 3		#rad. mesh pts in SOL
com.nycore[0] =1		#rad. mesh pts in core
...
from hdf5 import *
hdf5_restore('../data/d3d_cfdupg.h5')

bbb.exmain()
#if bbb.iterm == 1:    #successfully converged run
#   create rdd3d_cfdupg.8x4.pdb # Save important stuff in pdb file.
#   write bbb.nis,bbb.ups,bbb.tes,bbb.tis,bbb.ngs,bbb.tgs,bbb.phis
#   close
lngs = bbb.ngs.copy()
hdf5_save('rdd3d_cfdupg_8x4.h5')
