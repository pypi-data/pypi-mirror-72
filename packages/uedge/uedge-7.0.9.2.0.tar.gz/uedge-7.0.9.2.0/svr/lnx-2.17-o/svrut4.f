...
c  end of atob
c-----------------------------------------------------------------------
      subroutine ilut (n,a,ja,ia,lfil,tol,alu,jlu,ju,iwk, wu,wl,jr,jwl,
     &   jwu,ierr)
cProlog
c-----------------------------------------------------------------------
      implicit none
      ...
      doubleprecision a(*), alu(*), wu(n+1), wl(n), tol
      integer ja(*),ia(n+1),jlu(*),ju(n),jr(n), jwu(n), jwl(n), lfil, 
     &   iwk, ierr
c----------------------------------------------------------------------*
...
c
         if (wu(1) .eq. 0.0d0) wu(1) = (0.0001d0 + tol)*tnorm
c
         alu(ii) = 1.0d0 / wu(1)
c
...
cProlog
      ...
      integer ja(*),jb(*),jc(*),ia(nrow+1),ib(nrow+1),ic(nrow+1), iw(
     &   ncol)
...
c-----------------------------------------------------------------------
      ...
      subroutine aplsb (nrow,ncol,a,ja,ia,s,b,jb,ib, c,jc,ic,nzmax,iw,
     &   ierr)
cProlog
      implicit none
      integer nrow, ncol, nzmax, ierr, len, j, ii, ka, jcol, kb
      integer jpos, k
      doubleprecision a(*),b(*),c(*),s
      integer ja(*),jb(*),jc(*),ia(nrow+1),ib(nrow+1), ic(nrow+1),iw(
     &   ncol)
c-----------------------------------------------------------------------
...
c
      do 40 i = 1, n
         ...
         do 41 k=jlu(i),ju(i)-1
            if (abs(alu(k)/1d50)*abs(x(jlu(k))/1d50) .gt. 1d200) then
...
c-----------------------------------------------------------------------
      ...
      subroutine roscal (nrow, job, nrm, a, ja, ia, diag, b, jb, ib)
cProlog
      implicit none
      integer nrow, job, nrm, j
      doubleprecision a(*), b(*), diag(nrow)
...
cProlog
      ...
      doubleprecision a(*), diag(nrow), scal
...
cProlog
      ...
      doubleprecision a(*), b(*), diag(nrow), scal
      integer ja(*),jb(*), ia(nrow+1),ib(nrow+1)
c-----------------------------------------------------------------------
...
cProlog
      implicit none
      integer n, job, ipos, i, k, j, nnext
      integer ia(n+1),iao(n+1),ja(*),jao(*)
      doubleprecision a(*),ao(*)
c-----------------------------------------------------------------------
...
cProlog
      ...
      integer k, j, i, ko, l
      doubleprecision diag(ndiag,idiag), a(*), ao(*)
...
c-----------------------------------------------------------------------
      implicit none
      integer ifmt, job, iounit, ihead, i, nnext, iend
      character title*72,key*8,type*3,ptrfmt*16,indfmt*16,valfmt*20, 
     &   guesol*2, rhstyp*3,ix*80
      integer totcrd, ptrcrd, indcrd, valcrd, rhscrd, nrow, ncol, nnz, 
     &   nrhs, len, nperli
      integer ja(*), ia(*)
      doubleprecision a(*),rhs(*)
c--------------
c     compute pointer format
c--------------
      nnz = ia(ncol+1) -1
      len = int( log10(0.1d0+(nnz+1))) + 2
      nperli = 80/len
...
c     write 4-line or five line header
      write(iounit,10) title,key,totcrd,ptrcrd,indcrd,valcrd, rhscrd,
     &   type,nrow,ncol,nnz,nrhs,ptrfmt,indfmt,valfmt,valfmt
c-----------------------------------------------------------------------
...
c
      do 23002 j = 1, ndom+1
         iwk(j) = ndp1+mapptr(j)
23002 continue
      do 23004 j=1, mapptr(ndom+1)-1
         iwk(ndp1+j) = map(j)
23004 continue
c-----------------------------------------------------------------------
      do 23006 idom=1, ndom
         do 23008 k=mapptr(idom),mapptr(idom+1)-1
            mask(map(k)) = idom
23008    continue
23006 continue
      nextdom = 1
...
c-----------------------------------------------------------------------
      do 23010 idom =1, ndom
         ...
         init = iwk(j)
         nextdom = mapptr(jdom)
         call perphn(numnod,ja,ia,init,iwk(j),mask,maskval, nlev,riord,
     &      levels)
         call stripes (nlev,riord,levels,ip2,map(nextdom), mapptr(jdom),
     &      kdom)
         mapptr(jdom) = nextdom
         do 23012 j = jdom,jdom+kdom-1
...
cProlog
      implicit none
      integer n,ja(*),ia(*),init,iperm(*),mask(*),maskval, nlev,riord(*)
     &   ,levels(*)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
    ...
    1 continue
      ...
      if (nlev .gt. nlevp) then
         ...
         do 23000 j=levels(nlev),levels(nlev+1)-1
            nod = riord(j)
...
cProlog
      integer nrow,ja(*),ia(nrow+1),jao(*),iao(nrow+1),perm(nrow), qperm
     &   (*),job
      doubleprecision a(*),ao(*)
c-----------------------------------------------------------------------
