...
c!include "../sptodp.h"
      ...
      subroutine nksol (n, u, savf, f, jac, su, sf, ftol, stptol, rwork, 
     &   lrw, iwork, liw, iopt, iterm, pset, psol, mf, mdif, ipflag, 
     &   icflag, icnstr, rlx, epscon1, epscon2, icntnu, adjf1)
cProlog
c-----------------------------------------------------------------------
...
c- icntnu change
      ...
      doubleprecision savf,u,rwork,su,sf
      doubleprecision stptol,epsmch,fnrm,f1nrm,f1nrmp,unrm
      doubleprecision ftol,stepmx,r1mach9,vnormnk,epsfac,sunrm,dnrm2_u,
     &   tau
      dimension savf(n),u(*),rwork(lrw),su(n),sf(n)
      dimension iwork(liw)
      doubleprecision rlx, epscon1, epscon2, adjf1
      integer icflag, icnstr, ivar
      dimension icnstr(n)
c
      doubleprecision zero,one,two,three
      logical mxtkn
...
c-----------------------------------------------------------------------
      doubleprecision eps, sqteta, rhom
      common /nks001/ eps, rhom, sqteta, locwmp, locimp, iersl, kmp, 
     &   mmax, methn, methk, ipflg, mfdif, nfe, nje, nni, nli, npe, nps, 
     &   ncfl, nbcf
c-----------------------------------------------------------------------
...
c first check for illegal values.
   ...
   10       continue
      ...
      elseif (methk .eq. 2) then
         lenk = 1 + 3*n + 3 + n*mmax + n + 2 + 2*mmax*(mmax+1) + mmax
ccc MVU: 15-jan-2020
      elseif (methk .eq. 3) then
         lenk = 1 + 3*n + 3 + n*mmax + n + 2 + 2*mmax*(mmax+1) + mmax
ccc MVU: 15-jan-2020
      endif
      lenwm = lenk + lenwmp
...
c insufficient storage in iwork
      ...
      if (stepmx .eq. zero) then
         unrm = vnormnk(n,u,su)
         sunrm = dnrm2_u(n,su,1)
         stepmx = 1.d3*max(unrm,sunrm)
      endif
...
c-----------------------------------------------------------------------
      if (methn .eq. 0) then
         call inexct(n,u,savf,f1nrm,rwork(lx),su,sf,stepmx,stptol, iret,
     &      rwork(lup),f1nrmp,mxtkn,f,icflag, icnstr, rlx)
      elseif (methn .eq. 1) then
         call dogdrv (n,rwork(lwm),lenwm,iwork(liwm),leniwm,u,savf, 
     &      f1nrm,rwork(lx),su,sf,stepmx,stptol,tau,iret, rwork(luprv),
     &      rwork(lfprv),rwork(lup),f1nrmp, mxtkn,f,jac,psol,icflag,
     &      icnstr,rlx)
         if (iersl .ne. 0) then
            iterm = iersl
            go to 500
         ...
         call lnsrch(n,u,savf,f1nrm,rwork(lx),su,sf,stepmx,stptol, iret,
     &      rwork(lup),f1nrmp,mxtkn,f,jac,icflag,icnstr, rlx,adjf1)
...
cpetsc      endif
      call nkstop(n,u,rwork(lup),savf,fnrm,su,sf,stptol,rwork(lx), ftol,
     &   iret,iter,itermx,mxtkn,ncscmx,iterm)
c
      do 300 i = 1,n
         u(i) = rwork(i+lup-1)
  300    continue
...
c-----------------------------------------------------------------------
      ...
      integer incpset, i
      doubleprecision u,unew,savf,fnorm,su,sf,stptol,ftol
      doubleprecision fmax,rlngth,wk,thsnd,two
...
c-----------------------------------------------------------------------
      doubleprecision u, s, su, one, rlngth, temp, zero
      integer i, n
...
c-----------------------------------------------------------------------
      implicit none
      integer n, i
      doubleprecision savf,sf,zero,temp,fnorm,fmax
...
c-----------------------------------------------------------------------
      implicit none
      integer n, iterm
      doubleprecision savf,sf,ftol,fmax,p01,fnorm
      dimension savf(n),sf(n)
      save
      data p01/1.d-2/
c
...
c----------------------- end of subroutine nkstp0 ----------------------
      ...
      doubleprecision function vnormnk (n, v, s)
cProlog
c-----------------------------------------------------------------------
...
c----------------------- end of function vnormnk -------------------------
      ...
      subroutine model(n, wm, lenwm, iwm, leniwm, u, savf, x, f, jac, su
     &   , sf, pset, psol)
cProlog
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      integer iwm
      integer i
      doubleprecision wm, x, su, sf, savf, u, onept5
...
c----------------------- end of subroutine model -----------------------
      ...
      subroutine solpk (n, wm, lenwm, iwm, leniwm, u, savf, x, su, sf, f
     &   , jac, psol)
cProlog
      implicit none
      ...
      external f, jac, psol
      integer n, iwm
      doubleprecision wm, x, su, sf, savf, u
      dimension u(*), savf(n), su(n), sf(n), wm(lenwm), iwm(leniwm), x(n
     &   )
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
  100 continue
      ...
      do 110 i = 1,n
  110    wm(i+ib-1) = x(i)
      call spiom (n, u, savf, wm(ib), su, sf, mmax, kmp, eps, f, jac, 
     &   psol, npsl, x, wm(iv), wm(ihes), iwm, miom, wm(locwmp), iwm(
     &   locimp), wm(iwk), ipflg, iflag, rhom)
      nni = nni + 1
      nli = nli + miom
      nps = nps + npsl
...
c-----------------------------------------------------------------------
  200 continue
      ...
      do 210 i = 1,n
  210    wm(i+ib-1) = x(i)
      call spigmr (n, u, savf, wm(ib), su, sf, mmax, mmaxp1, kmp, eps, f
     &   , jac, psol, npsl, x, wm(iv), wm(ihes), wm(iq), wm(ihsv), mgmr, 
     &   wm(locwmp), iwm(locimp), wm(iwk), methn, bnrm, ipflg, iflag, 
     &   rhom)
      nni = nni + 1
      nli = nli + mgmr
      nps = nps + npsl
      ...
      if (iflag .lt. 0) iersl = -1
      if (iersl .eq. 0) then
         call dcopy_u(mmax, wm(ib), 1, wm(ihes), 1)
         wm(1) = bnrm
         iwm(1) = mgmr
      endif
      return
...
ccc - this is direct solver
      do 23003 i=1,n
         x(i)=x(i)*sf(i)/su(i)
23003 continue
      call psol(n, u, savf, su, sf, f, jac, wm(iwk), wm(locwmp), iwm(
     &   locimp), x, iflag)
      npsl = 1
      bnrm = 0.d0
      mgmr = 1
...
c----------------------- end of subroutine solpk -----------------------
      ...
      subroutine spiom (n, u, savf, b, su, sf, mmax, iomp, eps, f, jac, 
     &   psol, npsl, x, v, hes, ipvt, miom, wmp, iwmp, wk, ipflg, iflag, 
     &   rho)
cProlog
      implicit none
      ...
      doubleprecision u, savf, b, su, sf, x, eps, v, hes, wmp, wk
...
c-----------------------------------------------------------------------
      ...
      doubleprecision bnrm, prod, rho, snormw, dnrm2_u, tem
c-----------------------------------------------------------------------
c     nks002 common block.
c-----------------------------------------------------------------------
      common /nks002/ iprint, iunit, iermsg
...
c-----------------------------------------------------------------------
         call atv (n, u, savf, v(1,m), su, sf, x, f, jac, psol, v(1,m+1)
     &      , wk, wmp, iwmp, ier, npsl)
...
c----------------------- end of subroutine spiom -----------------------
      ...
      subroutine atv (n, u, savf, v, su, sf, ftem, f, jac, psol, z, vtem
     &   , wmp, iwmp, ier, npsl)
cProlog
      implicit none
      ...
      external f, jac, psol
      integer n
      doubleprecision u, savf, v, su, sf, ftem, vtem, z, wmp
      dimension u(*), savf(n), v(n), su(n), sf(n), ftem(n), vtem(n), z(n
     &   )
      dimension iwmp(*), wmp(*)
c-----------------------------------------------------------------------
...
c     nks001 common block.
c-----------------------------------------------------------------------
      doubleprecision delt, sqteta, rhom
      common /nks001/ delt, rhom, sqteta, locwmp, locimp, iersl, kmp, 
     &   mmax, methn, methk, ipflg, mfdif, nfe, nje, nni, nli, npe, nps, 
     &   ncfl, nbcf
c
      doubleprecision sigma,tmp,utv,vtv,suitv,ddot_u,zero
      save
      data zero/0.0d0/
c
...
c ... Add a 1/(pseudo time-step) carried in u(n+2): TDR 8/21/95
      do 118 i = 1,n
c sf in numerator or dem??
         z(i) = z(i) + vtem(i)*sf(i)*u(n+2)
  118    continue
...
c ipflg = 1. apply inverse of right preconditioner
         ier = 0
         call psol (n, u, savf, su, sf, f, jac, ftem, wmp, iwmp, vtem, 
     &      ier)
         npsl = npsl + 1
         if (ier .ne. 0) return
         vtv = zero
...
c
      doubleprecision t
      integer idamax_u, j, k, km1, kp1, l, nm1
      save
c
...
c        find l = pivot index
c
      l = idamax_u(2,a(nm1,nm1),1) + nm1 - 1
      ipvt(nm1) = l
c
c        zero pivot implies this column already triangularized
c
...
c----------------------- end of subroutine shesl -----------------------
      ...
      subroutine spigmr (n, u, savf, b, su, sf, mmax, mmaxp1, igmp, eps, 
     &   f, jac, psol, npsl, x, v, hes, q, hessv, mgmr, wmp, iwmp, wk, 
     &   methn, bnrm, ipflg, iflag, rho)
cProlog
      implicit none
...
c note: the x array is used as work space in routines psol and atv.
c-----------------------------------------------------------------------
      doubleprecision v, hes, hessv, su, sf, q, b, savf, x, u, wk
      doubleprecision wmp, dnrm2_u
      ...
      doubleprecision bnrm,eps,prod,rho,snormw,tem
...
c----------------------- end of subroutine spigmr ----------------------
     ...
     &   su, sf, stepmx, stptol, tau, iret, uprev, fprev, unew, f1new, 
...
c-----------------------------------------------------------------------
      ...
      external f, jac, psol
      doubleprecision u, uprev, fprev, unew, f1nrm, f1prv, f1new, x, su, 
     &   sf
      doubleprecision beta, tau, stepmx, stptol, cpl, gml, wm, xl, savf
      dimension wm(lenwm), iwm(leniwm), x(n)
...
c-----------------------------------------------------------------------
     ...
     &   tau, wm(iynew), stepmx, dog1, nwttkn, cpl, gml, n, wm(iv), x, 
     &   xl, wm(iwk), wm(locwmp), iwm(locimp), u, su, sf, savf, f, jac, 
     &   psol)
      if (iersl .lt. 0) then
c nonrecoverable error from psol.  set iersl and return.
         iersl = 9
...
c-----------------------------------------------------------------------
      call trgupd (m, mp1, mmaxp1, n, np1, u, savf, f1nrm, x, xl, wm(
     &   iynew), su, sf, nwttkn, stepmx, beta, wm(ihsv), stptol, mxtkn, 
     &   tau, uprev, fprev, f1prv, unew, f1new, wm(iwk), ivio, iret, f)
c-----------------------------------------------------------------------
...
c----------------------- end of subroutine dogdrv ----------------------
     ...
     &   stepmx, dog1, nwttkn, cpl, gml, n, v, xnew, xnewl, wk, wmp, 
     &   iwmp, u, su, sf, savf, f, jac, psol)
cProlog
      implicit none
      ...
      external f, jac, psol
      doubleprecision ygm, ycp, beta, hes, tau, ynew, xnew, xnewl, v
      doubleprecision wk, wmp, u, su, sf, savf
      dimension ygm(m), ycp(m), hes(mmaxp1,m), ynew(mp1), xnew(n), v(n,m
     &   ), wk(n), wmp(*), iwmp(*), u(*), su(n), sf(n), savf(n)
      logical dog1, nwttkn
c-----------------------------------------------------------------------
...
c
c-----------------------------------------------------------------------
      doubleprecision a, b, c, cpl, gml, s, t, tem1, tem2, stepmx
...
c calculate cauchy point.
            ...
            do 20 i = 1,m
               ycp(i) = hes(1,i)
   20          continue
            tem1 = ddot_u(m, ycp, 1, ycp, 1)
            do 25 i = 1,mp1
               ynew(i) = 0.0d0
   25          continue
            do 30 i = 1,m
               call daxpy_u(mp1, ycp(i), hes(1,i), 1, ynew, 1)
   30          continue
            tem2 = ddot_u(mp1, ynew, 1, ynew, 1)
            t = beta*(tem1/tem2)
            call dscal_u(m, t, ycp, 1)
            cpl = dnrm2_u(m, ycp, 1)
c if this is the first iteration and no initial trust region was
c provided by the user, then set tau = min(cpl,stepmx).
            if (tau .eq. -1.0d0) tau = min(cpl, stepmx)
         endif
         if (tau .le. cpl) then
            t = tau/cpl
            do 40 i = 1,m
...
c             return
            ...
            do 50 i = 1,m
               ynew(i) = ycp(i) - ygm(i)
   50          continue
            a = ddot_u(m, ynew, 1, ynew, 1)
            b = ddot_u(m, ygm, 1, ynew, 1)
            c = gml*gml - tau*tau
            s = ( -b - sqrt(max(b*b-a*c,0.d0)) )/a
            do 60 i = 1,m
...
c-----------------------------------------------------------------------
      xnewl = dnrm2_u(m, ynew, 1)
      do 90 i = 1,n
         xnew(i) = 0.d0
   90    continue
      do 100 i = 1,m
         call daxpy_u(n, ynew(i), v(1,i), 1, xnew, 1)
  100    continue
      do 110 i = 1,n
...
c----------------------- end of subroutine dogstp ----------------------
     ...
     &   ynew, su, sf, nwttkn, stepmx, beta, hes, stptol, mxtkn, tau, 
     &   uprev, fprev, f1prv, upls, f1pls, wk, ivio, iret, f)
cProlog
      implicit none
      ...
      external f
      doubleprecision u, f1nrm, x, xl, ynew, su, sf, stepmx, beta, hes, 
     &   stptol, tau, uprev, fprev, f1prv, upls, f1pls, wk, savf
      dimension u(*), x(n), ynew(m), savf(n), su(n), sf(n), hes(mmaxp1,m
     &   ), uprev(n), fprev(n), upls(n), wk(np1)
c-----------------------------------------------------------------------
...
c
c-----------------------------------------------------------------------
      doubleprecision alpha, pt1, pt5, pt75, pt99, zero, two
      doubleprecision ddot_u, delf, dfpred, tautmp, fnrmp, rlngth, slpi
      doubleprecision vnormnk
      logical mxtkn, nwttkn
      save
      data alpha/1.d-4/, pt1/0.1d0/, pt5/0.5d0/, pt75/0.75d0/, pt99/
     &   0.99d0/, zero/0.0d0/, two/2.0d0/
c-----------------------------------------------------------------------
c     nks001 common block.
c-----------------------------------------------------------------------
      doubleprecision delt, sqteta, rhom
...
c-----------------------------------------------------------------------
      call dcopy_u(n, u, 1, upls, 1)
      do 20 i = 1,n
         u(i) = upls(i) + x(i)
   20    continue
      call f (n, u, savf)
      nfe = nfe + 1
      call dswap_u(n, u, 1, upls, 1)
      fnrmp = vnormnk(n, savf, sf)
      f1pls = pt5*fnrmp*fnrmp
      delf = f1pls - f1nrm
      slpi = -beta*ddot_u(m, hes, mmaxp1, ynew, 1)
      if (iret .ne. 3) f1prv = zero
      if ( (iret.eq.3) .and. ((f1pls.ge.f1prv) .or. (delf.gt.alpha*slpi)
     &   ) ) then
c reset upls to uprev and terminate global step.
         iret = 0
         call dcopy_u(n, uprev, 1, upls, 1)
         call dcopy_u(n, fprev, 1, savf, 1)
         f1pls = f1prv
         return
...
c f1(upls) sufficiently small.
   ...
   45       continue
         do 50 i = 1,m
            call daxpy_u(mp1, ynew(i), hes(1,i), 1, wk, 1)
   50       continue
         dfpred = slpi + pt5*ddot_u(mp1, wk, 1, wk, 1)
         if ( (iret.ne.2) .and. ( (abs(dfpred-delf).le.pt1*abs(delf)) 
     &      .or. (delf.le.slpi) ) .and. (.not.nwttkn) .and. (tau.le.pt99
     &      *stepmx) .and. (ivio.eq.0) ) then
c double tau and continue global step.
            iret = 3
            call dcopy_u(n, upls, 1, uprev, 1)
            call dcopy_u(n, savf, 1, fprev, 1)
            f1prv = f1pls
...
c----------------------- end of subroutine trgupd ----------------------
     ...
     &   iret, unew, f1nrmp, mxtkn, f, jac, icflag, icnstr, rlx, adjf1)
cProlog
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      external f, jac
      doubleprecision savf,u,unew,f1nrm,f1nrmp,p,su,sf
      doubleprecision stptol,stepmx,pnrm,ratio,ratio1,slpi,rlngth,rl
      doubleprecision fnrmp,rltmp,vnormnk
      doubleprecision beta,f1lo,f1nprv,rldiff,rlincr,rlmin,rllo, rlmax,
     &   rlprev
      doubleprecision rlx, adjf1
      integer ivar
      dimension savf(n),u(*),unew(n),p(n),su(n),sf(n),icnstr(n)
      doubleprecision pt1,pt1trl,pt99,one,two,alpha
      logical mxtkn
c-----------------------------------------------------------------------
c     nks001 common block.
c-----------------------------------------------------------------------
      doubleprecision delt, sqteta, rhom, tau
      common /nks001/ delt, rhom, sqteta, locwmp, locimp, iersl, kmp, 
     &   mmax, methn, methk, ipflg, mfdif, nfe, nje, nni, nli, npe, nps, 
     &   ncfl, nbcf
c-----------------------------------------------------------------------
...
c
      save
      data pt1/0.1d0/,alpha/1.d-4/,one/1.0d0/,two/2.0d0/
      data pt99/0.99d0/,beta/0.9d0/
c
      mxtkn = .false.
      pnrm = vnormnk(n,p,su)
      ratio = one
   ...
   16       format( ' ------ in routine lnsrch (ivio=1,pnrm=',e12.4, 
     &         ', var index = ',i6,') -- ')
            go to 11
         endif
      endif
...
c-----------------------------------------------------------------------
  115 continue
      call dcopy_u(n, u, 1, unew, 1)
      do 120 i = 1,n
         u(i) = unew(i) + rl*p(i)
  120    continue
      call f(n, u, savf)
      nfe = nfe + 1
      call dswap_u(n, u, 1, unew, 1)
      fnrmp = vnormnk(n,savf,sf)
...
c alpha-condition satisfied.  now check for beta-condition.
  ...
  140          u(i) = unew(i) + rl*p(i)
            ...
            f1nrmp = fnrmp*fnrmp/two
            if (iprint .gt. 1) write(iunit,125) rl,f1nrm,f1nrmp
            if ( (f1nrmp/adjf1 .le. f1nrm + alpha*slpi*rl) .and. (f1nrmp
     &         /adjf1 .lt. f1nrm + beta*slpi*rl) .and. (rl .lt. rlmax) ) 
     &            go to 130
         endif
         if ( (rl.lt.one) .or. ((rl.gt.one).and.(f1nrmp/adjf1.gt.f1nrm+
     &      alpha*slpi*rl)) ) then
            rllo = min(rl,rlprev)
  ...
  160          u(i) = unew(i) + rl*p(i)
     ...
     &         f1nrmp/adjf1 .lt. f1nrm + beta*slpi*rl) ) .and. (rldiff 
     &         .gt. rlmin) ) go to 150
            if (f1nrmp/adjf1 .lt. f1nrm + beta*slpi*rl) then
...
c     formats.
c-----------------------------------------------------------------------
 9000 format(// ' nksol ---  illegal value for mf.  mf must be between '
 ...
 9100 format(// ' nksol ---  insufficient length for rwork.'/
     &   '            rwork length given  - ',i8,/
     &   '            rwork length needed - ',i8)
 9110 format(// ' nksol ---  insufficient length for iwork.'/
...
c-----------------------------------------------------------------------
      ...
      integer n
      doubleprecision u, x
      doubleprecision tau, fac, zero
      doubleprecision rlx, mxchng, fac2, cutlo
      integer icnstr, i, iret, ivar
      dimension u(*), x(n), icnstr(n)
      data fac /0.6d0/, zero/0.d0/, fac2 /0.9d0/, cutlo /1.0d-200/
      save
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
 ...
 9030 format(// ' nksol ---  iterm = 4.'/
     &   '            the maximum allowable number of nonlinear'/
     &   '            iterations has been reached.')
 9040 format(// ' nksol ---  iterm = 5.'/
...
c-------------  end of subroutine infgen  ------------------------------
     ...
     &   iret, unew, f1nrmp, mxtkn, f, icflag, icnstr, rlx)
cProlog
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      external f
      doubleprecision savf,u,unew,f1nrm,f1nrmp,p,su,sf
      doubleprecision stepmx,stptol,pnrm,ratio,ratio1
      doubleprecision fnrmp,vnormnk
      doubleprecision rlx
      dimension savf(n),u(*),unew(n),p(n),su(n),sf(n),icnstr(n)
      doubleprecision pt99,one,two
      logical mxtkn
c-----------------------------------------------------------------------
c     nks001 common block.
c-----------------------------------------------------------------------
...
c
   ...
   15          continue
            pnrm = tau
            if (iprint .gt. 1) write(iunit,16) pnrm, ivar
   16       format( ' ------ in routine inexct (ivio=1,pnrm=',e12.4, 
     &         ', var index = ',i6,') -- ')
