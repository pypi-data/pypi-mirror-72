...
#SVG taken from https://commons.wikimedia.org/wiki/File:Letters_SVG.svg
...
<svg
   ...
   xml:space="preserve"><defs
     id="defs3599"><linearGradient
       ...
       gradientTransform="matrix(0.9763,-0.2164,0.2164,0.9763,-26.4711,60.2204)"><stop
         id="stop3591"
       ...
       xlink:href="#SVGID_1_"
       gradientUnits="userSpaceOnUse"
       gradientTransform="matrix(0.9763,-0.2164,0.2164,0.9763,-26.4711,60.2204)" /></defs><path
...
<body>
<p>Test</p>
<p><img src='test.svg' alt='test image'/></p>
</body>
</html>"""

css = "p { \ncolor: red;\n background-color: black;\n}"

import io, datetime, os, tempfile, unittest, subprocess, zipfile, functools
from lxml import etree
import epubC

#Tests assume 'standard' location of everything
class epubCTest(unittest.TestCase):
	namespaces = dict(
	...
	def setUp(self):
		self.testfile = os.path.join(tempfile.gettempdir(), "epubCTest.epub")
		self.buildtime = datetime.date.today()
		...
		with epubC.Epub("Title", "Author", "ID") as ep:
			...
			ep.add("test.svg", "image/svg+xml", io.StringIO(svg))

			ep.subject = ["Test", "epub"]
			ep.date = self.buildtime

			ep.write(self.testfile)

		self.epub = zipfile.ZipFile(self.testfile, "r")
	...
	def test_containerStructure(self):
		...
		self.assertEqual(self.epub.open("mimetype").readline(), "application/epub+zip".encode("utf-8"))

		ocf_tree = etree.parse(self.epub.open("META-INF/container.xml", "r"))
		rootfile = ocf_tree.xpath("//ocf:rootfile", namespaces=self.namespaces)[0]

		self.assertEqual(rootfile.get('full-path'), 'OEBPS/content.opf')

	def test_packageStructure(self):
		opf_tree = etree.parse(self.epub.open("OEBPS/content.opf", "r"))
		ncx_tree = etree.parse(self.epub.open("OEBPS/toc.ncx", "r"))

		opf_xpath = functools.partial(opf_tree.xpath, namespaces = self.namespaces)
		ncx_xpath = functools.partial(ncx_tree.xpath, namespaces = self.namespaces)

		self.assertEqual(opf_xpath("//opf:package")[0].get("version"), "2.0")
		self.assertEqual(opf_xpath("//opf:package")[0].get("unique-identifier"), "bookid")

		self.assertEqual(opf_xpath("//opf:metadata/dc:date")[0].text, self.buildtime.isoformat())
		self.assertEqual(opf_xpath("//opf:metadata/dc:title")[0].text, "Title")
		self.assertEqual(opf_xpath("//opf:metadata/dc:creator[@opf:role = 'aut']")[0].text, "Author")
		self.assertEqual(opf_xpath("//opf:metadata/dc:identifier[@id = 'bookid']")[0].text, "ID")
		self.assertCountEqual([subject.text for subject in opf_xpath("//opf:metadata/dc:subject")], ["Test", "epub"])


		self.assertCountEqual([item.get('href') for item in opf_xpath("//opf:manifest/opf:item[@href]")], 
							  ['toc.ncx', 'content.html', 'test.css', 'test.svg'])
		self.assertEqual(len(opf_xpath("//opf:spine[@toc='ncx']/opf:itemref[@idref]")), 1)

		self.assertEqual(ncx_xpath("//ncx:head/ncx:meta[@name='dtb:uid']")[0].get("content"), "ID")
		self.assertEqual(ncx_xpath("//ncx:docTitle/ncx:text")[0].text, "Title")
		self.assertEqual(ncx_xpath("//ncx:navPoint[@playOrder='1']/ncx:navLabel/ncx:text")[0].text, "Chapter 1")
		self.assertEqual(ncx_xpath("//ncx:navPoint[@playOrder='1']/ncx:content[@src]")[0].get("src"), "content.html")
