...
def DetectOutliers(sc, cluster_label, red_dim = 2, outlier_prob_thres = 10**-4):
    ...
    sc.addCellDataColumn(col_data = -np.log10(np.ones(sc.dim[1]) * outlier_prob_thres), col_name = 'Outlier_Thres')

    # First check if clustering has been performed
    if (sc.checkCellDataColumn(cluster_label) == False):
        raise ValueError("Clustering has not been done. Perform clustering first! ")

    else:
        print("Computing outliers . . .")
        # Get cluster labels
        labels = sc.getCellColumnValues(cluster_label)
        n_clusters = np.unique(labels)
        X = sc.getCounts()
        _, n_samples = X.shape

        # Sort according to F scores
        scores = sc.getGeneColumnValues('FEATS_F_Score')
        ...
        sc.addCellDataColumn(col_data = outliers, col_name = "FEATS_Outliers")
        sc.addCellDataColumn(col_data = squared_md, col_name = "FEATS_Msd")
        sc.addCellDataColumn(col_data = outlier_score, col_name = "Outlier_Score")
        sc.addCellDataColumn(col_data = oos, col_name = "FEATS_Oos")
