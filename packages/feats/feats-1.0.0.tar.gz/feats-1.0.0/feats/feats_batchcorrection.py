...
def IntegrateBatches(batches, name_by):
    ...
    for i in range(len(batches)): 
        ...
        batches[i].addCellDataColumn(col_data = batch_no, col_name = 'batch')
    
    # Find common genes in all the datasets
    keep_genes = set()
    for i in range(len(batches)):
        gene_list = batches[i].getGeneColumnValues(name_by[i]).tolist()
        if len(keep_genes) == 0:
    ...
    for i in range(len(batches)):
        ...
        genes = batches[i].getGeneColumnValues(name_by[i]).tolist()
...
def CorrectBatches(batches, correct_order, name_by = None, k = 20, sigma = 10, svd_dim = 0, adj_var = True):
    ...
    batch_info = batches.getCellColumnValues('batch')
