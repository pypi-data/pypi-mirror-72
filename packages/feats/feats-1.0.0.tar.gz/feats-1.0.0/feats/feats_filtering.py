...
def HVGFilter(sc, num_genes, name_by = 'gene_names'):
    ...
    if (sc.checkGeneDataColumn(name_by)):
        gene_names = sc.getGeneColumnValues(name_by)
    ...
    # Save the data
    sc.addGeneDataColumn(dispersion, "FEATS_dispersion")
    sc.addGeneDataColumn(var_bat, "FEATS_variance")
    sc.addGeneDataColumn(mu_bat, "FEATS_mean")
    sc.addGeneDataColumn(hvg, "FEATS_hvg")
...
def GeneFilter(sc_obj, min_cells, max_cells, expr_thres = 0):
...
def CellFilter(sc_obj, min_genes, max_genes, expr_thres = 0):
