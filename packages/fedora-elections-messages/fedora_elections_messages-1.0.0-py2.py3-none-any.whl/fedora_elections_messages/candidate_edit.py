...
class EditCandidateV1(CandidateMessage):
    """
    A sub-class of a Fedora message that defines a message schema for messages
    published by Elections when a candidate is edited.
    """

    topic = "fedora_elections.candidate.edit"

    body_schema = {
        "id": SCHEMA_URL + topic,
        "$schema": "http://json-schema.org/draft-04/schema#",
        "description": "Schema for messages sent when a candidate is edited",
        "type": "object",
        "properties": {
            "agent": {"type": "string"},
    ...
    def summary(self):
        ...
        if self.body["candidate"].get("fas_name"):
            msg = (
                '{agent} edited candidate "{candidate_name}" ({candidate_fas_name}) '
                'for election "{title}" ({alias})'
            )
        else:
            msg = '{agent} edited candidate "{candidate_name}" for election "{title}" ({alias})'
        return msg.format(
            agent=self.body["agent"],
