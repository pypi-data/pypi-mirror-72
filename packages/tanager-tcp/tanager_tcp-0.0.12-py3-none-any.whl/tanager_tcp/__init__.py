...
#         self.address_info_len=25 #Number of digits in the address including IP address and port info.
        ...
        try: 
            ...
            while next_message:
                ...
                next_message=self.sock.recv(HEADER_LEN+ADDRESS_LEN) #If all is as expected, should be b''. If full message didn't make it through in first sock.recv could have content.
                if len(self.return_message)>HEADER_LEN+ADDRESS_LEN:
                    raise Exception('Return message longer than expected.')

            #Check that the remote server received the correct message length  
            return_header=self.return_message[0:HEADER_LEN].decode('utf-8')        
            ...
            return_address=self.return_message[HEADER_LEN:].decode('utf-8')
