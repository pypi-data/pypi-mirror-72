        ...
        if credentials is not None:
            if isinstance(credentials, str):
                try:
                    ...
                    raise ValueError(
                        "credentials was not a well formed JSON string.", e
                    )
                ...
                if not self._verify_dictionary(credentials):
                    raise ValueError(
                        'credentials has neither a "refresh_token" nor a '
                        '"private_key" field.'
                    )
...
def _configure_colab_session_tfv2():
  ...
  """
    ...
    adc_filename = os.environ.get("GOOGLE_APPLICATION_CREDENTIALS", "/content/adc.json")
