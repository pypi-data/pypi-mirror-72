...
class TimeSeriesTransformer(AbstractTimeSeriesTransformer, FeaturizationInfoProvider):
    ...
    def __init__(self, pipeline_type: TimeSeriesPipelineType = TimeSeriesPipelineType.FULL,
        ...
        super(TimeSeriesTransformer, self).__init__(featurization_config, **kwargs)
    ...
    def _construct_pre_processing_pipeline(self,
        ...
        if self.parameters.get(TimeSeries.SHORT_SERIES_HANDLING)\
            ...
            if TimeSeriesInternal.LAG_LEAD_OPERATOR in self._transforms.keys():
                lag_operator = cast(LagLeadOperator, self._transforms.get(TimeSeriesInternal.LAG_LEAD_OPERATOR))
                lags = lag_operator.lags_to_construct.get(
                    TimeSeriesInternal.DUMMY_TARGET_COLUMN, [0])
                if isinstance(lags, int):
                    lags = [lags]
                self.parameters[TimeSeries.TARGET_LAGS] = lags
            else:
                self.parameters[TimeSeries.TARGET_LAGS] = [0]

            if TimeSeriesInternal.ROLLING_WINDOW_OPERATOR in self._transforms.keys():
                rw_op = cast(RollingWindow, self._transforms.get(
                    TimeSeriesInternal.ROLLING_WINDOW_OPERATOR))
                self.parameters[TimeSeries.TARGET_ROLLING_WINDOW_SIZE] = rw_op.window_size
            else:
                self.parameters[TimeSeries.TARGET_ROLLING_WINDOW_SIZE] = 0
