...
def _get_data_snapshot_helper(data: Any, column_names_and_types: Dict[str, np.dtype] = None) -> str:
...
def _get_data_snapshot(data: DataInputType, column_names_and_types: Dict[str, np.dtype] = None,
...
def _get_transformer_x(x: DataInputType,
    ...
    data_sanpshot_str = _get_data_snapshot(x, column_names_and_types=columns_types_mapping)
...
def _remove_y_nan_needed(is_timeseries: bool,
    ...
    if (not is_timeseries or featurization_config is None or
            ...
            featurization_config.transformer_params.get(TimeSeriesInternal.IMPUTER) is None):
        return True
    for cols, params in featurization_config.transformer_params.get(TimeSeriesInternal.IMPUTER):
        if params.get(TimeSeriesInternal.IMPUTER_STRATEGY) == TimeSeriesInternal.IMPUTER_CONSTANT \
