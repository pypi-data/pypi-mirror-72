...
class AbstractTimeSeriesTransformer(AutoMLTransformer):
    ...
    def __init__(self,
        ...
        if self._is_valid_featurization_config(featurization_config) \
                and featurization_config.drop_columns is not None and len(featurization_config.drop_columns) > 0:
        ...
        self._customized_target_imputer = self._get_customized_target_imputer()
    ...
    def _get_target_imputer(self, grain_df: Optional[pd.DataFrame] = None) -> TimeSeriesImputer:
        ...
        """
        if self._customized_target_imputer is not None:
            return self._customized_target_imputer
        else:
            return self._get_default_target_imputer(grain_df)

    def _get_customized_target_imputer(self) -> Optional[TimeSeriesImputer]:
        """
        Get target value imputer based on the featurization config.

        :return: Customized target imputer.
        """
        if self._is_valid_featurization_config_transformer(self.featurization_config):
            for cols, params in self.featurization_config.transformer_params[constants.TimeSeriesInternal.IMPUTER]:
                if self.target_column_name in cols:
                    strategy = params.get(constants.TimeSeriesInternal.IMPUTER_STRATEGY)
                    fill_value = params.get(constants.TimeSeriesInternal.IMPUTER_FILL_VALUE)
                    if strategy == constants.TimeSeriesInternal.IMPUTER_CONSTANT and fill_value is not None:
    ...
    def _get_x_imputer(self,
        ...
        if self._is_valid_featurization_config_transformer(self.featurization_config):
            for cols, params in self.featurization_config.transformer_params[constants.TimeSeriesInternal.IMPUTER]:
                ...
                strategy = params.get(constants.TimeSeriesInternal.IMPUTER_STRATEGY)
                fill_value = params.get(constants.TimeSeriesInternal.IMPUTER_FILL_VALUE)
                if strategy == constants.TimeSeriesInternal.IMPUTER_CONSTANT and \
                        fill_value is not None and len(cols) > 0:
                    for col in cols:
                        imputation_dict[col] = fill_value
    ...
    @staticmethod
    def _is_valid_featurization_config(featurization_config: Union[str, FeaturizationConfig]) -> bool:
        return featurization_config is not None and isinstance(featurization_config, FeaturizationConfig)

    @staticmethod
    def _is_valid_featurization_config_transformer(featurization_config: Union[str, FeaturizationConfig]) -> bool:
        return AbstractTimeSeriesTransformer._is_valid_featurization_config(featurization_config) \
            and featurization_config.transformer_params is not None
