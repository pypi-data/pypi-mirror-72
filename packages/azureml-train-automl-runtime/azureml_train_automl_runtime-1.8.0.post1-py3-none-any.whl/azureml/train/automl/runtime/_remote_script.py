    ...
    if cache_store is not None:
        try:
            ...
            if featurizer_container is None:
                ...
                logger.exception('Encountered caching exception in featurization collector run.')
                raise
...
def _recover_dataset(
    ...
    if dataset is None:
        ...
        dataset = _recover_dataset_internal(automl_settings_obj, cache_store, dataprep_json, entry_point,
                                            onnx_cvt, parent_run_id, script_directory)
    else:
...
def _recover_dataset_internal(automl_settings_obj: AzureAutoMLSettings,
    ...
    # _transform_and_validate_input_data can only return None when prep_type is specified
    ...
    if transformed_data_context is None:
        raise ClientException("Unexpectedly received null TransformedDataContext.", has_pii=False)
...
def fit_featurizers_wrapper(
    ...
    try:
        ...
        data_transformation.fit_and_cache_featurizers(
            ...
            featurization_json=featurization_json,
            cache_store=cache_store,
