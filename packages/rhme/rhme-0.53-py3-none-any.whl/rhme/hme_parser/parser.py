...
class Parser:
    ...
    def __main_parsing(self):
        ...
        while not Q.is_empty():
            ...
            while not S.is_empty():
                ...
                upperThreshold = listin[temp1]['ymin'] + ((1/7) * listin[temp1]['h'])
                lowerThreshold = listin[temp1]['ymin'] + ((6/7) * listin[temp1]['h'])

                helpers.debug('[parser.py] __main_parsing() | temp1 label: %s' % listin[temp1]['label'])

                '''
                HOW TO?
                set the rightWall attribute of s i to the minX coordinate of s i+1
                set the leftWall attribute of s i to the symbols' minX coordinate.
                set the topWall attribute of s i to RmaxY
                set the bottomWall attribute of s i to RminY
                '''
                ...
                R = [
                    {'above': [
                        [
                            listin[temp1]['xmin'] + ((1/7) * listin[temp1]['w']),
                            # listin[temp1]['xmin'], 
                            listin[temp1]['wall']['top']
                        ],
                        [
                            listin[temp1]['xmax'] - ((1/7) * listin[temp1]['w']),
                            upperThreshold
                        ]
                    ]},
                    {'below': [
    ...
    def preprocessing(self, symbols):
        ...
        for i in range(0,len(symbols)):
            ...
            # deixei como 1/3 33,33
            # e 2/3 66,66
            if re.search("^[0-9]$", str(s['label'])) or s['label'] == '19': # label 19 b
                s['centroid_class'] = 'Ascending'
                s['centroid'][1] = s['ymin'] + (2/3) * (s['ymax'] - s['ymin'])
                # s['centroid'][1] = s['ymin'] + (3/5) * (s['ymax'] - s['ymin']) # por ultimo
    ...
    def __hor(self, listin, index):
        ...
        If a horizontally overlaps a horizontal line hline, let a be the longest
        horizontal line which overlaps hline, or hline if no horizontal line over-laps hline

        If s is a horizontal line or bracket, 
        START is called on the region defined by the wall attributes of s 
        and the maxX bounding box coordinate of s (i.e. replacing leftWall)

        The remaining cases simply require a scan of the input list, 
        performing tests on 
        >coordinate positions of s and the identity and coordinates< 
        of each symbol encountered

        A call is then made to OVERLAP (again, to check with overlap with horizontal lines).

        In the general case, the leftmost symbol on the right of s in the HOR
        region is returned as a if it is present and not overlapped by a horizontal
        line.

        For horizontal lines and open brackets, START is applied to the region
        defined by the maxX bounding box coordinate of the symbol, and the re-maining three wall attributes.
        '''
        ...
        top = listin[index]['ymin'] + (listin[index]['h'] * (1/7)) # 1/6
        bottom = listin[index]['ymin'] + (listin[index]['h'] * (6/7)) # 5/6
        ...
        # if label in range(10,17) or label in [27, 28, 29, 30]:
        if label in range(10,17):
            ...
            for s in range(0, len(listin)):
                ...
                if not checked:
                    ...
                    # Adicionei o round() nos dois - e tirei | Adicionei o >=, <=, ... - era só >, <, ...
                    # Readicionei o round só no primeiro lado
