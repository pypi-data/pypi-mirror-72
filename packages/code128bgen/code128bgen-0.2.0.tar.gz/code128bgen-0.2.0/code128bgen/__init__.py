...
def build_font_common(raw_string):
    """
    Attempts to generate symbols for standard code128 fonts.
    :param raw_string:
    :return:
    """
    font_common = {
    ...
    codes = generate_codes_from_str(raw_string)
    return "".join([font_common.get(code, chr(int(code) + 32)) for code in codes])


def generate_codes_from_str(raw_string):
    """
    Generates codes from string.
    ...
    for i in raw_string:
        ...
        if 0 <= ord_num <= 31:
            raise ValueError('invalid character range for code128b barcode')
        ord_num -= 32
        if ord_num > 99:
...
if __name__ == '__main__':
    ...
    bar_widths_raw = generate_raw_bar_widths([FNC1, 42, 18, 40, 20, 50, CODE_A, 16])
