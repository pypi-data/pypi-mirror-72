...
class AbstractComponentTest(unittest.TestCase):
    ...
    def test_runs(self):
        """Basic smoke test"""
        inst = ControlComponent.from_env('test')
    ...
    def test_pm_reset(self):
        inst = ControlComponent.from_env('test', default_save=False)
    ...
    def test_report_connectors(self):
        ...
        self.assertIn(os.environ.get('HADRON_PM_PATH') + '/aistac_pm_control_task.json', report.get('uri'))

    def test_PM_from_env(self):
        os.environ['HADRON_PM_PATH'] = "work/test/contracts?A=24&B=fred"
        os.environ['HADRON_PM_TYPE'] = 'pickle'
        os.environ['HADRON_PM_MODULE'] = 'tests.handlers.test_handlers'
        os.environ['HADRON_PM_HANDLER'] = 'TestPersistHandler'
        ...
        self.assertEqual('work/test/contracts/aistac_pm_control_task.pickle', result.uri)
    ...
    def test_connector_file_pattern(self):
        ...
        state_connector = ConnectorContract(
            uri=manager.pm.file_pattern(prefix=f"{os.environ['HADRON_PM_PATH']}/data/", connector_name='version', versioned=True),
            module_name=manager.DEFAULT_MODULE,
            handler=manager.DEFAULT_PERSIST_HANDLER,
            version="v1.01")
        temporal_connector = ConnectorContract(
            uri=manager.pm.file_pattern(prefix=f"{os.environ['HADRON_PM_PATH']}/data/", connector_name='temporal', stamped='DAYS'),
            module_name=manager.DEFAULT_MODULE,
            handler=manager.DEFAULT_PERSIST_HANDLER)
        ...
        self.assertTrue(os.path.exists(f"{os.environ['HADRON_PM_PATH']}/data/aistac_CONTROL_task_version_v1.01.pickle"))
        manager.persist_canonical(connector_name='temporal_state', canonical=pd.DataFrame({'A': [1,2,3,4]}))
        dt = datetime.now().strftime("%Y%m%d")
        self.assertTrue(os.path.exists(f"{os.environ['HADRON_PM_PATH']}/data/aistac_CONTROL_task_temporal_{dt}.pickle"))
    ...
    def test_report_eviron(self):
        ...
        control = {'HADRON_CONTROL_HANDLER': 'not set',
                   'HADRON_CONTROL_MODULE': 'not set',
                   'HADRON_CONTROL_PATH': 'not set',
                   'HADRON_CONTROL_PERSIST_HANDLER': 'not set',
                   'HADRON_CONTROL_PERSIST_MODULE': 'not set',
                   'HADRON_CONTROL_PERSIST_PATH': 'not set',
                   'HADRON_CONTROL_SOURCE_HANDLER': 'not set',
                   'HADRON_CONTROL_SOURCE_MODULE': 'not set',
                   'HADRON_CONTROL_SOURCE_PATH': 'not set',
                   'HADRON_CONTROL_TEST_HANDLER': 'not set',
                   'HADRON_CONTROL_TEST_MODULE': 'not set',
                   'HADRON_CONTROL_TEST_PATH': 'not set',
                   'HADRON_CONTROL_TEST_PERSIST_HANDLER': 'not set',
                   'HADRON_CONTROL_TEST_PERSIST_MODULE': 'not set',
                   'HADRON_CONTROL_TEST_PERSIST_PATH': 'not set',
                   'HADRON_CONTROL_TEST_SOURCE_HANDLER': 'not set',
                   'HADRON_CONTROL_TEST_SOURCE_MODULE': 'not set',
                   'HADRON_CONTROL_TEST_SOURCE_PATH': 'not set',
                   'HADRON_DEFAULT_HANDLER': 'not set',
                   'HADRON_DEFAULT_MODULE': 'not set',
                   'HADRON_DEFAULT_PATH': 'not set',
                   'HADRON_DEFAULT_PERSIST_HANDLER': 'not set',
                   'HADRON_DEFAULT_PERSIST_MODULE': 'not set',
                   'HADRON_DEFAULT_PERSIST_PATH': 'not set',
                   'HADRON_DEFAULT_SOURCE_HANDLER': 'not set',
                   'HADRON_DEFAULT_SOURCE_MODULE': 'not set',
                   'HADRON_DEFAULT_SOURCE_PATH': 'not set',
                   'HADRON_PM_HANDLER': 'not set',
                   'HADRON_PM_MODULE': 'not set',
