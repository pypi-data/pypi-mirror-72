...
from take_forecast.tune.nodes import sarima_split_train_test
...
def test__try_default():
	foo = _try_default_(return_result, (ValueError,), -1)
	assert foo() == 1


def test__try_default__on_error():
	foo = _try_default_(raise_exception, (ValueError,), -1)
	assert foo() == -1


def test__try_default__on_unexpected_error():
...
def test__sarima_find_d(df: DF, column: str, threshold: float, d_stop: int):
	ts = df[column]
...
def test__sarima_find_s(df: DF, column: str, threshold: float, s_start: int, s_stop: int):
	ts = df[column]
...
def test__sarima_find_pq_max(df: DF, column: str, s: int):
	ts = df[column]
...
def test__sarima_find_spq_max(df: DF, column: str, s: int, n_cycles: int):
	ts = df[column]
...
def test__split_train_test(df: DF, column: str, r_split: int):
	ts = df[column]
	ts_train, ts_test = sarima_split_train_test(ts, r_split)
...
def test__sarima_find_best_hp__non_seasonal(df: DF, column: str, p_max: int, ur: int, q_max: int, criteria: str):
	ts = df[column]
...
def test__sarima_find_best_hp__seasonal(
		df: DF, column: str, p_max: int, ur: int, q_max: int, s: int, sP_max: int, sQ_max: int, criteria: str):
	ts = df[column]
	kwargs = sarima_find_best_hp(ts, p_max, ur, q_max, s, sP_max, sQ_max, criteria)
	assert isinstance(kwargs, dict)
	assert len(kwargs) == 2
