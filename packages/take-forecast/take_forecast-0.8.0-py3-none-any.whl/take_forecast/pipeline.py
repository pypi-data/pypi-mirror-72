...
from take_forecast import preprocess, tune, fit, predict, posprocess
...
def create_pipelines(**_) -> tp.Dict[str, Pipeline]:
	...
	for case in cases:
		...
		pl_predict = predict.create_pipeline(case=case)
		pl_posprocess = posprocess.create_pipeline(case=case)
		...
		pipelines['{case}_predict'.format(case=case)] = pl_predict
		pipelines['{case}_posprocess'.format(case=case)] = pl_posprocess
