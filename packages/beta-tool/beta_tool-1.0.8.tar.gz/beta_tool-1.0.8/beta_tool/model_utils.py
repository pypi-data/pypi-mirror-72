...
def encoder_train(data, features, model_save_path='/socrates_data/model_file/'):
    """
    类别特征 encoder 编码
    ...
    for feature in features:
        ...
        tmp_path = model_save_path + '_' + feature + '.pkl'
        try:
...
def encoder_prediction(data, features, model_save_path='/socrates_data/model_file/'):
    """
    类别特征 encoder 编码
    :param model_save_path:
    :param data:
...
def label_encoder(data, model_type, categorical_feature):
    ...
    if model_type == 'train':
        data = encoder_train(data, categorical_feature)
    elif model_type == 'predict':
        data = encoder_prediction(data, categorical_feature)
...
def classifier_model_evalution(y_eval, y_pred):
    print('confusion_matrix:\n %s', confusion_matrix(y_eval, y_pred))
    print('classification_report:\n %s', classification_report(y_eval, y_pred))
