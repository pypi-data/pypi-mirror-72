...
class NewGameObject():
    def __init__(self, imgs = None, size = None, body_type = pymunk.Body.KINEMATIC, sensor = False):
        ...
        if size:
            if isinstance(size, int):
                ...
                if isinstance(size[0][0], int):
                    self._gameObject.set_body(Body(body_type, 'POLY', size = size, sensor = sensor))
                    self.vertices = self._gameObject.body.shape.get_vertices()  
                else:
                    tiledmap_objects = TiledMapBodies(body_type, size, sensor)
    ...
    def slide_to(self, x, *y):
        if y:
            distance = vec(x, *y) - self._gameObject.pos
            if distance.length() < 1:
                self._gameObject.pos = vec(x, *y)
                return True
        else:
            distance = vec(x) - self._gameObject.pos
            if distance.length() < 1:
                self._gameObject.pos = vec(x)
                return True
        self._gameObject.pos += distance.normalize() * SPEED
        return False
    ...
    def set_color_mask(self, red, green, blue, alpha = 0):
    ...
    # 动画命令
    def next_image(self):
        self._gameObject.sprite.sprite_strategy.frame += 1
    ...
    def set_dt(self, dt, state = None):
        self._gameObject.sprite.sprite_strategy.set_dt(dt, state)
    ...
    # 音效
    def play_snd(self, snd = None, v=1):
        if snd:
            self.snd = pygame.mixer.Sound(snd)
            self.snd.set_volume(v)
            self.snd.stop()
            self.snd.play() 
        elif snd == '':
            pass
        else:
            if 'snd' in self.__dict__:
                self.snd.stop()   
        self.snd = snd
...
class Mouse(Character):
    def __init__(self, imgs = None, radius = 1):
        Character.__init__(self, imgs, radius)
        if imgs:
