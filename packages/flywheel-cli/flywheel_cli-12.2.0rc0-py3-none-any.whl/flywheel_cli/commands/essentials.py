...
def add_commands(subparsers, parsers, parents):
    ...
    dl_parser.add_argument(
        "-p", "--prefix", help="Prefix for downloaded directory structure",
    )
    dl_parser.add_argument("-i", "--include", help="Download only these types")
    dl_parser.add_argument(
        "-e", "--exclude", help="Download everything but these types"
    )
...
def download(args):
    ...
    try:
        src_path = sdk_impl.parse_resolver_path(args.src)
    ...
    if src_cont.container_type == "file":
        ...
        type_filter = flywheel.DownloadFilterDefinition(
            plus=args.include, minus=args.exclude
        )
...
def _get_row_for_container(
    cont, parent, user, show_ids
):  # pylint: disable=too-many-branches
    if user is None:
        level = "admin"
    elif "permissions" in cont:
        level = _get_permission_level(cont, user.id)
    else:
        level = _get_permission_level(parent, user.id)
...
def _get_permission_level(cont, uid):
    if cont is not None:
