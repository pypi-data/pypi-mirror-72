...
class testYieldCurveGetRate(unittest.TestCase):
    def testDiscountCurve(self):
        ...
        disC.values = dF  # force load data, normally load via reader's loadYCDate method
        disC.timeIndex = timeIndex
        self.assertAlmostEqual(disC.getRate(t1, t2), (1 + flatR) ** -1) # almostEqual: auto round to 7 decimals
