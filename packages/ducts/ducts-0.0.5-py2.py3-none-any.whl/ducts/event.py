...
@config_callback
def config(loader):
    loader.add_attr_list('system_plugin_base_modules', ['ducts'], help='event handler plugin path')
    loader.add_attr_path('system_plugin_path', './handler', help='event handler plugin path')
    loader.add_attr_path('plugin_path', './handler', help='event handler plugin path')
...
class HandlerManager(object):
    ...
    def __init__(self, ducts_home, loop, redis):
        ...
        self._handlers = {}
        self._plugin_dirs = []
        for module_name in self.conf.system_plugin_base_modules:
            module = importlib.import_module(module_name)
            self.load_plugins(Path(module.__file__).parent.joinpath(self.conf.system_plugin_path).resolve(), True)
        #self.load_plugins(self.conf.system_plugin_path if self.conf.system_plugin_path.is_absolute() else Path(__file__).parent.joinpath(self.conf.system_plugin_path).resolve(), True)
        self.load_plugins(self.conf.plugin_path if self.conf.plugin_path.is_absolute() else ducts_home.joinpath(self.conf.plugin_path).resolve(), False)
