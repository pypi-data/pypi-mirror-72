...
class DeviceVirtualDirectory(DeviceFileServer):
    ...
    def get_dir_content(self, drive, input_dir, file_filter=None):
        ...
        if source == LOCAL_FILESYSTEM:
            ...
            dirs_meta = []
            for d_dir in dirs:
                absolute_path = os.path.join(root_dir, d_dir)
                dirs_meta.append([0, os.path.getmtime(absolute_path)])
            for d_file in filtered_files:
                absolute_path = os.path.join(root_dir, d_file)
    ...
    def is_cached(self, drive, input_dir, file_name):
        ...
        path_hash, file_size, file_mod_time = self.get_meta_hash(drive, input_dir, file_name)
        path_hash = os.path.join(cache_directory, path_hash)
        if os.path.exists(path_hash):
            if file_size == os.path.getsize(path_hash) and file_mod_time == os.path.getmtime(path_hash):
