...
class fastRake(object):
    def __init__(self, language_code="fr", min_char_length=1, max_words_length=5, min_keyword_frequency=1,
                 min_words_length_adj=1, max_words_length_adj=1, min_phrase_freq_adj=2):

        self.__stop_words_list = get_stop_words(language_code)
...
if test and __name__ == '__main__':
    ...
    data = cls.review
    text = ''.join(data[2000:4000])
    # Split text into sentences
    sentenceList = split_sentences(text)
    stop_words_list =  get_stop_words('fr')
    stopwordpattern = build_stop_word_regex(stop_words_list)
    
    # generate candidate keywords
    phraseList = generate_candidate_keywords(sentenceList, stopwordpattern, stop_words_list,min_char_length=5, max_words_length=1)
    ...
    print(keywordcandidates.items())
    sortedKeywords = sorted(keywordcandidates.items(), key=lambda x : x[1], reverse=True)
    if debug: print(sortedKeywords[:50])

    totalKeywords = len(sortedKeywords)
    if debug: print(totalKeywords)
    test = {"étais moche" : 2, "sucer thierry" :1, "manger pomme" : 3}
    ...
    rake = fastRake(language_code="fr", max_words_length=1)
    keywords = rake.run(' '.join(data[:2000]))
    print(keywords[:15])