...
class ND_Model(object):
    ...
    def DM_iterator(self):
        ...
        all_indices = [list(range(0, x[1][1])) for x in list(self.extra_dims.items())]
        ...
        M_base_indices = [slice(0, x[1][1]) for x in list(self.M_base_dims.items())]
        D_base_indices = [slice(0, x[1][1]) for x in
                          list(self.Data.D_base_dims.items())]
    ...
    def retrieve_lams_and_WtWms(self, iteration):
        ...
        if(self.Data.extra_mask is None):
            dimensions = list(self.regularizations.keys())
        else:
