...
class ND_Data(object):
    ...
    def __init__(self, model, extra_dims, data_weighting_key, settings):
        ...
        self.D_base_size = [x[1][1] for x in list(self.D_base_dims.items())]
    ...
    def D_iterator(self):
        ...
        """
        all_indices = [list(range(0, x[1][1])) for x in self.extra_dims.items()]
    ...
    def _allocate_D(self):
        ...
        new_shape = []
        for dimension in list(self.D_base_dims.keys()):
            new_shape.append(self.D_base_dims[dimension][1])

        for dimension in list(self.extra_dims.keys()):
