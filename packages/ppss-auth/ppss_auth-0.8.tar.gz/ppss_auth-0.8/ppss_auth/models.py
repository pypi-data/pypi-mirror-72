...
def __createUsers(session,users,groups):
    usermap = {}
    for u in users:
        u = u.split("=")
        uname = str.strip(u[0])
        ...
        if not user:
            ...
            if Conf.defaultpassword:
                user.setPassword(Conf.defaultpassword)
            session.add(user)
        usermap[uname] = user
        for g in grouplist:
...
def initdb(session=None,createdefault=False,engine = None):
    ...
    if createdefault:
        ...
        if Conf.adminname:
            root = __createUsers(session,[Conf.adminname+"="+u"sysadmin"],groupmap)
            if Conf.adminpass:
...
class PPSsuser(Base,commonTable):
    ...
    def checkLogin(cls,user,password,dbsession):
        ...
        return PPSsuser.checkCryptedLogin(cls,user,s.hexdigest(),dbsession)
