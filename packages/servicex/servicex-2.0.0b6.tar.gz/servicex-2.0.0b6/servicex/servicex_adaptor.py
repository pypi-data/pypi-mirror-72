...
class ServiceXAdaptor:
    ...
    async def _login(self):
        ...
        async with (await self._get_client()).post(url, json={
    ...
    async def submit_query(self, json_query: Dict[str, str]) -> str:
        """
        ...
        async with (await self._get_client()).post(f'{self._endpoint}/servicex/transformation',
    ...
    async def get_transform_status(self, request_id: str) -> \
        ...
        async with (await self._get_client()).get(
...
async def transform_status_stream(sa: ServiceXAdaptor,
                                  request_id: str) \
    ...
    while not done:
        next_processed = await sa.get_transform_status(request_id)
