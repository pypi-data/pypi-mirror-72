...
class catalog:
    ...
    def __init__(self, alphas=[], deltas=[], gamma_1s=[], gamma_2s=[],
        ...
        if self.ctx['prec'] < 32:
            raise Exception("Float size below 32 bits not allowed")
    ...
    def get_map(self, type, bin=0, alpha_rot=0.0, delta_rot=0.0,
                pix=-1, trimming=True, mirror=False):
        """
        ...
        # masking
        shear_map_1[np.logical_not(weights > 0.0)] = hp.pixelfunc.UNSEEN
        shear_map_2[np.logical_not(weights > 0.0)] = hp.pixelfunc.UNSEEN
