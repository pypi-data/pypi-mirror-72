...
class likelihood:
    ...
    def set_means(self, means, systematics=True,
        ...
        if len(self.S_inv) > 0:
            ...
            if use_SVD:
                # use only non-systematics to build SVD basis
                if not systematics:
                    params = {}
                    ...
                    chosen = np.ones(means.shape[0], dtype=bool)

                u, s, vh = np.linalg.svd(means[chosen],
                                         full_matrices=True)
    ...
    def build_interpolator(self, interpolate_precision_matrix=False,
        ...
        if not interpolate_precision_matrix:
