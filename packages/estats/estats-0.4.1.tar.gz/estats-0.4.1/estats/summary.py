...
class summary:
    ...
    # ACCESING OBJECTS
    def get_data(self, statistic='Peaks', rescale=False):
        """
        Get full data array for a certain statistic

        :param statistic: Statistic for which to return data array
        :param rescale: If True rescale each databin to [0, 1] range
        :return : All data vector realisations for the statistic
        """
        if statistic == 'all':
            if rescale:
                raise Exception("Rescaling not supported for return all "
                                "data mode")
            return self.data
        else:
            data = self.data[statistic]
            if rescale:
                maxs = np.max(data, axis=0)
                mins = np.min(data, axis=0)
                data = (data - mins) / (maxs - mins)
            return np.asarray(self.data[statistic])
    ...
    # SETTING OBJECTS
    def set_data(self, data, statistic='Peaks', rescale=False):
        """
        Set full data array
    ...
    def join_statistics(self, statistics=['Peaks', 'CLs']):
        ...
        """
        # Loop over different statistics
        comp = self.get_meta(statistics[0])['NREALS'].astype(int)
        block_sizes = np.zeros(0, dtype=np.int16)
        for s in statistics:
            # check that meta data is the same
            try:
                check = np.equal(comp, self.get_meta(s)['NREALS'].astype(int))
            except ValueError:
                raise Exception("ERROR: Inconsistency when joining statistics")
            if check.size != np.sum(check):
                raise Exception("ERROR: Inconsistency when joining statistics")
    ...
    def join_redshift_bins(self):
        ...
        for statistic in self.data.keys():
            ...
            # check that meta data is the same for all of them
            comp = self.get_meta(statistic)['NREALS'][idx].astype(int)
            for bin in bins:
                check = np.equal(
                    comp,
                    self.get_meta(
                        statistic)['NREALS'][bin_idxs[bin]].astype(int))
                if check.size != np.sum(check):
                    raise Exception(
                        "ERROR: Inconsistency when joining redshift bins")
    ...
    def get_full_summary(self, statistic='Peaks', invert=False,
                         ...
                         plot_fiducial_incov=False,
                         Fiducials=[],
                         label='', scale_to_unity=False, reorder=False,
        ...
        :param scale_to_unity: If True attempts to renormalize
                               data vectors to [0,1] range.
        ...
        # optionally scale all data bins to 1 (division by fiducial datavector)
        # stabilises covariance and inversion
        if scale_to_unity:
            if len(Fiducials) == 0:
                raise Exception(
                    "If you wish to do unity scaling please provide a "
                    "list for the fiducial parameter settings")
            self._scale_to_unity(statistic, Fiducials, means)
    ...
    def _scale_to_unity(self, statistic, fiducials, means):
        """
        Rescales data vectors to 0,1 range
        """
        # get fiducial data vector
        fid = [True] * self.meta[statistic].shape[0]
        for ii, entry in enumerate(fiducials):
            fid &= self.meta[statistic][:, 2 + ii] == entry

        fiducial_datavector = means[fid, :]
        self.data[statistic] = self.data[statistic] / fiducial_datavector
    ...
    def _process_data(self, raw_data, stat, trimmed_mask_dict={},
                      scale_to_unity=False, logger=None):
        """
        Load a STATS file in the correct format
        """

        if scale_to_unity:
            vprint("Scaling data vectors to unity for better numerical \
                    stabilitiy when inverting covariance matrix.",
                   logger=self.logger, verbose=self.verbose)
        ...
        return plugin(raw_data, self.ctx, scale_to_unity)
    ...
    def _slice_data(self, errors=[], stat='Peaks', logger=None):
        ...
        for bin in bins:
            ...
            new_data = _slicer(data=self.data[stat][idx],
