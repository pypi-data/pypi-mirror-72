...
class ParseConf(object):
    ...
    def conf_parse(self):
        ...
        try:
            ...
            self.delete_model_file = eval(conf_parser.get("model", "delete_model_file"))
...
def model_popen(jobs_pid, script_path, base_path, model_id=-1, gpu_use=-1, port_use=0, model_use=None, cfg=None,
                model_path=None, model_module=None, delete_model_file=False):
    """
    ...
    cmd_pattern = '{python} {script_path} {base_path} {model_id} {cfg} {port_use} ' \
                  '{model_use} {model_path} {model_module} {delete_model_file}'
...
def start_task(conf):
    ...
    if args.is_auth:
        ...
        auth_module = __import__(args.auth_module)
        authenticate = getattr(auth_module, args.auth_method)
        authentication.authenticate = authenticate
        if not authenticate():
            sys.exit(-1)
        decrypt_model_method = getattr(auth_module, args.decrypt_model_method)
        authentication.decrypt_model = decrypt_model_method
        logging.debug("set decrypt model success: {}".format(authentication.decrypt_model))
    ...
    for model_id in args.activate_models:
        ...
        logging.info(
            "Model {} using GPU: {}, PORT: {}, MODEL: {}".format(model_id, gpu_use, port_use,
                                                                 model_use))
        delete_model_file = 1 if args.delete_model_file else 0
        p = multiprocessing.Process(target=model_popen,
                                    args=(jobs_pid, model_script_path, base_path, model_id, gpu_use, port_use,
                                          model_use, config_path, model_path, args.model_module, delete_model_file))
        p.start()
    ...
    # 删除模型文件
    if args.delete_model_file:
        start_time = time.time()
        temp_path = os.path.join(os.path.dirname(model_path), '..', 'model', 'temp')
        model_path = os.path.join(os.path.dirname(model_path), '..', 'model')
        while True:
            ...
            if not os.path.exists(temp_path):
                continue
            file_num = len([lists for lists in os.listdir(temp_path) if os.path.isfile(os.path.join(temp_path, lists))])
