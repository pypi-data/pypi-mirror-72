...
class DataStore:
    ...
    @staticmethod
    def refresh_dm(score_pre_calc=True,start_time='20050101',print_time=False):
        print(f'start load at {datetime.datetime.now()}')
        t1 = time.time()
        dm = FundDataManager(start_time=start_time,end_time=datetime.datetime.now().strftime('%Y%m%d'))
        dm.init(score_pre_calc=score_pre_calc,print_time=print_time)
