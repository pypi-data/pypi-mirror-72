...
class FundIndicatorProcessorWeekly(object):
    ...
    def _process_fund_indicator_item(self, fund_id, time_range, fund_ret, index_id, index_ret):
        ...
        df = df.replace(np.Inf,None).dropna(subset=['beta_w', 'alpha_w','track_err_w']).reset_index()
        if df.empty:
            return None
        return df
    ...
    def process(self, start_date, end_date):
        ...
        try:
            ...
            end_date_weekday = end_date_dt.weekday()
            if end_date_weekday != 4: # weekday 0对应周一  4对应周五
                print(f'weekly indicator only update on Friday, not today {end_date_weekday}')
                return failed_tasks
            else:
                print(f'weekly indicator only update on Friday, today {end_date_weekday} is Friday')
