...
from .fund_info_filter import filter_fund_info
...
class FundDataManager(DataManager):
    ...
    def init(self, index_list=None, score_pre_calc=True, print_time=False):
        ...
        with DerivedDatabaseConnector().managed_session() as derived_session:
            ...
            _fund_indicator_weekly = _fund_indicator_weekly.pivot(index='datetime', columns='fund_id')
            _fund_indicator_weekly.index = pd.Series(_fund_indicator_weekly.index).dt.date
            _dt = sorted(list(set(_fund_indicator_weekly.index).union(set(trim_trading_days))))#交易日，和现有indicator index 周日做并集
            _fund_indicator_weekly = _fund_indicator_weekly.reindex(_dt).fillna(method='ffill').stack().swaplevel()
            _fund_indicator_weekly = _fund_indicator_weekly.groupby(level='fund_id', as_index=False).apply(lambda x: x.loc[(slice(None), slice(None, self.end_date_dic[x.index.get_level_values(0).array[0]])), :])
            _fund_indicator_weekly = _fund_indicator_weekly.droplevel(0).reset_index()
            self.dts.fund_indicator = pd.merge(self.dts.fund_indicator, _fund_indicator_weekly, on=['fund_id','datetime'], how='outer')
