...
class DataStore:
    ...
    @staticmethod
    def refresh_dm(score_pre_calc=True):
        t1 = time.time()
        dm = FundDataManager(start_time='20050101',end_time=datetime.datetime.now().strftime('%Y%m%d'))
        dm.init(score_pre_calc=score_pre_calc)
