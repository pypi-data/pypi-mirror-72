...
from .utils import (
    ...
    _var_generator, to_args_from_keywords, _monad_generator)
...
class hep_render_context:
    '''
    Helper class to track the context
    '''
    def __init__(self, context: render_context, old_context: hep_render_context = None):
        self._df_context = context
        if old_context is None:
            self._monad_generator = _monad_generator()
            self._vg = _var_generator()
        else:
            self._monad_generator = old_context._monad_generator
            self._vg = old_context._vg

    @property
    def context(self) -> render_context:
        return self._df_context

    @property
    def vg(self) -> _var_generator:
        return self._vg

    def monad_ref(self) -> str:
        return self._monad_generator.new_monad_ref()
...
def _render_expresion_as_transform(tracker: _statement_tracker, context: hep_render_context,
                                   a: ast.AST):
    '''
    ...
    if term.term == 'main_sequence':
        ...
        vn = context.vg.new_term(_unwrap_list(tracker.sequence.result_type)
        ...
        st_select = statement_select(a, in_type, out_type, vn, term, context.vg)
        tracker.statements.append(st_select)
        tracker.sequence = st_select


def _render_callable(a: ast.AST, callable: ast_Callable, context: hep_render_context,
                     tracker: _statement_tracker) -> term_info:
    # And the thing we want to call we can now render.
    expr, new_context_df = render_callable(callable, context.context, callable.dataframe)
    new_context = hep_render_context(new_context_df, context)
    ...
    if root_expr is tracker.sequence._ast:
        ...
        with tracker.substitute_ast(
            ...
            select_var = context.vg.new_term(seq_as_object.result_type)
...
class _statement_tracker:
    ...
    '''
    def __init__(self, start_sequence: statement_base, vg: _var_generator, parent: Optional[_statement_tracker]):
    ...
    def carry_monad_forward(self, a: ast.AST) -> int:
        ...
        for s in self.statements[index + 1:]:
            m_index = s.carry_monad_forward(m_index, self.vg)
...
class _map_to_data(_statement_tracker, ast.NodeVisitor):
    ...
    '''
    def __init__(self, base_sequence: statement_base, context: hep_render_context,
        ...
        _statement_tracker.__init__(self, base_sequence, context.vg, p_tracker)
    ...
    def visit_ast_Filter(self, a: ast_Filter):
        ...
        var_name = self.context.vg.new_term(_unwrap_list(seq_type) if _is_list(seq_type) else seq_type)
        with self.substitute_ast(self.sequence._ast,
            ...
            st = statement_where(a, self.sequence.result_type, var_name, term, self.context.vg)
    ...
    def visit_Call(self, a: ast.Call):
        ...
        if isinstance(a.func, ast.Attribute):
            ...
            var_name = self.context.vg.new_term(_unwrap_if_possible(self.sequence.result_type))
    ...
    def append_call(self, a: ast.AST, name_of_method: str, args: Optional[List[str]]) -> None:
        ...
        iterator_name = self.context.vg.new_term(f_input_type)
        ...
        select = statement_select(a, self.sequence.result_type, result_type,
                                  iterator_name, term_info(expr, f_result_type), self.context.vg)
        self.statements.append(select)
        self.sequence = select


def _render_expression(current_sequence: statement_base, a: ast.AST,
                       context: hep_render_context, p_tracker: Optional[_statement_tracker]) \
    ...
    class render_expression(_statement_tracker, ast.NodeVisitor):
        def __init__(self, current_sequence: statement_base, context: hep_render_context,
                     p_tracker: Optional[_statement_tracker]):
            ast.NodeVisitor.__init__(self)
            _statement_tracker.__init__(self, current_sequence, context.vg, p_tracker)
        ...
        def binary_op_statement(self, operator: ast.AST, a_left: ast.AST, a_right: ast.AST):
            ...
            def do_statements(s: List[statement_base], t: term_info) \
                ...
                if t.term != 'main_sequence':
                    return t, t
                if len(s) == 0:
                    r = self.context.vg.new_term(self.sequence.result_type)
                    return r, r
                if not _is_list(s[-1].result_type):
                    stem = self.context.vg.new_term(self.sequence.result_type)
                    r = stem
                    for single in s:
                    ...
                    self.statements += s
                    self.sequence = s[-1]
                    r = self.context.vg.new_term(_unwrap_list(self.sequence.result_type))
                    return r, r
        ...
        def visit_BoolOp(self, a: ast.BoolOp):
            ...
            var_name = self.context.vg.new_term(self.sequence.result_type)
            with self.substitute_ast(self.sequence._ast,
                                     _ast_VarRef(var_name)):
        ...
        def visit_UnaryOp(self, a: ast.UnaryOp):
            ...
            st = statement_select(a, in_type, out_type, itr_unwrapped, term_info(expr, bool,
                                  operand.monad_refs), self.context.vg)
            self.statements.append(st)
            self.sequence = st
            self.term_stack.append(term_info('main_sequence', List[bool]))
            ...
            if isinstance(a.func, ast.Attribute):
                if a.func.attr not in _known_simple_math_functions:
                    ...
                    if v.term != 'main_sequence':
                        ...
                        var_name = self.context.vg.new_term(float)
...
def _resolve_expr_inline(curret_sequence: statement_base, expr: ast.AST, context: hep_render_context,
                         p_tracker: _statement_tracker) \
        -> term_info:
    '''
