...
class _monad_generator:
    '''
    Utility class to track monand index on a per query basis
    '''
    def __init__(self):
        self.monad_index: int = 0

    def new_monad_ref(self):
        '''
        Return a new, unique, string that can be used as a monad reference
        '''
        assert self.monad_index < 1000
        v = f'<mr-{self.monad_index:03d}>'
        self.monad_index += 1
        return v


class _var_generator:
    '''
    Utility class to track unique variable names on a per-query basis
    '''
    def __init__(self):
    ...
    def new_var_name(self):
        '''
        Returns the string for a new variable name. Each one is unique in a query.
        '''
