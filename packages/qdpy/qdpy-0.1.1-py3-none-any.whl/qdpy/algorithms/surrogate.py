...
class SAIL(SurrogateAssistedAlg):
    ...
    def _acquisition_eval_fn(self, inds: Sequence[IndividualLike]) -> Sequence[Tuple[FitnessValuesLike, FeaturesLike]]:
    ...
    def _create_gaussian_regressor(self) -> None:
        ...
        for ind in self.container:
            dataX.append(ind)
            dataY.append(list(ind.fitness.values) + list(ind.features))
