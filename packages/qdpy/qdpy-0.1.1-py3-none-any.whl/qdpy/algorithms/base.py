...
class QDAlgorithm(abc.ABC, QDAlgorithmLike, Summarisable, Saveable, Copyable, CreatableFromConfig):
    ...
    def tell(self, individual: IndividualLike, fitness: Optional[Any] = None,
        ...
        if elapsed is not None:
            ind.elapsed = elapsed
        xattr: Mapping[str, Any] = self._tell_before_container_update(ind)
        added: bool = self.container.update([ind]) == 1 if tell_container else False
        self._tell_after_container_update(ind, added, xattr)
        return added


    def _tell_before_container_update(self, individual: IndividualLike) -> Mapping[str, Any]:
    ...
    def optimise(self, eval_fn: Callable, budget: Optional[int] = None, 
            batch_mode: bool = True, executor: Optional[ExecutorLike] = None,
            send_several_suggestions_to_fn: bool = False, sampling: Optional[Sampling] = None) -> IndividualLike:
        """TODO"""
        ...
        # Init sampling
        sampling = self._sampling if sampling is None else sampling
        if sampling is None:
            evaluate = eval_fn
        else:
            evaluate = partial(sampling, eval_fn)
