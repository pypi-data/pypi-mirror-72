...
DATE_FORMATS = [
    'ISO',
    '%Y-%m-%d', '%m/%d/%Y',   # '2019-04-17' (ISO format), '04/17/2019'
...
DATETIME_FORMATS = ['ISO']
DATETIME_FORMATS.extend([d + ' ' + t for t in TIME_FORMATS[1:] for d in DATE_FORMATS[1:]])
DATETIME_FORMATS.extend(DATE_FORMATS[1:])
DATETIME_FORMATS.extend(TIME_FORMATS[1:])

# Fallback iso parsing
if sys.version_info < (3, 7):
    # DATE_FORMATS.extend(['%Y-%m-%d'])  # Already being parsed
    # TIME_FORMATS.extend(['%H:%M:%S.%f'])
    DATETIME_FORMATS.extend(['%Y-%m-%dT%H:%M:%S.%f', '%Y-%m-%dT%H:%M:%S', '%Y-%m-%dT%H:%M'])
...
def make_datetime(date_string, formats=None):
    ...
    for fmt in formats:
        try:
            if str(fmt).upper() == 'ISO':
                return datetime.datetime.fromisoformat(date_string)  # ... Added in Python 3.7
            else:
