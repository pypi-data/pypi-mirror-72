...
from dnac.utils.os.System import System
...
class DnacMapExporter:
    ...
    @staticmethod
    def do_export(dnac_cluster, anchor_site_id, archive_file):
        ...
        result = dnac_cluster.mapService.export_site(anchor_site_id, temp_archive_file)
        if not result:
            print('...Failed')
    ...
    def start_export(self):
        ...
        result = self.dnac_cluster.mapService.export_site(self.anchor_site.id, temp_archive_file)
        if not result:
            print('???Failed to export: Aborted')
            sys.exit(1)
        status, content_type, content = result
