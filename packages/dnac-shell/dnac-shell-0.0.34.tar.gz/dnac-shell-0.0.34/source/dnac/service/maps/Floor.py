...
class Floor:
    ...
    def jsonTemplate(self):
        return '''
{"type":4,"contact":"{{tvCONTACT}}","maintainAspectRatio":false,"geometry":{{ tvGEOMETRY }},"parentGroupUuid":"{{tvPARENT_ID}}","incomplete":false,"name":"{{tvSITE_NAME}}"}
	 	       '''
        ...
        if cbs:
            if (isinstance(cbs, list)):
                ...
                if 'name' in cbs.keys() and 'groupHierarchy' in cbs.keys() and 'groupNameHierarchy' in cbs.keys():
                    ...
                    parsed_floor.hierarchy = namehier
                    return parsed_floor
