...
class Evolve(object):
    ...
    def evolve(cls, initialbinarytable, **kwargs):
        ...
        **kwargs:
        ...
        if 'dtp' not in list(initialbinarytable.keys()):
            initialbinarytable = initialbinarytable.assign(dtp=kwargs.pop('dtp', initialbinarytable['tphysf']))
        if 'randomseed' not in list(initialbinarytable.keys()):
        ...
        if 'bin_num' not in list(initialbinarytable.keys()):
        ...
        for k,v in list(BSEDict.items()):
            if k in list(initialbinarytable.keys()):
        ...
        if (pd.Series(FLATTENED_NATAL_KICK_COLUMNS).isin(list(initialbinarytable.keys())).all()) and ('natal_kick_array' not in BSEDict):
        ...
        if (pd.Series(QCRIT_COLUMNS).isin(list(initialbinarytable.keys())).all()) and ('qcrit_array' not in BSEDict):
            initialbinarytable = initialbinarytable.assign(qcrit_array=initialbinarytable[QCRIT_COLUMNS].values.tolist())

        if (pd.Series(FPRIMC_COLUMNS).isin(list(initialbinarytable.keys())).all()) and ('fprimc_array' not in BSEDict):
