...
def filter_bpp_bcm(bcm, bpp, method, kstar1_range, kstar2_range):
    ...
    for meth, use in list(method.items()):
        if (meth == 'binary_state'):
            ...
            merger_objects_to_track.extend(list(["{0}{1}".format(str(x[0]).zfill(2),str(x[1]).zfill(2)) for x in list(itertools.product(kstar1_range, kstar2_range))]))
            merger_objects_to_track.extend(list(["{0}{1}".format(str(x[0]).zfill(2),str(x[1]).zfill(2)) for x in list(itertools.product(kstar2_range, kstar1_range))]))
...
def conv_select(bcm_save, bpp_save, final_kstar_1, final_kstar_2, method, conv_lims):
    ...
    if conv_lims:
        for key in list(conv_lims.keys()):
...
def dat_un_transform(dat_sample, dat_set, dat_list):
    ...
    for ii,column in zip(list(range(len(dat_list))),dat_list):
...
def error_check(BSEDict, filters=None, convergence=None, sampling=None):
    ...
    if filters is not None:
        ...
        for option in ['binary_state']:
            if option not in list(filters.keys()):
    ...
    if convergence is not None:
        ...
        for option in ['convergence_params', 'convergence_filter', 'match', 'convergence_limits']:
            if option not in list(convergence.keys()):
    ...
    if sampling is not None:
        ...
        for option in ['sampling_method', 'SF_start', 'SF_duration', 'metallicity']:
            if option not in list(sampling.keys()):
    ...
    if convergence is not None:
        ...
        if convergence[flag]:
            for item, key in zip(list(convergence.items()), list(convergence.keys())):
                if len(item) != 2:
...
def convert_kstar_evol_type(bpp):
    ...
    kstar_string_to_int_dict = {v:k for k,v in list(kstar_int_to_string_dict.items())}
    ...
    evolve_type_string_to_int_dict = {v:k for k,v in list(evolve_type_int_to_string_dict.items())}
