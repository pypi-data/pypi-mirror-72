...
class TransformerTask:
    ...
    def single_predict(self, example: InputExample):
        self._taskModel.model.eval()
        input_ids = torch.tensor(self._taskModel.tokenizer.encode(
            example.text_a, example.text_b,
            max_length=self.task_args.data_args.max_seq_length,
            pad_to_max_length=True
        )).unsqueeze(0)  # Batch size 1

        input_ids.to(self.task_args.training_args.device)

        return self._taskModel.model(input_ids)[0]
