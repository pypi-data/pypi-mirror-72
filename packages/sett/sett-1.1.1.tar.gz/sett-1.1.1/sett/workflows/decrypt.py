...
from ..core.archive import check_tar, unpack, extract, \
    METADATA_FILE, METADATA_FILE_SIG, DATA_ARCHIVE, DATA_FILE_ENCRYPTED, \
    CHECKSUM_FILE
from ..core.crypt import decrypt as core_decrypt, extract_pub_key_ids, \
    validated_keys_by_ids, verify_signature, fingerprint2keyid
...
def verify_metadata_signature(tar_file, config):
    ...
    with tarfile.open(tar_file) as tar:
        try:
            ...
            if not sig:
                raise UserError(
                    f"{tar_file}: Metadata is not signed by the sender")
        except KeyError:
            raise UserError(f"{tar_file}: Metadata signature not found")
        fpr = verify_signature(tar.extractfile(METADATA_FILE), sig,
                               config.gpg_store)
        validated_keys_by_ids((fpr,),
                              config.gpg_store,
...
""", "\n\t".join(files), output_dir, dry_run)
    ...
    for tar_file in files:
        verify_metadata_signature(tar_file, config)
