...
validated_keys_by_ids = to_user_error(
    (RuntimeError, gpg.cmd.GPGError))(crypt.validated_keys_by_ids)
refresh_keys = to_user_error(
    (RuntimeError, gpg.cmd.GPGError))(crypt.refresh_keys)
...
def verify_signature(src: Union[bytes, io.IOBase], sig: bytes,
                     gpg_store: GPGStore) -> str:
    """Verify a file with a detached signature.

    :param src: file object of bytes to be verified.
    :param sig: signature to be used for verification.
    :param gpg_store: key database as gnupg object.
    :return: fingerprint of the key used for the signature.
    :raises UserError:
    """
    try:
        return gpg_store.verify(src, sig)
    except gpg.cmd.GPGError as e:
        raise UserError(f"Signature verification failed. {e}")
...
def retrieve_keys(gpg_store: GPGStore,
    ...
    if with_refresh_keys:
        pub_keys = list(refresh_keys(keys=pub_keys,
