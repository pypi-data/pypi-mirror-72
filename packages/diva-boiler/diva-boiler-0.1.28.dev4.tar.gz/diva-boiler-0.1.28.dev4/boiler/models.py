...
class Activity:
    ...
    def truncate_detections(self) -> 'Activity':
        """Return the activity with actors truncated to a minimal frame range."""
        new_actors = [
            a.inject_keyframe(a.begin).inject_keyframe(a.end).truncate_framerange()
            for a in self.actors
        ]
        return attr.evolve(self, actors=new_actors)
