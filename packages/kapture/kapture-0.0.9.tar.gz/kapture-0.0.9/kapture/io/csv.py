...
def records_lidar_from_file(filepath: str, lidar_ids: Set[str]) -> kapture.RecordsLidar:
    """
    Reads records_lidar from CSV file.

    :param filepath: input file path
    :param lidar_ids: input set of valid lidar device ids.
                        If the records_lidar contains unknown devices, they will be ignored.
...
def records_wifi_from_file(filepath: str, wifi_ids: Set[str]) -> kapture.RecordsWifi:
    """
    Reads RecordsWifi from CSV file.

    :param filepath: input file path
    :param wifi_ids: input set of valid wifi device ids.
                        If the records_wifi contains unknown devices, they will be ignored.
    :return: Wifi records
    """
...
def keypoints_from_dir(kapture_dirpath: str, images_paths: Optional[Set[str]]) -> kapture.Keypoints:
    """
    Read and build keypoints from images_filenames if given, or directly from actual files in kapture_dirpath.
...
def descriptors_from_dir(kapture_dirpath: str, images_paths: Set[str]) -> kapture.Descriptors:
    """
    Read and build descriptors from images_filenames if given, or directly from actual files in kapture_dirpath.
...
def global_features_from_dir(kapture_dirpath: str, images_paths: Set[str]) -> kapture.GlobalFeatures:
    """
    Read and build Global features from images_filenames if given, or directly from actual files in kapture_dirpath.
...
def matches_from_dir(kapture_dirpath: str,
    ...
    """
    Read and build Matches from images_filenames if given, or directly from actual files in kapture_dirpath.
...
# Kapture Read #########################################################################################################
def kapture_from_dir(kapture_dirpath: str, matches_pairsfile_path: Optional[str] = None) -> kapture.Kapture:
    """
    Reads kapture directory.
    ...
    csv_filepaths = {dtype: path.join(kapture_dirpath, filename) for dtype, filename in CSV_FILENAMES.items()}
    data_dirpaths = {dtype: path.join(kapture_dirpath, dir_name) for dtype, dir_name in
                     kapture.io.features.FEATURES_DATA_DIRNAMES.items()}

    kapture_data = kapture.Kapture()
    # sensors
    sensor_ids = None
    sensors_file_path = csv_filepaths[kapture.Sensors]
    if path.exists(sensors_file_path):
        logger.debug(f'loading sensors')
    ...
    # rigs
    if path.exists(csv_filepaths[kapture.Rigs]):
        logger.debug(f'loading rigs')
        assert sensor_ids is not None
        kapture_data.rigs = rigs_from_file(csv_filepaths[kapture.Rigs], sensor_ids)
        # update sensor_ids with rig_id
        sensor_ids.update(kapture_data.rigs.keys())

    # trajectories
    if path.exists(csv_filepaths[kapture.Trajectories]):
        logger.debug(f'loading trajectories')
        assert sensor_ids is not None
        kapture_data.trajectories = trajectories_from_file(csv_filepaths[kapture.Trajectories], sensor_ids)

    # records camera
    if path.exists(csv_filepaths[kapture.RecordsCamera]):
    ...
    # records Wifi
    if path.exists(csv_filepaths[kapture.RecordsWifi]):
        logger.debug(f'loading wifi')
        assert kapture_data.sensors is not None
        wifi_sensor_ids = set([sensor_id
                               for sensor_id in kapture_data.sensors.keys()
                               if kapture_data.sensors[sensor_id].sensor_type == 'wifi'])
        assert wifi_sensor_ids is not None
        kapture_data.records_wifi = records_wifi_from_file(csv_filepaths[kapture.RecordsWifi], wifi_sensor_ids)

    # records lidar
    if path.exists(csv_filepaths[kapture.RecordsLidar]):
        logger.debug(f'loading lidar')
        assert kapture_data.sensors is not None
        lidar_sensor_ids = set([sensor_id
                                for sensor_id in kapture_data.sensors.keys()
                                if kapture_data.sensors[sensor_id].sensor_type == 'lidar'])
        assert lidar_sensor_ids is not None
        kapture_data.records_lidar = records_lidar_from_file(csv_filepaths[kapture.RecordsLidar], lidar_sensor_ids)
    ...
    # keypoints
    if path.exists(csv_filepaths[kapture.Keypoints]):
        logger.debug(f'loading keypoints')
        assert kapture_data.records_camera is not None
        kapture_data.keypoints = keypoints_from_dir(kapture_dirpath, image_filenames)

    # descriptors
    if path.exists(csv_filepaths[kapture.Descriptors]):
        logger.debug(f'loading descriptors')
        assert kapture_data.records_camera is not None
        kapture_data.descriptors = descriptors_from_dir(kapture_dirpath, image_filenames)

    # global_features
    if path.exists(csv_filepaths[kapture.GlobalFeatures]):
        logger.debug(f'loading global features')
        assert kapture_data.records_camera is not None
        kapture_data.global_features = global_features_from_dir(kapture_dirpath, image_filenames)

    # matches
    if path.exists(data_dirpaths[kapture.Matches]):
        logger.debug(f'loading matches')
        assert kapture_data.records_camera is not None
        kapture_data.matches = matches_from_dir(kapture_dirpath, image_filenames, matches_pairsfile_path)

    # points3d
    if path.exists(csv_filepaths[kapture.Points3d]):
        logger.debug(f'loading points 3d')
        kapture_data.points3d = points3d_from_file(csv_filepaths[kapture.Points3d])

    # observations
    if path.exists(csv_filepaths[kapture.Observations]):
        logger.debug(f'loading observations')
        assert kapture_data.keypoints is not None
        assert kapture_data.points3d is not None
        kapture_data.observations = observations_from_file(get_csv_fullpath(kapture.Observations, kapture_dirpath),
