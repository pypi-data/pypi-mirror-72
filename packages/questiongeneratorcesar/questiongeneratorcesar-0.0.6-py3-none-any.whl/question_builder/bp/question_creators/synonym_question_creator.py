...
from .question_creator_exception import WordNotInMultisynonymDictionary, WordNotInSoundsLikeDictionary
import random
...
class SynonymQuestionCreator(QuestionCreator):
    ...
    def _get_baits(self, target_word, pos):
        if(target_word in word2soundslike):
            return self.baits_sounds_code, random.sample(word2soundslike[target_word], N_BAITS)
        else:
